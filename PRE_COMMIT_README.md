# oGRAC pre-commit Developer Guide

## What Was Ported From DSS

GitCode PR `opengauss/DSS!700` added a local pre-commit workflow:
...
oGRAC is also mainly a C/C++ repository, with CMake, shell, SQL, and Python support files. The oGRAC version keeps the same operating model and uses checks that fit this repository.

## Checks Enabled For oGRAC
...
```bash
.pre-commit-venv/bin/pre-commit run --files pkg/src/common/cm_defs.h
...
- `setup-pre-commit.sh` installs `pre-commit<3.0.0` so Python 3.7 environments remain supported.
