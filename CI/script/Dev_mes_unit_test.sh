...
init_and_start_cms1()
{
	...
	su - ogracdba -c 'cms gcc -reset -f' >> ${GRUN_LOG}
	su - ogracdba -c 'cms node -add 0 node0 127.0.0.1 1720' >> ${GRUN_LOG}
	su - ogracdba -c 'cms res -add db -type db -attr "check_timeout=1000000000"' >> ${GRUN_LOG}
    su - ogracdba -c 'cms res -edit db -attr "HB_TIMEOUT=1000000000"' >> ${GRUN_LOG}
    su - ogracdba -c 'cms node -list' >> ${GRUN_LOG}
	su - ogracdba -c 'cms res -list' >> ${GRUN_LOG}
    echo "start to start cms 1"   
    nohup su - ogracdba -c 'cms server -start' >> ${GRUN_LOG} 2>&1 &
}
...
make_code()
{
    ...
    sh Makefile.sh make_regress_test ${lcov_build_flag} >> ${GRUN_LOG} 2>&1
    if [ "$?" != "0" ]; then
...
run_mes_test()
{
    ...
    su - ogracdba -c "${code_path}/output/bin/mes_test --gtest_output=xml:${code_path}/gtest_result/" >> ${GRUN_LOG} 2>&1
    if [ "$?" != "0" ]; then
