...
REGRESS_PATH=${ROOT_PATH}/pkg/test/backup_test
...
function install_ogracdb() {
    CREATEDB_SQL=${REGRESS_PATH}/create_cluster_database.backup.sql
    ...
    su - ${RUN_TEST_USER} -c "OGSQL_SSL_QUIET=TRUE ${TEST_DATA_DIR}/install/bin/ogsql sys/${SYS_PASSWD}@127.0.0.1:1611 -c 'select * from DV_VERSION' 2>&1"
...
gen_lcov_report()
{
    ...
    coverage_info_name="${ROOT_PATH}/lcov_output/Dev_Basic_test_coverage.info"
    coverage_report_name="${ROOT_PATH}/lcov_output/Dev_Basic_test_coverage.report"
...
function clear_old_install() {
    kill -9 $(pidof ogracd)
    kill -9 $(pidof gssd)
    kill -9 $(pidof cms)
    rm -rf /home/regress/ograc_data/*  /home/ogracdba/data/*  /home/ogracdba/install/*  /data/data/*
...
function parse_parameter() {
    ...
    eval set -- "${ARGS}"
    declare -g LCOV_ENABLE=TRUE
    declare -g RUN_TEST_USER="ogracdba"
    declare -g CORE_DIR="/home/core"
    declare -g BASIC_TEST_LIST="basic_test"
    while true
    do
        case "$1" in
            -c | --coverage)
    ...
    declare -g TEST_DATA_DIR="/home/regress/og_regress"
