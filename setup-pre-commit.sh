#!/usr/bin/env bash
# =============================================================================
# setup-pre-commit.sh -- install oGRAC pre-commit checks in an isolated venv.
#
# Design:
...
git rev-parse --show-toplevel >/dev/null 2>&1 || die "run this script inside the oGRAC git repository"
...
[ -x "$VENV_PY" ] || die "virtual environment python not found"
