...
#define MAX_DEC_BYTE_SZ MAX_DEC_BYTE_BY_PREC(OG_MAX_NUM_SAVING_PREC)
...
static inline void cm_dec4_copy(dec4_t *dst, const dec4_t *src)
{
    ...
    switch (src->ncells) {
        ...
        default:
            ...
            OG_THROW_ERROR(ERR_ASSERT_ERROR, "copy_dec4: invalid decimal");
            cm_zero_dec4(dst);
            break;
    }
}
...
{
    return cm_dec4_to_text(dec, OG_MAX_DEC_OUTPUT_ALL_PREC, text);
}
