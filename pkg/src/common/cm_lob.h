...
/* lob value can from kernel(lob_locator_t) or vm_pool(vm_lob_t)
  * vm_cli_lob_t is same with ogconn_lob_t to preserve lob info in order to transform to client */
...
#define OG_LOB_FROM_KERNEL         0
#define OG_LOB_FROM_VMPOOL         1
#define OG_LOB_FROM_NORMAL         2
#define OG_IS_VALID_LOB_TYPE(type)                                                                                     \
    ((type) == OG_LOB_FROM_KERNEL || (type) == OG_LOB_FROM_VMPOOL || (type) == OG_LOB_FROM_NORMAL)

static inline void cm_reset_vm_lob(vm_lob_t *vlob)
{
    vlob->size = 0;
    vlob->type = OG_LOB_FROM_VMPOOL;
    vlob->entry_vmid = OG_INVALID_ID32;
    vlob->last_vmid = OG_INVALID_ID32;
}

static inline void cm_reset_normal_lob(normal_lob_t *nlob)
{
    nlob->size = 0;
    nlob->type = OG_LOB_FROM_NORMAL;
    nlob->value.len = 0;
    nlob->value.str = NULL;
}
