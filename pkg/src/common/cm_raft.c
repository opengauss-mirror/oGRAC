...
status_t raft_load_lib(raft_procs_t *procs)
{
    ...
    if (procs->lib_handle == NULL) {
        OG_THROW_ERROR(ERR_LOAD_LIBRARY, "libconsistency.so", cm_get_os_error());
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "InitConsistency", (void **)(&procs->init_consistency)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "QueryInfo", (void **)(&procs->query_info)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "AddMember", (void **)(&procs->add_member)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "DeleteMember", (void **)(&procs->delete_member)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "InitDBEngines", (void **)(&procs->init_db_engines)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "Register", (void **)(&procs->register_callback)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "SendMessage", (void **)(&procs->send_message)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "ExecWriteLogCmd", (void **)(&procs->exec_writelog_cmd)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "PromoteLeader", (void **)(&procs->promote_leader)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "StopConsistency", (void **)(&procs->stop_consistency)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "GetVersion", (void **)(&procs->get_version)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "SetParam", (void **)(&procs->set_param)) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (raft_load_symbol(procs->lib_handle, "Monitor", (void **)(&procs->monitor)) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t raft_load_lib(raft_procs_t *procs)
{
    OG_THROW_ERROR(ERR_CAPABILITY_NOT_SUPPORT, "raft");
    return OG_ERROR;
}
