...
/*
* @addtogroup NLS
* @brief The head file for oGRAC's Globalization of Language --
...
#define CM_SET_NLSPARAM(nls_val, text_val)                             \
    do {                                                               \
        ...
        OG_RETURN_IFERR(cm_text2str((text_val), __nls_val->str, MAX_NLS_PARAM_LENGTH)); \
...
static inline status_t cm_text2nlsvalue(const text_t *text, nlsvalue_t *nls_text)
{
    ...
    if (SECUREC_UNLIKELY(errcode != EOK)) {
        OG_THROW_ERROR(ERR_CLT_INVALID_VALUE, "text2nlsvalue error", (uint32)errcode);
        return OG_ERROR;
    }
...
#define OG_DEFALUT_SESSION_NLS_PARAMS (&g_default_session_nlsparams)
