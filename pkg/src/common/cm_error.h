...
typedef enum en_errno {
    ...
    ERR_SYSTEM_BUSY = 139,
    ERR_SCSI_LOCK_OCCUPIED = 140,
    ERR_SCSI_REG_CONFLICT = 141,
    ERR_SKLIST_ERR = 142,
...
/* sharding error: 1200 - 1299 */
    ...
    // SPM, range [1700, 1799]
    ERR_SPM_NOT_ENABLED = 1700,
    ERR_SPM_NOT_SUPPORTED = 1701,
    ERR_SPM_NOT_FOUND = 1702,
    ERR_SPM_DUPLICATE_PLAN = 1703,
    ERR_SPM_FIX_EXISTS = 1704,
    ERR_SPM_STATUS_NOT_ALLOWED = 1705,
    ...
    ERR_JSONB_SYNTAX_ERROR = 2508,
    ERR_JSONB_EXP_ERROR = 2509,
    ...
    ERR_MES_XNET_CONN = 2752,
    ERR_MES_XNET_LISTEN = 2753,
    ERR_MES_XNET_MSG_POOL = 2754,

    // CMS, range [2800, 2998]
    ERR_CMS_OPEN_DISK = 2800,
    ERR_CMS_SEEK_DISK = 2801,
    ERR_CMS_READ_DISK = 2802,
    ERR_CMS_WRITE_DISK = 2803,
    ERR_CMS_CREATE_SOCKET = 2804,
    ERR_CMS_OBJECT_EXISTS = 2805,
    ERR_CMS_OBJECT_NOT_FOUND = 2806,
    ERR_CMS_NUM_EXCEED = 2807,
    ERR_CMS_DEFAULT_RES_GROUP = 2808,
    ERR_CMS_RES_GROUP_NOT_NULL = 2809,
    ERR_CMS_RES_INVALID_ATTR = 2810,
    ERR_CMS_SERVER_RUNNING = 2811,
    ERR_CMS_SAME_RESOURCE_TYPE = 2812,
    ERR_CMS_GCC_EXPORT = 2813,
    ERR_CMS_GCC_IMPORT = 2814,
    ERR_CMS_INVALID_RES_ATTRS = 2815,
    ERR_CMS_GCC_LOCK = 2816,
    ERR_CMS_GCC_NODE_UNREGISTERED = 2817,
    ERR_CMS_PORT_OUT_RANGE = 2818,
    ERR_CMS_INVALID_PATH = 2819,
    ERR_CMS_GCC_VERSION_MISMATCH = 2820,
    ERR_CMS_INVALID_VOTEDISK = 2821,
    ERR_CMS_INVALID_GCC = 2822,
    ERR_CMS_CONVERT_VALUE = 2823,
    ERR_CMS_VOTEDISK_AUTHORITY = 2824,
    ...
    ERR_DCS_MSG_EAGAIN = 3001,
    // GDV,range[3400, 3499]
    ERR_GDV_ALLOC_SQL_SESSION = 3100,
    ERR_GDV_PREP_AND_EXEC = 3101,

    ERR_CLUSTER_DDL_DISABLED = 3200,

    // Tools
    ERR_EXPORT = 3301,
    ERR_IMPORT = 3302,
    ERR_CM_OPEN_DISK = 3600,
    ...
    ERR_WRITE_OPT_IN_DISK_PROTECT = 4009,

    // BACKUP AND RESTORE
    ERR_BACKUP_IN_STANDBY = 4200,
    ERR_BACKUP_INCREMENT_BLOCK = 4201,
...
typedef struct st_error_info_t {
    ...
    char t2s_buf1[OG_T2S_LARGER_BUFFER_SIZE];
    char t2s_buf2[OG_T2S_BUFFER_SIZE];
    char message[OG_MESSAGE_BUFFER_SIZE];
    struct { // first position of err occurs
        char err_file[OG_MAX_NAME_LEN];
        uint32 err_line;
    };
    bool8 is_ignored;
    bool8 is_ignore_log;
...
typedef struct st_tls_plc_error_t {
    ...
    uint16 start_pos[OG_MAX_PLC_CNT]; // Store the start position of g_tls_error.message when entering plc_do_compile.
} tls_plc_error_t;

typedef status_t (*cm_error_handler)(const char *file, uint32 line, og_errno_t code, const char *format,
    va_list args);
...
status_t cm_inherit_ctstore_error(const char *file, uint32 line, og_errno_t code, const char *errmsg);
...
#define OG_ERRNO g_tls_error.code
...
{
    g_tls_plc_error.plc_flag = OG_TRUE;
}
...
#define OG_THROW_ERROR_TRY_SRC(loc, err_no, ...)                                                           \
    do {                                                                                                     \
        cm_set_error((char *)__FILE__, (uint32)__LINE__, err_no, g_error_desc[err_no], ##__VA_ARGS__);       \
        if (loc != NULL) {                                                                                 \
            cm_set_error_loc(*loc);                                                                        \
...
#define OG_THROW_ERROR_EX(err_no, format, ...)                                              \
    do {                                                                                    \
        cm_set_error_ex((char *)__FILE__, (uint32)__LINE__, err_no, format, ##__VA_ARGS__); \
    } while (0)

#define OG_SRC_THROW_ERROR_EX(loc, err_no, format, ...)                                     \
    do {                                                                                    \
        if (g_tls_plc_error.plc_flag) {                                                                    \
            cm_set_error_loc(loc);                                                                         \
            cm_set_error_ex((char *)__FILE__, (uint32)__LINE__, err_no, format, ##__VA_ARGS__);            \
...
//CAUTION: only for db inherit error from ogstore instance in ogstore_interface.c
#define OG_THROW_CTSTORE_ERROR(err_no, ...)                                                  \
    do {                                                                                 \
...
void cm_set_error(const char *file, uint32 line, og_errno_t code, const char *format, ...) OG_CHECK_FMT(4, 5);
void cm_set_hint(const char *format, ...) OG_CHECK_FMT(1, 2);
void cm_set_error_ex(const char *file, uint32 line, og_errno_t code, const char *format, ...) OG_CHECK_FMT(4, 5);
