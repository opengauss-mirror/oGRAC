...
typedef struct st_debug_config_item {
    char *name;
    char *default_value;
    char curr_value[OG_PARAM_BUFFER_SIZE];
    char *range;
    char *datatype;
    config_verify_t verify;
...
typedef struct st_config {
    ...
    char file_buf[OG_MAX_CONFIG_FILE_SIZE];
    char *value_buf;
    uint32 value_offset;
    ...
    config_item_t *name_map[OG_CONFIG_HASH_BUCKETS];
    config_item_t *alias_map[OG_CONFIG_ALIAS_HASH_BUCKETS];
