...
{
    uint32 copy_size;
    CM_ASSERT(buf_size > OG_QUATO_LEN + 1);
    copy_size = (text->len + OG_QUATO_LEN >= buf_size) ? buf_size - OG_QUATO_LEN - 1 : text->len;
    buf[0] = '\'';
    if (copy_size > 0) {
        MEMS_RETURN_IFERR(memcpy_sp(buf + 1, copy_size, text->str, copy_size));
    }
    buf[copy_size + 1] = '\'';
    buf[copy_size + OG_QUATO_LEN] = '\0';
    return OG_SUCCESS;
}
...
{
    char buf[OG_MAX_NUMBER_LEN + 1] = {0};
    text_t text = *text_src;

    cm_trim_text(&text);

    if (text.len > OG_MAX_NUMBER_LEN) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR,
            "Convert uint16 failed, the length of text can't be larger than %u, text = %s",
            OG_MAX_NUMBER_LEN, T2S(&text));
        return OG_ERROR;
    }
    OG_RETURN_IFERR(cm_text2str(&text, buf, OG_MAX_NUMBER_LEN + 1));
...
num_errno_t cm_numpart2int(num_part_t *np, int32 *value)
{
    ...
    if (np->digit_text.len > OG_MAX_INT32_PREC ||
    ...
    if (np->digit_text.len == OG_MAX_INT32_PREC) {
        ...
        if (cmp_ret > 0) {
            ...
            *value = np->is_neg ? OG_MIN_INT32 : OG_MAX_INT32;
            return NERR_SUCCESS;
        }
    }
...
status_t cm_text2bool(const text_t *bool_text, bool32 *val)
{
    ...
    if (bool_text == NULL) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "bool_text != NULL");
        return OG_ERROR;
    }
    ...
    do {
        OG_BREAK_IF_TRUE(CM_IS_EMPTY(&text));  // empty text is not allowed
        ...
        OG_BREAK_IF_TRUE(text.len < OG_MIN_BOOL_STRLEN || text.len > OG_MAX_BOOL_STRLEN);
    ...
    OG_THROW_ERROR(ERR_SQL_SYNTAX_ERROR, "invalid BOOLEAN text");
    return OG_ERROR;
}
...
{
    char buf[OG_MAX_NUMBER_LEN + 1] = {0};  // '00000000000000000000001'
    text_t text = *text_src;
...
num_errno_t cm_numpart2uint32(const num_part_t *np, uint32 *value)
{
    ...
    if (np->digit_text.len > OG_MAX_UINT32_PREC ||
...
{
    char buf[OG_MAX_NUMBER_LEN + 1] = {0};  // '00000000000000000000000000000001'
    text_t text = *text_src;
...
num_errno_t cm_numpart2bigint(const num_part_t *np, int64 *value)
{
    ...
    OG_RETVALUE_IFTRUE((np->digit_text.len > OG_MAX_INT64_PREC || np->has_dot || np->has_expn), NERR_ERROR);

    if (np->digit_text.len == OG_MAX_INT64_PREC) {
        int32 cmp_ret = cm_compare_digitext(&np->digit_text,
                                            np->is_neg ? &g_neg_bigint_ceil : &g_pos_bigint_ceil);
        if (cmp_ret > 0) {
            return NERR_OVERFLOW;
        } else if (cmp_ret == 0) {
            *value = np->is_neg ? OG_MIN_INT64 : OG_MAX_INT64;
            return NERR_SUCCESS;
        }
    }
    ...
    for (uint32 i = 0; i < np->digit_text.len; ++i) {
        if (!CM_IS_DIGIT(np->digit_text.str[i])) {
            OG_THROW_ERROR_EX(ERR_ASSERT_ERROR, "np->digit_text.str(%c) should be a digit", np->digit_text.str[i]);
            return NERR_ERROR;
        }
...
{
    if (np->digit_text.len > OG_MAX_UINT64_PREC ||
...
status_t cm_str2int(const char *str, int32 *value)
{
    ...
    if (cm_is_err(err)) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "Convert int32 failed, text = %s", str);
        return OG_ERROR;
    }
...
status_t cm_str2uint16(const char *str, uint16 *value)
{
    ...
    int64 val_int64 = strtol(str, &err, CM_DEFAULT_DIGIT_RADIX);
    if (cm_is_err(err)) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "Convert uint16 failed, text = %s", str);
        return OG_ERROR;
    }
...
status_t cm_str2uint32(const char *str, uint32 *value)
{
    ...
    if (cm_is_err(err)) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "Convert uint32 failed, text = %s", str);
        return OG_ERROR;
    }
...
status_t cm_str2bigint(const char *str, int64 *value)
{
    ...
    *value = strtoll(str, &err, CM_DEFAULT_DIGIT_RADIX);
    if (cm_is_err(err)) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "Convert int64 failed, text = %s", str);
        return OG_ERROR;
    }
...
{
    char buf[OG_MAX_REAL_INPUT_STRLEN + 1] = {0};
    text_t text = *text_src;

    cm_trim_text(&text);

    if (text.len > OG_MAX_REAL_INPUT_STRLEN) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR,
                          "Convert double failed, the length(%u) of text can't be larger than %u, text = %s", text.len,
                          OG_MAX_REAL_INPUT_STRLEN, T2S(&text));
        return OG_ERROR;
    }
    OG_RETURN_IFERR(cm_text2str(&text, buf, OG_MAX_REAL_INPUT_STRLEN + 1));
...
num_errno_t cm_numpart2real(num_part_t *np, double *value)
{
    ...
    if (np->sci_expn > OG_MAX_REAL_EXPN) {
        return NERR_OVERFLOW;
    }
    ...
    } else if (np->sci_expn < OG_MIN_REAL_EXPN) {
        *value = 0.0;
        return NERR_SUCCESS;
    }
    ...
    int32 code = snprintf_s(CM_GET_TAIL(&np->digit_text), OG_MAX_NUM_PART_BUFF - np->digit_text.len,
        OG_MAX_NUM_PART_BUFF - np->digit_text.len - 1, "E%d", expn);
...
status_t cm_str2real(const char *str, double *value)
{
    ...
    *value = strtod(str, &err);
    if (cm_is_err(err)) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "Convert double failed, text = %s", str);
        return OG_ERROR;
    }
...
num_errno_t cm_text2int16_ex(const text_t *text, int32 *val)
{
    ...
    OG_RETVALUE_IFTRUE((i >= text->len), NERR_NO_DIGIT);
    ...
    *val = 0;
    ...
    OG_RETVALUE_IFTRUE((text->len >= (6 + i)), NERR_INVALID_LEN);

    for (;;) {
        // invalid number text with unexpected characters
        OG_RETVALUE_IFTRUE((!CM_IS_DIGIT(c)), NERR_UNEXPECTED_CHAR);
        *val = (*val) * CM_DEFAULT_DIGIT_RADIX + CM_C2D(c);

        ++i;
        OG_BREAK_IF_TRUE(i >= text->len);
        c = text->str[i];
    }
    // overflow
    OG_RETVALUE_IFTRUE((*val != (int16)(*val)), NERR_OVERFLOW);
...
static bool32 cm_diag_int(const text_t *text, const digitext_t *dtext, num_part_t *np)
{
    ...
    OG_RETVALUE_IFTRUE((num_text.len == 0), OG_FALSE);
    ...
    OG_RETVALUE_IFTRUE((num_text.len == dtext->len && cm_compare_text(&num_text, &num_dtext) > 0), OG_FALSE);

    if (np != NULL) {
        cm_text2digitext(&num_text, &np->digit_text);
        np->digit_text.len = num_text.len;
        np->has_dot = OG_FALSE;
        np->has_expn = OG_FALSE;
        np->is_neg = is_neg;
    }
 ...
 * return false. */
static inline num_errno_t cm_decide_decimal_type(const num_part_t *np, og_type_t *type)
{
    if (np->sci_expn < OG_MAX_REAL_EXPN) {
        ...
        *type = (np->sci_expn > MAX_NUMERIC_EXPN || np->sci_expn < MIN_NUMERIC_EXPN) ? OG_TYPE_REAL : OG_TYPE_NUMBER;
        return NERR_SUCCESS;
    } else if (np->sci_expn == OG_MAX_REAL_EXPN) {
        OG_RETVALUE_IFTRUE((cm_compare_digitext(&np->digit_text, &g_double_ceil) > 0), NERR_OVERFLOW);
        // less than the maximal representable double
        *type = OG_TYPE_REAL;
        return NERR_SUCCESS;
    }

    // sci_exp > OG_MAX_REAL_EXPN
    return NERR_OVERFLOW;
}

/* Decide type of an integer num_part, if the num_part is
 * + in the range of an int32, type = OG_TYPE_INTEGER;
 * + in the range of an bigint, type = OG_TYPE_BIGINT;
 * + else, return number type */
static inline num_errno_t cm_decide_integer_type(const num_part_t *np, og_type_t *type)
{
    const digitext_t *cmp_text = NULL;
    /* Rule 4: no dot and no expn */
    /* Rule 4.1: the precision less than the maximal length of an int32 */
    if (np->digit_text.len < OG_MAX_INT32_PREC) {
        *type = OG_TYPE_INTEGER;
        return NERR_SUCCESS;
    }
        ...
        *type = OG_TYPE_INTEGER;
        if (np->is_neg) {
            ...
            if (cm_compare_digitext(&np->digit_text, &g_pos_int32_ceil) > 0) {
                *type = (cm_compare_digitext(&np->digit_text, &g_uint32_ceil) <= 0) ? OG_TYPE_UINT32 : OG_TYPE_BIGINT;
            }
        }
    ...
    if (np->digit_text.len == OG_MAX_INT64_PREC) {
        ...
        *type = (cm_compare_digitext(&np->digit_text, cmp_text) > 0) ? OG_TYPE_NUMBER : OG_TYPE_BIGINT;
        return NERR_SUCCESS;
    }
    ...
    if (np->digit_text.len == OG_MAX_UINT64_PREC) {
        if (np->is_neg) {
            ...
            *type = (cm_compare_digitext(&np->digit_text, &g_uint64_ceil) > 0) ? OG_TYPE_NUMBER : OG_TYPE_UINT64;
        }
        return NERR_SUCCESS;
    }

    if (np->digit_text.len <= OG_MAX_NUM_PRECISION) {
        *type = OG_TYPE_NUMBER;
        return NERR_SUCCESS;
    }
    return NERR_ERROR;
...
num_errno_t cm_decide_numtype(const num_part_t *np, og_type_t *type)
{
    ...
    if (np->has_expn || np->has_dot || np->digit_text.len > OG_MAX_INT64_PREC) {
...
*/
num_errno_t cm_is_number_with_sign(const text_t *text, og_type_t *type)
{
    num_part_t np;
    num_errno_t err_no;
...
num_errno_t cm_is_number(const text_t *text, og_type_t *type)
{
...
static inline num_errno_t cm_parse_num_expn(text_t *expn_text, int32 *expn)
{
    ...
    if (i >= expn_text->len) { /* if no exponent digits, return error  */
        OG_RETVALUE_IFTRUE((i >= expn_text->len), NERR_NO_EXPN_DIGIT);
    }
    ...
    tmp_exp = 0;
    for (;;) {
        OG_RETVALUE_IFTRUE((!CM_IS_DIGIT(c)), NERR_EXPN_WITH_NCHAR);
    ...
    OG_RETVALUE_IFTRUE((!is_negexp && tmp_exp > CM_MAX_EXPN), NERR_OVERFLOW);
...
num_errno_t cm_split_num_text(const text_t *num_text, num_part_t *np)
{
    ...
    bool32 leading_flag = OG_TRUE; /** used to ignore leading zeros */
    ...
    OG_RETVALUE_IFTRUE((text.len == 0 || text.len >= SIZE_M(1)), NERR_INVALID_LEN);  // text.len > 2^15
    ...
    /* check again */
    OG_RETVALUE_IFTRUE((i >= (int32)text.len), NERR_NO_DIGIT);
    ...
    for (; i < (int32)text.len; ++i) {
        ...
        if (CM_IS_DOT(c)) {
            ...
            OG_RETVALUE_IFTRUE((dot_offset >= 0), NERR_MULTIPLE_DOTS);
        ...
        OG_RETVALUE_IFTRUE((!CM_IS_EXPN_CHAR(c)), NERR_UNEXPECTED_CHAR);

        // Exclude: 'E0012', '.E0012', '-E0012', '+.E0012', .etc
        OG_RETVALUE_IFTRUE((precision < 0), NERR_UNEXPECTED_CHAR);
        ...
        OG_RETVALUE_IFTRUE((nerr != NERR_SUCCESS), nerr);
        np->has_expn = OG_TRUE;
        break;
    }  // end for

    OG_RETVALUE_IFTRUE((precision < 0), NERR_NO_DIGIT);
    ...
    if (np->digit_text.len > num_text->len || np->digit_text.len >= OG_MAX_NUM_PART_BUFF) {
        OG_THROW_ERROR_EX(ERR_ASSERT_ERROR,
            "np->digit_text.len(%u) <= num_text->len(%u) and np->digit_text.len(%u) < OG_MAX_NUM_PART_BUFF(%d)",
            np->digit_text.len, num_text->len, np->digit_text.len, OG_MAX_NUM_PART_BUFF);
        return NERR_ERROR;
    }
    return NERR_SUCCESS;
}
...
status_t cm_text2size(const text_t *text, int64 *value)
{
    ...
    OG_RETURN_IFERR(cm_text2real(&num, &size));
...
status_t cm_text2microsecond(const text_t *text, uint64 *value)
{
    ...
    if (displace == 1 || displace == 0) {
        if (cm_text2uint32(&num, &second) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    if (displace == 2) {
        if (cm_text2uint64(&num, &microsecond) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
num_errno_t cm_numpart2size(const num_part_t *np, int64 *value)
{
    ...
    OG_RETVALUE_IFTRUE((*value < 0), NERR_EXPECTED_POS_INT);
    ...
    // overflow
    OG_RETVALUE_IFTRUE((*value > (OG_MAX_INT64 >> unit)), NERR_OVERFLOW);
...
void cm_str_reverse(char *dst, const char *src, uint32 dst_len)
{
    ...
    if (len >= dst_len) {
        OG_THROW_ERROR_EX(ERR_ASSERT_ERROR, "len(%lu) < dst_len(%u)", len, dst_len);
        return;
    }
...
bool32 cm_is_unsigned_int(const char *num)
{
    ...
    if (len > OG_MAX_INT32_STRLEN) {
        OG_THROW_ERROR_EX(ERR_GENERIC_INTERNAL_ERROR, "'%s' is too long for unsigned int", num);
        return OG_FALSE;
    }

    for (i = 0; i < len; i++) {
        if (!(num[i] >= '0' && num[i] <= '9')) {
            OG_THROW_ERROR_EX(ERR_GENERIC_INTERNAL_ERROR, "character '%c' is invalid for unsigned int", num[i]);
            return OG_FALSE;
        }
    }
...
void cm_truncate_text(text_t *text, uint32 max_len)
{
    if (text == NULL || text->str == NULL) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "text != NULL and text->str != NULL");
        return;
    }
...
status_t cm_replace_quotation(char *src, char *dest, uint32 dest_len, bool32 *exist_flag)
{
    ...
    *exist_flag = OG_FALSE;
    ...
    if (*exist_flag) {
        ...
        while (*src_tmp != '\0') {
            if (count >= dest_len - 1) {
                OG_THROW_ERROR(ERR_OUT_OF_INDEX, "dest_tmp", dest_len);
                return OG_ERROR;
            }
            count++;
            *dest_tmp = *src_tmp;
...
bool32 cm_is_multiple_sql(text_t *sql)
{
    ...
    if (pos == OG_INVALID_ID32 || (pos == sql->len - 1)) {
        return OG_FALSE;
    }
...
status_t cm_replace_regexp_spec_chars(text_t *regexp, char *ret, uint32 ret_size)
{
    ...
    uint32 chars = 0;
    uint32 dep = 0;
    bool32 in_char_class = OG_FALSE;
    char *pre = NULL;
    bool32 is_escape = OG_FALSE;
    bool32 pre_is_escape = OG_FALSE;

    for (i = 0; i < regexp->len; i++) {
        if (j >= ret_size) {
            OG_THROW_ERROR(ERR_BUFFER_OVERFLOW, j + 1, ret_size);
            return OG_ERROR;
        }
        is_escape = OG_FALSE;
        if (regexp->str[i] == '[') {
