...
keyword_map_item_t g_key_pattern[] = {
    ...
    // set OGSQL_SSL_KEY_PWD =
    { "SET.*?\\s*.*?OGSQL_SSL_KEY_PASSWD",                              "SQL CONTAINS:SET OGSQL_SSL_KEY_PASSWD" },
    // conn user/pwd
...
void cm_text_reg_match(text_t *text, const char *pattern, int32 *pos, charset_type_t charset)
{
    ...
    if (OG_SUCCESS != cm_regexp_compile(&code, pattern, &match_param, charset)) {
        return;
    }

    assist.code = code;
    if (OG_SUCCESS != cm_regexp_instr(pos, &assist, OG_TRUE)) {
        cm_regexp_free(code);
        return;
    }
...
static inline void cm_get_star_pos(text_t *text, bool32 end, uint32 *comm_pos, charset_type_t charset)
{
    ...
    assist.code = code;
    if (OG_SUCCESS != cm_regexp_instr(&pos, &assist, end)) {
        cm_regexp_free(code);
        return;
    }
...
void cm_text_star_to_one(text_t *text)
{
    ...
    for (;;) {
        if (left_text.len > 0) {
            ...
            cm_get_star_pos(&left_text, OG_TRUE, &end, CHARSET_GBK);
            ...
            cm_get_star_pos(&left_text, OG_FALSE, &start, CHARSET_GBK);
            len = (end - start);
