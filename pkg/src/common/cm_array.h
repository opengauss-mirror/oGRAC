...
typedef struct st_array_head_t {
    ...
    uint32 offset;  /* the first value offset equal to OG_VMEM_PAGE_SIZE * n.m,
...
#define MAX_DIR_COUNT_IN_ONE_VM   (OG_VMEM_PAGE_SIZE / sizeof(elem_dir_t))
#define COMPRESS_ARRAY   OG_TRUE
#define UNCOMPRESS_ARRAY OG_FALSE
#define ARRAY_USED_VM_PAGES 2
#define ARRAY_UNUSED_SPACE_IN_VM (OG_VMEM_PAGE_SIZE / 128)
...
{
    vm_page_t *page = NULL;
    OG_RETURN_IFERR(vm_open(se, pool, v->value.vm_lob.entry_vmid, &page));
...
/* 1) after sql_compress_array_xxx, head_offset is equal to ctrl_size
   2) before sql_compress_array_xxx, head_offset is euqal to (n * OG_VMEM_PAGE_SIZE)
      notice: when ctrl_size is equal to (n * OG_VMEM_PAGE_SIZE), it is the same scene with 1)
*/
    ...
    uint32* total_size, uint32* head_offset)
{
    ...
    OG_RETURN_IFERR(cm_get_array_head_offset(session, pool, v, head_offset));

    if (*head_offset == ctrl_size) {
        *total_size = v->value.vm_lob.size;
    } else {
        *total_size = v->value.vm_lob.size - (OG_VMEM_PAGE_SIZE - ctrl_size % OG_VMEM_PAGE_SIZE);
    }
...
{
    vm_page_t *page = NULL;
    OG_RETURN_IFERR(vm_open(session, pool, v_array->value.vm_lob.entry_vmid, &page));
    array_head_t *head = (array_head_t *)page->data;
...
/* after compress the space between last dir and first data
   the offset of array_head_t is n.m * OG_VMEM_PAGE_SIZE,
   and when m > 0 , dir page count should add one */
static inline uint32 cm_get_dir_page_count(uint32 head_offset)
{
    uint32 count = head_offset / OG_VMEM_PAGE_SIZE;
    return ((head_offset % OG_VMEM_PAGE_SIZE > 0) ? (count + 1) : count);
}
...
static inline elem_dir_t* cm_get_array_end_dir(char* data, uint32 dir_end, bool32 is_last_dir_page)
{
    ...
    if (is_last_dir_page) {
        end_dir = (elem_dir_t *)(data + (dir_end % OG_VMEM_PAGE_SIZE));
    } else {
        end_dir = (elem_dir_t *)(data + OG_VMEM_PAGE_SIZE - sizeof(elem_dir_t));
    }
    return end_dir;
}
