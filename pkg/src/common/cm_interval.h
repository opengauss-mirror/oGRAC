...
#define OG_IS_YM_UNIT(resid)       ((resid) == IU_YEAR || (resid) == IU_MONTH || (resid) == IU_QUARTER)
#define OG_IS_DAY_UNIT(resid)      ((resid) == IU_DAY || (resid) == IU_WEEK)
#define OG_IS_TIME_UNIT(resid)                                                                                         \
    ((resid) == IU_HOUR || (resid) == IU_MINUTE || (resid) == IU_SECOND || (resid) == IU_MICROSECOND)
#define OG_IS_DATETIME_UNIT(resid) (OG_IS_YM_UNIT(resid) || OG_IS_DAY_UNIT(resid) || OG_IS_TIME_UNIT(resid))
...
#define OG_MAX_YMINTERVAL ((interval_ym_t)(ITVL_MAX_SQL_YEAR * ITVL_MONTHS_PER_YEAR + ITVL_MAX_SQL_MONTH))
#define OG_MIN_YMINTERVAL (-(OG_MAX_YMINTERVAL))

#define OG_MAX_DSINTERVAL \
...
#define OG_MIN_DSINTERVAL (-(OG_MAX_DSINTERVAL))

#define CM_IS_VALID_DSINTERVAL(dsitvl) \
    (((dsitvl) >= OG_MIN_DSINTERVAL) && ((dsitvl) <= OG_MAX_DSINTERVAL))
...
{
    if (a < OG_MIN_DSINTERVAL || a > OG_MAX_DSINTERVAL) {
        OG_THROW_ERROR_EX(ERR_ASSERT_ERROR, "OG_MIN_DSINTERVAL(%lld) <= a(%lld) <= OG_MAX_DSINTERVAL(%lld)",
                          OG_MIN_DSINTERVAL, a, OG_MAX_DSINTERVAL);
        return OG_ERROR;
    }
    if (b < OG_MIN_DSINTERVAL || b > OG_MAX_DSINTERVAL) {
        OG_THROW_ERROR_EX(ERR_ASSERT_ERROR, "OG_MIN_DSINTERVAL(%lld) <= b(%lld) <= OG_MAX_DSINTERVAL(%lld)",
                          OG_MIN_DSINTERVAL, b, OG_MAX_DSINTERVAL);
        return OG_ERROR;
    }

    *res = a + b;
    if (*res >= OG_MIN_DSINTERVAL && *res <= OG_MAX_DSINTERVAL) {
        return OG_SUCCESS;
    }
...
{
    if (a < OG_MIN_YMINTERVAL || a > OG_MAX_YMINTERVAL) {
        OG_THROW_ERROR_EX(ERR_ASSERT_ERROR, "OG_MIN_YMINTERVAL(%d) <= a(%d) <= OG_MAX_YMINTERVAL(%d)",
            OG_MIN_YMINTERVAL, a, OG_MAX_YMINTERVAL);
        return OG_ERROR;
    }
    if (b < OG_MIN_YMINTERVAL || b > OG_MAX_YMINTERVAL) {
        OG_THROW_ERROR_EX(ERR_ASSERT_ERROR, "OG_MIN_YMINTERVAL(%d) <= b(%d) <= OG_MAX_YMINTERVAL(%d)",
            OG_MIN_YMINTERVAL, b, OG_MAX_YMINTERVAL);
        return OG_ERROR;
    }

    *res = a + b;
    if (*res >= OG_MIN_YMINTERVAL && *res <= OG_MAX_YMINTERVAL) {
        return OG_SUCCESS;
    }
...
#define CM_CHECK_ITVL_FIELD(val, MAX_ISO_VALUE, field_name)                        \
    if (fabs((val)) >= (double)((MAX_ISO_VALUE) + 1)) {                            \
        OG_THROW_ERROR(ERR_INVALID_INTERVAL_FIELD, (field_name), (MAX_ISO_VALUE)); \
        return OG_ERROR;                                                           \
    }
...
static inline status_t cm_year2yminterval(double year, interval_ym_t *ymitvl)
{
    ...
    */
    if ((fabs(*ymitvl) / ITVL_MONTHS_PER_YEAR) >= (ITVL_MAX_ISO_YEAR + 1)) {
        OG_THROW_ERROR(ERR_INVALID_INTERVAL_FIELD, "YEAR", (ITVL_MAX_ISO_YEAR));
        return OG_ERROR;
    }
