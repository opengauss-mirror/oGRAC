...
{
    OG_RETURN_IFERR(vmc_alloc(owner, size, buf));
    if (size != 0) {
...
{
    if (mpool_create(&area->marea, "variant memory pool", init_pages, area->marea.page_count, &pool->mpool) != OG_SUCCESS) {
        return OG_ERROR;
    }
    
    if (mpool_create(&area->large_marea, "large variant memory pool", 0,
                     area->large_marea.page_count, &pool->large_mpool) != OG_SUCCESS) {
