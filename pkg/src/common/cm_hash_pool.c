...
static status_t try_extend_free_list(cm_hash_pool_t *pool)
{
    ...
    if (pool->hwm >= HASH_MAX_SIZE(pool)) {
        OG_THROW_ERROR(ERR_TOO_MANY_OBJECTS, HASH_MAX_SIZE(pool), HASH_NAME(pool));
        return OG_ERROR;
    }
    uint32 size = (ENTRY_SIZE(pool) + HASH_ITEM_SIZE) * HASH_MEM_EXTENT_SIZE;
    extent = (cm_hash_item_t *)malloc(size);
    if (extent == NULL) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, pool->profile.name);
        return OG_ERROR;
    }
...
static status_t remove_from_free_list(cm_hash_pool_t *pool, cm_hash_item_t **item)
{
    ...
    if (free_list->count == 0) {
        if (try_extend_free_list(pool) != OG_SUCCESS) {
...
status_t cm_hash_pool_create(cm_hash_profile_t *profile, cm_hash_pool_t *pool)
{
    ...
    if (pool->buckets == NULL) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, mem_size, profile->name);
        return OG_ERROR;
    }
...
void *cm_hash_pool_match_lock(cm_hash_pool_t *pool, void *data)
{
    ...
    item = find_in_hash_bucket(pool, bucket, data, OG_TRUE);
    if (item != NULL) {
...
status_t cm_hash_pool_add(cm_hash_pool_t *pool, void *data)
{
    ...
    cm_spin_lock(&bucket->lock, NULL);
    item = find_in_hash_bucket(pool, bucket, data, OG_FALSE);
    if (item != NULL) {
        cm_spin_unlock(&bucket->lock);
        return OG_ERROR;
    }

    if (remove_from_free_list(pool, &item) != OG_SUCCESS) {
        cm_spin_unlock(&bucket->lock);
        return OG_ERROR;
    }
