 ...
 * this function would return OG_ERROR and report an error
 */
status_t cm_text2tzoffset(text_t *text, timezone_info_t *tz)
{
    ...
    switch (retval) {
        case NERR_SUCCESS:
            break;
        case NERR_ERROR:
            OG_THROW_ERROR(ERR_VALUE_ERROR, "string does not match the timezone offset format");
            return OG_ERROR;
        case NERR_UNEXPECTED_CHAR:
            OG_THROW_ERROR(ERR_VALUE_ERROR, "unrecognized character in the specified timezone offset");
            return OG_ERROR;
        case NERR_OVERFLOW:
            OG_THROW_ERROR(ERR_VALUE_ERROR,
                "invalid time zone offset. time zone offset must be between -12:00 and 14:00");
            return OG_ERROR;
        default:
            OG_THROW_ERROR(ERR_VALUE_ERROR, "unexpected return value");
            return OG_ERROR;
    }
...
int16 cm_get_local_tzoffset(void)
{
    ...
    if (GetTimeZoneInformation(&tmp) == TIME_ZONE_ID_INVALID) {
        OG_LOG_RUN_WAR("error occurred during calling GetTimeZoneInformation(). errno: \"%lu\"", GetLastError());
    } else {
    ...
    if (localtime_r(&utc_time, &result) != NULL) {
        ...
        OG_LOG_RUN_WAR("error occurred during calling localtime_r(). errno: \"%d\"", errno);
    }
