    ...
    uint32 total_count, uint32 size, bool32 is_compress)
{
    ...
    if (vm_open(session, vm_pool, vmid, &page) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (is_compress) {
        ...
        array_head->offset = dir_page_count * OG_VMEM_PAGE_SIZE;
    }
...
{
    /* A: suppose that dir_size is (x.y * (OG_VMEM_PAGE_SIZE - sizeof(array_head_t)) + n.m * OG_VMEM_PAGE_SIZE),
    ...
    for (uint32 offset = 0; offset < curr_dir_count; offset++) {
        ...
        if (dir->offset != ELEMENT_NULL_OFFSET) {
            if (is_compress) {
                dir->offset -= (OG_VMEM_PAGE_SIZE - ctrl_size % OG_VMEM_PAGE_SIZE);
            } else {
                dir->offset += (OG_VMEM_PAGE_SIZE - ctrl_size % OG_VMEM_PAGE_SIZE);
            }
        }
    ...
    uint32 size, uint32 total_dir_cnt, bool32 compress)
{
    ...
    OG_RETURN_IFERR(array_modify_head_offset(session, vm_pool, vmid, total_dir_cnt, size, compress));
    while (dir_cnt != total_dir_cnt) {
        CM_ASSERT(vmid != OG_INVALID_ID32);
        OG_RETURN_IFERR(vm_open(session, vm_pool, vmid, &page));
        
        if (vmid == vlob->entry_vmid) {
            cur_dir_cnt = array_modify_dir_offset(page->data, dir_cnt, ctrl_size, total_dir_cnt, OG_TRUE, compress);
        } else {
            cur_dir_cnt = array_modify_dir_offset(page->data, dir_cnt, ctrl_size, total_dir_cnt, OG_FALSE, compress);
        }
        dir_cnt += cur_dir_cnt;
...
{
    if (vm_alloc_and_append(aa->session, aa->pool, aa->list) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
*/
static status_t array_extend_dir_page(array_assist_t *aa, vm_lob_t *vlob)
{
    ...
    if (vlob->entry_vmid == OG_INVALID_ID32) {
        ...
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "array directory extend failed since page not init or alloc");
        return OG_ERROR;
    }
    ...
    if (vm_open(aa->session, aa->pool, vlob->entry_vmid, &page) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    head->offset += OG_VMEM_PAGE_SIZE;
    head->size += OG_VMEM_PAGE_SIZE;
    vlob->size += OG_VMEM_PAGE_SIZE;
    vm_close(aa->session, aa->pool, vlob->entry_vmid, VM_ENQUE_TAIL);
    return OG_SUCCESS;
}

static status_t array_update_dir_offset(array_assist_t *aa, vm_lob_t *vlob)
{
    array_head_t head;
    vm_page_t *page = NULL;
    elem_dir_t *dir = NULL;
    bool32 switch_page = OG_FALSE;

    OG_RETURN_IFERR(vm_open(aa->session, aa->pool, vlob->entry_vmid, &page));
    head = *(array_head_t *)(page->data);
    ...
    while (count < head.count) {
        if (switch_page) {
            ...
            OG_RETURN_IFERR(vm_open(aa->session, aa->pool, vmid, &page));
            dir = (elem_dir_t *)page->data;
            switch_page = OG_FALSE;
        }

        dir->offset += OG_VMEM_PAGE_SIZE;
        dir++;
        count++;

        if ((char *)dir == page->data + OG_VMEM_PAGE_SIZE) {
            switch_page = OG_TRUE;
        }
    }
...
uint32 array_get_vmid_by_offset(array_assist_t *aa, vm_lob_t *vlob, uint32 offset)
{
    ...
    count = offset / OG_VMEM_PAGE_SIZE;
    vmid = vlob->entry_vmid;
    while (count > 0 && vmid != OG_INVALID_ID32) {
        vmid = vm_get_ctrl(aa->pool, vmid)->sort_next;
        count--;
    }
...
*/
static uint32 array_get_free_bytes(array_assist_t *aa, vm_lob_t *vlob)
{
    uint32 free_bytes;
    vm_page_t *page = NULL;
    array_head_t *head = NULL;
    ...
    head = (array_head_t *)(page->data);
    if (head->size > 0 && (head->size % OG_VMEM_PAGE_SIZE == 0)) {
        free_bytes = 0;
    } else {
        free_bytes = OG_VMEM_PAGE_SIZE - head->size % OG_VMEM_PAGE_SIZE;
    }
    vm_close(aa->session, aa->pool, vlob->entry_vmid, VM_ENQUE_TAIL);

    /* allocate next page */
    if (free_bytes == 0) {
        if (array_extend_vm_page(aa, vlob) != OG_SUCCESS) {
            return 0;
        }

        free_bytes = OG_VMEM_PAGE_SIZE;
    }

    return free_bytes;
}

static status_t array_get_value_offset(array_assist_t *aa, vm_lob_t *vlob, uint32 *offset)
{
    array_head_t *head = NULL;
    vm_page_t *page = NULL;
...
static status_t array_get_value_vmid(array_assist_t *aa, vm_lob_t *vlob, uint32 *vmid, uint32 *offset)
{
    if (array_get_value_offset(aa, vlob, offset) != OG_SUCCESS) {
        return OG_ERROR;
    }

    *vmid = array_get_vmid_by_offset(aa, vlob, *offset);
    if (*vmid == OG_INVALID_ID32) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot get array value page");
        return OG_ERROR;
    }
...
static uint32 array_get_last_dir_vmid(array_assist_t *aa, vm_lob_t *vlob)
{
    uint32 vmid;
    uint32 dir_page_cnt;
    array_head_t head;
    ...
    vmid = vlob->entry_vmid;
    while (dir_page_cnt > 1 && vmid != OG_INVALID_ID32) {
        dir_page_cnt--;
        vmid = vm_get_ctrl(aa->pool, vmid)->sort_next;
    }
...
static status_t array_get_last_dir_offset(array_assist_t *aa, vm_lob_t *vlob, uint32 *offset)
{
    array_head_t *head = NULL;
    vm_page_t *page = NULL;
...
status_t array_get_dimension(array_assist_t *aa, vm_lob_t *vlob, uint32 *dimension)
{
    ...
    if (vmid == OG_INVALID_ID32) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot get array element dimension");
        return OG_ERROR;
    }

    if (array_get_last_dir_offset(aa, vlob, &offset) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (offset == 0) {
        *dimension = 0;
        ...
        if (vm_open(aa->session, aa->pool, vmid, &page) != OG_SUCCESS) {
            return OG_ERROR;
        }

        dir = (elem_dir_t *)(page->data + offset % OG_VMEM_PAGE_SIZE);
...
static status_t array_inc_head_size(array_assist_t *aa, vm_lob_t *vlob, uint32 inc)
{
    array_head_t *head = NULL;
    vm_page_t *page = NULL;
...
static status_t array_inc_head_count(array_assist_t *aa, vm_lob_t *vlob, uint32 inc)
{
    array_head_t *head = NULL;
    vm_page_t *page = NULL;
...
static status_t array_append_directory(array_assist_t *aa, uint32 subscript, uint32 size,
                                bool8 is_null, bool32 last, vm_lob_t *vlob)
{
    ...
    if (array_get_last_dir_end(aa, vlob, &aa->dir_curr) != OG_SUCCESS) {
        return OG_ERROR;
    }

    /* need extend pages for element directory if the current page has not enough space */
    if (aa->dir_curr > 0 && aa->dir_curr % OG_VMEM_PAGE_SIZE == 0) {
        if (array_extend_dir_page(aa, vlob) != OG_SUCCESS) {
            return OG_ERROR;
        }

        // update offset in old directory
        OG_RETURN_IFERR(array_update_dir_offset(aa, vlob));
    }
    ...
    if (aa->dir_curr > sizeof(elem_dir_t)) {
        ...
        if (dir_vmid == OG_INVALID_ID32) {
            OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot get array prev element directory");
            return OG_ERROR;
        }

        if (vm_open(aa->session, aa->pool, dir_vmid, &page) != OG_SUCCESS) {
            return OG_ERROR;
        }

        dir = (elem_dir_t *)(page->data + (aa->dir_curr - sizeof(elem_dir_t)) % OG_VMEM_PAGE_SIZE);
        dir->last = OG_FALSE;
    ...
    if (dir_vmid == OG_INVALID_ID32) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot get array directory page");
        return OG_ERROR;
    }
    ...
    dir = (elem_dir_t *)(page->data + aa->dir_curr % OG_VMEM_PAGE_SIZE);
    /* set subscript */
    dir->subscript = (int32)subscript;
    /* set offset */
    if (array_get_value_offset(aa, vlob, &dir->offset) != OG_SUCCESS) {
        vm_close(aa->session, aa->pool, dir_vmid, VM_ENQUE_TAIL);
        return OG_ERROR;
    }
...
static status_t array_append_value(array_assist_t *aa, char *value, uint32 size_input, vm_lob_t *vlob)
{
    errno_t err;
    uint32 free_bytes;
    uint32 write_bytes;
    ...
    while (size > 0) {
        ...
        if (free_bytes == 0) {
            OG_THROW_ERROR(ERR_NO_FREE_VMEM, "can't allocate page from virtual memory pool");
            return OG_ERROR;
        }

        if (array_get_value_vmid(aa, vlob, &val_vmid, &val_offset) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (vm_open(aa->session, aa->pool, val_vmid, &page) != OG_SUCCESS) {
            return OG_ERROR;
        }

        write_bytes = MIN(free_bytes, size);
        val_addr = page->data + val_offset % OG_VMEM_PAGE_SIZE;
        ...
        if (array_inc_head_size(aa, vlob, write_bytes) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
{
    if (array_append_directory(aa, subscript, size, is_null, last, vlob) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (array_inc_head_count(aa, vlob, 1) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static inline status_t cm_check_bufsize_and_set_stat(uint32 buf_size, uint32 need_len, status_t* status)
{
    if (buf_size < need_len + 1) {
        OG_THROW_ERROR(ERR_BUFFER_OVERFLOW, need_len + 1, buf_size);
        *status = OG_ERROR;
    } else {
        *status = OG_SUCCESS;
    }
...
static status_t cm_element_as_string(array_assist_t *aa, const nlsparams_t *nls, var_array_t *var,
                              elem_dir_t *dir, text_t *text, uint32 max_buf_size)
{
    ...
    switch (var->type) {
        ...
        case OG_TYPE_INTEGER:
            OG_BREAK_IF_ERROR(cm_check_bufsize_and_set_stat(buf_size, OG_MAX_INT32_STRLEN, &status));
            cm_int2text(*(int32 *)data, &ele_text);
            break;

        case OG_TYPE_BOOLEAN:
            OG_BREAK_IF_ERROR(cm_check_bufsize_and_set_stat(buf_size, OG_MAX_BOOL_STRLEN, &status));
            cm_bool2text(*(bool32 *)data, &ele_text);
            break;

        case OG_TYPE_BIGINT:
            OG_BREAK_IF_ERROR(cm_check_bufsize_and_set_stat(buf_size, OG_MAX_INT64_STRLEN, &status));
            cm_bigint2text(*(int64 *)data, &ele_text);
            break;

        case OG_TYPE_REAL:
            OG_BREAK_IF_ERROR(cm_check_bufsize_and_set_stat(buf_size, OG_MAX_REAL_OUTPUT_STRLEN, &status));
            cm_real2text(*(double *)data, &ele_text);
            break;

        case OG_TYPE_NUMBER:
        case OG_TYPE_DECIMAL:
            status = cm_dec4_to_text((const dec4_t *)data, OG_MAX_DEC_OUTPUT_PREC, &ele_text);
            break;
        case OG_TYPE_NUMBER2:
            cm_dec2_copy_ex(&d2, (const payload_t *)data, (uint8)dir->size);
            status = cm_dec2_to_text(&d2, OG_MAX_DEC_OUTPUT_PREC, &ele_text);
            break;
        case OG_TYPE_DATE:
...
status_t cm_array2text(const nlsparams_t *nls, var_array_t *var, text_t *text)
{
    ...
    OG_RETURN_IFERR(cm_concat_string(text, max_buf_size, "{"));
    while (dir_start < dir_end && dir_vmid != OG_INVALID_ID32) {
        if (switch_page && vm_open(aa.session, aa.pool, dir_vmid, &dir_page) != OG_SUCCESS) {
            return OG_ERROR;
        }
        dir = (elem_dir_t *)(dir_page->data + dir_start % OG_VMEM_PAGE_SIZE);
        /* convert element value to string */
        if (cm_element_as_string(&aa, nls, var, dir, text, max_buf_size) != OG_SUCCESS) {
            vm_close(aa.session, aa.pool, dir_vmid, VM_ENQUE_TAIL);
            return OG_ERROR;
        }

        if (dir_start + sizeof(elem_dir_t) < dir_end) {
            if (cm_concat_string(text, max_buf_size, ",") != OG_SUCCESS) {
                vm_close(aa.session, aa.pool, dir_vmid, VM_ENQUE_TAIL);
                return OG_ERROR;
            }
        }
        ...
        curr_vmid = dir_vmid;
        if (dir_start % OG_VMEM_PAGE_SIZE + sizeof(elem_dir_t) >= OG_VMEM_PAGE_SIZE) {
        ...
        if (switch_page == OG_TRUE || dir_start >= dir_end || dir_vmid == OG_INVALID_ID32) {
            vm_close(aa.session, aa.pool, curr_vmid, VM_ENQUE_TAIL);
        }
    }

    return cm_concat_string(text, max_buf_size, "}");
}
...
status_t array_init(array_assist_t *aa, handle_t session, vm_pool_t *pool, id_list_t *list, vm_lob_t *vlob)
{
    ...
    if (vm_open(session, pool, vlob->entry_vmid, &page) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    aa->head->size = OG_VMEM_PAGE_SIZE;
    aa->head->count = 0;
    aa->head->offset = OG_VMEM_PAGE_SIZE;
                            ...
                            uint32 *id, uint32 *offset, array_search_mode mode)
{
    ...
    if (vm_open(aa->session, aa->pool, src_lob->entry_vmid, &page) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    for (uint32 i = 1; i <= dir_page_count; i++) {
        ...
        last_dir_page = ((i == dir_page_count) ? OG_TRUE : OG_FALSE);
...
static status_t cm_find_first_dir(array_assist_t *aa, vm_lob_t *src_lob, int subscript, uint32 *id, uint32 *offset)
{
    ...
    if (subscript == OG_INVALID_ID32 || subscript == 1) {
        if (array_get_element_count(aa, src_lob, &count) != OG_SUCCESS) {
            return OG_ERROR;
        }
                                   ...
                                   array_assist_t *dst_aa, vm_lob_t *dst_lob, uint32 new_subscript)
{
    ...
    /* copy dir */
    if (array_append_directory(dst_aa, new_subscript, dir->size, ELEMENT_IS_NULL(dir), OG_TRUE,
                               dst_lob) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (array_inc_head_count(dst_aa, dst_lob, 1) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (val_vmid == OG_INVALID_ID32) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot get next page to store array element value");
        return OG_ERROR;
    }
    ...
    offset = dir->offset;
    while (size > 0) {
        if (vm_open(src_aa->session, src_aa->pool, val_vmid, &val_page) != OG_SUCCESS) {
            return OG_ERROR;
        }

        value = val_page->data + offset % OG_VMEM_PAGE_SIZE;
        copy_size = MIN(size, OG_VMEM_PAGE_SIZE - offset % OG_VMEM_PAGE_SIZE);
        if (array_append_value(dst_aa, value, copy_size, dst_lob) != OG_SUCCESS) {
...
static status_t cm_get_elements_value(subarray_assist_t *sa)
{
    ...
    bool32 swith_page = OG_TRUE;
    uint32 curr_vmid;
    uint32 new_subscript = 1; /* temp array subscript should start with 1 */

    while (sa->dir_vmid != OG_INVALID_ID32) {
        if (swith_page) {
            if (vm_open(session, pool, sa->dir_vmid, &dir_page) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
        ...
        if (cm_subscript_in_range(dir->subscript, sa->start, sa->end)) {
            if (array_copy_element(sa->src_aa, sa->src_vlob, dir,
                                   &sa->dst_aa, sa->dst_vlob, new_subscript) != OG_SUCCESS) {
        ...
        sa->dir_offset = (sa->dir_offset + sizeof(elem_dir_t)) % OG_VMEM_PAGE_SIZE;
        if (sa->dir_offset == 0) {
        ...
        if (swith_page || sa->dir_vmid == OG_INVALID_ID32) {
...
status_t array_get_subarray(array_assist_t *aa, vm_lob_t *src_lob, vm_lob_t *dst_lob, int32 start, int32 end)
{
    ...
    if (array_init(&sa.dst_aa, aa->session, aa->pool, aa->list, dst_lob) != OG_SUCCESS) {
        return OG_ERROR;
    }

    /* try to find the start element's directory in the vm lob, return page id & directory offset */
    if (cm_find_first_dir(aa, src_lob, start, &sa.dir_vmid, &sa.dir_offset) != OG_SUCCESS) {
        return OG_ERROR;
    } else if (sa.dir_vmid != OG_INVALID_ID32) {
        sa.src_aa = aa;
        sa.src_vlob = src_lob;
        sa.dst_vlob = dst_lob;
        sa.start = start;
...
status_t array_get_value_by_dir(array_assist_t *aa, char *buf, uint32 size_input, vm_lob_t *vlob, elem_dir_t *dir)
{
    errno_t err;
    uint32 remain_size;
    uint32 copy_size;
    ...
    if (val_vmid == OG_INVALID_ID32) {
        OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot get array element value");
        return OG_ERROR;
    }
    ...
    ele_val = page->data + dir->offset % OG_VMEM_PAGE_SIZE;
    remain_size = dir->size;

    if (remain_size > size) {
        vm_close(aa->session, aa->pool, val_vmid, VM_ENQUE_TAIL);
        OG_THROW_ERROR(ERR_BUFFER_OVERFLOW, dir->size, size);
        return OG_ERROR;
    }

    copy_size = MIN(remain_size, OG_VMEM_PAGE_SIZE - dir->offset % OG_VMEM_PAGE_SIZE);
    while (copy_size > 0) {
        ...
        buf += copy_size;
        size -= copy_size;
        copy_size = MIN(OG_VMEM_PAGE_SIZE, remain_size);
        if (copy_size > 0) {
...
status_t array_get_element_info(array_assist_t *aa, uint32 *size, uint32 *offset, vm_lob_t *vlob, uint32 subscript)
{
    ...
    if (cm_find_equal_dir(aa, vlob, (int)subscript, &dir_vmid, &dir_offset) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (vm_open(aa->session, aa->pool, dir_vmid, &dir_page) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t array_get_element_by_subscript(array_assist_t *aa, char *buf, uint32 size, vm_lob_t *vlob, uint32 subscript)
{
    ...
    if (array_get_value_by_dir(aa, buf, size, vlob, &dir) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
*/
static status_t array_update_element_by_dir(array_assist_t *aa, char *data, uint32 size,
                                     elem_dir_t *dir, vm_lob_t *vlob)
{
    ...
    if (dir->size >= size) {
        val_vmid = array_get_vmid_by_offset(aa, vlob, dir->offset);
        if (val_vmid == OG_INVALID_ID32) {
            OG_THROW_ERROR(ERR_ASSERT_ERROR, "cannot update array element value");
            return OG_ERROR;
        }

        if (vm_open(aa->session, aa->pool, val_vmid, &val_page) != OG_SUCCESS) {
            return OG_ERROR;
        }

        ret = memcpy_sp(val_page->data + dir->offset % OG_VMEM_PAGE_SIZE, size, data, size);
        if (ret != EOK) {
            vm_close(aa->session, aa->pool, val_vmid, VM_ENQUE_TAIL);
        ...
        */
        ...
        if (array_append_value(aa, data, size, vlob) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
                               ...
                               bool32 first, bool32 last, uint32 offset)
{
    ...
    if (end + sizeof(elem_dir_t) > OG_VMEM_PAGE_SIZE) {
        if (last) {
            ...
            if (array_update_dir_offset(aa, vlob) != OG_SUCCESS) {
                vm_close(aa->session, aa->pool, vmid, VM_ENQUE_TAIL);
                return OG_ERROR;
            }
        ...
        if (vm_open(aa->session, aa->pool, next_vmid, &next_page) != OG_SUCCESS) {
            vm_close(aa->session, aa->pool, vmid, VM_ENQUE_TAIL);
            return OG_ERROR;
        }
...
static status_t array_insert_element(array_assist_t *aa, char *data, uint32 size, uint32 subscript, vm_lob_t *vlob,
                              elem_dir_t *dir, uint32 dir_vmid, uint32 dir_offset)
{
    /* insert a new directory before the directory found */
    if (array_shift_dir(aa, vlob, dir_vmid, dir_offset) != OG_SUCCESS) {
        return OG_ERROR;
    }
                                           ...
                                           uint32 subscript, vm_lob_t *vlob)
{
    ...
    if (cm_find_first_dir(aa, vlob, (int)subscript, &dir_vmid, &dir_offset) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    */
    if (dir_vmid == OG_INVALID_ID32) {
        return array_append_element(aa, subscript, data, size, is_null, OG_TRUE, vlob);
    }
    ...
    dir = (elem_dir_t *)(page->data + dir_offset % OG_VMEM_PAGE_SIZE);
    if (subscript == (uint32)dir->subscript) {
        if (array_update_element_by_dir(aa, data, size, dir, vlob) != OG_SUCCESS) {
            vm_close(aa->session, aa->pool, dir_vmid, VM_ENQUE_TAIL);
            return OG_ERROR;
        }
    } else if (subscript < (uint32)dir->subscript) {
        /* insert a new directory before the directory found */
        if (array_insert_element(aa, data, size, subscript, vlob, dir, dir_vmid, dir_offset) != OG_SUCCESS) {
            vm_close(aa->session, aa->pool, dir_vmid, VM_ENQUE_TAIL);
            return OG_ERROR;
        }
    }
...
{
    bool32 complete = OG_TRUE;
    bool32 has_quote = OG_FALSE;
...
status_t array_convert_inline_lob(handle_t session, vm_pool_t *pool, var_array_t *v, char *buf, uint32 buf_len)
{
    ...
    while (vmid != OG_INVALID_ID32) {
        OG_RETURN_IFERR(vm_open(session, pool, vmid, &page));
        copy_size = MIN(remain_size, OG_VMEM_PAGE_SIZE);
    ...
    for (i = 0; i < v->count; i++) {
        ...
        /* if the element is null, can not update the offset value */
        if (dir->offset != ELEMENT_NULL_OFFSET) {
            dir->offset -= (OG_VMEM_PAGE_SIZE - (ctrl_size + sizeof(array_head_t)) % OG_VMEM_PAGE_SIZE);
        }
    }
