...
// 8 bytes align
#define OG_PUSH_RESERVE_SIZE 8
#define OG_PUSH_OFFSET_POS 4
...
{
    uint32 last_offset;
    uint32 actual_size = CM_ALIGN8(size) + OG_PUSH_RESERVE_SIZE;
    uint8 *ptr = stack->buf + stack->push_offset - actual_size + OG_PUSH_RESERVE_SIZE;
...
static inline void cm_pop(cm_stack_t *stack)
{
    ...
    stack->push_offset = *(uint32 *)(stack->buf + stack->push_offset + OG_PUSH_OFFSET_POS);
...
static inline void cm_keep_stack_variant(cm_stack_t *stack, char *buf)
{
    ...
    if (buf < (char *)(stack->buf + stack->heap_offset) ||
        buf >= (char *)stack->buf + stack->push_offset + OG_PUSH_RESERVE_SIZE) {
        return;
    }

    stack->push_offset = (uint32)(buf - (char *)stack->buf - OG_PUSH_RESERVE_SIZE);
...
#define OG_MAX_TEST_HEAP_DEPTH  (uint32)256
#define OG_MAX_TEST_STACK_DEPTH (uint32)1024

typedef struct st_stack {
    uint8 *buf;
    uint32 push_offset;
    uint32 heap_offset;
    void *heap_addr[OG_MAX_TEST_HEAP_DEPTH];
    uint32 push_depth;
    void *stack_addr[OG_MAX_TEST_STACK_DEPTH];
} cm_stack_t;

inline void cm_stack_reset(cm_stack_t *stack)
{
    uint32 i;
    for (i = 0; i < OG_MAX_TEST_STACK_DEPTH && stack->stack_addr[i] != NULL; ++i) {
...
static inline void *cm_push(cm_stack_t *stack, uint32 size)
{
    ...
    if (stack->push_depth + 1 > OG_MAX_TEST_STACK_DEPTH) {
        return NULL;
    }
    if (size == 0) {
        return NULL;
...
static inline status_t cm_stack_alloc(void *owner, uint32 size, void **ptr)
{
    ...
    if (stack->heap_offset + 1 > OG_MAX_TEST_HEAP_DEPTH) {
        *ptr = NULL;
        return OG_ERROR;
    }
    if (size == 0) {
...
static inline void cm_keep_stack_variant(cm_stack_t *stack, char *buf)
{
    ...
    for (i = 0; stack->stack_addr[i] != NULL && i < OG_MAX_TEST_STACK_DEPTH; i++) {
