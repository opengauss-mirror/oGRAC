...
#define OG_MAX_TUPLE_SIZE 10
#define OG_MAX_INT64_SIZE 22
/* A tuple of continuous words with size */
typedef struct {
    uint32 size;
    char *words[OG_MAX_TUPLE_SIZE];
} word_tuple_t;
...
#define OG_MAX_NUM_PART_BUFF (OG_MAX_DEC_OUTPUT_ALL_PREC)

typedef struct st_digitext {
    char str[OG_MAX_NUM_PART_BUFF];
    uint32 len;
} digitext_t;
...
static inline status_t cm_text_copy(text_t *dest, uint32 buf_len, text_t *src)
{
    if (src->len != 0) {
 ...
 * exceeds the maximal buf size return OG_FALSE; else return OG_TRUE
 */
...
{
    (void)cm_text2str(num_text, dig_text->str, OG_MAX_NUM_PART_BUFF);
...
static inline void cm_int2text(int32 value, text_t *text)
{
    ...
    iret_snprintf = snprintf_s(text->str, OG_MAX_INT32_STRLEN + 1, OG_MAX_INT32_STRLEN, PRINT_FMT_INTEGER, value);
    if (iret_snprintf == -1) {
...
static inline void cm_uint32_to_text(uint32 value, text_t *text)
{
    int iret_snprintf;
    CM_POINTER(text);
    iret_snprintf = snprintf_s(text->str, OG_MAX_UINT32_STRLEN + 1, OG_MAX_UINT32_STRLEN, PRINT_FMT_UINT32, value);
    if (iret_snprintf == -1) {
        OG_THROW_ERROR(ERR_SYSTEM_CALL, (iret_snprintf));
        return;
    }
...
static inline void cm_bool2text(bool32 value, text_t *text)
{
    int iret_snprintf;
    CM_POINTER(text);
    iret_snprintf = snprintf_s(text->str, OG_MAX_BOOL_STRLEN + 1, OG_MAX_BOOL_STRLEN, value ? "TRUE" : "FALSE");
    if (iret_snprintf == -1) {
        OG_THROW_ERROR(ERR_SYSTEM_CALL, (iret_snprintf));
        return;
    }
...
static inline void cm_bigint2text(int64 value, text_t *text)
{
    ...
    CM_POINTER(text);
#ifdef WIN32
    iret_snprintf = snprintf_s(text->str, OG_MAX_INT64_STRLEN + 1, OG_MAX_INT64_STRLEN, PRINT_FMT_BIGINT, value);
    if (SECUREC_UNLIKELY(iret_snprintf == -1)) {
        OG_THROW_ERROR(ERR_SYSTEM_CALL, iret_snprintf);
        return OG_ERROR;
    }
#else
    iret_snprintf = snprintf_s(text->str, OG_MAX_INT64_STRLEN + 1, OG_MAX_INT64_STRLEN, PRINT_FMT_BIGINT,
                               (long long)value);
#endif
    if (iret_snprintf == -1) {
        OG_THROW_ERROR(ERR_SYSTEM_CALL, (iret_snprintf));
...
static inline void cm_uint64_to_text(uint64 value, text_t *text)
{
    int iret_snprintf;
    CM_POINTER(text);
#ifdef WIN32
    iret_snprintf = snprintf_s(text->str, OG_MAX_UINT64_STRLEN + 1, OG_MAX_UINT64_STRLEN, PRINT_FMT_UINT64, value);
    if (SECUREC_UNLIKELY(iret_snprintf == -1)) {
        OG_THROW_ERROR(ERR_SYSTEM_CALL, iret_snprintf);
        return OG_ERROR;
    }
#else
    iret_snprintf = snprintf_s(text->str, OG_MAX_UINT64_STRLEN + 1, OG_MAX_UINT64_STRLEN, PRINT_FMT_UINT64,
...
static inline void cm_real2text(double real_val, text_t *text)
{
    int iret_snprintf;
    CM_POINTER(text);
    CM_SNPRINTF_REAL(iret_snprintf, text->str, real_val, OG_MAX_REAL_OUTPUT_STRLEN + 1);
...
static inline status_t cm_text_dup(const text_t *src, text_t **dst)
{
    ...
    if (dst_path == NULL) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)sizeof(text_t), "duplicate text");
        return OG_ERROR;
    }
    ...
    if (src->len == 0) {
        CM_FREE_PTR(dst_path);
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)(src->len + 1), "duplicate text");
        return OG_ERROR;
    }
    ...
    if (cm_text2str(src, dst_path->str, src->len + 1) != OG_SUCCESS) {
        CM_FREE_PTR(dst_path->str);
        CM_FREE_PTR(dst_path);
        return OG_ERROR;
    }
