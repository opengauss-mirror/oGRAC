...
static bool32 volatile g_system_initialized = OG_FALSE;
static spinlock_t g_system_lock;
static char g_program_name[OG_FILE_NAME_BUFFER_SIZE + 1] = {0};
static char g_user_name[OG_NAME_BUFFER_SIZE] = {0};
static char g_host_name[OG_HOST_NAME_BUFFER_SIZE] = {0};
static char g_platform_name[OG_NAME_BUFFER_SIZE] = {0};
static uint64 g_process_id = 0;
static uint32 g_nprocs = 0;

static void cm_get_host_name()
{
    (void)gethostname(g_host_name, OG_HOST_NAME_BUFFER_SIZE);
}
...
static void cm_get_program_name(void)
{
...
#ifdef WIN32
    len = GetModuleFileName(NULL, g_program_name, OG_FILE_NAME_BUFFER_SIZE);
#elif defined(AIX)
    pid_t pid;
    ...
    while (getprocs64(&processInfo, sizeof(processInfo), 0, 0, &pid, 1) > 0) {
        ...
        {
            len = getargs(&processInfo, sizeof(processInfo), g_program_name, OG_FILE_NAME_BUFFER_SIZE);
            break;
        }
    }
#else /* linux */
    len = readlink("/proc/self/exe", g_program_name, OG_FILE_NAME_BUFFER_SIZE);
    if (len > 0) {
    ...
    if (len == 0) {
        OG_THROW_ERROR(ERR_INIT_SYSTEM, cm_get_os_error());
        PRTS_RETVOID_IFERR(snprintf_s(g_program_name, OG_FILE_NAME_BUFFER_SIZE, OG_FILE_NAME_BUFFER_SIZE - 1,
                                      "<empty>"));
    }
}
...
{
#ifdef WIN32
    uint32 size = OG_NAME_BUFFER_SIZE;
    ...
    if (strncpy_s(g_user_name, OG_NAME_BUFFER_SIZE, pw->pw_name, name_len) != EOK) {
        OG_LOG_RUN_ERR("Secure C lib has thrown an error while getting user name");
        return;
    }
#endif
}
...
static void cm_get_platform_name(void)
{
    ...
    if (strncpy_s(g_platform_name, OG_NAME_BUFFER_SIZE, platform_name, strlen(platform_name)) != EOK) {
        OG_LOG_RUN_ERR("Secure C lib has thrown an error while getting platform name");
        return;
    }
#else
...
status_t cm_get_file_host_name(char *path, char *host_name)
{
    ...
    name_len = strlen(pw->pw_name);
    MEMS_RETURN_IFERR(strncpy_s(host_name, OG_NAME_BUFFER_SIZE, pw->pw_name, name_len));
