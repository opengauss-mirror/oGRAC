...
bool32 g_vma_mem_check = OG_FALSE;
static const cpid_t g_invalid_cpid = { .pool_id = OG_INVALID_ID32, .cached_page_id = OG_INVALID_ID32 };
#define INVALID_CPID g_invalid_cpid
#define IS_INVALID_CPID(cpid) ((cpid).pool_id == OG_INVALID_ID32 || (cpid).cached_page_id == OG_INVALID_ID32)
...
static inline void vm_create_func_stack_core(vm_pool_t *pool, uint32 vmid)
{
    ...
    if (size <= OG_INIT_BLACK_BOX_DEPTH) {
        CM_FREE_PTR(stacks);
        return;
    }

    uint32 remain_size = OG_VM_FUNC_STACK_SIZE - 1;
    for (i = OG_INIT_BLACK_BOX_DEPTH; i < size; i++) {
        uint32 len = strlen(stacks[i]) + 2;
        if (len > remain_size) {
            break;
        }

        if (snprintf_s(pool->func_stacks[vmid]->stack + OG_VM_FUNC_STACK_SIZE - 1 - remain_size,
...
static inline void vm_create_func_stack(vm_pool_t *pool, uint32 vmid)
{
    ...
    if (pool->func_stacks == NULL) {
        ...
        if (need_size == 0 || need_size / g_vm_max_stack_count != sizeof(vm_func_stack_t *)) {
            OG_LOG_RUN_ERR("not enough memory");
            return;
        }
...
void test_memory_pool_maps(memory_pool_t *pool)
{
    ...
    for (i = 0; i < pool->free_pages.count - 1; i++) {
        next = pool->maps[prev];
        CM_ASSERT(next != OG_INVALID_ID32);
        prev = next;
    }
    CM_ASSERT(prev == pool->free_pages.last);

    CM_ASSERT(pool->maps[prev] == OG_INVALID_ID32);
    cm_spin_unlock(&pool->lock);
}
...
static void test_memory_area_maps(memory_area_t *area)
{
    ...
    CM_ASSERT(area->maps[prev] == OG_INVALID_ID32);
    cm_spin_unlock(&area->lock);
}
#endif  // DEBUG
...
void marea_attach(const char *name, char *buf, size_t size, uint32 page_size, memory_area_t *area)
{
    ...
    OG_INIT_SPIN_LOCK(area->lock);
    area->buf = buf;
    len = (uint32)strlen(name);
    CM_ASSERT(len < OG_NAME_BUFFER_SIZE);
    PRTS_RETVOID_IFERR(snprintf_s(area->name, OG_NAME_BUFFER_SIZE, (size_t)len, "%s", name));

    OG_RETVOID_IFTRUE(page_count == 0);

    area->offset = (uint64)page_count * (uint64)sizeof(uint32);
    ...
    area->free_pages.first = OG_INVALID_ID32;
    area->free_pages.last = OG_INVALID_ID32;

    for (uint32 i = 0; i < area->page_count - 1; i++) {
        area->maps[i] = i + 1;
    }

    area->maps[area->page_count - 1] = OG_INVALID_ID32;
}
...
status_t marea_create(const char *name, size_t size, uint32 page_size, memory_area_t *area)
{
    ...
    if (area_size == 0 || area_size / (size_t)page_size != page_count) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)0, name);
        return OG_ERROR;
    }

    char *buf = (char *)malloc(area_size);
    if (buf == NULL) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)area_size, name);
        return OG_ERROR;
    }
...
status_t marea_alloc_buf(memory_area_t *area, uint32 page_count, char **buf)
{
    ...
    if (area->offset + size > area->size) {
        OG_THROW_ERROR(ERR_ALLOC_GA_MEMORY, area->name);
        return OG_ERROR;
    }
...
status_t mpool_extend(memory_pool_t *pool, uint32 count, mem_extent_t *extent)
{
    if (!mpool_try_extend(pool, count, extent)) {
        OG_THROW_ERROR(ERR_ALLOC_GA_MEMORY, pool->name);
        return OG_ERROR;
    }
                      ...
                      memory_pool_t *pool)
{
    ...
    OG_INIT_SPIN_LOCK(pool->lock);
    pool->area = area;
    pool->buf = area->buf;
    pool->page_buf = area->page_buf;

    len = (uint32)strlen(name);
    if (len >= OG_NAME_BUFFER_SIZE) {
        OG_THROW_ERROR(ERR_BUFFER_OVERFLOW, len, OG_NAME_BUFFER_SIZE - 1);
        return OG_ERROR;
    }
    PRTS_RETURN_IFERR(snprintf_s(pool->name, OG_NAME_BUFFER_SIZE, (size_t)len, "%s", name));

    pool->free_pages.count = page_count;
    pool->maps = area->maps;
    pool->page_count = page_count;
    pool->page_size = (uint32)area->page_size;
    pool->opt_count = opt_count;

    if (page_count == 0) {
        pool->free_pages.first = OG_INVALID_ID32;
        pool->free_pages.last = OG_INVALID_ID32;
    } else {
        cm_spin_lock(&area->lock, NULL);
        pool->free_pages.first = area->page_hwm;
        pool->free_pages.last = area->page_hwm + page_count - 1;
        area->maps[pool->free_pages.last] = OG_INVALID_ID32;
        if (marea_alloc_buf(area, page_count, NULL) != OG_SUCCESS) {
            cm_spin_unlock(&area->lock);
            return OG_ERROR;
        }
        cm_spin_unlock(&area->lock);
...
void mpool_attach(const char *name, char *buf, int64 buf_size, uint32 page_size, memory_pool_t *pool)
{
    ...
    len = (uint32)strlen(name);
    CM_ASSERT(len < OG_NAME_BUFFER_SIZE);
    PRTS_RETVOID_IFERR(snprintf_s(pool->name, OG_NAME_BUFFER_SIZE, (size_t)len, "%s", name));

    pool->area = NULL;
    pool->buf = buf;
    pool->maps = (uint32 *)buf;
    pool->page_size = page_size;
    pool->page_count = (uint32)((buf_size - OG_MAX_ALIGN_SIZE_4K) / (page_size + sizeof(uint32)));
    pool->page_buf = buf + CM_CALC_ALIGN((uint64)pool->page_count * (uint64)sizeof(uint32), OG_MAX_ALIGN_SIZE_4K);
    ...
    pool->maps[pool->page_count - 1] = OG_INVALID_ID32;
}
...
void mpool_free(memory_pool_t *pool, uint32 pool_caches)
{
    ...
    if (pool_caches == 0 || area_free_pages < area->page_count * OG_VMA_LW_FACTOR) {
    ...
    area->maps[extra_pages.last] = OG_INVALID_ID32;
...
void mctx_init(memory_pool_t *pool, memory_context_t *context)
{
    ...
    context->pages.first = OG_INVALID_ID32;
    context->pages.last = OG_INVALID_ID32;
    context->alloc_pos = pool->page_size;
    context->curr_page_addr = NULL;
    context->curr_page_id = OG_INVALID_ID32;
}
...
status_t mctx_alloc_exhausted(memory_context_t *context, uint32 size, void **buf, uint32 *buf_size)
{
    if (!mctx_try_alloc_exhausted(context, size, buf, buf_size)) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, context->pool->name);
        return OG_ERROR;
    }
...
status_t mctx_copy_text2str(memory_context_t *context, const text_t *src, char **dst)
{
    ...
    if (mctx_alloc(context, src->len + 1, (void **)dst) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t mctx_copy_text(memory_context_t *context, const text_t *src, text_t *dst)
{
    ...
    if (mctx_alloc(context, src->len, (void **)&dst->str) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
{
    if (mctx_copy_text(context, (text_t *)src, (text_t *)dst) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (upper_name == OG_TRUE) {
        cm_text_upper(dst);
    }
    return OG_SUCCESS;
}
...
void vm_init_pool(vm_pool_t *pool, char *buf, int64 buf_size, const vm_swapper_t *swapper, vm_statis_t stat)
{
    ...
    pool->buffer = buf;
    pool->page_count = (uint32)((buf_size - OG_MAX_ALIGN_SIZE_4K) / (OG_VMEM_PAGE_SIZE + sizeof(vm_page_t)));
    ...
    pool->page_buffer = pool->buffer + CM_CALC_ALIGN(pool->page_count * (uint64)sizeof(vm_page_t),
                                                     OG_MAX_ALIGN_SIZE_4K);
    MEMS_RETVOID_IFERR(memset_sp(pool->map_pages, OG_MAX_VMEM_MAP_PAGES * sizeof(uint32), 0xFF,
                                 OG_MAX_VMEM_MAP_PAGES * sizeof(uint32)));
    pool->map_count = 1;
    pool->map_pages[0].pool_id = pool->pool_id;
    pool->map_pages[0].cached_page_id = 0;
    page = vm_get_page(pool, 0);
    MEMS_RETVOID_IFERR(memset_sp(page->data, OG_VMEM_PAGE_SIZE, 0, OG_VMEM_PAGE_SIZE));

    pool->ctrl_count = VM_CTRLS_PER_PAGE;
    pool->page_hwm = 1;
    pool->swapper = *swapper;
    pool->free_ctrls.first = OG_INVALID_ID32;
    pool->free_ctrls.last = OG_INVALID_ID32;
    pool->free_pages.first = OG_INVALID_ID32;
    pool->free_pages.last = OG_INVALID_ID32;
...
static inline void vm_init_ctrl(vm_ctrl_t *ctrl)
{
    ...
    ctrl->swid = OG_INVALID_ID64;
    ctrl->cipher_len = 0;
    ctrl->prev = OG_INVALID_ID32;
    ctrl->next = OG_INVALID_ID32;
    ctrl->sort_next = OG_INVALID_ID32;
    ctrl->free = OG_FALSE;
    ctrl->closed = OG_TRUE;
    ctrl->swapping = OG_FALSE;
    ctrl->lock = 0;
}
...
static inline void vm_remove_page_list_head(vm_pool_t *pool, id_list_t *list, vm_page_t *page)
{
    ...
    if (list->count == 0) {
        ...
        next_page->prev.cached_page_id = OG_INVALID_ID32;
    }

    page->next.cached_page_id = OG_INVALID_ID32;
    page->prev.cached_page_id = OG_INVALID_ID32;
}
...
static inline void vm_remove_page_from_list(vm_pool_t *pool, vm_list_t *list, vm_page_t *page)
{
    ...
    if (list->count == 0) {
        ...
        if (page->next.cached_page_id != OG_INVALID_ID32) {
        ...
        if (page->prev.cached_page_id != OG_INVALID_ID32) {
...
static inline void vm_enque_closed_page(handle_t session, vm_pool_t *pool, cpid_t id, vm_enque_mode_t mode)
{
    ...
    CM_ASSERT(page->next.cached_page_id == OG_INVALID_ID32 && page->prev.cached_page_id == OG_INVALID_ID32);
    vm_page_pool_t *page_pool = &pool->close_page_pools[id.cached_page_id % OG_VM_CLOSE_PAGE_LIST_CNT];
...
static status_t vm_alloc_from_close_page_pool(handle_t session, vm_pool_t *pool, vm_page_pool_t *page_pool, cpid_t *id)
{
    ...
    for (page_id = list->first;; page_id = page->next) {
        ...
        // ctrl is closed.
        if (ctrl->ref_num == 0) {
            CM_ASSERT(ctrl->cpid.cached_page_id != OG_INVALID_ID32);
            CM_ASSERT(ctrl->swid == OG_INVALID_ID64);
...
static status_t vm_alloc_close_page(handle_t session, vm_pool_t *pool, cpid_t *id)
{
    ...
    for (uint32 i = 0; i < OG_VM_CLOSE_PAGE_LIST_CNT; i++) {
        page_pool = &pool->close_page_pools[(pool_idx + i) % OG_VM_CLOSE_PAGE_LIST_CNT];
        (void)cm_atomic32_inc((atomic32_t *)&pool->close_pool_idx);
        if (page_pool->pages.count > 0) {
            cm_spin_lock(&page_pool->lock, NULL);
            if (page_pool->pages.count > 0) {
                if (vm_alloc_from_close_page_pool(session, pool, page_pool, id) == OG_SUCCESS) {
                    cm_spin_unlock(&page_pool->lock);
                    ...
                    ctrl = vm_get_ctrl(pool, page->vmid);
                    UNPROTECT_PAGE(page);
                    if (pool->swapper.out(session, page, &ctrl->swid, &ctrl->cipher_len) != OG_SUCCESS) {
...
static status_t vm_alloc_page(handle_t session, vm_pool_t *pool, cpid_t *id)
{
    ...
    if (vm_alloc_close_page(session, pool, id) != OG_SUCCESS) {
        OG_THROW_ERROR(ERR_NO_FREE_VMEM, "can not alloc page from close pages");
        return OG_ERROR;
    }
    return OG_SUCCESS;
}
...
static inline status_t vm_extend_ctrls(handle_t session, vm_pool_t *pool)
{
    ...
    if (pool->map_count + VM_MIN_CACHE_PAGES >= pool->page_count) {
        OG_THROW_ERROR(ERR_NO_FREE_VMEM, "no free buffer for materialized result set");
        cm_spin_unlock(&pool->lock);
        return OG_ERROR;
    }
    pool->extending_ctrls = OG_TRUE;
    cm_spin_unlock(&pool->lock);

    map_id = pool->ctrl_hwm / VM_CTRLS_PER_PAGE;
    if (vm_alloc_page(session, pool, &pool->map_pages[map_id]) != OG_SUCCESS) {
        return OG_ERROR;
    }
    page = vm_get_cpid_page(pool, pool->map_pages[map_id]);
    MEMS_RETURN_IFERR(memset_sp(page->data, OG_VMEM_PAGE_SIZE, 0, OG_VMEM_PAGE_SIZE));
    pool->ctrl_count += VM_CTRLS_PER_PAGE;
    pool->map_count++;
    CM_MFENCE;
    pool->extending_ctrls = OG_FALSE;
...
static inline vm_ctrl_t *vm_alloc_from_free_ctrls(vm_pool_t *pool, uint32 *id)
{
    vm_ctrl_t *ctrl = NULL;
...
status_t vm_alloc(handle_t session, vm_pool_t *pool, uint32 *id)
{
    ...
    cm_spin_lock(&pool->lock, NULL);
    if (pool->free_ctrls.count > 0) {
        ctrl = vm_alloc_from_free_ctrls(pool, id);
        cm_spin_unlock(&pool->lock);
        CM_ASSERT(ctrl->free);
        vm_init_ctrl(ctrl);
        pool->vm_stat(session, VM_STAT_ALLOC);
        return OG_SUCCESS;
    }
    ...
    if (pool->page_hwm == pool->page_count && pool->free_pages.count == 0 &&
        pool->get_swap_extents != OG_INVALID_ID32 &&
        pool->swap_count + RESERVED_SWAP_EXTENTS >= pool->get_swap_extents) {
        cm_spin_unlock(&pool->lock);
        OG_THROW_ERROR(ERR_NO_FREE_VMEM,
    ...
    if (pool->page_hwm == pool->page_count &&
        ...
        cm_spin_unlock(&pool->lock);
        OG_THROW_ERROR(ERR_NO_FREE_VMEM, "no available temporary buffer page");
        return OG_ERROR;
    }
    ...
    for (;;) {
        if (SECUREC_UNLIKELY(pool->ctrl_hwm == pool->ctrl_count)) {
            cm_spin_unlock(&pool->lock);
            if (vm_extend_ctrls(session, pool) != OG_SUCCESS) {
                return OG_ERROR;
            }
            cm_spin_lock(&pool->lock, NULL);
            continue;
...
status_t vm_alloc_and_append(handle_t session, vm_pool_t *pool, id_list_t *list)
{
    ...
    if (vm_alloc(session, pool, &id) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t vm_open(handle_t session, vm_pool_t *pool, uint32 id, vm_page_t **page)
{
    ...
    if (SECUREC_UNLIKELY(ctrl->free)) {
        ...
        OG_THROW_ERROR_EX(ERR_VM, "vm page is already free, vmid: %u", id);
        return OG_ERROR;
    }
    ...
    if (!IS_INVALID_CPID(ctrl->cpid)) {
        ...
        *page = vm_get_cpid_page(pool, ctrl->cpid);
        UNPROTECT_PAGE(*page);
        ctrl->ref_num = 1;
        ctrl->closed = OG_FALSE;
        cm_spin_unlock(&ctrl->lock);
        vm_deque_closed_page(session, pool, ctrl->cpid);
        CM_ASSERT((*page)->vmid == id);
        pool->vm_stat(session, VM_STAT_OPEN);
        pool->vm_stat(session, VM_STAT_END);
        return OG_SUCCESS;
    }

    // allocate one cached page
    if (vm_alloc_page(session, pool, &ctrl->cpid) != OG_SUCCESS) {
        ctrl->cpid = INVALID_CPID;
        cm_spin_unlock(&ctrl->lock);
        pool->vm_stat(session, VM_STAT_END);
        return OG_ERROR;
    ...
    *page = vm_get_cpid_page(pool, ctrl->cpid);
    ...
    (*page)->prev.cached_page_id = OG_INVALID_ID32;
    if (ctrl->swid != OG_INVALID_ID64) {
        if (pool->swapper.in(session, ctrl->swid, ctrl->cipher_len, *page) != OG_SUCCESS) {
            cm_spin_unlock(&ctrl->lock);
            vm_free_cpid_page(session, pool, ctrl, VM_ENQUE_HEAD);
            pool->vm_stat(session, VM_STAT_END);
            return OG_ERROR;
...
void *cm_realloc(void *ptr, size_t old_len, size_t new_len)
{
    if (new_len == 0 || new_len <= old_len) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)new_len, "reallocating memory");
        return NULL;
    }
    if (ptr == NULL) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)new_len, "reallocating memory");
...
static void protect_vm_init(void)
{
    g_pools_symbols = (pool_symbols_t *)malloc(sizeof(pool_symbols_t) * OG_MAX_TEMP_POOL_NUM);
    if (g_pools_symbols == NULL) {
        return;
    }
    errno_t ret = memset_sp(g_pools_symbols, sizeof(pool_symbols_t) * OG_MAX_TEMP_POOL_NUM, 0,
                            sizeof(pool_symbols_t) * OG_MAX_TEMP_POOL_NUM);
    if (ret != EOK) {
...
static pool_symbols_t* get_idle_pool_symbols(void)
{
    pool_symbols_t* pool_symbols = NULL;
    for (uint32 pool_id = 0; pool_id < OG_MAX_TEMP_POOL_NUM; pool_id++) {
...
static void _protech_vm_print_page_stack(vm_pool_t*  pool, uint32 id, void** stacks)
{
    vm_page_t* page = vm_get_page(pool, id);
    if (page == NULL) {
        OG_LOG_BLACKBOX("get vmpage by page id[%u] is null\r\n", id);
        return;
    }
    OG_LOG_BLACKBOX("page(%u,%u,%u,%u)\r\n", id, page->vmid, page->next.cached_page_id,
                    page->prev.cached_page_id);

    vm_ctrl_t *ctrl = vm_get_ctrl(pool, page->vmid);
    if (ctrl == NULL) {
        OG_LOG_BLACKBOX("get ctrl by vmid[%u] is null\r\n", page->vmid);
        return;
    }

    OG_LOG_BLACKBOX("ctrl(%llu,%u,%u,%u,%u,%u,%u,%u,%u,%u)\r\n",
        ctrl->swid,
        ctrl->cipher_len,
        ctrl->cpid.pool_id,
    ...
    if (ctrl->closed || ctrl->free) {
        OG_LOG_BLACKBOX("close or free stack is:\r\n");
...
static void _protech_vm_print_pool_stack(pool_symbols_t* pool_symbols)
{
    if (pool_symbols == NULL ||
    ...
    OG_LOG_BLACKBOX("VM POOL INFO:\r\n");
    OG_LOG_BLACKBOX("pool->map_count       = %u\r\n", pool_symbols->pool->map_count);
    OG_LOG_BLACKBOX("pool->ctrl_hwm        = %u\r\n", pool_symbols->pool->ctrl_hwm);
    OG_LOG_BLACKBOX("pool->ctrl_count      = %u\r\n", pool_symbols->pool->ctrl_count);
    OG_LOG_BLACKBOX("pool->page_hwm        = %u\r\n", pool_symbols->pool->page_hwm);
    OG_LOG_BLACKBOX("pool->page_count      = %u\r\n", pool_symbols->pool->page_count);
    OG_LOG_BLACKBOX("pool->get_swap_extents= %u\r\n", pool_symbols->pool->get_swap_extents);
    OG_LOG_BLACKBOX("pool->swap_count      = %u\r\n", pool_symbols->pool->swap_count);
    OG_LOG_BLACKBOX("pool->max_swap_count  = %u\r\n", pool_symbols->pool->max_swap_count);
...
void _protech_vm_print_stack(void)
{
    OG_LOG_BLACKBOX("PROTECT VM:enabled\r\n");
    for (uint32 pool_id = 0; pool_id < g_pools_count; pool_id++) {
...
void _protech_vm_print_stack(void)
{
    OG_LOG_BLACKBOX("PROTECT VM:disabled\r\n");
}
#endif
...
void mem_remove_from_coredump(void *begin, uint64 len)
{
    ...
    int rc;
    OG_LOG_RUN_ERR("Try remove mem from  coredump");
    rc = madvise(begin, len, MADV_DONTDUMP);
    if (rc == MADVICE_ERROR) {
        OG_LOG_RUN_ERR("Remove mem from coredump");
    }
#endif
}
...
const mtrl_rowid_t g_invalid_entry = { .vmid = OG_INVALID_ID32, .slot = OG_INVALID_ID32};
...
status_t vmctx_alloc(pvm_context_t vm_ctx, uint32 size, mtrl_rowid_t *row_id)
{
    ...
    bool32      need_close = OG_FALSE;

    VM_CTX_OPEN(vm_ctx);
    vmid = vm_ctx->vm_list.last;
    ctrl = vm_get_ctrl(VMCTX_POOL(vm_ctx), vmid);
    CM_ASSERT(ctrl->ref_num > 0);
    page = (vm_page_head_t *)VMCTX_CURR_PAGE(vm_ctx)->data;

    /* if requested size is exceeds max vm page size, return NULL */
    if ((size >= OG_VMEM_PAGE_SIZE) || (chunk_size > ALLOC_MAX_MEM_SIZE)) {
        OG_THROW_ERROR_EX(ERR_NO_FREE_VMEM, "alloc size (%u) exceed max page size %u, last id: %u", size,
                          OG_VMEM_PAGE_SIZE, vmid);
        return OG_ERROR;
    }
    ...
    *row_id = vm_ctx->free_list[idx];
    if (ROWID_ID2_UINT64(*row_id) != ROWID_ID2_UINT64(g_invalid_entry)) {
        vm_page_t *free_page = VMCTX_CURR_PAGE(vm_ctx);
        if (row_id->vmid != vmid) {
            if (vm_open(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), row_id->vmid, &free_page) != OG_SUCCESS) {
                return OG_ERROR;
            }
            need_close = OG_TRUE;
        }
        ...
        if (need_close) {
            vm_close(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), row_id->vmid, VM_ENQUE_TAIL);
        }
        return OG_SUCCESS;
    }
    ...
    if (chunk_size + VM_CHUNKHDRSZ + page->free_begin > OG_VMEM_PAGE_SIZE) {
        /* last page is not avail for current alloc, split the left room into free list */
        CM_ASSERT(page->free_begin <= OG_VMEM_PAGE_SIZE);
        uint32 avail_space = OG_VMEM_PAGE_SIZE - page->free_begin;
        while (avail_space >= ((1 << ALLOC_MINBITS) + VM_CHUNKHDRSZ)) {
            ...
            chunk = VM_GET_CHUNK(VMCTX_CURR_PAGE(vm_ctx), page->free_begin);
        ...
        vm_close(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), vmid, VM_ENQUE_TAIL);
        if (vm_alloc_and_append(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), &vm_ctx->vm_list) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (vmctx_open_page(vm_ctx) != OG_SUCCESS) {
    ...
    if (chunk_size == OG_VMEM_PAGE_SIZE) {
        chunk->size = ALLOC_MAX_MEM_SIZE;
        chunk->requested_size = size;
        page->free_begin = OG_VMEM_PAGE_SIZE;
    } else {
...
status_t vmctx_insert(pvm_context_t vm_ctx, const char *row, uint32 size, mtrl_rowid_t *row_id)
{
    ...
    /* if requested size is exceeds max vm page size, return NULL */
    if ((size >= OG_VMEM_PAGE_SIZE) || (chunk_size > ALLOC_MAX_MEM_SIZE)) {
        OG_THROW_ERROR_EX(ERR_NO_FREE_VMEM, "alloc size (%u) exceed max page size %u, last id: %u",
            size, OG_VMEM_PAGE_SIZE, vmid);
        return OG_ERROR;
    }
...
status_t vmctx_realloc(pvm_context_t vm_ctx, mtrl_rowid_t *row_id, uint32 size)
{
    ...
    if (vm_ctx->vm_list.last != row_id->vmid) {
        if (vm_open(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), row_id->vmid, &page) != OG_SUCCESS) {
            return OG_ERROR;
        }
        need_close = OG_TRUE;
    }
    ...
    data = cm_push(VMCTX_STACK(vm_ctx), size);
    ...
    if (ret != EOK) {
        cm_pop(VMCTX_STACK(vm_ctx));
        return OG_ERROR;
    }
    
    if (vmctx_free(vm_ctx, row_id) != OG_SUCCESS) {
        cm_pop(VMCTX_STACK(vm_ctx));
        return OG_ERROR;
    }
    
    if (vmctx_insert(vm_ctx, (const char *)data, size, row_id) != OG_SUCCESS) {
        cm_pop(VMCTX_STACK(vm_ctx));
        return OG_ERROR;
    }
    cm_pop(VMCTX_STACK(vm_ctx));
...
void vmctx_reset(pvm_context_t vm_ctx)
{
    ...
    vm_free_list(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), &vm_ctx->vm_list);
    MEMS_RETVOID_IFERR(memset_sp(vm_ctx->free_list, sizeof(vm_ctx->free_list), 0xFF, sizeof(vm_ctx->free_list)));
    vm_ctx->curr_page = NULL;
    vm_ctx->is_open = OG_FALSE; /* vm_ctx->session vm_ctx->stack vm_ctx->pool no need assign NULL */
}
...
status_t vmctx_open_page(pvm_context_t vm_ctx)
{
    ...
    if (vm_open(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), vmid, &vm_ctx->curr_page) != OG_SUCCESS) {
        OG_THROW_ERROR(ERR_VM, "fail to open the vm");
        return OG_ERROR;
    }
...
status_t  vmctx_check_memory(pvm_context_t vm_ctx)
{
    ...
    for (uint32 i = 0; i < ALLOCSET_NUM_FREELISTS; i++) {
        ...
        while (!IS_INVALID_MTRL_ROWID(curr)) {
            OPEN_VM_PTR(&curr, vm_ctx);
            CM_ASSERT(d_ptr != NULL);
            free_size += d_chunk->size + VM_CHUNKHDRSZ;
            next = d_chunk->next;
            CLOSE_VM_PTR(&curr, vm_ctx);
            curr = next;
        }
    }
    vm_page_head_t *page_head = (vm_page_head_t *)VMCTX_CURR_PAGE(vm_ctx)->data;
    ...
    if (vm_ctx->vm_list.count > 1) {
        ...
        while (id != vm_ctx->vm_list.last) {
            vm_page_t *page = NULL;
            vm_ctrl_t *tmp_ctrl = vm_get_ctrl(VMCTX_POOL(vm_ctx), id);
            CM_ASSERT(vm_open(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), id, &page) == OG_SUCCESS);
            alloc_size += ((vm_page_head_t *)page->data)->free_begin - VM_PAGEHDRSZ;
            vm_close(VMCTX_SESSION(vm_ctx), VMCTX_POOL(vm_ctx), id, VM_ENQUE_TAIL);
            id = tmp_ctrl->next;
        }
    }
    
    if (free_size != alloc_size) {
        OG_LOG_RUN_WAR("[VMCTX] vmctx memory may leak, alloc size %u, free size %u", alloc_size, free_size);
    }
    return OG_SUCCESS;
}
