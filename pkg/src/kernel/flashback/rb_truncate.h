 ...
 *
 * IDENTIFICATION
 * src/kernel/flashback/rb_truncate.h
 *
 * -------------------------------------------------------------------------
 */
...
typedef struct st_knl_rb_desc {
    ...
    char name[OG_NAME_BUFFER_SIZE];       // recycle bin object name
    uint32 uid;                           // owner user_id
    uint32 table_id;                      // table_id of current object
    char org_name[OG_NAME_BUFFER_SIZE];   // original object name
    char part_name[OG_NAME_BUFFER_SIZE];  // object partition name, NULL otherwise
