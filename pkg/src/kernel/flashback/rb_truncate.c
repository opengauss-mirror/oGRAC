 ...
 *
 * IDENTIFICATION
 * src/kernel/flashback/rb_truncate.c
 *
 * -------------------------------------------------------------------------
 */
...
void rb_convert_desc(knl_cursor_t *cursor, knl_rb_desc_t *desc)
{
    ...
    (void)cm_text2str(&text, desc->org_name, OG_NAME_BUFFER_SIZE);

    if (CURSOR_COLUMN_SIZE(cursor, SYS_RECYCLEBIN_COL_PARTITION_NAME) != OG_NULL_VALUE_LEN) {
        ...
        (void)cm_text2str(&text, desc->part_name, OG_NAME_BUFFER_SIZE);
    } else {
        desc->part_name[0] = '\0';
    }
    ...
    if (CURSOR_COLUMN_SIZE(cursor, SYS_RECYCLEBIN_COL_ENTRY) != OG_NULL_VALUE_LEN) {
    ...
    if (CURSOR_COLUMN_SIZE(cursor, SYS_RECYCLEBIN_COL_TCHG_SCN) != OG_NULL_VALUE_LEN) {
        desc->tchg_scn = *(knl_scn_t *)CURSOR_COLUMN_DATA(cursor, SYS_RECYCLEBIN_COL_TCHG_SCN);
    } else {
        desc->tchg_scn = OG_INVALID_ID64;
    }
...
static void rb_init_table_desc(knl_session_t *session, knl_table_desc_t *table_desc, knl_rb_desc_t *desc)
{
    ...
    ret = snprintf_s(desc->name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1,
                     "BIN$%u$%llX==$0", table_desc->id, desc->id);
    knl_securec_check_ss(ret);
    name_len = strlen(table_desc->name);
    ret = strncpy_s(desc->org_name, OG_NAME_BUFFER_SIZE, table_desc->name, name_len);
                                    ...
                                    knl_table_part_desc_t *part_desc, knl_rb_desc_t *desc)
{
    ...
    ret = strncpy_s(desc->org_name, OG_NAME_BUFFER_SIZE, table->desc.name, name_len);
    knl_securec_check(ret);
    name_len = strlen(part_desc->name);
    ret = strncpy_s(desc->part_name, OG_NAME_BUFFER_SIZE, part_desc->name, name_len);
    knl_securec_check(ret);
...
static void rb_init_index_desc(knl_session_t *session, knl_index_desc_t *index_desc, knl_rb_desc_t *desc)
{
    ...
    knl_get_index_name(index_desc, desc->org_name, OG_NAME_BUFFER_SIZE);
...
{
    desc->id = session->curr_lsn;
    errno_t ret = snprintf_s(desc->name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1,
                             "BIN$%u$%llX==$0", part_desc->table_id, desc->id);
    knl_securec_check_ss(ret);

    knl_get_index_name(&index->desc, desc->org_name, OG_NAME_BUFFER_SIZE);
    if (IS_SUB_IDXPART(part_desc)) {
        ...
        ret = snprintf_s(desc->part_name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1,
...
static void rb_init_lob_desc(knl_session_t *session, knl_lob_desc_t *lob_desc, knl_rb_desc_t *desc)
{
    ...
    ret = snprintf_s(desc->org_name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1,
...
static status_t rb_drop_index(knl_session_t *session, table_t *table, knl_rb_desc_t *desc)
{
    ...
    for (i = 0; i < table->index_set.total_count; i++) {
        ...
        if (db_write_sysrb(session, desc) != OG_SUCCESS) {
            return OG_ERROR;
        }

        /* update the origin index name to recycle bin object name in index$ */
        cm_str2text(desc->name, &text);
        if (db_update_index_name(session, index->desc.uid, index->desc.table_id, desc->org_name, &text) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t rb_drop_all_cons(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    if (db_drop_all_cons(session, table->desc.uid, table->desc.id, OG_TRUE) != OG_SUCCESS) {
        return OG_ERROR;
    }

    CM_SAVE_STACK(session->stack);
    char *buf = (char *)cm_push(session->stack, OG_NAME_BUFFER_SIZE);
    ...
    while (!cursor->eof) {
        ...
        if (type != CONS_TYPE_REFERENCE) {
            name_buf_size = OG_NAME_BUFFER_SIZE;
            ret = memset_sp(buf, name_buf_size, 0, name_buf_size);
            knl_securec_check(ret);
            ret = snprintf_s(buf, name_buf_size, OG_NAME_BUFFER_SIZE - 1,
...
status_t rb_drop_table(knl_session_t *session, knl_handle_t stmt, knl_dictionary_t *dc, bool32 commit)
{
    ...
    buf = (char *)cm_push(session->stack, OG_NAME_BUFFER_SIZE);
    name_len = strlen(desc.name);
    ret = strncpy_s(buf, OG_NAME_BUFFER_SIZE, desc.name, name_len);
    knl_securec_check(ret);
    ...
    if (db_update_table_name(session, table->desc.uid, table->desc.name, &text, OG_TRUE) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (rb_drop_index(session, table, &desc) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (rb_drop_all_cons(session, dc) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
    ...
    if (g_knl_callback.pl_db_drop_triggers(session, dc) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
    ...
    ret = strcpy_sp(redo.name, OG_NAME_BUFFER_SIZE, table->desc.name);
    knl_securec_check(ret);
...
{
    if (db_update_table_entry(session, &table->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
{
    if (db_update_table_part_entry(session, &table_part->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
{
    if (db_update_lob_entry(session, &lob->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
{
    if (db_update_lob_part_entry(session, &lob_part->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
{
    if (db_update_subtabpart_entry(session, &table_subpart->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t rb_truncate_index_subpart_segment(knl_session_t *session, index_part_t *index_subpart)
{
    ...
    if (db_upd_sub_idx_part_entry(session, &index_subpart->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
{
    if (db_update_sublobpart_entry(session, &lob_subpart->desc, INVALID_PAGID) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t rb_truncate_lob_parts(knl_session_t *session, table_t *table, lob_t *lob, knl_rb_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < table->part_table->desc.partcnt; i++) {
        ...
        if (rb_truncate_lob_part_segment(session, lob_part) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        for (uint32 j = 0; j < lob_part->desc.subpart_cnt; j++) {
            ...
            if (rb_truncate_lob_subpart_segment(session, lob_subpart) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
...
static status_t rb_truncate_lobs(knl_session_t *session, dc_entity_t *entity, table_t *table, knl_rb_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < entity->column_count; i++) {
        ...
        if (!IS_PART_TABLE(table)) {
            if (rb_truncate_lob_segment(session, lob) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        if (rb_truncate_lob_parts(session, table, lob, desc) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t rb_truncate_part_index(knl_session_t *session, index_t *index, knl_rb_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < index->part_index->desc.partcnt; i++) {
        ...
        if (!IS_PARENT_IDXPART(&index_part->desc)) {
            if (rb_truncate_index_part_segment(session, index_part) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        for (uint32 j = 0; j < index_part->desc.subpart_cnt; j++) {
            ...
            if (rb_truncate_index_subpart_segment(session, index_subpart) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
...
static status_t rb_truncate_index(knl_session_t *session, table_t *table, knl_rb_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (!IS_PART_INDEX(index)) {
            ...
            if (rb_truncate_index_segment(session, index) != OG_SUCCESS) {
                return OG_ERROR;
            }
            continue;
        }

        if (rb_truncate_part_index(session, index, desc) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t rb_truncate_part_table(knl_session_t *session, table_t *table, knl_rb_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < table->part_table->desc.partcnt; i++) {
        ...
        if (!IS_PARENT_TABPART(&table_part->desc)) {
            if (rb_truncate_table_part_segment(session, table_part) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        for (uint32 j = 0; j < table_part->desc.subpart_cnt; j++) {
            ...
            if (rb_truncate_table_subpart_segment(session, table_subpart) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
...
status_t rb_truncate_table(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    if (rb_truncate_table_segment(session, table) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (IS_PART_TABLE(table)) {
        if (rb_truncate_part_table(session, table, &desc) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }

    if (rb_truncate_index(session, table, &desc) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (entity->contain_lob) {
        if (rb_truncate_lobs(session, entity, table, &desc) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (IS_PART_INDEX(index)) {
            ...
            for (uint32 j = 0; j < index_part->desc.subpart_cnt; j++) {
                ...
                if (rb_truncate_index_subpart_segment(session, subpart) != OG_SUCCESS) {
                    return OG_ERROR;
                }
            }
            continue;
    ...
    knl_rb_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < entity->column_count; i++) {
        ...
        if (IS_PARENT_LOBPART(&lob_part->desc)) {
            ...
            for (uint32 j = 0; j < lob_part->desc.subpart_cnt; j++) {
                subpart = PART_GET_SUBENTITY(lob->part_lob, lob_part->subparts[j]);
                ...
                if (rb_truncate_lob_subpart_segment(session, subpart) != OG_SUCCESS) {
                    return OG_ERROR;
                }
            }
        } else {
...
status_t rb_truncate_table_part(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_part)
{
    ...
    if (IS_PARENT_TABPART(&table_part->desc)) {
        ...
        for (uint32 i = 0; i < table_part->desc.subpart_cnt; i++) {
            ...
            if (rb_truncate_table_subpart_segment(session, subpart) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    } else {
    ...
    if (rb_truncate_index_part(session, dc, table_part, &desc) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (entity->contain_lob) {
        if (rb_truncate_lob_part(session, dc, table_part, &desc) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    knl_rb_desc_t *desc, uint32 compart_no)
{
    ...
    if (db_need_invalidate_index(session, dc, table, (table_part_t *)subpart, &need_invalidate_index) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    uint32 compart_no)
{
    ...
    if (rb_truncate_index_subpart(session, dc, subpart, &desc, compart_no) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (entity->contain_lob) {
        if (rb_truncate_lob_subpart(session, dc, subpart, &desc, compart_no) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
