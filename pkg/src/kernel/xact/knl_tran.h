...
#define TX_NEED_READ_WAIT(se) ((se)->wxid.value != OG_INVALID_ID64 && TX_XA_CONSISTENCY(se))
#define RECORD_SQL_SIZE  SIZE_K(16)

#define XID_INST_ID(xid)      ((xid).xmap.seg_id / OG_MAX_UNDO_SEGMENT)
#define XMAP_INST_ID(xmap)    ((xmap).seg_id / OG_MAX_UNDO_SEGMENT)
#define XID_SEG_ID(xid)       ((xid).xmap.seg_id % OG_MAX_UNDO_SEGMENT)
#define XMAP_SEG_ID(xmap)     ((xmap).seg_id % OG_MAX_UNDO_SEGMENT)
...
typedef struct st_tx_area {
    ...
    thread_t rollback_proc[OG_MAX_ROLLBACK_PROC];
} tx_area_t;
...
static inline void tx_record_rowid(rowid_t rid)
{
    ...
    if (code == ERR_LOCK_TIMEOUT) {
        OG_LOG_RUN_ERR("lock timeout while waiting for row %u-%u-%u", rid.file, rid.page, rid.slot);
    }
}
