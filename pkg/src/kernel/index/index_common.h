 ...
 *
 * IDENTIFICATION
 * src/kernel/index/index_common.h
 *
 * -------------------------------------------------------------------------
 */
...
typedef struct st_idxpart_worker {
    ...
    struct st_idxpart_paral_ctx *ogx;
}idxpart_worker_t;
...
typedef struct st_idx_sort_worker {
    ...
    struct st_idx_paral_sort_ctx *ogx;
} idx_sort_worker_t;
...
typedef struct st_idx_multi_info {
    ...
    mtrl_context_t mtrl_ctx;
    mtrl_segment_t *segment;
    struct st_idx_build_worker *ogx;
} idx_multi_info_t;
...
typedef struct st_idx_paral_sort_ctx {
    ...
    btree_t *btree[OG_MAX_INDEX_COUNT_PERSQL];
    uint32 index_count;
    knl_dictionary_t *private_dc;
    idx_part_info_t part_info;
    union {
        id_list_t sort_info;
        id_list_t multi_sort_info[OG_MAX_INDEX_COUNT_PERSQL];
    };
    bool32 is_global;
    bool32 nologging;
    char err_msg[OG_MESSAGE_BUFFER_SIZE];
...
typedef struct st_idx_paral_rebuild_worker {
    ...
    knl_session_t *session;
    thread_t thread;
    struct st_idx_paral_rebuild_ctx *ogx;
...
typedef struct st_idx_paral_rebuild_ctx {
    ...
    char err_msg[OG_MESSAGE_BUFFER_SIZE];
    struct st_idx_paral_rebuild_worker workers[OG_MAX_REBUILD_INDEX_PARALLELISM];
...
status_t btree_create_part_entry(knl_session_t *session, btree_t *btree, index_part_t *index_part,
                                 knl_part_locate_t part_loc);
void btree_concat_extent(knl_session_t *session, btree_t *btree, page_id_t extent, uint32 extent_size, bool32
    is_degrade);
...
status_t idx_start_rebuild_workers(knl_session_t *session,
    idx_paral_rebuild_ctx_t *ogx, knl_parts_locate_t parts_loc);
