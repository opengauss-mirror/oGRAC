                       ...
                       btree_info_t *info, void *cbo_index_handle)
{
    ...
    for (uint32 i = 0; i < column_count; i++) {
        ...
        result = btree_cmp_column_data((void *)data1, (void *)data2, type, &offset, OG_TRUE);
        if (result != 0) {
            btree_set_comb_ndv(info, i, column_count);
            if (is_distinct) {
                info->distinct_keys++;
    ...
    page_id_t *prev_page_id, pcrb_key_t *prev_compare_key)
{
    ...
    if (btree_check_segment_scn(page, PAGE_TYPE_PCRB_NODE, seg_scn) != OG_SUCCESS) {
                                   ...
                                   btree_info_t *info, page_id_t page_id_input)
{
    ...
    compare_key = (pcrb_key_t *)cm_push(session->stack, OG_KEY_BUF_SIZE);
