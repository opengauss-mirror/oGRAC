...
* Output          : is_same : return OG_TRUE if one key is exactly equal to the other.
    ...
    uint16 *offset)
{
    ...
    if (flag == SCAN_KEY_NORMAL) {
        ...
        result = btree_cmp_column_data((void *)data1, data2, column->datatype, offset, OG_FALSE);
...
bool32 bt_chk_leaf_recycled(knl_session_t *session, btree_t *btree, btree_page_t *btree_page, knl_scn_t snap_scn)
{
    ...
    if (!btree_page->is_recycled) {
        ...
        OG_LOG_RUN_WAR("index %s leaf is recycled.page id %u-%u, seg_scn %llu, is recycled %u,"
            "snap_scn %llu, ow_recycle_scn %llu",
            btree->index->desc.name, (uint32)AS_PAGID(btree_page->head.id).file,
                (uint32)AS_PAGID(btree_page->head.id).page,
            btree_page->seg_scn, btree_page->is_recycled, snap_scn, ow_recycle_scn);
    }

    OG_LOG_DEBUG_INF("index %s leaf is recycled.page id %u-%u, seg_scn %llu, is recycled %u,"
        "snap_scn %llu, ow_recycle_scn %llu",
    ...
    uint8 isolevel, knl_scn_t query_scn)
{
    if (btree_page->level > 0 || isolevel == ISOLATION_CURR_COMMITTED ||
        OG_INVALID_SCN(btree->chg_stats.ow_del_scn)) {
        return OG_SUCCESS;
    }

    if (query_scn < KNL_GET_SCN(&btree->chg_stats.ow_del_scn)) {
        OG_THROW_ERROR(ERR_KEYS_RECYCLED);
        OG_LOG_RUN_ERR("index %s keys may recycled, details: query_scn %llu,"
...
static status_t bt_chk_page(knl_session_t *session, btree_search_t *search_info, btree_page_t *btree_page)
{
    ...
    if (btree_check_segment_scn(btree_page, PAGE_TYPE_BTREE_NODE, search_info->seg_scn) != OG_SUCCESS) {
        OG_THROW_ERROR(ERR_INDEX_ALREADY_DROPPED, index->desc.name);
        return OG_ERROR;
    }
    
    if (SECUREC_UNLIKELY(!DB_IS_PRIMARY(&session->kernel->db))) {
        if (btree_check_min_scn(search_info->query_scn, search_info->btree->min_scn, btree_page->level) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    btree_path_info_t *path_info, bool32 cmp_rowid, bool32 *is_found)
{
    ...
    */
    ...
    for (;;) {
        if (btree_enter_locate_page(session, search_info, level, page_id, &page) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (bt_chk_page(session, search_info, page) != OG_SUCCESS) {
            buf_leave_page(session, OG_FALSE);
            session->index_root = NULL;
            return OG_ERROR;
        }
    ...
    btree_key_t *key, bool32 *is_found)
{
    ...
    if (txn_info.status == (uint8)XACT_END) {
        ...
        if (txn_info.is_owscn) {
            tx_record_sql(session);
            OG_LOG_RUN_ERR("snapshot too old, detail: key owscn %llu, query scn %llu",
        ...
        if (itl->xid.value == cursor->xid) {
            ...
            if (TX_XA_CONSISTENCY(session) &&
                ...
                txn_info.scn < cursor->query_scn) {
                OG_LOG_DEBUG_INF("need read wait.prepare_scn[%llu] < query_scn[%llu]", txn_info.scn, cursor->query_scn);
    ...
    if (btree_check_visible_with_undo(session, cursor, btree, key, &part_id, &parent_partid, is_found) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    *is_found = OG_FALSE;
    ...
    while (cursor->key_loc.slot < page->keys) {
        ...
        *is_found = btree_match_cond(cursor, page);
        if (*is_found) {
            if (btree_check_visible(session, cursor, dir, key, is_found) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }

        if (*is_found) {
            int32 ret = memcpy_sp(cursor->scan_range.l_buf, OG_KEY_BUF_SIZE, key, (size_t)key->size);
    ...
    *is_found = OG_FALSE;
    ...
    for (;;) {
        ...
        *is_found = btree_match_cond(cursor, page);
        ...
        if (*is_found) {
            ret = memcpy_sp(cursor->scan_range.r_buf, OG_KEY_BUF_SIZE, key, (size_t)key->size);
            knl_securec_check(ret);
    ...
    knl_scan_key_t *scan_key, btree_path_info_t *path_info)
{
    ...
    if (btree_find_leaf(session, search_info, scan_key, path_info, cmp_rowid, &is_found) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    btree_path_info_t *path_info)
{
    ...
    if (search_info->is_dsc_scan) {
        if (path_info->path[0].slot == 0) {
            ...
            if (buf_read_page(session, prev_id, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
                return OG_ERROR;
            }
            page = BTREE_CURR_PAGE(session);
    ...
    if (path_info->path[0].slot + 1 >= page->keys) {
        ...
        if (buf_read_page(session, next_id, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    knl_scan_key_t *scan_key, btree_path_info_t *path_info)
{
    ...
    if (status != OG_SUCCESS || cursor->eof) {
        if (BTREE_LOCATE_NEXT_KEY(search_info, cursor)) {
...
static status_t btree_locate_with_find(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    if (btree_locate_key(session, &search_info, cursor, scan_key, &path_info) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    page_id_t *next_page, knl_scn_t *next_snap_scn, bool32 *check_next)
{
    ...
    *check_next = OG_FALSE;
    ...
    if (cursor->index_ffs) {
        if (buf_read_prefetch_page(session, *next_page, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (buf_read_page(session, *next_page, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t btree_locate_next_page(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    for (;;) {
        ...
        if (btree_enter_next_page(session, cursor, &next_page, &next_snap_scn, &check_next) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t btree_relocate_curr_page(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    for (;;) {
        ...
        if (buf_read_page(session, cursor->key_loc.page_id, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
            return OG_ERROR;
        }
        page = BTREE_CURR_PAGE(session);
        if (page_soft_damaged(&page->head)) {
            OG_THROW_ERROR(ERR_PAGE_SOFT_DAMAGED, AS_PAGID(&page->head.id).file, AS_PAGID(&page->head.id).page);
            return OG_ERROR;
        }
...
static status_t btree_locate_curr_page(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    if (TX_NEED_READ_WAIT(session)) {
        OG_LOG_DEBUG_INF("locate key on curr btree page need begin read wait.");
    ...
    if (btree_relocate_curr_page(session, cursor, scan_key) != OG_SUCCESS) {
        buf_leave_page(session, OG_FALSE);
        return OG_ERROR;
    }
...
static void btree_cleanout_page(knl_session_t *session, knl_cursor_t *cursor, page_id_t page_id)
{
    ...
    if (btree_check_segment_scn(page, PAGE_TYPE_BTREE_NODE, cursor->key_loc.seg_scn) != OG_SUCCESS ||
        page->is_recycled) {
        buf_leave_page(session, OG_FALSE);
        log_atomic_op_end(session);
        unlock_table_without_xact(session, cursor->dc_entity, lock_inuse);
...
static status_t btree_locate_next_scan_key(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (scan_key->flags[0] == SCAN_KEY_NORMAL) {
        char *data = btree_get_column(scan_key, column->datatype, 0, &len, OG_FALSE);
    ...
    if (btree_locate_key(session, &search_info, cursor, &next_scan_key, &path_info) != OG_SUCCESS) {
    ...
    knl_scan_key_t *scan_key)
{
    ...
    if (!btree_get_bitmap(&key->bitmap, 0)) {
        ...
        data = btree_get_column(&next_scan_key, column->datatype, 0, &len, OG_FALSE);
    ...
    for (uint32 i = 1; i < index->desc.column_count; i++) {
        ...
        if (scan_key->flags[i] == SCAN_KEY_NORMAL) {
            column = dc_get_column(index->entity, index->desc.columns[i]);
            data = btree_get_column(&cursor->scan_range.org_key, column->datatype, i, &len, OG_FALSE);
...
static status_t btree_set_next_range(knl_session_t *session, knl_cursor_t *cursor, btree_t *btree)
{
    ...
    if (btree_locate_next_scan_key(session, cursor) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    int32 result = btree_compare_key(index, end_key, key, OG_TRUE, NULL);
...
void btree_init_match_cond(knl_cursor_t *cursor, bool32 is_pcr)
{
    ...
    if (cursor->scan_range.is_equal || cursor->skip_index_match ||
        ...
        cursor->key_loc.match_left = OG_FALSE;
        cursor->key_loc.match_right = OG_FALSE;
        return;
    }
...
static void btree_init_fetch(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    */
    ...
    btree_init_match_cond(cursor, OG_FALSE);
    if (cursor->index_dsc) {
...
status_t btree_fetch(knl_handle_t handle, knl_cursor_t *cursor)
{
    ...
    for (;;) {
        ...
        if (btree_find_key(session, cursor, scan_key) != OG_SUCCESS) {
        ...
        if (btree_check_eof(session, cursor) != OG_SUCCESS) {
            SEG_STATS_RECORD(session, temp_stat, &btree->stat);
            return OG_ERROR;
        }
        ...
        if (TX_NEED_READ_WAIT(session)) {
            OG_LOG_DEBUG_INF("fetch btree need read wait.");
            continue;
        }
...
status_t bt_chk_exist_pre(knl_session_t *session, btree_t *btree, btree_search_t *search_info)
{
    ...
    search_info->btree = btree;
    search_info->is_dsc_scan = OG_FALSE;
    search_info->is_equal = OG_TRUE;
    ...
    search_info->use_cr_pool = OG_TRUE;
...
status_t btree_check_key_exist(knl_session_t *session, btree_t *btree, char *data, bool32 *exists)
{
    ...
    for (;;) {
        if (btree_find_leaf(session, &search_info, &scan_key, &path_info, cmp_rowid, &found) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (!found) {
            *exists = OG_FALSE;
...
static status_t btree_fetch_depended_asc(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    while (cursor->key_loc.slot < page->keys) {
        ...
        if (cursor->key_loc.slot_end == INVALID_SLOT || cursor->key_loc.slot_end < cursor->key_loc.slot) {
            result = btree_compare_key(btree->index, filter_key, key, OG_FALSE, NULL);
            if (result < 0) {
                if (cursor->key_loc.page_cache == NO_PAGE_CACHE) {
                    buf_leave_page(session, OG_FALSE);
                }
        ...
        if (btree_check_exist(session, dir, key, &cursor->is_found, &is_wait) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
status_t btree_fetch_depended(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    for (;;) {
        ...
        if (btree_fetch_depended_asc(session, cursor) != OG_SUCCESS) {
            return OG_ERROR;
        }
