 ...
 *
 * IDENTIFICATION
 * src/kernel/catalog/knl_dblink.c
 *
 * -------------------------------------------------------------------------
 */
...
static status_t dc_get_dblink_def_id(knl_session_t *session, knl_dblink_def_t *def)
{
    ...
    def->id = OG_INVALID_ID32;

    for (uint32 i = 0; i < OG_MAX_DBLINKS; i++) {
        dblink = ogx->dblinks[i];
        if (dblink == NULL || dblink->status == DBLINK_STATUS_DROPPED) {
            if (def->id == OG_INVALID_ID32) {
                def->id = i;
            }
            continue;
        }
        ...
        if (cm_text_str_equal(&def->name.value, dblink->desc.name) && def->owner_id == dblink->desc.owner_id) {
            OG_THROW_ERROR(ERR_DUPLICATE_NAME, "dblink", T2S(&def->name.value));
            return OG_ERROR;
        }
    }
...
static status_t dc_insert_sys_links(knl_session_t *session, knl_dblink_def_t *def)
{
    ...
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_INSERT, SYS_LINK_ID, OG_INVALID_ID32);
...
static status_t dc_drop_sys_links(knl_session_t *knl_session, uint32 uid, text_t *name)
{
    ...
    do {
        ...
        if (cursor->eof) {
            OG_THROW_ERROR(ERR_DBLINK_NOT_EXIST, T2S(name));
            ret = OG_ERROR;
            break;
        }
...
static status_t dc_add_dblink(dc_context_t *ogx, knl_dblink_def_t *def)
{
    dc_dblink_t *dblink = NULL;
    errno_t err;

    if (mctx_alloc(ogx->memory, sizeof(dc_dblink_t), (void **)&dblink) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static void dc_drop_dblink(knl_session_t *session, uint32 uid, text_t *name)
{
    ...
    for (uint32 i = 0; i < OG_MAX_DBLINKS; i++) {
        if (ogx->dblinks[i] == NULL) {
            continue;
        }

        cm_latch_x(&ogx->dblinks[i]->latch, session->id, NULL);

        if (!cm_text_str_equal(name, ogx->dblinks[i]->desc.name) || uid != ogx->dblinks[i]->desc.owner_id) {
            cm_unlatch(&ogx->dblinks[i]->latch, NULL);
            continue;
        }

        ogx->dblinks[i]->status = DBLINK_STATUS_DROPPED;
        cm_unlatch(&ogx->dblinks[i]->latch, NULL);
        return;
    }
}
...
status_t knl_create_dblink(knl_handle_t session, knl_dblink_def_t *def)
{
    ...
    if (DB_NOT_READY(knl_session)) {
        OG_THROW_ERROR(ERR_NO_DB_ACTIVE, "Database has not been created or is not open");
        return OG_ERROR;
    }

    dls_spin_lock(session, &ogx->paral_lock, NULL);

    if (dc_get_dblink_def_id(session, def) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }

    /* add the new dblink to dc */
    if (ogx->dblinks[def->id] == NULL) {
        if (dc_add_dblink(ogx, def) != OG_SUCCESS) {
            dls_spin_unlock(session, &ogx->paral_lock);
            return OG_ERROR;
        }
    } else {
    ...
    if (dc_insert_sys_links(knl_session, def) != OG_SUCCESS) {
        ogx->dblinks[def->id]->status = DBLINK_STATUS_DROPPED;
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }
...
status_t knl_drop_dblink_by_id(knl_handle_t session, uint32 id)
{
    knl_session_t *knl_session = (knl_session_t *)session;
    dc_context_t *ogx = &knl_session->kernel->dc_ctx;
    dc_dblink_t *dblink = ogx->dblinks[id];
    text_t name;

    dls_spin_lock(session, &ogx->paral_lock, NULL);
    name.str = dblink->desc.name;
    name.len = strlen(dblink->desc.name);
    if (dc_drop_sys_links(knl_session, dblink->desc.owner_id, &name) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }
...
{
    knl_session_t *knl_session = (knl_session_t *)session;
    mctx_destroy(knl_session->lnk_tab_dc->ogx);
...
status_t knl_load_dblinks(knl_handle_t session)
{
    ...
    cursor = knl_push_cursor(knl_session);
    knl_open_sys_cursor(knl_session, cursor, CURSOR_ACTION_SELECT, SYS_LINK_ID, OG_INVALID_ID32);
    ...
    for (;;) {
        ...
        ret = mctx_alloc(ogx->memory, sizeof(dc_dblink_t), (void **)&dblink);
        OG_BREAK_IF_ERROR(ret);
        err = memset_sp(&dblink->latch, sizeof(latch_t), 0, sizeof(latch_t));
        knl_securec_check(err);
        ...
        dblink->status = DBLINK_STATUS_NORMAL;
        ogx->dblinks[dblink->desc.id] = dblink;
    }
...
static status_t dc_open_dblink(knl_session_t *session, text_t *name, dc_dblink_t **dblink)
{
    dc_context_t *ogx = &session->kernel->dc_ctx;
    ...
    for (uint32 i = 0; i < OG_MAX_DBLINKS; i++) {
        ...
        cm_latch_s(&ogx->dblinks[i]->latch, session->id, OG_FALSE, NULL);

        if (ogx->dblinks[i]->status != DBLINK_STATUS_NORMAL ||
            !cm_text_str_equal(name, ogx->dblinks[i]->desc.name) ||
            session->uid != ogx->dblinks[i]->desc.owner_id) {
            cm_unlatch(&ogx->dblinks[i]->latch, NULL);
            continue;
        }
...
static void dc_close_dblink(knl_session_t *session, dc_dblink_t *dblink)
{
    cm_unlatch(&dblink->latch, NULL);
}

static void dc_init_lnk_tab_entry(knl_session_t *session, dc_dblink_t *dblink,
    text_t *tab_user, text_t *tab_name, dc_entry_t *entry)
{
    errno_t err;

    err = memcpy_sp(entry->name, OG_NAME_BUFFER_SIZE, tab_name->str, tab_name->len);
    knl_securec_check(err);
    entry->name[tab_name->len] = '\0';
    err = memcpy_sp(entry->user_name, OG_NAME_BUFFER_SIZE, tab_user->str, tab_user->len);
    knl_securec_check(err);
    ...
    knl_session_t *session, dc_entity_t *entity)
{
    ...
    if (dc_alloc_mem(dc_ctx, entity->memory, sizeof(routing_group_t) * routing_info->group_count,
        (void **)&routing_info->groups) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    knl_session_t *session, dc_entity_t *entity)
{
    ...
    err = memcpy_sp(desc->name, OG_NAME_BUFFER_SIZE, entity->entry->name, OG_NAME_BUFFER_SIZE);
    knl_securec_check(err);
...
static status_t dc_create_lnk_tab_entity(knl_session_t *session, memory_context_t *ogx, dc_dblink_t *dblink,
    dc_entry_t *entry)
{
    dc_context_t         *dc_ctx = &session->kernel->dc_ctx;
    ...
    callback_data.ogx = ogx;
    callback_data.node_id = dblink->desc.node_id;
    callback_data.tab_user = entry->user_name;
    callback_data.tab_name = entry->name;
    if (g_knl_callback.load_lnk_tab_dc(session, &callback_data) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_alloc_mem(dc_ctx, ogx, sizeof(dc_entity_t), (void **)&entry->entity) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    entry->entity->memory = ogx;
    entry->entity->valid = OG_TRUE;
    entry->entity->column_count = callback_data.col_cnt;

    if (dc_create_lnk_tab_routing(&callback_data, session, entry->entity) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_create_lnk_tab_cols(&callback_data, session, entry->entity) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_init_lnk_tab_def(&callback_data, session, entry->entity) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t dc_create_lnk_tab_entry(knl_session_t *session, memory_context_t *ogx, dc_dblink_t *dblink,
    text_t *tab_user, text_t *tab_name, dc_entry_t **entry)
{
    dc_entry_t *ptr = NULL;
    errno_t err;
    ...
    if (dc_alloc_mem(dc_ctx, ogx, sizeof(dc_entry_t), (void **)&ptr) != OG_SUCCESS) {
        return OG_ERROR;
    }

    err = memset_sp(ptr, sizeof(dc_entry_t), 0, sizeof(dc_entry_t));
    knl_securec_check(err);
    err = memcpy_sp(ptr->name, OG_NAME_BUFFER_SIZE, tab_name->str, tab_name->len);
    knl_securec_check(err);
    ptr->name[tab_name->len] = '\0';

    err = memcpy_sp(ptr->user_name, OG_NAME_BUFFER_SIZE, tab_user->str, tab_user->len);
    knl_securec_check(err);
    ptr->user_name[tab_user->len] = '\0';

    if (dc_alloc_mem(dc_ctx, ogx, sizeof(dc_appendix_t), (void **)&ptr->appendix) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    ptr->id = OG_DBLINK_ENTRY_START_ID + session->lnk_tab_count;
    ptr->used = OG_TRUE;
    ptr->ready = OG_FALSE;
...
{
    memory_context_t *ogx = session->lnk_tab_dc->ogx;
    dc_entry_t *entry = NULL;

    if (session->lnk_tab_count >= session->lnk_tab_capacity) {
        OG_THROW_ERROR(ERR_TOO_MANY_OBJECTS, session->lnk_tab_capacity, "dblink tables");
        return OG_ERROR;
    }

    if (dc_create_lnk_tab_entry(session, ogx, dblink, tab_user, tab_name, &entry) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_create_lnk_tab_entity(session, ogx, dblink, entry) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t dc_init_lnk_tab(knl_handle_t session)
{
    ...
    if (dc_alloc_mem(dc_ctx, context, sizeof(knl_lnk_tab_dc_t), (void **)&lnk_tab_dc) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    knl_dictionary_t *dc, bool32 *found)
{
    ...
    if (lnk_tab_dc == NULL) {
        if (dc_init_lnk_tab(session) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
bool32 knl_find_lnk_tab_dc(knl_handle_t session, text_t *lnk_name, text_t *tab_name)
{
    ...
    // find dblink dc
    if (dc_open_dblink(knl_session, lnk_name, &dblink) != OG_SUCCESS) {
        return OG_FALSE;
    }
    ...
    sql_text_t *tab_user, text_t *tab_name, knl_dictionary_t *dc)
{
    ...
    if (dc_find_lnk_tab(knl_session, dblink, &tab_user->value, tab_name, dc, &found) != OG_SUCCESS) {
        dc_close_dblink(session, dblink);
        return OG_ERROR;
    }

    if (!found) {
        if (dc_create_lnk_tab(session, dblink, &tab_user->value, tab_name, dc) != OG_SUCCESS) {
            dc_close_dblink(session, dblink);
            return OG_ERROR;
        }
    }
