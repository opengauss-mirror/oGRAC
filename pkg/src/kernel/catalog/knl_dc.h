...
#define DC_GROUP_COUNT                       (OG_SHARED_PAGE_SIZE / sizeof(pointer_t))
#define DC_GROUP_SIZE                        (OG_SHARED_PAGE_SIZE / sizeof(pointer_t))
#define DC_GROUP_CURRVAL_COUNT               (OG_SHARED_PAGE_SIZE / sizeof(dc_currval_t))
#define DC_SESSION_GROUP_COUNT               ((OG_MAX_SESSIONS + DC_GROUP_CURRVAL_COUNT - 1) / DC_GROUP_CURRVAL_COUNT)
#define DC_CACHED_SERIAL_VALUE(value, start)                                \
    (((value) - (start)) / OG_SERIAL_CACHE_COUNT * OG_SERIAL_CACHE_COUNT +  \
    ((start) == 0 ? 1 : (start)) + OG_SERIAL_CACHE_COUNT)
...
#define DC_HASH_SIZE (OG_SHARED_PAGE_SIZE / sizeof(dc_bucket_t))
#define USER_PRIV_GROUP_COUNT             (OG_MAX_USERS / DC_GROUP_SIZE + 1)
...
typedef struct st_trig_set {
    trig_item_t items[OG_MAX_TRIGGER_COUNT];
    uint32 trig_count;
} trig_set_t;
...
typedef struct st_synonym_link {
    ...
    char user[OG_NAME_BUFFER_SIZE];  // link user
    char name[OG_NAME_BUFFER_SIZE];  // link name
...
typedef struct st_dc_entry {
    bilist_node_t node;  // !!!this member must be the first one
    spinlock_t lock;
    uint32 id;
    ...
    char user_name[OG_NAME_BUFFER_SIZE]; /* table user name */
    char name[OG_NAME_BUFFER_SIZE];      /* table name */
...
#define OG_SYS_PRIVS_BYTES     (OG_SYS_PRIVS_COUNT / 8 + 1)
#define DC_OBJ_PRIV_ENTRY_SIZE (OG_SHARED_PAGE_SIZE / sizeof(pointer_t))

typedef struct st_dc_obj_priv_item {
    uint32 objowner;                    /* object's owner user ID */
    char objname[OG_NAME_BUFFER_SIZE];  /* object's name */
    ...
    uint32 grantor[OG_OBJ_PRIVS_COUNT]; /* grantor uid for each object privilege,
                                           OG_INVALID_ID32 means the privilege is inherited from roles */
} dc_obj_priv_item;
...
typedef struct st_dc_user_privs_item {
    ...
    uint32 grantor[OG_USER_PRIVS_COUNT];           /* grantor uid for each user privilege */
} dc_user_priv_item_t;
...
typedef struct st_knl_role_desc {
    ...
    char name[OG_NAME_BUFFER_SIZE];         /* role name */
    char password[OG_PASSWORD_BUFFER_SIZE]; /* role pwd */
} knl_role_desc_t;
...
typedef struct st_dc_role {
    ...
    uint8 sys_privs[OG_SYS_PRIVS_BYTES]; /* system privileges directly granted to the role,
                                                           data source: SYS_PRIVS$, grantee_type = 1 */
    uint8 admin_opt[OG_SYS_PRIVS_BYTES]; /* system privileges with admin option ? 1: yes, 0: no */
...
typedef struct st_dc_user {
    ...
    uint8 sys_privs[OG_SYS_PRIVS_BYTES];     /* system privileges directly granted to the user,
                                                               data source: SYS_PRIVS$, grantee_type = 1 */
    uint8 admin_opt[OG_SYS_PRIVS_BYTES];     /* system privileges with admin option ? 1: yes, 0: no */
    uint8 all_sys_privs[OG_SYS_PRIVS_BYTES]; /* all system privileges merged from user and granted roles */
    uint8 ter_admin_opt[OG_SYS_PRIVS_BYTES]; /* admin option for each privilege merged from user and roles */
    ...
    uint32 entry_hwm;
    uint32 entry_lwm;
    bilist_t free_entries;
    spinlock_t free_entries_lock;
    cm_list_head grant_obj_privs; /* object privilegs the user directly granted to others */
    volatile bool8 is_loaded;
    bool8 has_nologging;
    user_status_t status;
    drlock_t s_lock;                       /* avoid changing user status concurrently */
    drlatch_t user_latch;                  /* avoid concurrent execution of drop user and create/drop object */
    drlatch_t lib_latch;
...
typedef struct st_knl_tenant_desc {
    ...
    date_t ctime;
    uint32 ts_num;
    uint8 ts_bitmap[OG_SPACES_BITMAP_SIZE];
...
typedef struct st_dc_context {
    ...
    dc_user_t *users[OG_MAX_USERS];
    dc_role_t *roles[OG_MAX_ROLES];
    dc_bucket_t *tenant_buckets;
    dc_tenant_t *tenants[OG_MAX_TENANTS];
    dc_dblink_t *dblinks[OG_MAX_DBLINKS];
...
static inline bool32 dc_is_reserved_entry(uint32 uid, uint32 entry_id)
{
    ...
    if (entry_id < OG_RESERVED_SYSID || (entry_id >= OG_EX_SYSID_START && entry_id < OG_EX_SYSID_END)) {
        return OG_TRUE;
    }
...
#define DC_IS_READY(session)            ((session)->kernel->db.status >= DB_STATUS_WAIT_CLEAN)
...
#define DC_MAX_POOL_PAGES(kernel) \
...
// for create user sync_ddl replay node, do not init entries in user, to avoid buf_read page inside replay_logic lock
...
//performance case use dtc_dc_locked_by_id instead
bool32 dc_locked_by_self(knl_session_t *session, dc_entry_t *entry);
