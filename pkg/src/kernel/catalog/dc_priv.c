...
static void dc_clr_objpriv_by_uid(dc_obj_priv_t *obj_privs, uint32 ownerid)
{
    uint32 oid;
    dc_obj_priv_entry_t *entry = NULL;
    cm_spin_lock(&obj_privs->lock, NULL);
...
static void dc_clr_role_objpriv_by_uid(dc_context_t *ogx, dc_role_t *role, uint32 ownerid)
{
    uint32 oid;
    dc_obj_priv_entry_t *entry = NULL;
...
static void dc_clear_others_objprivs(dc_context_t *ogx, dc_user_t *user)
{
    uint32 id;

    for (id = 0; id < OG_MAX_USERS; id++) {
        if (!ogx->users[id] || id == user->desc.id) {
            continue;
        }

        if (ogx->users[id]->status == USER_STATUS_NORMAL) {
            dc_clr_objpriv_by_uid(&ogx->users[id]->obj_privs, user->desc.id);
        }
    }

    for (id = 0; id < OG_MAX_ROLES; id++) {
        if (!ogx->roles[id]) {
            continue;
        }

        dc_clr_role_objpriv_by_uid(ogx, ogx->roles[id], user->desc.id);
        /* need update all the role' children privileges */
    }
}

static void dc_revoke_objpriv_by_grantor(dc_context_t *ogx, dc_user_t *user, dc_obj_priv_item *priv_item, uint32 privid)
{
    ...
    cm_list_for_each_safe(item, temp, &user->grant_obj_privs)
    {
        ...
        if (entry->priv_item.objowner == priv_item->objowner && entry->priv_item.objtype == priv_item->objtype &&
            ...
            if (entry->grantee_type == TYPE_USER) {
                grantee_user = ogx->users[entry->grantee_id];
                (void)dc_revoke_objpriv_from_user_by_id(ogx, grantee_user, priv_item, privid);
            } else if (entry->grantee_type == TYPE_ROLE) {
                grantee_role = ogx->roles[entry->grantee_id];
                (void)dc_revoke_objpriv_from_role_by_id(ogx, grantee_role, priv_item, privid);
            }
            cm_list_remove(item);
        }
    }
...
static void dc_clear_grantor_objpriv_item(dc_context_t *ogx, dc_obj_priv_entry_t *entry, dc_obj_priv_item *priv_item,
    uint32 privid, uint32 grantee_id, uint32 grantee_type)
{
    ...
    if (grant_uid < OG_MAX_USERS && ogx->users[grant_uid] != NULL &&
        ogx->users[grant_uid]->status == USER_STATUS_NORMAL) {
        cm_list_for_each_safe(item, temp, &ogx->users[grant_uid]->grant_obj_privs)
        {
...
void dc_revoke_objpriv_from_user_by_id(dc_context_t *ogx, dc_user_t *user, dc_obj_priv_item *priv_item,
    uint32 privid)
{
    dc_obj_priv_entry_t *entry = NULL;
    text_t obj_name;
    ...
    if (dc_find_objpriv_entry(&user->obj_privs, priv_item->objowner, &obj_name, (uint32)priv_item->objtype, &entry)) {
        if (!DC_HAS_OBJ_PRIV(entry->priv_item.direct_grant, privid)) {
            entry->priv_item.grantor[privid] = OG_INVALID_ID32;
            return;
        } else {
            dc_clear_grantor_objpriv_item(ogx, entry, priv_item, privid, user->desc.id, TYPE_USER);
        }
    } else {
        return;
    }
    ...
    dc_update_user_objpriv_info(ogx, user, priv_item);

    dc_revoke_objpriv_by_grantor(ogx, user, priv_item, privid);
}

void dc_revoke_userpriv_from_user_by_id(dc_context_t *ogx, dc_user_t *user, uint32 grantee, uint32 privid)
{
    dc_user_priv_entry_t *entry = NULL;

    if (dc_find_user_priv_entry(&user->user_privs, grantee, &entry)) {
        if (!DC_HAS_OBJ_PRIV(entry->user_priv_item.privid_map, privid)) {
            entry->user_priv_item.grantor[privid] = OG_INVALID_ID32;
            return;
        }
    } else {
        return;
...
void dc_revoke_objpriv_from_role_by_id(dc_context_t *ogx, dc_role_t *role,
    dc_obj_priv_item *priv_item, uint32 privid)
{
    ...
    if (dc_find_objpriv_entry(&role->obj_privs, priv_item->objowner, &obj_name, (uint32)priv_item->objtype, &entry)) {
        if (!DC_HAS_OBJ_PRIV(entry->priv_item.direct_grant, privid)) {
            ...
            dc_clear_grantor_objpriv_item(ogx, entry, priv_item, privid, role->desc.id, TYPE_ROLE);
        }
    } else {
        return;
    }
    ...
    dc_update_user_objpriv_by_role(ogx, role, priv_item);
...
static void dc_clear_grantee_objprivs(dc_context_t *ogx, dc_user_t *user)
{
    dc_user_t *grantee_user = NULL;
    dc_role_t *grantee_role = NULL;
    cm_list_head *item = NULL;
    ...
    cm_list_for_each_safe(item, temp, &user->grant_obj_privs)
    {
        ...
        if (entry->grantee_type == TYPE_USER) {
            ...
            if (grantee_user->status == USER_STATUS_NORMAL) {
                dc_revoke_objpriv_from_user_by_id(ogx, grantee_user, &entry->priv_item, entry->priv_id);
            }
        } else if (entry->grantee_type == TYPE_ROLE) {
            grantee_role = ogx->roles[entry->grantee_id];
            /* the role may be dropped before */
            if (grantee_role != NULL) {
                dc_revoke_objpriv_from_role_by_id(ogx, grantee_role, &entry->priv_item, entry->priv_id);
            }
        }
        cm_list_remove(item);
    }
...
void dc_clear_grantor_objprivs(dc_context_t *ogx, dc_obj_priv_t *obj_privs, uint32 grantee_id, uint32 grantee_type)
{
    uint32 oid;
    uint32 pid;
    uint32 grantor_uid;
    ...
    for (oid = 0; oid < obj_privs->hwm; oid++) {
        ...
        if (entry != NULL && entry->valid) {
            for (pid = 0; pid < OG_OBJ_PRIVS_COUNT; pid++) {
                grantor_uid = entry->priv_item.grantor[pid];
                if (grantor_uid < OG_MAX_USERS) {
                    grantor = ogx->users[grantor_uid];
...
static void dc_update_role_owner(dc_context_t *ogx, uint32 uid)
{
    dc_role_t *role = NULL;

    for (uint32 i = 0; i < OG_MAX_ROLES; i++) {
        role = ogx->roles[i];
...
void dc_clear_role_priv(knl_session_t *session, dc_role_t *role)
{
    ...
    if (role == NULL) {
        OG_LOG_RUN_ERR("[DC] load role privilege failed, the role not exist");
        return;
    }
    dls_spin_lock(session, &role->lock, NULL);
    dc_clear_grantor_objprivs(ogx, &role->obj_privs, role->desc.id, TYPE_ROLE);
    ...
    cm_list_for_each_safe(item1, temp1, &role->child_roles)
    {
        ...
        dls_spin_lock(session, &child->lock, NULL);
        ...
        dls_spin_unlock(session, &child->lock);
    ...
    cm_list_for_each_safe(item1, temp1, &role->parent)
    {
        ...
        dls_spin_lock(session, &parent->lock, NULL);
        ...
        dls_spin_unlock(session, &parent->lock);
    }
...
void dc_clear_user_priv(dc_context_t *ogx, dc_user_t *user)
{
    dc_user_granted *child_user = NULL;
    ...
    dc_update_role_owner(ogx, user->desc.id);

    cm_list_init(&user->parent);
    cm_list_init(&user->grant_obj_privs);
}

static status_t dc_load_sys_priv(knl_session_t *session, knl_cursor_t *cursor)
{
    dc_context_t *ogx = &session->kernel->dc_ctx;
    ...
    if (grantee_type == 0) {
        ...
        if (grantee_id >= OG_MAX_USERS || !ogx->users[grantee_id]) {
            cm_spin_unlock(&ogx->lock);
            OG_THROW_ERROR(ERR_GRANTEE_EXCEED_MAX, "grantee", OG_MAX_USERS);
            return OG_ERROR;
        }

        dc_user_t *user = ogx->users[grantee_id];
        ...
        cm_spin_lock(&ogx->lock, NULL);
        if (grantee_id >= OG_MAX_ROLES || !ogx->roles[grantee_id]) {
            cm_spin_unlock(&ogx->lock);
            OG_THROW_ERROR(ERR_GRANTEE_EXCEED_MAX, "grantee", OG_MAX_ROLES);
            return OG_ERROR;
        }

        dc_role_t *role = ogx->roles[grantee_id];
...
static status_t dc_load_objpriv(knl_session_t *session, dc_context_t *ogx, knl_cursor_t *cursor)
{
    uint32 grantee_id;
    uint32 grantee_type;
    uint32 owner_id;
    ...
    if (grantee_type == 0) {
        ...
        user = ogx->users[grantee_id];
        if (!dc_find_objpriv_entry(&user->obj_privs, owner_id, &objname, objtype, &entry)) {
            if (dc_alloc_objpriv_entry(ogx, &user->obj_privs, user->memory, owner_id,
                &objname, objtype, &entry) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
        ...
        role = ogx->roles[grantee_id];
        if (!dc_find_objpriv_entry(&role->obj_privs, owner_id, &objname, objtype, &entry)) {
            if (dc_alloc_objpriv_entry(ogx, &role->obj_privs, role->memory, owner_id,
                &objname, objtype, &entry) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    ...
    if (OG_SUCCESS == dc_open_user_by_id(session, grant_uid, &grantor_user)) {
...
static status_t dc_load_user_priv(knl_session_t *session, dc_context_t *ogx, knl_cursor_t *cursor)
{
    ...
    if (!dc_find_user_priv_entry(&user->user_privs, grantee, &entry)) {
        if (dc_alloc_user_priv_entry(ogx, &user->user_privs, user->memory, grantee, &entry) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t dc_load_role_priv(knl_session_t *session, dc_context_t *ogx, knl_cursor_t *cursor)
{
    uint32 grantee_id;
    uint32 grantee_type;
    dc_user_t *user = NULL;
    ...
    role = ogx->roles[granted_role_id];
    if (role == NULL) {
        OG_THROW_ERROR(ERR_OBJECT_ID_NOT_EXIST, "granted role", granted_role_id);
        OG_LOG_RUN_ERR("[DC] failed to load role id:%u", granted_role_id);
        return OG_ERROR;
    }
    /* user type */
    if (grantee_type == 0) {
        ...
        if (grantee_id >= OG_MAX_USERS || !ogx->users[grantee_id]) {
            cm_spin_unlock(&ogx->lock);
            OG_THROW_ERROR(ERR_GRANTEE_EXCEED_MAX, "grantee", OG_MAX_USERS);
            OG_LOG_RUN_ERR("[DC] failed to load user id:%u", grantee_id);
            return OG_ERROR;
        }

        user = ogx->users[grantee_id];
        if (dc_alloc_mem(ogx, role->memory, sizeof(dc_user_granted), (void **)&user_grant) != OG_SUCCESS) {
            cm_spin_unlock(&ogx->lock);
            return OG_ERROR;
        }
        ...
        if (grantee_id >= OG_MAX_ROLES || !ogx->roles[grantee_id]) {
            cm_spin_unlock(&ogx->lock);
            OG_THROW_ERROR(ERR_GRANTEE_EXCEED_MAX, "grantee", OG_MAX_ROLES);
            OG_LOG_RUN_ERR("[DC] failed to load role id:%u", grantee_id);
            return OG_ERROR;
        }

        role2 = ogx->roles[grantee_id];
...
status_t dc_load_sys_privs_by_id(knl_session_t *session, uint32 id, uint32 type)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), l_border, OG_TYPE_INTEGER, (void *)&id, sizeof(uint32),
        IX_COL_SYS_PRIVS_001_GRANTEE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_border, OG_TYPE_INTEGER, (void *)&type, sizeof(uint32),
        IX_COL_SYS_PRIVS_001_GRANTEE_TYPE);
    knl_set_key_flag(l_border, SCAN_KEY_LEFT_INFINITE, IX_COL_SYS_PRIVS_001_RIVILEGE);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_border, OG_TYPE_INTEGER, (void *)&id, sizeof(uint32),
        IX_COL_SYS_PRIVS_001_GRANTEE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_border, OG_TYPE_INTEGER, (void *)&type, sizeof(uint32),
        IX_COL_SYS_PRIVS_001_GRANTEE_TYPE);
    ...
    while (!cursor->eof) {
        if (dc_load_sys_priv(session, cursor) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
...
static status_t dc_load_role_privs_as_grantee(knl_session_t *session, uint32 id, uint32 type)
{
    knl_cursor_t *cursor = NULL;
    knl_scan_key_t *l_border = NULL;
    knl_scan_key_t *r_border = NULL;
    ...
    while (!cursor->eof) {
        if (dc_load_role_priv(session, ogx, cursor) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
...
static status_t dc_load_role_privs_as_granted(knl_session_t *session, uint32 id)
{
    knl_cursor_t *cursor = NULL;
    knl_scan_key_t *l_border = NULL;
    knl_scan_key_t *r_border = NULL;
...
{
    if (dc_load_role_privs_as_grantee(session, id, type) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t dc_load_obj_privs_by_id(knl_session_t *session, uint32 id, uint32 type)
{
    ...
    while (!cursor->eof) {
        if (dc_load_objpriv(session, ogx, cursor) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
...
status_t dc_load_user_privs_by_id(knl_session_t *session, uint32 uid)
{
    ...
    while (!cursor->eof) {
        if (dc_load_user_priv(session, ogx, cursor) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
...
static status_t dc_load_sys_privs(knl_session_t *session, dc_context_t *ogx)
{
    knl_cursor_t *cursor = NULL;
    ...
    /* restore privileges from SYS_PRIVS$ table: privileges directly granted to the user */
    cursor = knl_push_cursor(session);
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_SELECT, SYS_PRIVS_ID, OG_INVALID_ID32);
...
static status_t dc_load_role_privs(knl_session_t *session, dc_context_t *ogx)
{
    knl_cursor_t *cursor = NULL;
    ...
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_SELECT, SYS_USER_ROLES_ID, OG_INVALID_ID32);
...
static status_t dc_load_obj_privs(knl_session_t *session, dc_context_t *ogx)
{
    knl_cursor_t *cursor = NULL;
    ...
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_SELECT, OBJECT_PRIVS_ID, OG_INVALID_ID32);
...
static status_t dc_load_user_privs(knl_session_t *session, dc_context_t *ogx)
{
    knl_cursor_t *cursor = NULL;
    ...
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_SELECT, SYS_USER_PRIVS_ID, OG_INVALID_ID32);
...
status_t dc_load_privileges(knl_session_t *session, dc_context_t *ogx)
{
    if (dc_load_sys_privs(session, ogx) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_load_role_privs(session, ogx) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_load_obj_privs(session, ogx) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_load_user_privs(session, ogx) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static bool32 dc_find_user_grant_objpriv(dc_user_t *user, uint32 grantee_type, uint32 grantee_id,
    ...
    dc_obj_priv_item *priv_item, uint32 priv_id)
{
    ...
    if (!dc_find_user_grant_objpriv(user, grantee_type, grantee_id, priv_item, &grant_obj_priv, priv_id)) {
        if (dc_alloc_mem(ogx, user->memory, sizeof(dc_grant_obj_priv),
            (void **)&grant_obj_priv) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        objname_len = OG_NAME_BUFFER_SIZE - 1;
        err = strncpy_s(grant_obj_priv->priv_item.objname, OG_NAME_BUFFER_SIZE, priv_item->objname,
            objname_len);
        knl_securec_check(err);
                                 ...
                                 text_t *objname, object_type_t objtype, uint32 privid)
{
    ...
    if (dc_open_user(session, curr_user, &user) != OG_SUCCESS) {
        return OG_FALSE;
    }
...
bool32 dc_sys_priv_with_option(knl_session_t *session, text_t *user, uint32 priv_id)
{
    ...
    for (i = 0; i < OG_MAX_USERS; i++) {
        cm_spin_lock(&ogx->lock, NULL);
        if (ogx->users[i] != NULL && cm_text_str_equal(user, ogx->users[i]->desc.name)) {
            if (DC_HAS_SYS_PRIV(ogx->users[i]->all_sys_privs, priv_id) &&
                DC_HAS_SYS_OPT(ogx->users[i]->ter_admin_opt, priv_id)) {
                cm_spin_unlock(&ogx->lock);
                return OG_TRUE;
            }
        }
    ...
    if (DC_HAS_SYS_PRIV(ogx->users[DB_PUB_USER_ID]->all_sys_privs, priv_id) &&
        DC_HAS_SYS_OPT(ogx->users[DB_PUB_USER_ID]->ter_admin_opt, priv_id)) {
        return OG_TRUE;
    }
...
bool32 dc_grant_role_with_option(knl_session_t *session, text_t *username, text_t *rolename, bool32 with_option)
{
    ...
    if (!dc_get_role_id(session, rolename, &rid)) {
        OG_THROW_ERROR(ERR_ROLE_NOT_EXIST, T2S(rolename));
        return OG_FALSE;
    }
...
static void dc_update_obj_entry(dc_obj_priv_entry_t *entry, const char *oldname, text_t *newname)
{
    if (!entry->valid) {
        return;
    }
    ...
    if (cm_str_equal(oldname, entry->priv_item.objname)) {
        (void)cm_text2str(newname, entry->priv_item.objname, OG_NAME_BUFFER_SIZE);
    }

    cm_spin_unlock(&entry->bucket->lock);
}

void dc_drop_object_privs(dc_context_t *ogx, uint32 objowner, char *objname, uint32 objtype)
{
    uint32 i;
    text_t name;
    dc_user_t *user = NULL;
...
static void dc_collect_roles_privs(dc_role_t *role, uint8 *sys_privs, uint8 *admin_opt)
{
    uint32 i;
    cm_list_head *item = NULL;
    dc_granted_role *parent = NULL;
    ...
    cm_list_for_each(item, &role->parent)
    {
        ...
        for (i = 0; i < OG_SYS_PRIVS_BYTES; i++) {
...
*/
static void dc_collect_user_priv(dc_user_t *user, uint8 *sys_privs, uint8 *admin_opt, uint32 array_len)
{
    uint32 i;
    cm_list_head *item = NULL;
    dc_granted_role *parent = NULL;
    ...
    knl_panic(array_len <= OG_SYS_PRIVS_BYTES);
...
{
    uint32 i;
    uint8 sys_privs[OG_SYS_PRIVS_BYTES];
    uint8 admin_opt[OG_SYS_PRIVS_BYTES];
    errno_t err;

    err = memset_sp(sys_privs, OG_SYS_PRIVS_BYTES, 0x0, OG_SYS_PRIVS_BYTES);
    knl_securec_check(err);
    err = memset_sp(admin_opt, OG_SYS_PRIVS_BYTES, 0x0, OG_SYS_PRIVS_BYTES);
    knl_securec_check(err);

    /* collect roles that granted to the user */
    dc_collect_user_priv(user, sys_privs, admin_opt, OG_SYS_PRIVS_BYTES);
    ...
    err = memcpy_sp(user->all_sys_privs, OG_SYS_PRIVS_BYTES, sys_privs, OG_SYS_PRIVS_BYTES);
    knl_securec_check(err);
    err = memcpy_sp(user->ter_admin_opt, OG_SYS_PRIVS_BYTES, admin_opt, OG_SYS_PRIVS_BYTES);
    knl_securec_check(err);

    return;
}

static void dc_collect_roles_objprivs(dc_role_t *role, uint32 *objprivs, dc_obj_priv_item *priv_item)
{
    text_t objname;
    cm_list_head *item = NULL;
    dc_granted_role *parent = NULL;
...
*  (include indirectly granted through other roles) in to a list
*/
static void dc_collect_user_objpriv(dc_user_t *user, uint32 *objprivs, dc_obj_priv_item *priv_item)
{
    text_t objname;
    cm_list_head *item = NULL;
    dc_granted_role *parent = NULL;
...
void dc_update_user_objpriv_info(dc_context_t *ogx, dc_user_t *user, dc_obj_priv_item *priv_item)
{
    uint32 priv = 0;
    text_t objname;
    dc_obj_priv_entry_t *entry = NULL;
    ...
    if (!dc_find_objpriv_entry(&user->obj_privs, priv_item->objowner, &objname, priv_item->objtype, &entry)) {
        ...
        if (dc_alloc_objpriv_entry(ogx, &user->obj_privs, user->memory, priv_item->objowner,
            &objname, priv_item->objtype, &entry) != OG_SUCCESS) {
            return;
        }
    }
...
static void dc_merge_role_objpriv_to_user(knl_session_t *session, dc_role_t *role, dc_user_t *user)
{
    uint32 eid;
    text_t objname;
    dc_obj_priv_entry_t *entry = NULL;
...
static void dc_merge_parent_objprivs(knl_session_t *session, dc_user_t *user, cm_list_head *rolelist)
{
    cm_list_head *item1 = NULL;
    dc_role_t *role = NULL;
    dc_granted_role *parent = NULL;
...
* @see NA
*/
void dc_update_user_objpriv_by_role(dc_context_t *ogx, dc_role_t *role, dc_obj_priv_item *priv_item)
{
    cm_list_head *item = NULL;
    dc_user_granted *child_user = NULL;
    dc_granted_role *child_role = NULL;
    ...
    {
        child_user = cm_list_entry(item, dc_user_granted, node);
        dc_update_user_objpriv_info(ogx, child_user->user_granted, priv_item);
    }

    cm_list_for_each(item, &role->child_roles)
    {
        child_role = cm_list_entry(item, dc_granted_role, node);
        dc_update_user_objpriv_by_role(ogx, child_role->granted_role, priv_item);
    }
}
...
static bool32 dc_try_reuse_userpriv_entry(dc_user_priv_t *user_privs, dc_user_priv_entry_t **dc_entry)
{
    dc_user_priv_entry_t *entry = NULL;
        ...
        if (user != NULL && user->status == USER_STATUS_NORMAL) {
            ...
            cm_list_for_each(item, &user->grant_obj_privs)
            {
                ...
                if (cm_str_equal(grant_obj_priv->priv_item.objname, oldname) &&
                    ...
                    (void)cm_text2str(newname, grant_obj_priv->priv_item.objname, OG_NAME_BUFFER_SIZE);
                }
            }
        }
    }
...
static void dc_init_objpriv_entry(dc_obj_priv_entry_t *entry, uint32 owner_uid, text_t *obj_name, uint32 obj_type)
{
    uint32 pid;
    /* fill the privilege information */
    (void)cm_text2str(obj_name, entry->priv_item.objname, OG_NAME_BUFFER_SIZE);
    ...
    for (pid = 0; pid < OG_OBJ_PRIVS_COUNT; pid++) {
        entry->priv_item.grantor[pid] = OG_INVALID_ID32;
    }
    return;
}
status_t dc_alloc_objpriv_entry(dc_context_t *ogx, dc_obj_priv_t *obj_privs, memory_context_t *memory, uint32 owner_uid,
                                text_t *obj_name, uint32 obj_type, dc_obj_priv_entry_t **dc_entry)
{
    ...
    if (!dc_try_reuse_objpriv_entry(obj_privs, &entry)) {
        if (oid >= DC_GROUP_SIZE * DC_GROUP_SIZE) {
            ...
            OG_LOG_RUN_ERR("[DC] failed to alloc objpriv entry");
            cm_spin_unlock(&obj_privs->lock);
            return OG_ERROR;
        }
        ...
        do {
            ...
            if (dc_alloc_mem(ogx, memory, sizeof(dc_obj_priv_entry_t), (void **)&entry) != OG_SUCCESS) {
...
status_t dc_alloc_user_priv_entry(dc_context_t *ogx, dc_user_priv_t *user_privs, memory_context_t *memory,
                                  uint32 grantee, dc_user_priv_entry_t **dc_entry)
{
    ...
    if (!dc_try_reuse_userpriv_entry(user_privs, &entry)) {
        if (oid >= USER_PRIV_GROUP_COUNT * DC_GROUP_SIZE) {
            ...
            OG_LOG_RUN_ERR("[DC] failed to alloc userpriv entry");
            cm_spin_unlock(&user_privs->lock);
            return OG_ERROR;
        }
        ...
        do {
            ...
            if (dc_alloc_mem(ogx, memory, sizeof(dc_user_priv_entry_t), (void **)&entry) != OG_SUCCESS) {
                OG_THROW_ERROR(ERR_DC_BUFFER_FULL);
                cm_spin_unlock(&user_privs->lock);
                return OG_ERROR;
            }
    ...
    for (pid = 0; pid < OG_USER_PRIVS_COUNT; pid++) {
        entry->user_priv_item.grantor[pid] = OG_INVALID_ID32;
    }
