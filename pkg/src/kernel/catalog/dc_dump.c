...
status_t dc_dump_prepare(cm_dump_t *dump, dc_dump_info_t *info, char *file_name, uint32 name_size)
{
    if (!CM_IS_EMPTY(&info->dump_file)) {
        ...
        if (info->dump_file.len >= name_size) {
            OG_THROW_ERROR(ERR_INVALID_FILE_NAME, T2S(&info->dump_file), name_size);
            return OG_ERROR;
        }
        ...
        if (cm_file_exist(file_name)) {
            OG_THROW_ERROR(ERR_FILE_ALREADY_EXIST, file_name, "failed to dump catalog");
            return OG_ERROR;
        } else if (cm_create_file(file_name, O_RDWR | O_BINARY | O_SYNC, &dump->handle) != OG_SUCCESS) {
            return OG_ERROR;
        }
        return OG_SUCCESS;
    }

    if (cm_open_file(file_name, O_RDWR | O_CREAT | O_APPEND, &dump->handle) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (file_size > MAX_DUMP_FILE_SIZE) {
        OG_LOG_RUN_ERR("[DB] the size of dump file %s is more than %d, need delete first",
            file_name, MAX_DUMP_FILE_SIZE);
        OG_THROW_ERROR(ERR_DATAFILE_RESIZE_EXCEED, file_size, MAX_DUMP_FILE_SIZE);
...
static status_t dc_dump_index(cm_dump_t *dump, index_t *index)
{
    ...
    if (btree != NULL) {
        ...
        cm_dump(dump, "wait_recycle: %u\n", btree->wait_recycle);
        cm_dump(dump, "is_recycling: %u\n", btree->is_recycling);
        cm_dump(dump, "alloc_pages: %lld\n", btree->chg_stats.alloc_pages);
        cm_dump(dump, "delete_size: %lld\n", btree->chg_stats.delete_size);
        cm_dump(dump, "empty_size: %llu\n", btree->chg_stats.empty_size);
        cm_dump(dump, "first_empty_size: %llu\n", btree->chg_stats.first_empty_size);
        cm_dump(dump, "ow_del_scn: %llu\n", btree->chg_stats.ow_del_scn);
...
status_t dc_dump_table(knl_session_t *session, cm_dump_t *dump, dc_dump_info_t info)
{
    ...
    if (knl_open_dc(session, &info.user_name, &info.table_name, &dc) != OG_SUCCESS) {
        return OG_ERROR;
    }

    errno_t ret = memset_sp(file_name, OG_MAX_FILE_NAME_LEN, 0, OG_MAX_FILE_NAME_LEN);
    knl_securec_check(ret);
    ret = snprintf_s(file_name, OG_MAX_FILE_NAME_LEN, OG_MAX_FILE_NAME_LEN - 1, "%s/trc/%s_%s_DUMP.trc",
        session->kernel->home, T2S(&info.user_name), T2S_EX(&info.table_name));
    knl_securec_check_ss(ret);

    if (dc_dump_prepare(dump, &info, file_name, OG_MAX_FILE_NAME_LEN) != OG_SUCCESS) {
        knl_close_dc(&dc);
        return OG_ERROR;
    }
    ...
    dc_entity_t *entity = DC_ENTITY(&dc);
    dump->offset = 0;
    if (dc_dump_table_entry(dump, info, dc) != OG_SUCCESS) {
        knl_close_dc(&dc);
        cm_close_file(dump->handle);
        return OG_ERROR;
    }
    ...
    for (uint32 i = 0; i < entity->table.index_set.total_count; i++) {
        ...
        if (dc_dump_index(dump, index) != OG_SUCCESS) {
            knl_close_dc(&dc);
            cm_close_file(dump->handle);
            return OG_ERROR;
        }
    ...
    if (cm_dump_flush(dump) != OG_SUCCESS) {
        knl_close_dc(&dc);
        cm_close_file(dump->handle);
        return OG_ERROR;
    }
...
status_t dc_dump_user(knl_session_t *session, cm_dump_t *dump, dc_dump_info_t info)
{
    ...
    if (dc_open_user(session, &info.user_name, &user) != OG_SUCCESS) {
        return OG_ERROR;
    }

    errno_t ret = memset_sp(file_name, OG_MAX_FILE_NAME_LEN, 0, OG_MAX_FILE_NAME_LEN);
    knl_securec_check(ret);
    ret = snprintf_s(file_name, OG_MAX_FILE_NAME_LEN, OG_MAX_FILE_NAME_LEN - 1, "%s/trc/USER_%s_DUMP.trc",
    ...
    cm_dump(dump, "user_latch: %d /* avoid concurrent execution of drop user and create/drop object */\n",
            user->user_latch.latch.stat);
    cm_dump(dump, "lib_latch: %d  /* latch for pl */\n", user->lib_latch.latch.stat);
    CM_DUMP_WRITE_FILE(dump);
