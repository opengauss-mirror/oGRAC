...
static inline void abr_set_task(lsnd_abr_task_t *task, uint16 file, uint32 page, char *buf, uint32 buf_size)
{
    ...
    task->running = OG_TRUE;
    task->timestamp = cm_current_time();
}

static bool32 abr_create_task(knl_session_t *session, buf_ctrl_t *ctrl, lsnd_abr_task_t *task)
{
    ...
    if (!task->running && !task->succeeded) {
        ...
        OG_LOG_RUN_INF("[ABR] create ABR task for file %u page %u with lsnd id %u", file, page, task->lsnd_id);
...
void abr_finish_task(lsnd_abr_task_t *task, bool32 succeeded, const char *buf, uint32 buf_size)
{
    ...
    task->running = OG_FALSE;
    OG_LOG_RUN_INF("[ABR] finish ABR task state %d for file %u page %u with lsnd id %u",
...
bool32 abr_notify_task(knl_session_t *session, buf_ctrl_t *ctrl, lsnd_abr_task_t **task_handle)
{
    ...
    bool32 notified = OG_FALSE;
    uint16 file = ctrl->page_id.file;
    uint32 page = ctrl->page_id.page;

    if (!session->kernel->attr.enable_abr) {
        OG_LOG_RUN_INF("[ABR] failed to create ABR task for file %u page %u due to abr disabled", file, page);
        return OG_FALSE;
    }

    if (lsnd_ctx->est_standby_num == 0) {
        OG_LOG_RUN_INF("[ABR] failed to create ABR task for file %u page %u due to no valid standby", file, page);
        return OG_FALSE;
    }

    while (cm_current_time() - now < abr_timeout) {
        cm_latch_s(&lsnd_ctx->latch, SESSION_ID_LSND, OG_FALSE, NULL);
        ...
        if (session->killed || session->canceled || session->force_kill || !session->kernel->attr.enable_abr) {
            OG_LOG_RUN_INF("[ABR] failed to notify for file %u page %u due to session killed %d or abr disabled %d",
    ...
    if (!notified) {
        OG_LOG_RUN_INF("[ABR] failed to notify ABR for file %u page %u due to timeout", file, page);
    }
...
bool32 abr_wait_task_done(knl_session_t *session, lsnd_abr_task_t *task_handle)
{
    ...
    while (cm_current_time() - now < abr_time_out) {
        ...
        if (session->killed || session->canceled || session->force_kill || !session->kernel->attr.enable_abr) {
            OG_LOG_RUN_INF("[ABR] failed to wait ABR task done due to session killed %d or abr disabled %d",
                           (session->killed || session->canceled), !session->kernel->attr.enable_abr);
            abr_finish_task(task, OG_FALSE, NULL, OG_INVALID_ID32);
            return OG_FALSE;
        }
        cm_sleep(10);
    }
    OG_LOG_RUN_INF("[ABR] failed to wait ABR task done due to timeout");
    abr_finish_task(task, OG_FALSE, NULL, OG_INVALID_ID32);
    return OG_FALSE;
}
...
void abr_try_save_page(knl_session_t *session, page_head_t *page)
{
    ...
    if (spc_write_datafile(session, df, handle, offset, page, PAGE_SIZE(*page)) != OG_SUCCESS) {
        ...
        OG_LOG_RUN_WAR("[ABR] failed to write page (file %u, page %u) to datafile %s",
                       (uint32)page_id->file, page_id->page, df->ctrl->name);
    }

    if (db_fdatasync_file(session, *handle) != OG_SUCCESS) {
        OG_LOG_RUN_WAR("[ABR] failed to fdatasync datafile %s", df->ctrl->name);
        spc_close_datafile(df, handle);
    }
}
...
status_t abr_send_page_fetch_req(lsnd_t *lsnd, lsnd_abr_task_t *task)
{
    ...
    if (cs_write_stream(&lsnd->pipe, (char *)&rep_msg_header, sizeof(rep_msg_header_t),
        ...
        OG_LOG_RUN_ERR("[ABR] failed to send abr request header to standby");
        return OG_ERROR;
    }

    if (cs_write_stream(&lsnd->pipe, (char *)&abr_req, sizeof(rep_abr_req_t),
                        (int32)cm_atomic_get(&lsnd->session->kernel->attr.repl_pkg_size)) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[ABR] failed to send abr request data to standby");
        return OG_ERROR;
    }
...
bool32 abr_repair_page_from_standy(knl_session_t *session, buf_ctrl_t *ctrl)
{
    ...
    OG_LOG_RUN_WAR("[ABR] find corrupted page(file %u, page %u)", ctrl->page_id.file, ctrl->page_id.page);
    if (abr_wait_task_done(session, handle)) {
        if (CHECK_PAGE_PCN(ctrl->page)) {
            abr_try_save_page(session, ctrl->page);
            OG_LOG_RUN_WAR("[ABR] corrupted page(file %u, page %u) has been fixed successfully",
                           ctrl->page_id.file, ctrl->page_id.page);
            return OG_TRUE;
        } else {
            OG_LOG_RUN_ERR("[ABR] failed to fix corrupted page(file %u, page %u)",
                           ctrl->page_id.file, ctrl->page_id.page);
        }
    }
...
status_t abr_wait_paral_rcy_compelte(knl_session_t *session)
{
    ...
    while (rcy->wait_stats_view[WAIT_RELAPY_COUNT] == curr_wait_count) {
        ...
        if (abr_timeout == 0) {
            OG_LOG_RUN_ERR("[Log Receiver] ABR standby wait replay timeout");
            return OG_ERROR;
        }
...
bool32 abr_verify_pageid(knl_session_t *session, page_id_t page_id)
{
    ...
    if (page_id.file == dw_file_id && page_id.page > SPACE_ENTRY_PAGE && page_id.page < DW_SPC_HWM_START) {
        OG_THROW_ERROR(ERR_INVALID_PAGE_ID, ", double write page is unsupported");
        return OG_FALSE;
    }

    df = DATAFILE_GET(session, page_id.file);
    if (!df->ctrl->used || !DATAFILE_IS_ONLINE(df)) {
        OG_THROW_ERROR(ERR_INVALID_PAGE_ID, ", datafile is unused");
        return OG_FALSE;
    }
    ...
    if (SPACE_IS_NOLOGGING(space)) {
        if (page_id.page != SPACE_ENTRY_PAGE || df->file_no != 0) { // space head repairing is supported
            OG_THROW_ERROR(ERR_INVALID_PAGE_ID, ", temporary or nologging tablespace page is unsupported");
            return OG_FALSE;
        }
    }

    if ((uint64)df->ctrl->size < (uint64)(page_id.page + 1) * DEFAULT_PAGE_SIZE(session)) {
        OG_THROW_ERROR(ERR_INVALID_PAGE_ID, ", block offset is out of datafile's size");
        return OG_FALSE;
    }
    ...
    *find = OG_FALSE;
    ...
    for (;;) {
        if (knl_decompress(bak->record.attr.compress, compress_ctx, last_package, compress_buf + left_size,
            BACKUP_BUFFER_SIZE(bak) - left_size) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t abr_search_page_from_compress(knl_session_t *session, bak_page_search_t *search_ctx, bool32 *find)
{
    ...
    *find = OG_FALSE;
    ...
    bak->compress_ctx.finished = OG_FALSE;
    ...
    while (!last_package) {
        if (cm_read_device_nocheck(search_ctx->file_type, search_ctx->handle, offset, search_ctx->read_buf.aligned_buf,
            BACKUP_BUFFER_SIZE(bak),
        ...
        if (abr_decompress_and_search_bakfile(bak, search_ctx, last_package, (uint32)read_size, find) != OG_SUCCESS) {
            status = OG_ERROR;
            break;
        }
    offset += read_size;
...
static status_t abr_get_disk_page_id(bak_page_search_t *search_ctx, int64 offset, page_id_t *page_id)
{
    ...
    if (cm_read_device(search_ctx->file_type, handle, offset, page, search_ctx->page_size) != OG_SUCCESS) {
...
static status_t abr_binary_search_bakfile(knl_session_t *session, bak_page_search_t *search_ctx, bool32 *find)
{
    ...
    *find = OG_FALSE;
    ...
    while (low_offset <= high_offset) {
        ...
        if (abr_get_disk_page_id(search_ctx, mid_offset * page_size, &tmp_page_id) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        if (PAGID_LT(tmp_page_id, page_id)) {
            ...
            if (cm_read_device(search_ctx->file_type, search_ctx->handle, mid_offset * page_size,
                               (void *)search_ctx->read_buf.aligned_buf, page_size) != OG_SUCCESS) {
                return OG_ERROR;
            }
            *find = OG_TRUE;
            return OG_SUCCESS;
...
static status_t abr_flush_repaired_page(knl_session_t *session, page_head_t *page_repaired)
{
    ...
    if (cks_level == (uint32)CKS_OFF) {
        PAGE_CHECKSUM(page_repaired, DEFAULT_PAGE_SIZE(session)) = OG_INVALID_CHECKSUM;
    } else {
    ...
    errcode = snprintf_s(file_name, OG_FILE_NAME_BUFFER_SIZE, OG_FILE_NAME_BUFFER_SIZE - 1, "%s/data/page_%u_%u",
                         session->kernel->home, page_id.file, page_id.page);
    knl_securec_check_ss(errcode);

    if (cm_create_file(file_name, O_BINARY | O_SYNC | O_RDWR | O_EXCL, &handle) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (cm_write_file(handle, page_repaired, (int32)DEFAULT_PAGE_SIZE(session)) != OG_SUCCESS) {
                                          ...
                                          page_head_t *page_backup)
{
    ...
    OG_LOG_RUN_INF("[ABR] begine to relplay page (%u-%u) from log file:%u, block id:%u, lfn:%llu\n",
    ...
    if (bak->lfn > 0) {
        ...
        OG_LOG_RUN_INF("abr replay page to latest bak->lfn %llu", bak->lfn);
    } else {
        rcy->abr_db_status = DB_STATUS_MOUNT;
        lrp_point = dtc_my_ctrl(session)->lrp_point;
        OG_LOG_RUN_INF("abr replay page to latest dtc my ctrl [%llu-%llu]", (uint64)lrp_point.lfn, (uint64)lrp_point.lsn);
    }

    OG_LOG_RUN_INF("[ABR] recovery expected least end with file:%u,point:%u,lfn:%llu",
                                         ...
                                         page_id_t page_id, uint32 page_size)
{
    ...
    for (uint32 id = 0; id < head->file_count; id++) {
        ...
        OG_LOG_RUN_INF("[ABR] abr_bak_index_contain_page for name:%s id:%u type:%u page_id.file:%u", bak->files[id].spc_name, bak->files[id].id,
...
static status_t abr_open_bak_file(knl_session_t *session, const char *path, bak_local_t *bak_file, bak_file_type_t file_type,
                           uint32 index, uint32 file_id, uint32 sec_id)
{
    ...
    OG_LOG_RUN_INF("[ABR] abr_open_bak_file bak_file->name is %s", bak_file->name);
    if (cm_open_device(bak_file->name, cm_device_type(bak_file->name), O_BINARY | O_SYNC | O_RDWR, &bak_file->handle)
        != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[BACKUP] failed to open backupset file, path is %s", path);
        return OG_ERROR;
    }
    return OG_SUCCESS;
}
                                         ...
                                         bak_dependence_t *next_depend)
{
    ...
    OG_LOG_DEBUG_INF("[ABR] backupset path is %s", path);

    if (abr_open_bak_file(session, path, bak_handle, BACKUP_HEAD_FILE, 0, 0, 0) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (rst_restore_backupset_head(session, OG_TRUE) != OG_SUCCESS) {
        cm_close_device(cm_device_type(bak_handle->name), &bak_handle->handle);
        bak_handle->handle = OG_INVALID_HANDLE;
        return OG_ERROR;
    }
    cm_close_device(cm_device_type(bak_handle->name), &bak_handle->handle);
    bak_handle->handle = OG_INVALID_HANDLE;
    ...
    if (id != OG_INVALID_ID32) {
        ...
        OG_LOG_RUN_INF("[ABR] abr_open_bak_file bak->files[id].id:%u bak_file->name is %s type:%u id:%u", bak->files[id].id , bak_handle->name, bak_handle->type, id);
        if (abr_open_bak_file(session, path, bak_handle, BACKUP_DATA_FILE, id,
                              bak->files[id].id, bak->files[id].sec_id) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    bak_handle->handle = OG_INVALID_HANDLE;  // bak_handle->handle will not be used
    search_ctx->rcy_point = bak_head->ctrlinfo.rcy_point;

    if (search_ctx->max_rcy_point.asn == OG_INVALID_ASN) {
        if (!DB_CLUSTER_NO_CMS){
            knl_panic(bak_head->ctrlinfo.rcy_point.asn != OG_INVALID_ASN);
        }
        search_ctx->max_rcy_point = bak_head->ctrlinfo.rcy_point;
    }

    OG_LOG_RUN_INF("[ABR] backup rcy point [%llu-%u-%u] lfn:%llu in backup %s",
...
static status_t abr_search_page_from_bakfile(knl_session_t *session, bak_page_search_t *search_ctx, bool32 *find)
{
    ...
    if (bak->record.attr.compress != COMPRESS_NONE) {
        if (abr_search_page_from_compress(session, search_ctx, find) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("[ABR] failed to search page from compressed file");
            return OG_ERROR;
        }
    } else if (bak->record.attr.level == 1) {
        if (abr_binary_search_bakfile(session, search_ctx, find) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("[ABR] failed to binary search page from buckup file");
            return OG_ERROR;
        }
    } else {
        ...
        if (file_size < 0 || (offset + search_ctx->page_size) > (uint64)file_size) {
            OG_LOG_RUN_ERR("[ABR] invalid backup file size %lld, page offset is %llu", file_size, offset);
            return OG_ERROR;
        }

        if (cm_read_device(search_ctx->file_type, search_ctx->handle, offset, (void *)search_ctx->read_buf.aligned_buf,
                           search_ctx->page_size) != OG_SUCCESS) {
            return OG_ERROR;
        }
        OG_LOG_DEBUG_INF("[ABR] search_ctx->sec_start %llu, offset %llu", search_ctx->sec_start, offset);
...
static status_t abr_search_page_from_backupset(knl_session_t *session, bak_page_search_t *search_ctx, bool32 *find,
                                        const char *bak_path)
{
    bak_context_t *ogx = &session->kernel->backup_ctx;
    bak_t *bak = &ogx->bak;
    ...
    *find = OG_FALSE;
    ret = strcpy_sp(path, OG_FILE_NAME_BUFFER_SIZE, bak_path);
    knl_securec_check(ret);

    while (!(*find)) {
        start_time = cm_now();
        if (abr_bakfile_contain_page(session, search_ctx, path, &next_incr_backup) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (search_ctx->handle != OG_INVALID_HANDLE) {
            if (abr_search_page_from_bakfile(session, search_ctx, find) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
        OG_LOG_RUN_INF("[ABR] page search time %lldms, is found %u, backupset %s",
        ...
        ret = strcpy_sp(path, OG_FILE_NAME_BUFFER_SIZE, next_incr_backup.file_dest);
        knl_securec_check(ret);
    }
    ...
    if (abr_open_bak_file(session, bak_path, &bak->local, BACKUP_HEAD_FILE, 0, 0, 0) != OG_SUCCESS) {
        return OG_ERROR;
    }
 ...
 * If failed, we continue repair, otherwise, return OG_SUCCESS
 */
bool32 abr_precheck_corrupted_page(knl_session_t *session, page_id_t page_id)
{
    ...
    char *buf = (char *)cm_push(session->stack, (uint32)(DEFAULT_PAGE_SIZE(session) + OG_MAX_ALIGN_SIZE_4K));
    ...
    if (PAGE_CHECKSUM(page, DEFAULT_PAGE_SIZE(session)) != OG_INVALID_CHECKSUM &&
        ...
        OG_LOG_RUN_WAR("[ABR] input page is not corrupted, block recover will not continue");
        cm_pop(session->stack);
        return OG_TRUE;
    }
...
status_t abr_repair_page_from_backup(knl_session_t *session, bak_page_search_t *search_ctx, const char *path)
{
    ...
    if (abr_search_page_from_backupset(session, search_ctx, &find, path) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    OG_LOG_RUN_INF("[ABR] total search time %lldms, is found %u",
                   (cm_now() - start_time) / (int32)MICROSECS_PER_MILLISEC, (uint32)find);

    if (find) {
        if (PAGE_CHECKSUM(page, DEFAULT_PAGE_SIZE(session)) != OG_INVALID_CHECKSUM &&
            !page_verify_checksum(page, DEFAULT_PAGE_SIZE(session))) {
            OG_THROW_ERROR(ERR_CHECKSUM_FAILED, path);
            return OG_ERROR;
        }
        ...
        OG_LOG_RUN_INF("[ABR] find page (%u-%u) in backup", page_id.file, page_id.page);
    } else {
        ret = memset_sp(page, DEFAULT_PAGE_SIZE(session), 0, DEFAULT_PAGE_SIZE(session));
        knl_securec_check(ret);
        OG_LOG_RUN_INF("[ABR] dose not find page (%u-%u) in backup", page_id.file, page_id.page);
    }

    start_time = cm_now();
    if (abr_replay_page_to_latest(session, rcy_point, page_id, page) != OG_SUCCESS) {
        return OG_ERROR;
    }
    OG_LOG_RUN_INF("[ABR] page replay time %lldms, from point [%llu-%u-%u] lfn: %llu",
    ...
    if (abr_flush_repaired_page(session, page) != OG_SUCCESS) {
        return OG_ERROR;
    }

    OG_LOG_RUN_INF("[ABR] page (%u-%u) repair succesefully, lsn %llu, checksum %u",
...
static void abr_clear_search_context(knl_session_t *session, bak_page_search_t *search_ctx)
{
    ...
    search_ctx->handle = OG_INVALID_HANDLE;
...
status_t abr_restore_block_recover(knl_session_t *session, knl_restore_t *param)
{
    ...
    bak->backup_buf_size = OG_MIN_BACKUP_BUF_SIZE;

    if (cm_text2str(&param->path, bak->record.path, OG_FILE_NAME_BUFFER_SIZE) != OG_SUCCESS) {
        cm_spin_unlock(&session->kernel->lock);
        return OG_ERROR;
    }

    OG_LOG_RUN_INF("[ABR] start reparing corrupted page (file %u, page %u), buckupset path:%s",
                   page_id.file, page_id.page, bak->record.path);

    search_ctx.handle = OG_INVALID_HANDLE;
    search_ctx.page_id = page_id;
    search_ctx.page_size = DEFAULT_PAGE_SIZE(session);
    search_ctx.max_rcy_point.asn = OG_INVALID_ASN;

    if (cm_aligned_malloc((int64)BACKUP_BUFFER_SIZE(bak), "ABR", &search_ctx.read_buf) != OG_SUCCESS) {
        OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)(BACKUP_BUFFER_SIZE(bak)), "page repair");
        OG_LOG_RUN_ERR("[ABR] failed to malloc read buffer");
        cm_spin_unlock(&session->kernel->lock);
        return OG_ERROR;
    }
    ...
    if (abr_repair_page_from_backup(session, &search_ctx, bak->record.path) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[ABR] failed to repair page");
        abr_clear_search_context(session, &search_ctx);
        cm_spin_unlock(&session->kernel->lock);
        return OG_ERROR;
    }
...
static status_t abr_recover_file_lastest(knl_session_t *session, knl_restore_t *param)
{
    ...
    if (bak->record.device == DEVICE_DISK) {
        if (abr_open_bak_file(session, bak->record.path, &bak->local, BACKUP_HEAD_FILE, 0, 0, 0) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (rst_restore_backupset_head(session, OG_FALSE) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    OG_LOG_RUN_INF("[ABR] begin to recover file: id %u, from log file:%u, block id:%u, lfn:%llu\n",
        bak->rst_file.file_id, rcy_point.asn, rcy_point.block_id, (uint64)rcy_point.lfn);

    OG_LOG_RUN_INF("[ABR] recovery file: id %u, expected least end with file:%u,point:%u,lfn:%llu",
    ...
    session->kernel->rcy_ctx.is_file_repair = OG_TRUE;
    if (abr_replay_file_log(session, rcy_point) != OG_SUCCESS) {
        session->kernel->rcy_ctx.is_file_repair = OG_FALSE;
        return OG_ERROR;
    }
...
static status_t abr_init_rst_info(knl_session_t *session, bak_context_t *ogx, knl_restore_t *param)
{
    datafile_ctrl_t *df_ctrl = NULL;

    if (param->file_repair == OG_INVALID_FILEID) {
        df_ctrl = abr_get_fileid_by_name(session, &param->file_repair_name);
        if (df_ctrl == NULL) {
            OG_THROW_ERROR(ERR_FILE_NOT_EXIST, "database", "specifical");
            return OG_ERROR;
        }

        ogx->bak.rst_file.file_id = df_ctrl->id;
    } else {
        ogx->bak.rst_file.file_id = param->file_repair;
    }

    ogx->bak.rst_file.file_type = RESTORE_DATAFILE;
    ogx->bak.rst_file.exist_repair_file = OG_FALSE;
...
static inline status_t abr_set_build_completed(knl_session_t *session, bak_t *bak, bool32 is_completed)
{
    ...
    if (db_save_core_ctrl(session) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[RESTORE] set build completed failed when repair file: %d", bak->rst_file.file_id);
        return OG_ERROR;
    }
...
status_t abr_restore_copy_ctrl(knl_session_t *session, knl_restore_t *param) {
    OG_LOG_RUN_INF("[RESTORE] start copy ctrl");
    ...
    if (db->ctrlfiles.count <= 0) {
        OG_LOG_RUN_ERR("[RESTORE] ctrl files not exist");
        return OG_ERROR;
    }
    ctrlfile = &db->ctrlfiles.items[0];

    if (cm_aligned_malloc((int64)(CTRL_MAX_PAGES_CLUSTERED * OG_DFLT_CTRL_BLOCK_SIZE), "ctrl", &buf) !=
        OG_SUCCESS) {
        OG_LOG_RUN_ERR("make ctrl file malloc memory failed");
        return OG_ERROR;
    }
    if (ctrlfile->handle == OG_INVALID_HANDLE) {
    ...
    if (cm_read_device(ctrlfile->type, ctrlfile->handle, 0,
                       buf.aligned_buf, (int64)CTRL_MAX_PAGES_CLUSTERED * OG_DFLT_CTRL_BLOCK_SIZE) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[RESTORE] failed to read %s offset %lld", ctrlfile->name, (int64)CORE_CTRL_PAGE_ID * ctrlfile->block_size);
        cm_aligned_free(&buf);
        return OG_ERROR;
    }

    OG_LOG_RUN_INF("[RESTORE] ctrl file path %s\n", param->path.str);
    if (cm_create_file(param->path.str, O_BINARY | O_SYNC | O_RDWR | O_EXCL, &dst_file) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to create control file %s\n", param->path.str);
        cm_aligned_free(&buf);
        return OG_ERROR;
    }

    if (cm_write_file(dst_file, buf.aligned_buf, CTRL_MAX_PAGES_CLUSTERED * OG_DFLT_CTRL_BLOCK_SIZE) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to write size %u\n", CTRL_MAX_PAGES_CLUSTERED * OG_DFLT_CTRL_BLOCK_SIZE);
    ...
    cm_aligned_free(&buf);
    cm_close_file(dst_file);
    OG_LOG_RUN_INF("[RESTORE] finish copy ctrl");
    return OG_SUCCESS;
}

status_t abr_restore_flush_page(knl_session_t *session, knl_restore_t *param){
    OG_LOG_RUN_INF("[RESTORE] start flush page");
    int32 handle = OG_INVALID_HANDLE;
    char *buf = NULL;
    int32 read_size;
    page_head_t *page_head = NULL;
    ...
    OG_LOG_RUN_INF("data file path %s\n", param->path.str);
    buf = (char *)cm_push(session->stack, DEFAULT_PAGE_SIZE(session));
    if (buf == NULL) {
        OG_LOG_RUN_ERR("failed to malloc buffer size %u\n", DEFAULT_PAGE_SIZE(session));
        return OG_ERROR;
    }
    if (cm_open_file(param->path.str, O_SYNC | O_RDWR | O_BINARY, &handle) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to open data file %s\n", param->path.str);
        cm_pop(session->stack);
        return OG_ERROR;
    }
    if (cm_read_file(handle, buf, DEFAULT_PAGE_SIZE(session), &read_size) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to read data file %s\n", param->path.str);
        cm_pop(session->stack);
        cm_close_file(handle);
        return OG_ERROR;
    }
    ...
    if (df_handle == -1) {
        if (spc_open_datafile(session, df, &df_handle) != OG_SUCCESS) {
    ...
    OG_LOG_RUN_INF("[RESTORE] cm write device file:%u page:%u df_name:%s offset:%llu", page_id->file, page_id->page, df->ctrl->name, offset);
    if(cm_write_device(df->ctrl->type, df_handle, offset, page_head, PAGE_SIZE(*page_head)) != OG_SUCCESS){
        OG_LOG_RUN_ERR("[RESTORE] failed write file:%s handle:%u", df->ctrl->name, handle);
        cm_pop(session->stack);
        cm_close_file(handle);
        return OG_ERROR;
    }
    ...
    cm_pop(session->stack);
    cm_close_file(handle);
    OG_LOG_RUN_INF("[RESTORE] finish flush page file:%u page:%u", page_id->file, page_id->page);
    return OG_SUCCESS;
}
...
status_t abr_restore_file_recover(knl_session_t *session, knl_restore_t *param)
{
    ...
    if (abr_init_rst_info(session, ogx, param) != OG_SUCCESS) {
        bak_unset_running(session, ogx);
        return OG_ERROR;
    }

    OG_LOG_RUN_INF("[RESTORE] restore file: %d start, device:%d, policy:%s, paral count %u, path:%s",
        bak->rst_file.file_id, param->device, T2S(&param->policy), paral_count, T2S_EX(&param->path));

    if (abr_set_build_completed(session, bak, OG_FALSE) != OG_SUCCESS) {
        bak_unset_running(session, ogx);
        return OG_ERROR;
    }
    ...
    if (rst_proc(session) != OG_SUCCESS) {
        bak_end(session, OG_TRUE);
        return OG_ERROR;
    }

    OG_LOG_RUN_INF("[RESTORE] recover file: %u start, device:%d, policy:%s, path:%s",
        bak->rst_file.file_id, param->device, T2S(&param->policy), T2S_EX(&param->path));

    if (bak_load_tablespaces(session) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[RECOVERY] recover file %u failed when load tablespace in mount mode", bak->rst_file.file_id);
        abr_recover_fail_clean(session);
        return OG_ERROR;
    }

    if (abr_recover_file_lastest(session, param) != OG_SUCCESS) {
        abr_recover_fail_clean(session);
        return OG_ERROR;
    }

    if (abr_set_build_completed(session, bak, OG_TRUE) != OG_SUCCESS) {
        abr_recover_fail_clean(session);
        return OG_ERROR;
    }

    OG_LOG_RUN_INF("[RECOVERY] recovery file %u success", bak->rst_file.file_id);
