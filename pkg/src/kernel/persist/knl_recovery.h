...
typedef struct st_rcy_context {
    ...
    thread_t preload_thread[OG_MAX_PARAL_RCY];
    rcy_preload_info_t preload_info[OG_MAX_PARAL_RCY];
...
static void inline rcy_eventfd_init(rcy_context_t *ogx)
{
    for (uint32 i = 0; i < OG_MAX_PARAL_RCY; i++) {
        ogx->bucket[i].eventfd.efd = -1;
        ogx->bucket[i].eventfd.epfd = -1;
    }
}
