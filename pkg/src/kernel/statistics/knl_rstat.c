 ...
 *
 * IDENTIFICATION
 * src/kernel/statistics/knl_rstat.c
 *
 * -------------------------------------------------------------------------
 */
...
#define IS_FULL_SAMPLE(ff)                                         (fabs(ff) < OG_REAL_PRECISION)
#define IS_STATS_TABLE_TYPE(table)                                 \
    (((table)->desc.type == TABLE_TYPE_HEAP) || ((table)->desc.type == TABLE_TYPE_NOLOGGING))
#define IS_ANALYZE_ALL_PARTS(table_stats)                         ((table_stats)->part_stats.part_id == OG_INVALID_ID32)
#define STATS_NEED_AMPLIFY(sample_ratio, is_parent)             (((sample_ratio) > OG_REAL_PRECISION) && (!(is_parent)))
...
static void stats_internal_commit(knl_session_t *session, stats_table_t *table_stats)
{
...
static void stats_internal_rollback(knl_session_t *session, stats_table_t *table_stats)
{
    if (stats_no_persistent(table_stats)) {
        return;
    }
...
int32 stats_compare_data_ex(void *data1, uint16 size_1, void *data2, uint16 size_2, knl_column_t *column)
{
    ...
    if (size1 > column->size && size1 != OG_NULL_VALUE_LEN) {
        size1 = column->size;
    }

    if (size2 > column->size && size2 != OG_NULL_VALUE_LEN) {
        size2 = column->size;
    }
...
static inline status_t stats_pcrb_compare_mtrl_key(mtrl_segment_t *segment, char *data1, char *data2, int32 *result)
{
    ...
    *result = pcrb_compare_key(profile, &scan_key, (pcrb_key_t *)data2, OG_TRUE, NULL);
    return OG_SUCCESS;
}
...
static inline status_t stats_btree_compare_mtrl_key(mtrl_segment_t *segment, char *data1, char *data2, int32 *result)
{
    ...
    *result = btree_compare_key(btree->index, &scan_key, (btree_key_t *)data2, OG_TRUE, NULL);
    return OG_SUCCESS;
}

static status_t stats_mtrl_sort_cmp(mtrl_segment_t *segment, char *data1, char *data2, int32 *result)
{
    mtrl_row_t row1;
    mtrl_row_t row2;
...
static status_t stats_match_histgram(void *handle, bool32 *match)
{
    ...
    *match = OG_FALSE;
    ...
    switch (cond->match_type) {
        case MATCH_PART:
            if (CURSOR_COLUMN_SIZE(cursor, HIST_PART_ID) != OG_NULL_VALUE_LEN) {
        ...
        case MATCH_COLUMN:
            if (CURSOR_COLUMN_SIZE(cursor, HIST_COLUMN_ID) != OG_NULL_VALUE_LEN) {
...
static status_t stats_match_histhead(void *handle, bool32 *match)
{
    ...
    *match = OG_FALSE;
    ...
    switch (cond->match_type) {
        case MATCH_PART:
            if (CURSOR_COLUMN_SIZE(cursor, HIST_HEAD_PART_ID) != OG_NULL_VALUE_LEN) {
        ...
        case MATCH_SUBPART:
            if (CURSOR_COLUMN_SIZE(cursor, HIST_HEAD_SUBPART_ID) != OG_NULL_VALUE_LEN) {
        ...
        case MATCH_COLUMN:
            if (CURSOR_COLUMN_SIZE(cursor, HIST_HEAD_COLUMN_ID) != OG_NULL_VALUE_LEN) {
...
static status_t stats_match_sys_dmls(void *handle, bool32 *match)
{
    ...
    *match = OG_FALSE;
    ...
    if (cond->part_id == OG_INVALID_ID32) {
        *match = OG_TRUE;
        return OG_SUCCESS;
    }

    if (CURSOR_COLUMN_SIZE(cursor, STATS_MON_MODS_PART_ID) != OG_NULL_VALUE_LEN) {
...
static status_t stats_create_report_file(const char *stats_file_name, int32 *report_file)
{
    char stats_report_name[STATS_MAX_REPORT_NAME_LEN] = { 0 }; /* 4 bytes for ".csv" */
    int32 ret;

    if (cm_file_exist(stats_file_name)) {
        OG_LOG_RUN_INF("[STATS] Stats report file %s already exits", stats_file_name);
        return OG_ERROR;
    }
    ...
    if (cm_file_exist(stats_report_name) && cm_remove_file(stats_report_name) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[STATS] failed to remove remained stats report file %s", stats_report_name);
        return OG_ERROR;
    }

    if (cm_create_file(stats_report_name, O_BINARY | O_SYNC | O_RDWR | O_EXCL, report_file) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[STATS] failed to create stats report file %s", stats_report_name);
...
static status_t stats_init_report_file(knl_session_t *session, dc_entity_t *entity, stats_option_t *stats_option)
{
    char stats_file_name[OG_MAX_FILE_NAME_LEN];
    char stats_file_dir[OG_MAX_FILE_NAME_LEN];
    int32 ret;
    log_param_t *log_param = cm_log_param_instance();

    ret = sprintf_s(stats_file_dir, OG_MAX_FILE_NAME_LEN, STATS_REPORT_DIR, log_param->log_home);
    knl_securec_check_ss(ret);

    if (!cm_dir_exist(stats_file_dir)) {
        if (cm_create_dir(stats_file_dir) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("[STATS] failed to create dir %s", stats_file_dir);
            return OG_ERROR;
        }
    }

    ret = sprintf_s(stats_file_name, OG_MAX_FILE_NAME_LEN, STATS_REPORT_FORMAT, stats_file_dir,
                    entity->entry->name, "TAB");
    knl_securec_check_ss(ret);

    if (stats_create_report_file(stats_file_name, &stats_option->report_tab_file) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (stats_create_report_file(stats_file_name, &stats_option->report_idx_file) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (stats_create_report_file(stats_file_name, &stats_option->report_col_file) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static void stats_close_report_file(bool8 is_report, stats_option_t *stats_option)
{
    if (is_report) {
...
static status_t stats_values_convert_str(knl_column_t *column, text_t *res_value, text_t *value)
{
    ...
    switch (column->datatype) {
        ...
        case OG_TYPE_DATE:
            if (cm_date2text(*(date_t *)res_value->str, NULL, value, STATS_MAX_BUCKET_SIZE) != OG_SUCCESS) {
                return OG_ERROR;
            }
            break;
        ...
        case OG_TYPE_TIMESTAMP_LTZ:
            if (cm_timestamp2text(*(date_t *)res_value->str, NULL, value, STATS_MAX_BUCKET_SIZE) != OG_SUCCESS) {
                return OG_ERROR;
            }
            break;
        ...
        case OG_TYPE_TIMESTAMP_TZ:
            if (cm_timestamp_tz2text((timestamp_tz_t *)res_value->str, NULL, value,
                STATS_MAX_BUCKET_SIZE) != OG_SUCCESS) {
                return OG_ERROR;
            }
            break;
        ...
        case OG_TYPE_DECIMAL:
            if (cm_dec_4_to_8(&dec, (dec4_t *)res_value->str, res_value->len) != OG_SUCCESS) {
                return OG_ERROR;
            }
            (void)cm_dec_to_text(&dec, STATS_DEC_BUCKET_SIZE, value);
            break;
        case OG_TYPE_NUMBER2:
            OG_RETURN_IFERR(cm_dec_2_to_8(&dec, (payload_t *)res_value->str, res_value->len));
            (void)cm_dec_to_text(&dec, STATS_DEC_BUCKET_SIZE, value);
            break;
        ...
        default:
            ...
            if (cm_bin2text(&bin, OG_FALSE, value) != OG_SUCCESS) {
                return OG_ERROR;
            }
            break;
    }
...
void stats_set_load_info(stats_load_info_t *load_info, dc_entity_t *entity, bool32 load_subpart, uint32 part_id)
{
    ...
    if (entity->cbo_table_stats != NULL && STATS_GLOBAL_CBO_STATS_EXIST(entity)) {
        ...
        load_info->parent_part_id = OG_INVALID_ID32;
    }
}
                                              ...
                                              int32 report_file, text_t *value, text_t *endpoint_value)
{
    ...
    if (hist_infos != NULL) {
        if (stats_values_convert_str(column, &hist_infos->ep_value, &old_value) != OG_SUCCESS) {
            return OG_ERROR;
        }
                                            ...
                                            dc_entity_t *entity, stats_table_t *tab_stats)
{
    ...
    if (stats_values_convert_str(column, &res_value, &value) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (tab_stats->part_stats.part_id != OG_INVALID_ID32) {
    ...
    if (stats_write_report_hist_value(column_handler, cbo_col, report_file, &value, &endpoint_value) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static void stats_write_double_deviation_rate(double ori_value, double verify_value, int32 report_file)
{
    ...
    if (verify_value > OG_REAL_PRECISION) {
                                         ...
                                         stats_table_t *table_stats)
{
    ...
    if (!entity->stat_exists) {
        OG_LOG_RUN_ERR("[STATS] fail to write report because of the statistics' absence of the table %s.",
            entity->entry->name);
        return;
    }
    ...
    if (stats_idx->part_id == OG_INVALID_ID32) {
                                          ...
                                          text_t *value, bool32 is_part)
{
    ...
    if (is_part) {
        ...
        stats_write_uint32_deviation_rate((uint32)cbo_tab->avg_row_len, (uint32)stats_tab->part_stats.info.avg_row_len,
                                          report_file);
        stats_write_uint32_deviation_rate(cbo_tab->sample_size, (uint32)stats_tab->part_stats.info.sample_size,
            report_file);
    } else {
        ...
        stats_write_uint32_deviation_rate((uint32)cbo_tab->avg_row_len, (uint32)stats_tab->tab_info.avg_row_len,
            report_file);
                                             ...
                                             bool32 is_part)
{
    ...
    if (stats_tab->part_stats.part_id == OG_INVALID_ID32) {
...
static void stats_init_column_handler(knl_session_t *session, stats_col_handler_t *column_handler,
    stats_col_context_t *col_ctx, stats_tab_context_t *tab_ctx)
{
    ...
    column_handler->min_value.len = OG_NULL_VALUE_LEN;
    column_handler->max_value.str = column_handler->max_buf;
    column_handler->max_value.len = OG_NULL_VALUE_LEN;
    ...
    column_handler->g_rid.vmid = OG_INVALID_ID32;
...
static bool32 stats_check_same_page_step(stats_sampler_t *stat_sample, uint32 randvalue)
{
    uint32 i;
...
static void stats_random_page_step(stats_sampler_t *stat_sample)
{
    uint32 randvalue = 0;
    bool32 is_exist = OG_TRUE;
...
static status_t stats_next_page_in_extent(knl_session_t *session, knl_cursor_t *cursor, stats_sampler_t *stat_sample,
                                   uint8 *type)
{
    page_id_t page_id;
    ...
    if (!spc_validate_page_id(session, page_id)) {
        OG_THROW_ERROR(ERR_OBJECT_ALREADY_DROPPED, name);
        return OG_ERROR;
    }
...
static void stats_sample_init(knl_session_t *session, stats_sampler_t *stats_sampler, uint32 extents_count, space_t *space,
                       uint64 sample_size, double sample_ratio)
{
    ...
    steps_len = OG_MAX_EXTENT_SIZE * sizeof(uint32);
...
status_t stats_delete_histhead_by_subpart(knl_session_t *session, table_part_t *sub_part, bool32 is_nologging)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&sub_part->desc.uid,
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, (void *)&sub_part->desc.uid,
        sizeof(uint32), IX_COL_HIST_HEAD_003_USER_ID);
...
status_t stats_delete_histhead_by_part(knl_session_t *session, knl_dictionary_t *dc, uint32 part_id)
{
    ...
    knl_init_index_scan(cursor, OG_FALSE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&dc->uid,
                     sizeof(uint32), IX_COL_HIST_HEAD_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&dc->oid,
                     sizeof(uint32), IX_COL_HIST_HEAD_003_TABLE_ID);
    knl_set_key_flag(&cursor->scan_range.l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_HEAD_003_COL_ID);
    knl_set_key_flag(&cursor->scan_range.l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_HEAD_003_SPARE1);
    knl_set_key_flag(&cursor->scan_range.l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_HEAD_003_SPARE2);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, (void *)&dc->uid,
                     sizeof(uint32), IX_COL_HIST_HEAD_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, (void *)&dc->oid,
                     sizeof(uint32), IX_COL_HIST_HEAD_003_TABLE_ID);
    knl_set_key_flag(&cursor->scan_range.r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_HEAD_003_COL_ID);
    knl_set_key_flag(&cursor->scan_range.r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_HEAD_003_SPARE1);
    knl_set_key_flag(&cursor->scan_range.r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_HEAD_003_SPARE2);
    ...
    cond.subpart_id = OG_INVALID_ID32;
    cond.col_id = OG_INVALID_ID32;
...
static status_t stats_delete_histhead_by_column(knl_session_t *session, knl_cursor_t *cursor, knl_column_t *column,
                                         stats_table_t *table_stats, uint32 part_id, bool32 is_subpart)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&column->uid,
        sizeof(uint32), IX_COL_HIST_HEAD_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&column->table_id,
        sizeof(uint32), IX_COL_HIST_HEAD_003_TABLE_ID);
    ...
    if (is_part) {
        knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_BIGINT, (void *)&part,
    ...
    if (is_subpart) {
        uint64 sub_part = (part == OG_INVALID_ID32) ? OG_INVALID_ID32 : table_stats->part_stats.sub_stats->part_id;
        knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_BIGINT, (void *)&sub_part,
...
static status_t stats_delete_histgrams(knl_session_t *session, knl_cursor_t *cursor, knl_column_t *column, bool32 is_dynamic,
    bool32 is_nologging)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&column->uid, sizeof(uint32),
                     IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&column->table_id, sizeof(uint32),
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&column->uid, sizeof(uint32),
                     IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&column->table_id, sizeof(uint32),
                     IX_COL_HIST_003_TABLE_ID);
    ...
    bool32 is_nologging)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&subpart->desc.uid, sizeof(uint32),
        IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&subpart->desc.table_id, sizeof(uint32),
        IX_COL_HIST_003_TABLE_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_003_COL_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_003_PART_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_003_ENDPOINT);

    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&subpart->desc.uid, sizeof(uint32),
        IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&subpart->desc.table_id, sizeof(uint32),
        IX_COL_HIST_003_TABLE_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_COL_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_PART_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_ENDPOINT);
...
static void stats_set_part_histgram_scan_key(knl_cursor_t *cursor, knl_column_t *column, uint32 part_id)
{
    knl_scan_key_t *l_key = &cursor->scan_range.l_key;
    knl_scan_key_t *r_key = &cursor->scan_range.r_key;
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&column->uid, sizeof(uint32),
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&column->table_id, sizeof(uint32),
        IX_COL_HIST_003_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&column->id, sizeof(uint32),
        IX_COL_HIST_003_COL_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&part_id, sizeof(uint32),
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&column->table_id, sizeof(uint32),
        IX_COL_HIST_003_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&column->id, sizeof(uint32),
        IX_COL_HIST_003_COL_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&part_id, sizeof(uint32),
        IX_COL_HIST_003_PART_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_ENDPOINT);
}

static status_t stats_delete_histgram_subpart_column(knl_session_t *session, knl_cursor_t *cursor, knl_column_t *column,
...
static status_t stats_delete_histgram_part_column(knl_session_t *session, knl_cursor_t *cursor, knl_column_t *column,
                                       ...
                                       uint32 part_id)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&dc->uid, sizeof(uint32),
                     IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, OG_TYPE_INTEGER, (void *)&dc->oid, sizeof(uint32),
                     IX_COL_HIST_003_TABLE_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_003_COL_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_003_PART_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_HIST_003_ENDPOINT);

    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&dc->uid, sizeof(uint32),
                     IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), r_key, OG_TYPE_INTEGER, (void *)&dc->oid, sizeof(uint32),
                     IX_COL_HIST_003_TABLE_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_COL_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_PART_ID);
    knl_set_key_flag(r_key, SCAN_KEY_RIGHT_INFINITE, IX_COL_HIST_003_ENDPOINT);
...
status_t stats_drop_hists(knl_session_t *session, uint32 uid, uint32 oid, bool32 is_nologging)
{
    ...
    if (stats_delete_from_sys_histgram(session, cursor, uid, oid, is_nologging) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (stats_delete_from_sys_hist_head(session, cursor, uid, oid, is_nologging) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
...
status_t stats_put_result_value(row_assist_t *ra, text_t *res_value, og_type_t type)
{
    ...
    if (res_value->len == OG_NULL_VALUE_LEN) {
        row_put_null(ra);
        return OG_SUCCESS;
    }

    if (res_value->len == 0 && !OG_IS_NUMBER_TYPE(type)) {
    ...
    switch (type) {
        ...
        case OG_TYPE_TIMESTAMP_TZ_FAKE:
        case OG_TYPE_TIMESTAMP_LTZ:
            if (cm_timestamp2text(*(timestamp_t *)res_value->str, NULL, &value, STATS_MAX_BUCKET_SIZE) != OG_SUCCESS) {
                return OG_ERROR;
            }
            break;
        ...
        case OG_TYPE_NUMBER3:
        case OG_TYPE_DECIMAL:
            if (cm_dec_4_to_8(&d8, (dec4_t *)res_value->str, res_value->len) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        case OG_TYPE_NUMBER2:
            OG_RETURN_IFERR(cm_dec_2_to_8(&d8, (payload_t *)res_value->str, res_value->len));
            (void)cm_dec_to_text(&d8, STATS_DEC_BUCKET_SIZE, &value);
            break;
...
static void stats_set_histgram_scan_key(knl_cursor_t *cursor, knl_column_t *column, uint32 part_id, bool32 is_part)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, (void *)&column->uid,
                     sizeof(uint32), IX_COL_HIST_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, (void *)&column->table_id,
                     sizeof(uint32), IX_COL_HIST_003_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, (void *)&column->id,
                     sizeof(uint32), IX_COL_HIST_003_COL_ID);
    ...
    stats_table_t *table_stats)
{
    ...
    uint64 subpart_id = (part_stats->is_subpart) ? part_stats->sub_stats->part_id : OG_INVALID_ID32;
    ...
    for (;;) {
        ...
        if (!part_stats->is_subpart) {
            if (bucket_num < curr_buckets) {
                stats_set_hist_rowids(cursor, hist_rowids, hist_assit, &bucket_num, OG_FALSE);
                continue;
            }
        ...
        bool32 subpart_found = (CURSOR_COLUMN_SIZE(cursor, HIST_SUBPART_ID) != OG_NULL_VALUE_LEN) &&
            (subpart_id == *(uint64*)CURSOR_COLUMN_DATA(cursor, HIST_SUBPART_ID));
        if (subpart_found) {
            if (bucket_num < curr_buckets) {
                stats_set_hist_rowids(cursor, hist_rowids, hist_assit, &bucket_num, OG_TRUE);
                continue;
            }
            // drop part and analyze table concurrently,some histgrams of drop part can remain. we need clean them.
            if (knl_internal_delete(session, cursor) != OG_SUCCESS) {
                                           ...
                                           stats_table_t *table_stats)
{
    ...
    for (;;) {
        ...
        if (!part_stats->is_subpart) {
            column_handler->hist_exits = OG_TRUE;
            break;
        }
...
static status_t stats_delete_old_histgram(knl_session_t *session, knl_cursor_t *cursor, stats_hist_rowids_t
    *hist_buckets,
                                          stats_table_t *table_stats)
{
    stats_open_histgram_cursor(session, cursor, CURSOR_ACTION_DELETE, OG_INVALID_ID32, table_stats->is_nologging);
...
static status_t stats_fetch_distinct(stats_col_handler_t *column_handler, bool32 is_gather)
{
    mtrl_context_t *ogx = &column_handler->mtrl.mtrl_ctx;
    mtrl_cursor_t *mtrl_cur = &column_handler->mtrl.mtrl_cur;
    ...
    while (!(group_changed || mtrl_cur->eof)) {
        if (mtrl_fetch_group(ogx, mtrl_cur, &group_changed) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (is_gather) {
            if (STATS_GET_ROW_SIZE(mtrl_cur) != OG_NULL_VALUE_LEN) {
                column_handler->hist_info.dnv_per_num++;
            }
        } else {
            if (STATS_GET_ROW_SIZE(mtrl_cur) == OG_NULL_VALUE_LEN && !mtrl_cur->eof) {
...
static status_t stats_calc_distinct_num(knl_session_t *session, stats_col_handler_t *column_handler)
{
    ...
    for (;;) {
        if (stats_fetch_distinct(column_handler, OG_FALSE) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    switch (rs_col->datatype) {
        ...
        case OG_TYPE_NUMBER2:
            if (len == 0 && ra->is_csf) {
                ...
                cm_dec2_copy_ex(&dec2, (const payload_t *)ptr, len);
                return row_put_dec2_core(ra, &dec2);
            }
        default:
            bin.bytes = (uint8 *)ptr;
            bin.size = len;
...
static status_t stats_make_mtrl_rs_row(mtrl_cursor_t *cursor, knl_column_t *rs_col, char *buf, bool32 *has_null)
{
    ...
    cm_row_init(&ra, buf, OG_MAX_ROW_SIZE, STATS_MAP_ROW_COUNT, row->is_csf);
    ...
    if (stats_put_mtrl_row(&ra, rs_col, ptr, len) != OG_SUCCESS) {
        return OG_ERROR;
    }
                                      ...
                                      stats_table_t *tab_stats)
{
    ...
    for (;;) {
        if (stats_next_sample_page(session, cursor, tab_stats, tab_sample, &type) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t stats_insert_row(knl_session_t *session, knl_cursor_t *cursor, mtrl_context_t  *temp_ctx,
                          uint32 seg_id, stats_table_t *table_stats)
{
    ...
    char *row = (char *)cm_push(session->stack, OG_MAX_ROW_SIZE + sizeof(rowid_t));

    segment = temp_ctx->segments[seg_id];
    if (mtrl_open_page(temp_ctx, segment->vm_list.last, &segment->curr_page) != OG_SUCCESS) {
    ...
    for (;;) {
        ...
        if (mtrl_insert_row(temp_ctx, seg_id, row, &rid) != OG_SUCCESS) {
            mtrl_close_page(temp_ctx, segment->vm_list.last);
            segment->curr_page = NULL;
            cm_pop(session->stack);
            return OG_ERROR;
...
static status_t get_next_tmptab_page(knl_session_t *session, stats_table_t *table_stats, knl_cursor_t *cursor)
{
    ...
    if (table_stats->temp_table->prev_sample_pageid == OG_INVALID_ID32) {
    ...
    if (curr_sample_pageid == OG_INVALID_ID32) {
        return OG_SUCCESS;
    }

    if (buf_enter_temp_page_nolock(session, curr_sample_pageid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open heap migr vm page (%u).", curr_sample_pageid);
        return OG_ERROR;
    }
                                             ...
                                             uint32 seg_id, stats_table_t *table_stats)
{
    ...
    for (;;) {
        ...
        if (get_next_tmptab_page(session, table_stats, cursor) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (stats_insert_row(session, cursor, temp_ctx, seg_id, table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    session->stat_sample = OG_FALSE;

    return OG_SUCCESS;
}

static bool8 stats_set_sample(stats_sampler_t *stats_sample)
{
...
static status_t stats_get_next_sample_page_by_map(knl_session_t *session, knl_cursor_t *cursor, stats_sampler_t
    *stats_sample,
...
{
    errno_t ret = memset_sp(map_sampler->steps, sizeof(uint32) * HEAP_MAX_MAP_LEVEL, 0, sizeof(uint32) *
        HEAP_MAX_MAP_LEVEL);
    knl_securec_check(ret);
    ...
    map_sampler->count = 0;
    if (sample_ratio < OG_REAL_PRECISION) {
        map_sampler->interval = (double)OG_MAX_UINT32;
    } else {
...
static status_t stats_insert_mtrl_sample_by_map(knl_session_t *session, knl_cursor_t *cursor, mtrl_context_t *temp_ctx,
                                                stats_sampler_t *stats_sample, uint32 seg_id, stats_table_t
                                                    *table_stats)
{
    ...
    for (;;) {
        ...
        for (uint32 i = 0; i< map_sampler.count; i++) {
            if (stats_get_next_sample_page_by_map(session, cursor, stats_sample, table_stats, sample_pages[i]) !=
                OG_SUCCESS) {
                return OG_ERROR;
            }
...
{
    bool32 is_sample = (stats_sample->sample_size != OG_INVALID_ID32) ? OG_TRUE : OG_FALSE;
    ...
    for (;;) {
        if (is_sample) {
            if (sample_by_map) {
                if (stats_insert_mtrl_sample_by_map(session, cursor, temp_ctx, stats_sample, seg_id, table_stats) !=
                    OG_SUCCESS) {
                    session->stat_sample = OG_FALSE;
                    return OG_ERROR;
                }
            } else {
                if (stats_get_sample_page(session, cursor, stats_sample, table_stats) != OG_SUCCESS) {
                    session->stat_sample = OG_FALSE;
                    return OG_ERROR;
                }
            }
        } else {
            if (stats_get_nonsample_page(session, cursor, table_stats) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
                                          ...
                                          mtrl_cursor_t *cursor, uint32 temp_seg)
{
    ...
    if (mtrl_open_page(temp_ctx, rid.vmid, &page) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t stats_mtrl_distinct(knl_session_t *session, stats_col_handler_t *column_handler)
{
    ...
    if (stats_open_mtrl_rs_cursor(column_handler->g_rid, temp_ctx, cursor, temp_seg) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    for (;;) {
        if (mtrl_fetch_rs(temp_ctx, cursor, OG_TRUE) != OG_SUCCESS) {
        ...
        if (stats_make_mtrl_rs_row(cursor, column_handler->column, buf, &column_handler->has_null) != OG_SUCCESS) {
            mtrl_close_cursor(temp_ctx, cursor);
            return OG_ERROR;
        }

        if (mtrl_insert_row(ogx, dist_seg, buf, &rid) != OG_SUCCESS) {
            mtrl_close_cursor(temp_ctx, cursor);
            return OG_ERROR;
        }
    }
...
static uint32 stats_get_table_pages(knl_session_t *session, table_t *table, uint32 *extent_count)
{
    ...
    if (!spc_validate_page_id(session, table->desc.entry)) {
        OG_LOG_RUN_ERR("stats table error: table name: %s has been droped", table->desc.name);
        return estimate_blocks;
    }
...
static uint32 stats_get_table_part_pages(knl_session_t *session, table_part_t *table_part, uint32 *extent_count)
{
    uint32 estimate_blocks = 0;
    heap_segment_t *segment = NULL;
    space_t *space = NULL;
    ...
    if (!spc_validate_page_id(session, table_part->desc.entry)) {
        OG_LOG_RUN_ERR("stats table part error: table part name: %s has been droped", table_part->desc.name);
        return estimate_blocks;
    }
    ...
    if (page->type != PAGE_TYPE_HEAP_HEAD || segment->seg_scn != table_part->desc.seg_scn) {
        OG_LOG_RUN_ERR("stats table table_part error: table table_part name: %s has been droped",
                                    ...
                                    double *sample_ratio, stats_table_t *table_stats)
{
    ...
    if (*sample_ratio > OG_REAL_PRECISION) {
        ...
        stats_sample->sample_size = OG_INVALID_ID32;
    }
}
/*
 ...
 */
static status_t stats_create_global_mtrl_table(knl_session_t *session, knl_dictionary_t *dc, mtrl_context_t *temp_ctx,
                                        uint32 seg_id, stats_table_t *table_stats)
{
    knl_cursor_t *cursor = NULL;
    ...
    knl_close_cursor(session, cursor);
    CM_RESTORE_STACK(session->stack);
    SYNC_POINT_GLOBAL_START(COLLECT_STATISTICS_CREATE_TEMP_TABLE_FAIL, &result, OG_ERROR);
    SYNC_POINT_GLOBAL_END;
    return result;
}

static status_t stats_create_mtrl_table(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    mtrl_context_t *temp_ctx = tab_ctx->mtrl_tab_ctx;
    ...
    if (!STATS_IS_ANALYZE_TEMP_TABLE(table_stats)) {
        if (stats_insert_mtrl_heap_table(session, cursor, temp_ctx, &stats_sample, seg_id, table_stats) != OG_SUCCESS) {
        ...
        if (stats_insert_mtrl_temp_table(session, cursor, temp_ctx, seg_id, table_stats) != OG_SUCCESS) {
            mtrl_close_segment(temp_ctx, seg_id);
            knl_close_cursor(session, cursor);
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
...
static status_t stats_create_distinct_values(knl_session_t *session, stats_col_handler_t *column_handler)
{
    ...
    if (mtrl_create_segment(ogx, MTRL_SEGMENT_DISTINCT, (handle_t)column_handler->column, seg_id) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (mtrl_open_segment(ogx, *seg_id) != OG_SUCCESS) {
        return OG_ERROR;
    }
 
    if (stats_mtrl_distinct(session, column_handler) != OG_SUCCESS) {
        mtrl_close_segment(ogx, *seg_id);
        return OG_ERROR;
    }

    mtrl_close_segment(ogx, *seg_id);
    if (mtrl_sort_segment(ogx, *seg_id) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (mtrl_open_cursor(ogx, *seg_id, mtrl_cur) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t stats_update_sys_subindexpart(knl_session_t *session, stats_index_t *stats_index)
{
    row_assist_t ra;
    ...
    if (stats_try_begin_auton_rm(session, stats_index->is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &stats_index->uid, sizeof(uint32),
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &stats_index->idx_id, sizeof(uint32),
        IX_COL_SYS_INDEXSUBPART001_INDEX_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &stats_index->part_id, sizeof(uint32),
        IX_COL_SYS_INDEXSUBPART001_PARENT_PART_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &stats_index->subpart_id, sizeof(uint32),
        IX_COL_SYS_INDEXSUBPART001_SUB_PART_ID);

    if (knl_fetch(session, cursor) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        stats_try_end_auton_rm(session, OG_ERROR, stats_index->is_dynamic);
        return OG_ERROR;
    }

    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_INDEX_ALREADY_DROPPED, T2S(&stats_index->name));
        stats_try_end_auton_rm(session, OG_ERROR, stats_index->is_dynamic);
        return OG_ERROR;
    }
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->info.leaf_blocks));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->info.distinct_keys));
    (void)row_put_real(&ra, stats_index->avg_leaf_key);
    (void)row_put_real(&ra, stats_index->avg_data_key);
    (void)row_put_date(&ra, cm_now());
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->info.empty_leaves));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->clus_factor));  // clus_factor
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->sample_size));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->info.comb_cols_2_ndv));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->info.comb_cols_3_ndv));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, stats_index->info.comb_cols_4_ndv));
    ...
    stats_try_end_auton_rm(session, OG_SUCCESS, stats_index->is_dynamic);
    return OG_SUCCESS;
}

static status_t stats_update_sys_indexpart(knl_session_t *session, stats_index_t *stats_index)
{
    row_assist_t ra;
    knl_scan_key_t *key = NULL;
    uint16 size;
...
static void stats_reset_index_stats(stats_index_t *stats_idx)
{
                                     ...
                                     index_t *idx, char *key_buf)
{
    ...
    if (knl_make_key(session, knl_cur, idx, key_buf) != OG_SUCCESS) {
...
{
    mtrl_rowid_t       rid;
    mtrl_context_t    *ogx = &stats_idx->mtrl.mtrl_ctx;
    ...
    if (stats_open_mtrl_rs_cursor(stats_idx->g_rid, temp_ctx, cursor, temp_seg) != OG_SUCCESS) {
        return OG_ERROR;
    }

    char *key = (char *)cm_push(session->stack, OG_KEY_BUF_SIZE);
    errno_t ret = memset_sp(key, OG_KEY_BUF_SIZE, 0, OG_KEY_BUF_SIZE);
    knl_securec_check(ret);
    ...
    for (;;) {
        ...
        if (stats_make_index_key(session, cursor, idx, key) != OG_SUCCESS) {
            mtrl_close_cursor(temp_ctx, cursor);
            cm_pop(session->stack);
            return OG_ERROR;
        }

        if (mtrl_insert_row(ogx, dist_seg, key, &rid) != OG_SUCCESS) {
            mtrl_close_cursor(temp_ctx, cursor);
            cm_pop(session->stack);
            return OG_ERROR;
        }
...
static status_t stats_create_mtrl_index(knl_session_t *session, index_t *idx, stats_index_t *stats_idx)
{
    ...
    if (idx->desc.cr_mode == CR_PAGE) {
        ...
        ogx->sort_cmp = stats_pcrb_compare_mtrl_key;
    } else {
        seg_type = MTRL_SEGMENT_RCR_BTREE;
        ogx->sort_cmp = stats_btree_compare_mtrl_key;
    }

    if (mtrl_create_segment(ogx, seg_type, stats_idx->btree, seg_id) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (stats_mtrl_index_key(session, idx, stats_idx) != OG_SUCCESS) {
        mtrl_close_segment(ogx, *seg_id);
        return OG_ERROR;
    }
                                  ...
                                  stats_table_t *table_stats, cbo_stats_index_t *cbo_index)
{
    ...
    ret = memcpy_sp(prev_key, OG_KEY_BUF_SIZE, key, (uint32)(key->size));
    knl_securec_check(ret);
}
...
static uint32 stats_calc_ndv_with_iteration(uint32 min_value_input, uint32 max_value_input, uint32 total_number,
    uint32 sampled_dist, double sampled_ratio)
{
    uint32 min_value = min_value_input;
    uint32 max_value = max_value_input;
    ...
    stats_table_t *table_stats)
{
    ...
    if (STATS_IS_ANALYZE_TEMP_TABLE(table_stats)) {
        ...
        stats_idx->info.height = temp_segment->level == OG_INVALID_ID32 ? 0 : temp_segment->level;
    } else {
        if (IS_INVALID_PAGID(btree_entry)) {
            return;
        }

        if (buf_read_page(session, btree_entry, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
            return;
        }
        ...
        if (page->type != PAGE_TYPE_BTREE_HEAD || segment->seg_scn != seg_scn) {
            buf_leave_page(session, OG_FALSE);
            OG_LOG_RUN_ERR("stats index error: found page scn is not seg scn, index name: %s", idx->desc.name);
            return;
        }
...
static void stats_estimate_total_idx_stats(index_t *idx, stats_index_t *stats_idx, cbo_stats_index_t *cbo_index)
{
    ...
    if (stats_idx->sample_ratio > OG_REAL_PRECISION) {
                                      ...
                                      stats_table_t *table_stats)
{
    ...
    if (idx->desc.cr_mode == CR_PAGE) {
        prev_pcrb_key = (pcrb_key_t *)cm_push(session->stack, OG_KEY_BUF_SIZE);
        ret = memset_sp(prev_pcrb_key, OG_KEY_BUF_SIZE, 0, OG_KEY_BUF_SIZE);
        knl_securec_check(ret);
    } else {
        prev_btree_key = (btree_key_t *)cm_push(session->stack, OG_KEY_BUF_SIZE);
        ret = memset_sp(prev_btree_key, OG_KEY_BUF_SIZE, 0, OG_KEY_BUF_SIZE);
        knl_securec_check(ret);
    }

    if (mtrl_open_cursor(ogx, dist_seg, cursor) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    for (;;) {
        if (mtrl_fetch_sort_key(ogx, cursor) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            mtrl_close_cursor(ogx, cursor);
            return OG_ERROR;
        }
    ...
    stats_table_t *table_stats)
{
    ...
    page_data_size = OG_VMEM_PAGE_SIZE - sizeof(temp_btree_page_t) - sizeof(temp_page_tail_t);
    ...
    btree_segment_t *segment)
{
    ...
    uint64 higher_level_page = 1;
    uint64 estimate_data_page = 0;
    uint32 recycled_cnt = segment->del_pages.count + segment->recycled_pages.count;
    uint32 unused_page = recycled_cnt + segment->ufp_count + 1; // 1 is map page
    space_t *space = SPACE_GET(session, segment->space_id);
    ...
    if (!spc_validate_page_id(session, AS_PAGID(segment->tree_info.root))) {
        OG_LOG_RUN_ERR("stats index error: index name: %s has been droped", idx->desc.name);
        return;
    }

    if (buf_read_page(session, AS_PAGID(segment->tree_info.root), LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[STATS] ERROR INFO: failed to read page %u-%u",
            AS_PAGID(segment->tree_info.root).file, AS_PAGID(segment->tree_info.root).page);
        return;
    }
    btree_page_t *page = BTREE_CURR_PAGE(session);

    if (btree_check_segment_scn(page, page_type, segment->seg_scn) != OG_SUCCESS) {
        buf_leave_page(session, OG_FALSE);
        OG_LOG_RUN_ERR("stats index error: found page scn is not seg scn, index name: %s", idx->desc.name);
        return;
    }
    ...
    btree_segment_t *segment)
{
    ...
    unused_page = segment->del_pages.count + segment->recycled_pages.count + segment->ufp_count + 1; // 1 is map page
    ...
    // key len must be smaller than max value of uint32
    avg_key_len = (uint32)(stats_idx->info.keys_total_size / stats_idx->info.keys);
    page_data_size = DEFAULT_PAGE_SIZE(session) - sizeof(btree_page_t) -
                                            ...
                                            stats_table_t *table_stats)
{
    ...
    uint32 index_segid = table_stats->temp_table->table_cache->index_segid;
    if (index_segid == OG_INVALID_ID32 || index_segid >= session->temp_mtrl->seg_count) {
        return;
    }
...
static void stats_estimate_page_info(knl_session_t *session, index_t *idx, stats_index_t *stats_idx)
{
    ...
    knl_scn_t seg_scn = STATS_IS_PART_INDEX(stats_idx) ? stats_idx->part_index->desc.seg_scn : idx->desc.seg_scn;
    space_t *space = STATS_IS_PART_INDEX(stats_idx) ? SPACE_GET(session, stats_idx->part_index->desc.space_id) :
        SPACE_GET(session, idx->desc.space_id);

    if (IS_INVALID_PAGID(btree_entry)) {
        return;
    }
    if (buf_read_page(session, btree_entry, LATCH_MODE_S, ENTER_PAGE_NORMAL) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[STATS] ERROR INFO: failed to read page %u-%u", btree_entry.file, btree_entry.page);
        return;
    }
    page_head_t *page = (page_head_t *)CURR_PAGE(session);
    btree_segment_t *segment = BTREE_GET_SEGMENT(session);
    if (page->type != PAGE_TYPE_BTREE_HEAD || segment->seg_scn != seg_scn) {
        buf_leave_page(session, OG_FALSE);
        OG_LOG_RUN_ERR("stats index error: found current seg scn is not origin seg scn, index name: %s",
            idx->desc.name);
        return;
    }
    ...
    if (stats_idx->info.distinct_keys == 0) {
        ...
        stats_idx->avg_data_key = segment_blocks - recycled_cnt;
    } else {
        stats_idx->avg_leaf_key = (double)stats_idx->info.leaf_blocks / (double)stats_idx->info.distinct_keys;
...
static void stats_init_index_handler(knl_session_t *session, stats_index_t *stats_idx, double sample_ratio)
{
    ...
    stats_idx->g_rid.vmid = OG_INVALID_ID32;
}

static void stats_calc_index_empty_size(knl_session_t *session, dc_entity_t *entity, index_t *idx, stats_index_t *stats_idx)
{
...
// when is_global is OG_TRUE, the situation of the index is part_index && part_id == INVALID should be allowed.
...
*/
static bool32 stats_dynamic_ignore_index(knl_session_t *session, dc_entity_t *entity, uint32 index_id,
                                  stats_table_t *table_stats)
{
    ...
    stats_index_t *stats_idx, stats_table_t *table_stats, bool8 is_global)
{
    ...
    if (stats_create_mtrl_index(session, idx, stats_idx) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (stats_gather_key_info(session, idx, stats_idx, table_stats) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (!STATS_IS_ANALYZE_TEMP_TABLE(table_stats)) {
        ...
        if (!IS_INVALID_PAGID(btree_entry)) {
            ...
            if (page->type != PAGE_TYPE_BTREE_HEAD || segment->seg_scn != seg_scn) {
                buf_leave_page(session, OG_FALSE);
                OG_LOG_DEBUG_WAR("stats index error: found current seg scn is not origin seg scn, index name: %s",
                    idx->desc.name);
                return OG_SUCCESS;
            }
            buf_leave_page(session, OG_FALSE);
    ...
    if (stats_persist_index_stats(session, stats_idx, table_stats, is_global) != OG_SUCCESS) {
        return OG_ERROR;
    }
                                                ...
                                                stats_table_t *tab_stats)
{
    ...
    if (stats_update_sys_index(session, stats_idx) != OG_SUCCESS) {
        return OG_ERROR;
    }
 ...
 * if current table is a part table, is_global should be OG_TRUE when gather global index's stats
 */
                              ...
                              mtrl_context_t *mtrl_tab_ctx, uint32 temp_seg, bool8 is_global)
{
    ...
    SYNC_POINT_GLOBAL_START(COLLECT_STATISTICS_INDEX_FAIL, &result, OG_ERROR);
    table = &entity->table;
    ...
    for (i = 0; i < table->index_set.count; i++) {
        ...
        stats_idx.part_id = OG_INVALID_ID32;
        ...
        if (IS_PART_INDEX(index) && !is_global) {
            if (stats_persist_empty_index_stats(session, &stats_idx, index, table_stats) != OG_SUCCESS) {
        ...
        if (stats_gather_index_entity(session, dc, index, &stats_idx, table_stats, is_global) != OG_SUCCESS) {
            mtrl_release_context(&stats_idx.mtrl.mtrl_ctx);
            cm_pop(session->stack);
            stats_internal_rollback(session, table_stats);
            return OG_ERROR;
                                        ...
                                        bool32 is_dynamic)
{
    ...
    key = &cursor->scan_range.l_key;
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &column->uid, sizeof(uint32),
                     IX_COL_SYS_COLUMN_001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &column->table_id, sizeof(uint32),
                     IX_COL_SYS_COLUMN_001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &column->id, sizeof(uint32),
    ...
    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_OBJECT_ALREADY_DROPPED, column->name);
        return OG_ERROR;
    }
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, column_handler->dist_num));  // num distinct

    if (stats_put_result_value(&ra, &column_handler->min_value, column_handler->column->datatype) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (stats_put_result_value(&ra, &column_handler->max_value, column_handler->column->datatype) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
...
static void stats_calc_max_min_value(stats_col_handler_t *column_handler, uint32 fetch_count, uint32 last_rows)
{
    ...
    if (len != OG_NULL_VALUE_LEN && len > STATS_MAX_BUCKET_SIZE) {
    ...
    if (fetch_count == 1) {
        ...
        if (len != 0 && len != OG_NULL_VALUE_LEN) {
     ...
     */
    if (len != OG_NULL_VALUE_LEN && fetch_count == last_rows) {
...
static void stats_generate_one_histgram(stats_col_handler_t *column_handler)
{
    ...
    if (bucket.len == OG_NULL_VALUE_LEN) {
        return;
    }

    column_handler->histgram.hist_num++;
    if (type == FREQUENCY && column_handler->simple_ratio > OG_REAL_PRECISION) {
                                         ...
                                         stats_table_t *table_stats, dc_entity_t *entity)
{
    ...
    column_handler->hist_exits = OG_FALSE;
    ...
    do {
        ...
        if (buck_hwm == 0) {
            ...
            if (stats_generate_histgram(session, column_handler, table_stats, entity) != OG_SUCCESS) {
                return OG_ERROR;
            }
                                            ...
                                            stats_table_t *table_stats, dc_entity_t *entity)
{
    ...
    do {
        if (stats_fetch_distinct(column_handler, OG_TRUE) != OG_SUCCESS) {
            return OG_ERROR;
        }
                                              ...
                                              stats_col_handler_t *column_handler, stats_table_t *table_stats)
{
    ...
    if (stats_calc_distinct_num(session, column_handler) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (mtrl_open_cursor(ogx, column_handler->mtrl.dist_seg_id, mtrl_cur) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (column_handler->dist_num > max_bucket_size) {
        // to handle high hist
        if (stats_create_height_hist(session, column_handler, table_stats, entity) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        // to handle frequency hist
        if (stats_create_frequency_hist(session, column_handler, table_stats, entity) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t stats_write_sys_hist_head(knl_session_t *session, stats_col_handler_t *column_handler, stats_table_t
    *table_stats,
                                   uint32 part_id, bool32 is_subpart)
{
    ...
    if (stats_delete_histhead_by_column(session, cursor, column, table_stats, part_id, is_subpart) != OG_SUCCESS) {
        return OG_ERROR;
    }
    
    CM_SAVE_STACK(session->stack);
    stats_open_hist_abstr_cursor(session, cursor, CURSOR_ACTION_INSERT, OG_INVALID_ID32, column_handler->is_nologging);
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, column_handler->total_rows));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, column_handler->null_num));
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, column_handler->dist_num));
    ...
    if (part_stats->is_subpart) {
        uint64 subpart = (part_id == OG_INVALID_ID32) ? OG_INVALID_ID32 : part_stats->sub_stats->part_id;
        (void)row_put_int64(&ra, subpart);
    } else {
        (void)row_put_null(&ra);
    }

    uint32 sample_size = (column_handler->simple_ratio < OG_REAL_PRECISION) ? column_handler->total_rows :
        (uint32)column_handler->total_rows * column_handler->simple_ratio;
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, sample_size));
...
static status_t stats_persist_global_column_stats(knl_session_t *session, stats_col_handler_t *column_handler,
                                           stats_table_t *table_stats, dc_entity_t *entity)
{
    stats_part_table_t *part_stats = &table_stats->part_stats;
    if (stats_no_persistent(table_stats)) {
    ...
    if (part_stats->part_id == OG_INVALID_ID32 && !part_stats->is_subpart) {
        if (stats_update_sys_column(session, column_handler, table_stats->is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t stats_update_sys_subtablepart(knl_session_t *session, table_part_t *table_sub, stats_table_info_t *tab_info,
                                       bool32 is_dynamic)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
    ...
    if (stats_try_begin_auton_rm(session, is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    key = &cursor->scan_range.l_key;
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_sub->desc.uid, sizeof(uint32),
        IX_COL_SYS_TABLESUBPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_sub->desc.table_id, sizeof(uint32),
        IX_COL_SYS_TABLESUBPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_sub->desc.parent_partid, sizeof(uint32),
        IX_COL_SYS_TABLESUBPART001_PARENT_PART_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_sub->desc.part_id, sizeof(uint32),
        IX_COL_SYS_TABLESUBPART001_SUB_PART_ID);

    if (knl_fetch(session, cursor) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        stats_try_end_auton_rm(session, OG_ERROR, is_dynamic);
        return OG_ERROR;
    }

    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_OBJECT_ALREADY_DROPPED, table_sub->desc.name);
        stats_try_end_auton_rm(session, OG_ERROR, is_dynamic);
        return OG_ERROR;
    }
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_info->rows));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_info->blocks));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_info->empty_block));
    (void)row_put_int32(&ra, tab_info->avg_row_len);
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_info->sample_size));
    ...
    CM_RESTORE_STACK(session->stack);
    stats_try_end_auton_rm(session, OG_SUCCESS, is_dynamic);
    return OG_SUCCESS;
}

static status_t stats_update_sys_tablepart(knl_session_t *session, knl_dictionary_t *dc, stats_table_t *tab_stats)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
    knl_scan_key_t *key = NULL;
    ...
    if (is_report) {
        stats_write_report_tab_value(session, DC_ENTITY(dc), tab_stats, OG_TRUE);
        return OG_SUCCESS;
    }
    ...
    if (stats_try_begin_auton_rm(session, tab_stats->is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    knl_init_index_scan(cursor,
                        OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &tab_stats->uid, sizeof(uint32),
                     IX_COL_SYS_TABLEPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &tab_stats->tab_id, sizeof(uint32),
                     IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &tab_stats->part_stats.part_id, sizeof(uint32),
                     IX_COL_SYS_TABLEPART001_PART_ID);

    if (knl_fetch(session, cursor) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        stats_try_end_auton_rm(session, OG_ERROR, tab_stats->is_dynamic);
        return OG_ERROR;
    }

    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_OBJECT_ALREADY_DROPPED, table_part->desc.name);
        stats_try_end_auton_rm(session, OG_ERROR, tab_stats->is_dynamic);
        return OG_ERROR;
    }
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->part_stats.info.rows));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->part_stats.info.blocks));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->part_stats.info.empty_block));
    (void)row_put_int32(&ra, tab_stats->part_stats.info.avg_row_len);
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->part_stats.info.sample_size));
    ...
    CM_RESTORE_STACK(session->stack);
    stats_try_end_auton_rm(session, OG_SUCCESS, tab_stats->is_dynamic);
    return OG_SUCCESS;
}

static void stats_estimate_column_stats(stats_table_t *table_stats, stats_col_handler_t *column_handler)
{
    ...
    if (estimate_sample_ratio > OG_REAL_PRECISION) {
    ...
    bool32 *global_exists, bool32 is_subpart)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &part_id, sizeof(uint64),
...
static void stats_set_global_extreme_value(stats_col_handler_t *col_handler, cbo_stats_column_t *col_stats)
{
    ...
    if (col_stats->low_value.len != OG_NULL_VALUE_LEN && col_stats->low_value.len != 0) {
        // if extreme_value of col_handler len is 65535(OG_NULL_VALUE_LEN), it means the analyzing column is no data,
        // so we need set extreme_value of other columns as global stats
        if (result < 0 || col_handler->min_value.len == OG_NULL_VALUE_LEN) {
    ...
    if (col_stats->high_value.len != OG_NULL_VALUE_LEN && col_stats->high_value.len != 0) {
        // if extreme_value of col_handler len is 65535(OG_NULL_VALUE_LEN), it means the column no data,
        // so we need set extreme_value of other columns as global stats
        if (result > 0 || col_handler->max_value.len == OG_NULL_VALUE_LEN) {
...
static void stats_set_global_col_stats(stats_col_handler_t *col_handler, cbo_stats_column_t *col_stats)
{
...
static status_t stats_update_global_histhead(knl_session_t *session, stats_col_handler_t *column_handler,
                                      stats_table_t *table_stats, dc_entity_t *entity)
{
    knl_column_t *column = column_handler->column;
    ...
    uint32 dist_num = 0;
    uint32 null_num = 0;
    bool32 is_part_key = OG_FALSE;
    bool32 global_stats_exist = OG_TRUE;
    ...
    if (stats_update_sys_histhead(session, column_handler, &global_stats_exist, is_subpart) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (!global_stats_exist) {
        if (stats_write_sys_hist_head(session, column_handler, table_stats, OG_INVALID_ID32,
            is_subpart) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
                                     ...
                                     stats_hist_entity_t *hist_entity, stats_table_t *table_stats)
{
    ...
    stats_open_histgram_cursor(session, cursor, CURSOR_ACTION_INSERT, OG_INVALID_ID32, table_stats->is_nologging);
    table = (table_t *)cursor->table;
    row_init(&ra, cursor->buf, max_size, table->desc.column_count);
    (void)row_put_int32(&ra, column->uid);
    (void)row_put_int32(&ra, column->table_id);
    ...
    if (stats_put_result_value(&ra, &bucket, column->datatype) != OG_SUCCESS) {
        return OG_ERROR;
    }

    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, endpoint));
                                      ...
                                      stats_table_t *table_stats)
{
    ...
    knl_cursor_t *cursor = column_handler->stats_cur;
    stats_open_histgram_cursor(session, cursor, CURSOR_ACTION_UPDATE, OG_INVALID_ID32, column_handler->is_nologging);
    cursor->scan_mode = SCAN_MODE_ROWID;
    cursor->fetch = TABLE_ACCESSOR(cursor)->do_rowid_fetch;
    ...
    for (uint32 i = 0; i < update_count; i++) {
        ...
        row_init(&ra, cursor->update_info.data, DEFAULT_PAGE_SIZE(session), STATS_UPDATE_SYS_HIST_COUNT);
        ...
        (void)row_put_int32(&ra, MIN(OG_MAX_INT32, histgrams.hist[i].endpoint));
        cm_decode_row(cursor->update_info.data, cursor->update_info.offsets, cursor->update_info.lens, &size);
    ...
    for (uint32 i = update_count; i < new_count; i++) {
        if (stats_write_histgram(session, cursor, column, &histgrams.hist[i], table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
                                          ...
                                          stats_table_t *table_stats)
{
    ...
    if (IS_ANALYZE_ALL_PARTS(table_stats)) {
        if (stats_delete_histgrams(session, cursor, column, table_stats->is_dynamic,
            column_handler->is_nologging) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (is_subpart) {
            ...
            if (stats_delete_histgram_part_column(session, cursor, column,
                table_stats->part_stats.part_id, column_handler->is_nologging) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
                                      ...
                                      stats_table_t *table_stats)
{
    ...
    */
    if (column_handler->total_rows == 0) {
        /* if part_id != OG_INVALID32 ,it means to gather one part statistics */
        if (stats_clean_all_histgrams(session, column_handler, table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }

    if (stats_check_histgram_exist(session, column_handler, table_stats) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (column_handler->hist_exits) {
        /* update + insert/delete */
        if (stats_get_old_buckets(session, column_handler, table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (stats_update_histgram(session, column_handler, table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (stats_delete_old_histgram(session, cursor, column_handler->hist_rowids, table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        /* insert new histgrams */
        for (uint32 i = 0; i < histgram.hist_num; i++) {
            if (stats_write_histgram(session, cursor, column, &histgram.hist[i], table_stats) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
                                           ...
                                           stats_table_t *table_stats, dc_entity_t *entity)
{
    ...
    if (!stats_no_persistent(table_stats)) {
        if (stats_persit_histgram(session, column_handler, table_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        if (stats_update_global_histhead(session, column_handler, table_stats, entity) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    SYNC_POINT_GLOBAL_START(COLLECT_STATISTICS_PERSISTENCE_THROUGH_RESULT_FAIL, &result, OG_ERROR);
    SYNC_POINT_GLOBAL_END;
    return result;
}
...
static status_t stats_parall_persist_stats(knl_session_t *session, stats_par_ctrl_t *ctrl)
{
    ...
    for (uint32 i = 0; i < OG_MAX_STATS_PARALL_THREADS; i++) {
        ...
        if (par_ctx->thread.closed || par_ctx->thread.result == OG_ERROR) {
            return OG_ERROR;
        }
        column_handler = par_ctx->col_ctx.col_handler;
        if (stats_persist_column_stats(session, column_handler, tab_ctx->table_stats, entity) != OG_SUCCESS) {
            par_ctx->thread.result = OG_ERROR;
            par_ctx->thread.closed = OG_TRUE;
            return OG_ERROR;
        }
        ctrl->finish_count++;
        par_ctx->is_wait = OG_FALSE;
    }

    errno_t ret = memset_sp(&ctrl->par_thread_queue.id_list, sizeof(uint16) * OG_MAX_STATS_PARALL_THREADS, 0xFF,
                            sizeof(uint16) * OG_MAX_STATS_PARALL_THREADS);
    knl_securec_check(ret);
    ctrl->par_thread_queue.pos = 0;
    ctrl->sort_finished = OG_FALSE;
    return OG_SUCCESS;
}
                                        ...
                                        stats_tab_context_t *tab_ctx, dc_entity_t *entity)
{
    ...
    if (stats_create_distinct_values(session, column_handler) != OG_SUCCESS) {
        mtrl_release_context(&column_handler->mtrl.mtrl_ctx);
        return OG_ERROR;
    }

    if (stats_handler_distinct_values(session, entity, column_handler, table_stats) != OG_SUCCESS) {
        mtrl_release_context(&column_handler->mtrl.mtrl_ctx);
        return OG_ERROR;
    }
...
static status_t stats_alloc_vm(knl_handle_t handle, mtrl_page_t **page, stats_vm_list *vm_list)
{
    ...
    *page = (mtrl_page_t *)vm_page->data;
    ...
    for (uint32 i = 0; i < STATS_PARALL_MAX_VM_PAGES; i++) {
        if (vm_list->id_list[i] == OG_INVALID_ID32) {
...
static void stats_close_vms(knl_handle_t session, stats_vm_list *vm_list)
{
    knl_session_t *se = (knl_session_t *)session;
    uint32 vmid = 0;
...
static status_t stats_push_cursor_from_vm(knl_handle_t handle, stats_vm_list *vm_list, knl_cursor_t **cursor)
{
    knl_session_t *session = (knl_session_t *)handle;
    char *ext_buf = NULL;
    uint32 ext_size;
    ...
    if (stats_alloc_vm(handle, &page, vm_list) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t stats_alloc_mem_from_vm(knl_handle_t handle, stats_vm_list *vm_list, char **ptr, uint32 size)
{
...
static void stats_parallel_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        ...
        if (!ctrl->parall_success || ctrl->all_finished) {
            stats_record_alive_thread(ctrl, OG_FALSE);
            return;
        }
        ...
        if (ctrl->all_thread_ready && ctrl->par_thread_queue.pos == ctrl->thread_count) {
            ctrl->sort_finished = OG_TRUE;
            cm_spin_unlock(&ctrl->read_lock);
            continue;
        }

        /* thread has finished sorting and in sorted queue,it must wait untildevelop_cse_b180_kernel_cleancode1
        ...
        if (stats_gather_one_column(session, &par_ctx->col_ctx, tab_ctx, entity) != OG_SUCCESS) {
            ctrl->parall_success = OG_FALSE;
            stats_record_alive_thread(ctrl, OG_FALSE);
        ...
        par_ctx->is_wait = OG_TRUE;
        stats_sorted_threads_enqueue(ctrl, par_ctx);
    }
}

static status_t stats_parallel_proc_clean(knl_session_t *session, stats_par_ctrl_t *ctrl)
{
    ...
    for (uint32 i = 0; i < ctrl->thread_count; i++) {
        ...
        if (ctrl->par_ctx[i].thread.result != OG_SUCCESS) {
            status = OG_ERROR;
            OG_THROW_ERROR(ERR_FAILED_PARALL_GATHER_STATS);
            continue;
        }
    }
...
static void stats_estimate_radio_by_type(knl_session_t *session, stats_table_t *stats_table, uint64 default_sample_size,
                                  uint64 estimate_blocks, uint64 specify_part_esti_blocks)
{
    ...
    switch (sample_type) {
        case STATS_AUTO_SAMPLE:
            ...
            if (specify_part_esti_blocks * DEFAULT_PAGE_SIZE(session) > default_sample_size) {
                stats_table->part_sample_ratio = ((double)default_sample_size) /
                                                 (specify_part_esti_blocks * DEFAULT_PAGE_SIZE(session));
            }
...
static void stats_force_estimate_sample_ratio(knl_session_t *session, stats_table_t *stats_table, uint64 estimate_blocks,
                                       uint64 specify_part_blocks)
{
    ...
    if (ori_sample_ratio < OG_REAL_PRECISION) {
        stats_table->estimate_sample_ratio = ((double)default_sample_size) /
                                                          (estimate_blocks * DEFAULT_PAGE_SIZE(session));

        if (specify_part_blocks * DEFAULT_PAGE_SIZE(session) > default_sample_size) {
            stats_table->part_sample_ratio = ((double)default_sample_size) /
                                                          (specify_part_blocks * DEFAULT_PAGE_SIZE(session));
        }
    } else {
        stats_table->estimate_sample_ratio = ((double)default_sample_size) /
                                                          (estimate_blocks * DEFAULT_PAGE_SIZE(session));

        if (specify_part_blocks * ori_sample_ratio * DEFAULT_PAGE_SIZE(session) > default_sample_size) {
            stats_table->part_sample_ratio = ((double)default_sample_size) /
                                                          (specify_part_blocks * DEFAULT_PAGE_SIZE(session));
        }
    }
...
static uint32 stats_get_pages_from_subparts(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_part)
{
    table_t *table = DC_TABLE(dc);
    ...
    for (uint32 i = 0; i < table_part->desc.subpart_cnt; i++) {
        ...
        if (!sub_part->heap.loaded) {
            if (dc_load_table_part_segment(session, dc->handle, (table_part_t *)sub_part) != OG_SUCCESS) {
...
static uint32 stats_get_pages_from_tablepart(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_part)
{
    table_t *table = DC_TABLE(dc);
    uint32 estimate_blocks = 0;
    uint32 extents = 0;
...
static void stats_estimate_sample_ratio(knl_session_t *session, knl_dictionary_t *dc, stats_table_t *stats_table,
                                 bool32 force_sample)
{
    table_t *table = DC_TABLE(dc);
    double ori_sample_ratio = stats_table->estimate_sample_ratio;
    ...
    if (force_sample) {
        ...
        if (estimate_blocks * ori_sample_ratio * DEFAULT_PAGE_SIZE(session) > default_sample_size) {
            stats_force_estimate_sample_ratio(session, stats_table, estimate_blocks, specify_part_blocks);
            return;
        }
    }
...
static void stats_add_columns_list(stats_cols_list_t *columns_list, uint16 col_id)
{
    for (uint32 j = 0; j < columns_list->max_count; j++) {
        ...
        if (columns_list->col_list[j] == OG_INVALID_ID16) {
...
static void stats_add_func_idx_columns(index_t *idx, stats_cols_list_t *idx_columns, uint32 icol_pos)
{
...
static void stats_get_idx_columns(index_t *idx, stats_cols_list_t *idx_columns)
{
    uint16 col_id = 0;
...
static void  stats_generate_all_cols_list(dc_entity_t *entity, stats_cols_list_t *col_list)
{
    knl_column_t *column = NULL;
...
static void  stats_generate_idx_cols_list(dc_entity_t *entity, stats_cols_list_t *col_list)
{
    table_t      *table = &entity->table;
    index_t      *idx = NULL;
...
static void  stats_generate_specified_cols_list(dc_entity_t *entity, stats_cols_list_t *col_list,
...
static void stats_generate_columns_list(dc_entity_t *entity, stats_table_t *table_stats, stats_method_opt_type_t method_opt,
                                 stats_cols_list_t *col_list)
{
    switch (method_opt) {
        ...
        case FOR_SPECIFIED_COLUMNS:
            stats_generate_specified_cols_list(entity, col_list, table_stats, OG_FALSE);
            break;
        case FOR_SPECIFIED_INDEXED_COLUMNS:
            stats_generate_specified_cols_list(entity, col_list, table_stats, OG_TRUE);
            break;
        default:
            stats_generate_all_cols_list(entity, col_list);
            break;
...
static void stats_init_columns_list(knl_session_t *session, stats_cols_list_t *column_list)
{
...
static void stats_init_column_context(knl_session_t *session, stats_col_context_t *col_ctx)
{
    knl_cursor_t *cursor = NULL;
...
static status_t stats_normal_gather_columns(knl_session_t *session, dc_entity_t *entity, stats_tab_context_t *tab_ctx,
                                     stats_cols_list_t *column_list)
{
    ...
    for (uint32 i = 0; i < column_list->column_count; i++) {
        ...
        if (stats_gather_one_column(session, &col_ctx, tab_ctx, entity) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }

        if (stats_persist_column_stats(session, col_ctx.col_handler, tab_ctx->table_stats, entity) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
    }
...
static status_t stats_wait_persist_end(knl_session_t *session, stats_par_ctrl_t *ctrl)
{
    status_t status = OG_SUCCESS;
    ...
    while (OG_TRUE) {
         ...
         * 2. there are some columns are sorting, some sort threads are working and the number of alive threads is not
             0.
        ...
        if (stats_parall_persist_stats(session, ctrl) != OG_SUCCESS) {
            ctrl->parall_success = OG_FALSE;
            status = OG_ERROR;
            break;
        }
...
static status_t stats_create_parall_threads(knl_session_t *session, stats_par_ctrl_t *ctrl, stats_vm_list *vm_list,
                                     uint32 thread_num)
{
    stats_par_context_t *par_ctx = NULL;
    status_t status = OG_SUCCESS;

    ctrl->all_thread_ready = OG_FALSE;
    for (uint32 i = 0; i < thread_num; i++) {
        ...
        if (stats_alloc_mem_from_vm(session, vm_list, (char**)&par_ctx->col_ctx.col_handler,
            sizeof(stats_col_handler_t)) != OG_SUCCESS) {
            status = OG_ERROR;
            break;
        }

        if (stats_push_cursor_from_vm(session, vm_list, &par_ctx->col_ctx.stats_cur) != OG_SUCCESS) {
            status = OG_ERROR;
            break;
        }

        if (cm_create_thread(stats_parallel_proc, 0, par_ctx, &par_ctx->thread) != OG_SUCCESS) {
            ctrl->parall_success = OG_FALSE;
            status = OG_ERROR;
            break;
        }

        stats_record_alive_thread(ctrl, OG_TRUE);
        ctrl->thread_count++;
    }

    if (status == OG_SUCCESS) {
        cm_spin_lock(&ctrl->read_lock, NULL);
        ctrl->all_thread_ready = OG_TRUE;
        cm_spin_unlock(&ctrl->read_lock);
    }
...
static status_t stats_parall_gather_columns(knl_session_t *session, dc_entity_t *entity, stats_tab_context_t *tab_ctx,
                                     stats_cols_list_t *column_list)
{
    ...
    ret = memset_sp(&ctrl.par_thread_queue.id_list, sizeof(uint16) * OG_MAX_STATS_PARALL_THREADS, 0xFF,
                    sizeof(uint16) * OG_MAX_STATS_PARALL_THREADS);
    knl_securec_check(ret);
    ...
    ctrl.parall_success = OG_TRUE;
    
    uint32 thread_num = (column_list->column_count > stats_max_paraller) ? stats_max_paraller :
    ...
    if (stats_parallel_proc_clean(session, &ctrl) != OG_SUCCESS) {
...
static status_t stats_gather_columns(knl_session_t *session, dc_entity_t *entity, stats_table_t *table_stats,
                              mtrl_context_t *mtrl_tab_ctx, uint32 mtrl_tab_seg)
{
    ...
    if (!STATS_DYNAMICAL_TRANS_GTT(entity->type, table_stats->is_dynamic)) {
        if (stats_try_begin_auton_rm(session, table_stats->is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t stats_gather_part_columns(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    stats_table_t *table_stats = tab_ctx->table_stats;
    mtrl_context_t *temp_ctx = tab_ctx->mtrl_tab_ctx;
    uint32 temp_seg = tab_ctx->mtrl_tab_seg;
    ...
    if (stats_create_global_mtrl_table(session, dc, temp_ctx, temp_seg, table_stats) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (stats_gather_columns(session, entity, table_stats, temp_ctx, temp_seg) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t stats_gather_global_columns_stats(knl_session_t *session, knl_dictionary_t *dc, stats_table_t *table_stats,
                                           mtrl_context_t *temp_ctx, uint32 temp_seg)
{
    ...
    if (table_stats->specify_part_id != OG_INVALID_ID32) {
        if (STATS_GLOBAL_CBO_STATS_EXIST(entity)) {
    ...
    table_stats->part_start_rid.vmid = OG_INVALID_ID32;
    table_stats->part_start_rid.slot = 0;
    table_stats->part_stats.part_id = OG_INVALID_ID32;
...
static uint32 stats_amplify_num_rows(uint32 num_rows, double sample_ratio)
{
    if (sample_ratio < OG_REAL_PRECISION) {
        return num_rows;
    }
    ...
    if (result_rows < OG_MAX_INT32) {
        return (uint32)result_rows;
    }

    return (uint32)OG_MAX_INT32;
}

static uint32 stats_add_num_rows(uint32 ori_rows, int32 curr_rows)
{
...
static void stats_estimate_whole_stats(stats_part_table_t *part_stats, double sample_ratio)
{
...
static void stats_increase_table_part_stats(cbo_stats_table_t *cbo_global_stats, stats_table_t *global_stats,
...
static void stats_calc_global_part_stats(dc_entity_t *entity, stats_table_t *global_stats, uint32 part_no, bool32 is_parent)
{
    ...
    if (cbo_part_stats == NULL) {
        ...
        global_stats->tab_info.row_len = (cbo_global_stats->avg_row_len * cbo_global_stats->rows) +
            part_stats->info.row_len;
    } else {
 ...
 */
static void stats_calc_global_table_stats(stats_table_t *table_stats, bool32 is_parent)
{
    stats_part_table_t *part_stats = &table_stats->part_stats;
...
static void stats_prepare_gather_subpart_index(knl_session_t *session, stats_table_t *table_stats, stats_index_t *stats_idx,
...
static void stats_init_part_index_info(stats_index_t *stats_idx, index_t *idx)
{
    stats_idx->uid = idx->desc.uid;
    stats_idx->name.str = idx->desc.name;
    // index name len is not greater than 68
...
static status_t stats_gather_part_index(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    stats_table_t *table_stats = tab_ctx->table_stats;
    ...
    for (uint32 i = 0; i < table->index_set.count; i++) {
        ...
        if (IS_PARENT_IDXPART(&index_part->desc)) {
            ...
            if (stats_update_sys_indexpart(session, &stats_idx) != OG_SUCCESS) {
                mtrl_release_context(&stats_idx.mtrl.mtrl_ctx);
                CM_RESTORE_STACK(session->stack);
                stats_internal_rollback(session, table_stats);
                return OG_ERROR;
        ...
        if (stats_gather_index_entity(session, dc, idx, &stats_idx, table_stats, OG_FALSE) != OG_SUCCESS) {
            mtrl_release_context(&stats_idx.mtrl.mtrl_ctx);
            CM_RESTORE_STACK(session->stack);
            stats_internal_rollback(session, table_stats);
            return OG_ERROR;
...
static status_t stats_update_empty_sys_indexparts(knl_session_t *session, index_t *idx, stats_index_t *index_stats, uint32
    part_id)
{
    ...
    for (uint32 i = 0; i < part_idx->desc.partcnt; i++) {
        ...
        if (IS_PARENT_IDXPART(&idx_part->desc)) {
            if (stats_update_empty_sys_subindexs(session, idx->part_index, idx_part, index_stats) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
        /*
        * if part_id is OG_INVALID_ID32, it means to gather all parts statistics,
        * we set all table part statistics to empty
        */
        if (part_id == OG_INVALID_ID32) {
            if (stats_update_sys_indexpart(session, index_stats) != OG_SUCCESS) {
                return OG_ERROR;
            }
            continue;
        }
    ...
    stats_table_t *table_stats)
{
    ...
    for (uint32 j = 0; j < table_part->desc.subpart_cnt; j++) {
        ...
        if (stats_update_sys_subtablepart(session, sub_part, &tab_info, table_stats->is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t stats_update_empty_sys_tablepart(knl_session_t *session, knl_dictionary_t *dc, stats_table_t *table_stats,
                                          uint32 part_id)
{
    table_t *table = DC_TABLE(dc);
    ...
    for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
        ...
        if (IS_PARENT_TABPART(&table_part->desc)) {
            if (stats_update_empty_sys_subparts(session, table, table_part, table_stats) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
        /*
        ...
        */
        if (part_id == OG_INVALID_ID32) {
            if (stats_update_sys_tablepart(session, dc, table_stats) != OG_SUCCESS) {
                return OG_ERROR;
            }
            continue;
        }
...
static status_t stats_try_gather_empty_subpart(knl_session_t *session, knl_dictionary_t *dc, table_part_t
    *table_subpart,
                                               bool32 *is_empty, bool32 is_dynamic)
{
    ...
    if (!table_subpart->heap.loaded) {
        if (dc_load_table_part_segment(session, dc->handle, table_subpart) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    for (uint32 i = 0; i < table->index_set.count; i++) {
        ...
        if (IS_PART_INDEX(idx)) {
            // to update indexpart$
            if (stats_update_sys_subindexpart(session, &index_stats) != OG_SUCCESS) {
    ...
    if (stats_update_sys_subtablepart(session, table_subpart, &tab_info, is_dynamic) != OG_SUCCESS) {
        stats_rollback(session, is_dynamic);
        return OG_ERROR;
    }
    stats_commit(session, is_dynamic);
                                            ...
                                            bool32 *is_empty, bool32 is_dynamic)
{
    ...
    if (stats_check_segment_valid(session, table_part, &seg) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    */
    ...
    for (uint32 i = 0; i < table->index_set.count; i++) {
        ...
        // to update indexpart$
        if (stats_update_empty_sys_indexparts(session, idx, &index_stats, table_part->desc.part_id) != OG_SUCCESS) {
            stats_internal_rollback(session, &tab_stats);
            return OG_ERROR;
        }
    }

    if (stats_update_empty_sys_tablepart(session, dc, &tab_stats, table_part->desc.part_id) != OG_SUCCESS) {
        stats_internal_rollback(session, &tab_stats);
        return OG_ERROR;
    }
...
*/
static void stats_calc_parent_part_stats(stats_table_t *table_stats)
{
...
static status_t stats_gather_table_one_subpart(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_subpart,
...
static status_t stats_gather_subpart_index(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    stats_table_t *table_stats = tab_ctx->table_stats;
    mtrl_context_t *mtrl_tab_ctx = tab_ctx->mtrl_tab_ctx;
    uint32 mtrl_tab_seg = tab_ctx->mtrl_tab_seg;
...
static status_t stats_gather_table_subpart(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_compart,
                                    stats_tab_context_t *tab_ctx, stats_part_table_t *sub_part)
{
    ...
    bool32 is_empty_part = OG_FALSE;
    table_stats->part_stats.is_subpart = OG_TRUE;
    ...
    for (uint32 i = 0; i < table_compart->desc.subpart_cnt; i++) {
        ...
        if (stats_gather_table_one_subpart(session, dc, table_subpart, tab_ctx, &is_empty_part) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        if (stats_gather_subpart_index(session, dc, tab_ctx) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    table_stats->part_stats.sub_stats->part_id = OG_INVALID_ID32;
...
static status_t stats_gather_single_table_part(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    stats_table_t *table_stats = tab_ctx->table_stats;
    stats_part_table_t *part_stats = &table_stats->part_stats;
    table_t *table = DC_TABLE(dc);
    ...
    if (is_parent) {
        ...
        if (stats_try_gather_empty_part(session, dc, table_part, &is_empty, table_stats->is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        if (stats_gather_part_columns(session, dc, tab_ctx) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    if (stats_gather_part_index(session, dc, tab_ctx) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static void stats_subtract_part_stats(stats_table_info_t *info, cbo_stats_table_t *part_stats)
{
                                               ...
                                               stats_table_info_t lose_info)
{
    ...
    key = &cursor->scan_range.l_key;
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table->desc.uid, sizeof(uint32),
        IX_COL_SYS_TABLE_001_USER_ID);
    // table name is smaller than 68
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_STRING, table->desc.name,
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, info.rows));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, info.blocks));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, info.empty_block));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, info.sample_size));
...
status_t stats_update_global_partstats(knl_session_t *session, knl_dictionary_t *dc, uint32 partid, uint32 subpart_no)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table->desc.uid, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_USER_ID);
    // table name is smaller than 68
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table->desc.id, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &partid, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_PART_ID);
    ...
    if (stats_update_global_by_subpart(session, dc, part_no, info) != OG_SUCCESS) {
        return OG_ERROR;
    }
                                       ...
                                       knl_dictionary_t *dc)
{
    ...
    if (is_report) {
        stats_write_report_tab_value(session, DC_ENTITY(dc), tab_stats, OG_FALSE);
        return OG_SUCCESS;
    }
    ...
    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_OBJECT_ALREADY_DROPPED, table->desc.name);
        stats_try_end_auton_rm(session, OG_ERROR, tab_stats->is_dynamic);
        return OG_ERROR;
    }
    ...
    cursor->update_info.count = STATS_SYS_TABLE_COLUMN_COUNT;
    row_init(&ra, cursor->update_info.data, HEAP_MAX_ROW_SIZE(session), STATS_SYS_TABLE_COLUMN_COUNT);
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->tab_info.rows));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->tab_info.blocks));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->tab_info.empty_block));
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->tab_info.sample_size));
    tab_stats->tab_info.analyze_time = cm_now();
    (void)row_put_int64(&ra, tab_stats->tab_info.analyze_time);
...
status_t stats_gather_table(knl_session_t *session, knl_dictionary_t *dc, stats_table_t *tab_stats)
{
    ...
    if (!tab_stats->is_part) {
        ...
        if (tab_stats->estimate_sample_ratio > OG_REAL_PRECISION) {
    ...
    if (stats_update_sys_table(session, tab_stats, dc) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t db_insert_sys_mon_mods(knl_session_t *session, knl_cursor_t *cursor, stats_table_mon_t *table_smon,
                                uint32 uid, uint32 id, uint32 part_id, bool32 is_part)
{
    table_t *table = NULL;
    row_assist_t ra;

    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_INSERT, SYS_MON_MODS_ALL_ID, OG_INVALID_ID32);
    table = (table_t *)cursor->table;
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, table_smon->inserts));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, table_smon->updates));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, table_smon->deletes));
    (void)row_put_date(&ra, table_smon->timestamp);
    (void)row_put_int32(&ra, 0);
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, table_smon->drop_segments));
    if (is_part) {
...
static status_t db_update_sys_mon_modes(knl_session_t *session, knl_cursor_t *cursor, stats_table_mon_t *table_smon)
{
    row_assist_t ra;
    uint16 size;
...
static status_t stats_check_empty_table_subpart(knl_session_t *session, knl_dictionary_t *dc, table_part_t
    *table_subpart,
                                                bool32 *is_empty)
{
    if (!table_subpart->heap.loaded) {
...
static status_t stats_check_empty_table_part(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_part,
    bool32 *is_empty)
{
    table_part_t *table_sub = NULL;
    bool32 is_empty_part = OG_TRUE;
    table_t *table = DC_TABLE(dc);
    ...
    for (uint32 i = 0; i < table_part->desc.subpart_cnt; i++) {
        ...
        if (stats_check_empty_table_subpart(session, dc, table_sub, &is_empty_part) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (is_empty_part != OG_TRUE) {
            break;
        }
    }
...
static status_t stats_check_empty_table(knl_session_t *session, knl_dictionary_t *dc, bool32 *is_empty)
{
    dc_entity_t *entity = DC_ENTITY(dc);
    table_t *table = &entity->table;
    part_table_t *part_table = table->part_table;
    ...
    if (IS_TEMP_TABLE_BY_DC(dc)) {
        ...
        for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
            ...
            if (stats_check_empty_table_part(session, dc, table_part, is_empty) != OG_SUCCESS) {
                return OG_ERROR;
            }
...
static bool32 stats_check_old_nologging(knl_session_t *session, knl_cursor_t *cursor)
{
    date_t analyze_time;
    ...
    if (CURSOR_COLUMN_SIZE(cursor, SYS_TABLE_COL_ANALYZETIME) != OG_NULL_VALUE_LEN) {
        analyze_time = *(date_t*)CURSOR_COLUMN_DATA(cursor, SYS_TABLE_COL_ANALYZETIME);
        if (knl_timestamp_to_scn(session, analyze_time, &analyze_scn) != OG_SUCCESS) {
            return OG_TRUE;
        }
...
status_t stats_clean_spc_stats(knl_session_t *session, space_t *space)
{
    ...
    stats_table_t tab_stats;
    errno_t ret;
    bool32 is_old_nologging = OG_FALSE;
    ...
    while (!cursor->eof) {
        ...
        if (space_id == space->ctrl->id) {
            ...
            if (stats_delete_table_stats(session, user_id, table_id, is_old_nologging) != OG_SUCCESS) {
                knl_rollback(session, NULL);
            } else {
                knl_commit(session);
            }
...
void stats_clean_nologging_stats(knl_session_t *session)
{
    ...
    if (db_truncate_sys_table(session, SYS_TEMP_HISTGRAM_ID) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to truncate SYS_TEMP_HISTGRAM when cleaning nologging data.");
    }

    if (db_truncate_sys_table(session, SYS_TEMP_HIST_HEAD_ID) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to truncate SYS_TEMP_HIST_HEAD when cleaning nologging data.");
    }
    ...
    for (uint32 i = 0; i < OG_MAX_SPACES; i++) {
        ...
        if (stats_clean_spc_stats(session, space) != OG_SUCCESS) {
            continue;
        }
    }
}
                                         ...
                                         bool32 is_dynamic)
{
    ...
    knl_column_t *column = NULL;
    errno_t ret;
    bool32 no_segment = OG_TRUE;

    if (stats_check_empty_table(session, dc, &no_segment) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    tab_stats.is_dynamic = is_dynamic;
    tab_stats.is_part = IS_PART_TABLE(table);
    ...
    for (uint32 i = 0; i < entity->column_count; i++) {
        ...
        if (stats_update_sys_column(session, column_handler, is_dynamic) != OG_SUCCESS) {
            cm_pop(session->stack);
            stats_internal_rollback(session, &tab_stats);
            stats_try_end_auton_rm(session, OG_ERROR, is_dynamic);
            return OG_ERROR;
    ...
    for (uint32 i = 0; i < table->index_set.count; i++) {
        ...
        if (IS_PART_INDEX(idx)) {
            // to update indexpart$
            if (stats_update_empty_sys_indexparts(session, idx, &index_stats, OG_INVALID_ID32) != OG_SUCCESS) {
                stats_internal_rollback(session, &tab_stats);
                return OG_ERROR;
            }
        } else {
            if (stats_update_sys_index(session, &index_stats) != OG_SUCCESS) {
                stats_internal_rollback(session, &tab_stats);
                return OG_ERROR;
            }
        }
    ...
    if (IS_PART_TABLE(table)) {
        // to update tablepart$
        if (stats_update_empty_sys_tablepart(session, dc, &tab_stats, OG_INVALID_ID32) != OG_SUCCESS) {
            stats_internal_rollback(session, &tab_stats);
            return OG_ERROR;
        }
    }
                                   ...
                                   bool8 single_part_analyze, uint32 specify_part_id)
{
    ...
    if (!IS_PART_TABLE(table)) {
        table_stats->is_part = OG_FALSE;
        table_stats->part_stats.part_id = OG_INVALID_ID32;
        table_stats->part_stats.part_no = OG_INVALID_ID32;
    }
    ...
    table_stats->now_rid.vmid = OG_INVALID_ID32;
}
 ...
 */
static status_t stats_gather_all_table_parts(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    stats_table_t *table_stats = tab_ctx->table_stats;
    mtrl_context_t *temp_ctx = tab_ctx->mtrl_tab_ctx;
    uint32 temp_seg = tab_ctx->mtrl_tab_seg;
    ...
    for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
        ...
        if (stats_gather_each_table_part(session, dc, tab_ctx, table_part) != OG_SUCCESS) {
            ...
            cm_reset_error();
            OG_LOG_RUN_WAR("OG-%d, fail to gather the %s partition %s, it or its subpartition has been dropped or truncated.",
    ...
    if (stats_gather_global_columns_stats(session, dc, table_stats, temp_ctx, temp_seg) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t stats_gather_part_table(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    stats_table_t *table_stats = tab_ctx->table_stats;
    dc_entity_t *entity = DC_ENTITY(dc);
...
static status_t stats_prepare_table_context(knl_session_t *session, stats_tab_context_t *table_ctx)
{
    mtrl_context_t *temp_ctx = table_ctx->mtrl_tab_ctx;

    stats_init_temp_ctx(session, temp_ctx);

    if (mtrl_create_segment(temp_ctx, MTRL_SEGMENT_TEMP, NULL, &table_ctx->mtrl_tab_seg) != OG_SUCCESS) {
        mtrl_release_context(temp_ctx);
        return OG_ERROR;
    }

    if (mtrl_open_segment(temp_ctx, table_ctx->mtrl_tab_seg) != OG_SUCCESS) {
        mtrl_release_context(temp_ctx);
        return OG_ERROR;
    }
                                 ...
                                 table_part_t *table_part, bool32 is_dynamic)
{
    ...
    table_t *table = DC_TABLE(dc);
    stats_init_table_stats(dc, &table_stats, stats_option, OG_TRUE, table_part->desc.part_id);
    ...
    if (stats_prepare_table_context(session, &tab_ctx) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (stats_gather_part_table(session, dc, &tab_ctx) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (stats_gather_indexes(session, dc, &table_stats, mtrl_tab_ctx, tab_ctx.mtrl_tab_seg, OG_FALSE) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }

    if (stats_gather_table(session, dc, &table_stats) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
    ...
    if (db_delete_mon_sysmods(session, dc->uid, dc->oid, table_part->desc.part_id, is_dynamic) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
                                   ...
                                   bool32 is_dynamic)
{
    ...
    if (is_report) {
        if (stats_init_report_file(session, DC_ENTITY(dc), &stats_option) != OG_SUCCESS) {
            stats_close_report_file(is_report, &stats_option);
            return OG_ERROR;
        }
    }
    
    if (stats_gather_empty_table(session, dc, &is_empty, is_dynamic) != OG_SUCCESS) {
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    }
    ...
    stats_init_table_stats(dc, &table_stats, stats_option, OG_FALSE, OG_INVALID_ID32);
    ...
    if (stats_prepare_table_context(session, &table_ctx) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    }
     ...
     */
    if (IS_PART_TABLE(table)) {
        if (stats_gather_part_table(session, dc, &table_ctx) != OG_SUCCESS) {
            mtrl_release_context(mtrl_tab_ctx);
            CM_RESTORE_STACK(session->stack);
            stats_close_report_file(is_report, &stats_option);
            return OG_ERROR;
        ...
        if (stats_create_mtrl_table(session, dc, &table_ctx) != OG_SUCCESS) {
            mtrl_release_context(mtrl_tab_ctx);
            CM_RESTORE_STACK(session->stack);
            stats_close_report_file(is_report, &stats_option);
            return OG_ERROR;
        ...
        if (stats_gather_columns(session, DC_ENTITY(dc), &table_stats, mtrl_tab_ctx, table_ctx.mtrl_tab_seg) !=
            OG_SUCCESS) {
            mtrl_release_context(mtrl_tab_ctx);
            CM_RESTORE_STACK(session->stack);
            stats_close_report_file(is_report, &stats_option);
    ...
    if (stats_gather_indexes(session, dc, &table_stats, mtrl_tab_ctx, table_ctx.mtrl_tab_seg, OG_TRUE) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    ...
    if (db_delete_mon_sysmods(session, table->desc.uid, table->desc.id, OG_INVALID_ID32,
                              is_dynamic) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
...
static void stats_init_temp_sample(knl_session_t *session, stats_table_t *table_stats)
{
    ...
    table_stats->temp_table->prev_sample_pageid = OG_INVALID_ID32;

    if (table_stats->estimate_sample_ratio < OG_REAL_PRECISION) {
                                 ...
                                 bool32 is_dynamic)
{
    ...
    if (is_report) {
        if (stats_init_report_file(session, entity, &stats_option) != OG_SUCCESS) {
            stats_close_report_file(is_report, &stats_option);
            return OG_ERROR;
        }
    }
    ...
    if (knl_ensure_temp_cache(session, entity, &table_stats.temp_table->table_cache) != OG_SUCCESS) {
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    }
    table_stats.is_dynamic = is_dynamic;
    ...
    if (cbo_alloc_temp_table_stats(session, entity, table_stats.temp_table->table_cache, is_dynamic) != OG_SUCCESS) {
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    }
    ...
    if (stats_create_mtrl_table(session, dc, &tab_ctx) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    ...
    if (stats_gather_columns(session, DC_ENTITY(dc), &table_stats, mtrl_tab_ctx, tab_ctx.mtrl_tab_seg) != OG_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        stats_close_report_file(is_report, &stats_option);
        return OG_ERROR;
    ...
    table_t *table, table_part_t *table_part)
{
    ...
    if (!IS_PARENT_TABPART(&table_part->desc)) {
        if (!STATS_IS_INVALID_PART_TABLE(table_part)) {
            heap_seg = HEAP_SEGMENT(session, table_part->heap.entry, table_part->heap.segment);
        }
        return heap_seg;
    }
    ...
    for (uint32 i = 0; i < table_part->desc.subpart_cnt; i++) {
        ...
        if (!STATS_IS_INVALID_PART_TABLE(table_subpart)) {
            return HEAP_SEGMENT(session, table_subpart->heap.entry, table_subpart->heap.segment);
        }
    }
...
static heap_segment_t *stats_get_heap_segment(knl_session_t *session, table_t *table)
{
    ...
    if (!IS_PART_TABLE(table)) {
        ...
        return HEAP_SEGMENT(session, table->heap.entry, table->heap.segment);
    }
    ...
    index_t *idx, index_part_t *index_part)
{
    ...
    if (!IS_PARENT_IDXPART(&index_part->desc)) {
        if (!STATS_IS_INVALID_PART_INDEX(index_part)) {
            btree_seg = BTREE_SEGMENT(session, index_part->btree.entry, index_part->btree.segment);
        }
    ...
    for (uint32 i = 0; i < index_part->desc.subpart_cnt; i++) {
        ...
        if (!STATS_IS_INVALID_PART_INDEX(index_subpart)) {
            return BTREE_SEGMENT(session, index_subpart->btree.entry, index_subpart->btree.segment);
        }
    }
...
static btree_segment_t *stats_get_btree_segment(knl_session_t *session, index_t *idx)
{
    ...
    if (!IS_PART_INDEX(idx)) {
        if (idx->btree.segment == NULL) {
            return NULL;
        }
        return BTREE_SEGMENT(session, idx->btree.entry, idx->btree.segment);
    }
                                             ...
                                             double sample_ratio)
{
    ...
    if (btree_level_first_page(session, btree, 1, &page_id) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (info->height == 1) {
        if (btree->index->desc.cr_mode == CR_PAGE) {
            compare_pkey = (pcrb_key_t *)cm_push(session->stack, OG_KEY_BUF_SIZE);
            compare_pkey->is_infinite = OG_TRUE;
            compare_pkey->size = sizeof(pcrb_key_t);
            pcrb_stats_leaf_page(session, btree, info, &page_id, &prev_page_id, compare_pkey);
        } else {
            compare_bkey = (btree_key_t *)cm_push(session->stack, OG_KEY_BUF_SIZE);
            compare_bkey->is_infinite = OG_TRUE;
            compare_bkey->size = sizeof(btree_key_t);
            btree_stats_leaf_page(session, btree, info, &page_id, &prev_page_id, compare_bkey);
        }
    } else {
        if (btree->index->desc.cr_mode == CR_PAGE) {
            if (pcrb_stats_leaf_by_parent(session, btree, sample_ratio, info, page_id) != OG_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                return OG_ERROR;
            }
        } else {
            if (btree_stats_leaf_by_parent(session, btree, sample_ratio, info, page_id) != OG_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                return OG_ERROR;
            }
        }
...
static inline void stats_init_stats_index(stats_index_t *stats_idx, index_t *idx)
{
    ...
    stats_idx->is_subpart = OG_FALSE;
    stats_idx->subpart_id = OG_INVALID_ID32;
    stats_idx->part_id = OG_INVALID_ID32;
    stats_idx->uid = idx->desc.uid;
    stats_idx->table_id = idx->desc.table_id;
    stats_idx->idx_id = idx->desc.id;
                                       ...
                                       btree_segment_t *segment, bool32 is_dynamic)
{
    ...
    if (stats_idx->info.distinct_keys == 0) {
        ...
        stats_idx->avg_data_key = (double)(btree_get_segment_page_count(space, segment) -
            recycled_cnt) / (double)stats_idx->info.distinct_keys;
    }
    ...
    } else if (stats_idx->subpart_id == OG_INVALID_ID32) {
        if (stats_update_sys_indexpart(session, stats_idx) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (stats_update_sys_subindexpart(session, stats_idx) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    index_part_t *index_part, double sample_ratio, bool32 is_dynamic)
{
    ...
    for (uint32 i = 0; i < index_part->desc.subpart_cnt; i++) {
        ...
        if (stats_sample_stats_one_btree(session, &index_subpart->btree,
            &stats_idx.info, sample_ratio) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (stats_save_btree_stats(session, idx, &stats_idx,
            (btree_segment_t *)index_subpart->btree.segment, is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    double sample_ratio, bool32 is_dynamic)
{
    ...
    for (uint32 i = 0; i < idx->part_index->desc.partcnt; i++) {
        ...
        if (!IS_PARENT_IDXPART(&index_part->desc)) {
            ...
            if (stats_save_btree_stats(session, idx, &stats_idx,
                (btree_segment_t *)index_part->btree.segment, is_dynamic) != OG_SUCCESS) {
                return OG_ERROR;
            }
        } else {
            if (stats_sample_stats_btree_subpart(session, idx, index_part,
                sample_ratio, is_dynamic) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
...
static status_t stats_sample_stats_btree(knl_session_t *session, index_t *idx, double sample_ratio, bool32 is_dynamic)
{
    ...
    if (!IS_PART_INDEX(idx)) {
        ...
        if (stats_sample_stats_one_btree(session, &idx->btree, &stats_idx.info, sample_ratio) != OG_SUCCESS) {
            return OG_ERROR;
        }

        if (stats_save_btree_stats(session, idx, &stats_idx,
            (btree_segment_t *)idx->btree.segment, is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
{
    if (btree->index->desc.cr_mode == CR_PAGE) {
        if (pcrb_full_stats_info(session, btree, info) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (btree_full_stats_info(session, btree, info) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    index_part_t *index_part, bool32 is_dynamic)
{
    ...
    for (uint32 i = 0; i < index_part->desc.subpart_cnt; i++) {
        ...
        if (stats_full_stats_one_btree(session, &index_subpart->btree, &stats_idx.info) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t stats_full_stats_btree_part(knl_session_t *session, index_t *idx, bool32 is_dynamic)
{
    ...
    for (uint32 i = 0; i < idx->part_index->desc.partcnt; i++) {
        ...
        if (!IS_PARENT_IDXPART(&index_part->desc)) {
            ...
            if (stats_full_stats_one_btree(session, &index_part->btree, &stats_idx.info) != OG_SUCCESS) {
                return OG_ERROR;
            }
            ...
            if (stats_full_stats_btree_subpart(session, idx, index_part, is_dynamic) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
...
static status_t stats_full_stats_btree(knl_session_t *session, index_t *idx, bool32 is_dynamic)
{
    ...
    if (!IS_PART_INDEX(idx)) {
        ...
        if (stats_full_stats_one_btree(session, &idx->btree, &stats_idx.info) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t stats_write_empty_btree_stats(knl_session_t *session, index_t *idx)
{
    ...
    if (stats_write_empty_sys_index(session, &stats_idx) != OG_SUCCESS) {
        return OG_ERROR;
    }
                                         ...
                                         double sample_ratio, bool32 is_dynamic)
{
    ...
    if (stats_check_segment_dropped(session, table, idx)) {
        if (stats_write_empty_btree_stats(session, idx) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    if (sample_ratio > OG_REAL_PRECISION) {
        if (stats_sample_stats_btree(session, idx, sample_ratio, is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (stats_full_stats_btree(session, idx, is_dynamic) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
                                     ...
                                     bool32 is_dynamic)
{
    ...
    if (stats_gather_btree_stats(session, dc, idx, def->sample_ratio, is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t db_write_sys_mon_modes(knl_session_t *session, stats_table_mon_t *table_smon, uint32 uid, uint32 id,
                                uint32 part_id, bool32 is_part)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &uid, sizeof(uint32), IX_COL_MODS_003_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &id, sizeof(uint32), IX_COL_MODS_003_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &part_id, sizeof(uint32),
    ...
    if (cursor->eof) {
        if (db_insert_sys_mon_mods(session, cursor, table_smon, uid, id, part_id, is_part) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
    } else {
        ...
        if (db_update_sys_mon_modes(session, cursor, table_smon) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
    }
...
static status_t db_flush_sys_mon_modes(knl_session_t *session, dc_entity_t *entity, uint32 uid, uint32 id)
{
    table_t *table = &entity->table;
    part_table_t *part_table = NULL;
    table_part_t *table_part = NULL;
    ...
    if (IS_PART_TABLE(table)) {
        ...
        for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
            ...
            if (db_write_sys_mon_modes(session, table_smon, uid, id, table_part->desc.part_id, OG_TRUE) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
        /* FOR ALL PART TABLE DML STATISTICS TO WRITE MON_MODES$ */
        table_smon = &entity->entry->appendix->table_smon;
        if (db_write_sys_mon_modes(session, table_smon, uid, id, OG_INVALID_ID32, OG_TRUE) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        table_smon = &entity->entry->appendix->table_smon;
        if (db_write_sys_mon_modes(session, table_smon, uid, id, OG_INVALID_ID32, OG_FALSE) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
status_t stats_flush_monitor_force(knl_session_t *session, dc_entity_t *entity)
{
    ...
    if (db_flush_sys_mon_modes(session, entity, entity->table.desc.uid, entity->table.desc.id) != OG_SUCCESS) {
...
status_t stats_flush_monitor_normal(knl_session_t *session)
{
    ...
    for (;;) {
        status = OG_SUCCESS;
        if (dc_scan_all_tables(session, &uid, &table_id, &eof) != OG_SUCCESS) {
            cm_reset_error();
            continue;
        }
        ...
        if (knl_open_dc_by_id(session, uid, table_id, &dc, OG_TRUE) != OG_SUCCESS) {
            OG_LOG_RUN_WAR("Open %u.%u monitor statistic failed normal, please gather statistics manually",
                           uid, table_id);
            cm_reset_error();
            continue;
        }
        ...
        if (lock_table_shared_directly(session, &dc) == OG_SUCCESS) {
            if (db_flush_sys_mon_modes(session, DC_ENTITY(&dc), uid, table_id) != OG_SUCCESS) {
                status = OG_ERROR;
            }
        } else {
            status = OG_ERROR;
        ...
        if (status == OG_ERROR) {
            OG_LOG_RUN_WAR("Flush %s.%s monitor statistic failed normal, please gather statistics manually",
...
static status_t stats_flush_monitor_from_lru(knl_session_t *session)
{
    status_t status = OG_SUCCESS;
    ...
    for (uint32 pos = 0; pos < STATS_FLUSH_ENTITY_NUM; pos++) {
        ...
        if (session->canceled || session->killed) {
            break;
        }
        ...
        if (knl_open_dc_by_id(session, uid, table_id, &dc, OG_TRUE) != OG_SUCCESS) {
            OG_LOG_RUN_WAR("Open %s.%s falied from dc lru, please gather statistics manually",
        ...
        if (status == OG_ERROR) {
            OG_LOG_RUN_WAR("Flush %s.%s monitor statistic failed from dc lru, please gather statistics manually",
                           DC_ENTITY(&dc)->entry->user->desc.name, DC_ENTITY(&dc)->table.desc.name);
            knl_rollback(session, NULL);
        } else {
            knl_commit(session);
...
void stats_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        ...
        if (0 == count % STATS_LRU_TABLE_MONITOR_INTERVAL) {
            db_set_with_switchctrl_lock(ctrl, &ogx->stats_gathering);
            if (!ogx->stats_gathering) {
                cm_sleep(sleep_time);
                continue;
            }
    ...
    OG_LOG_RUN_INF("stats thread closed");
...
status_t knl_flush_table_monitor(knl_handle_t session)
{
    ...
    if (stats_flush_monitor_normal(se) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t stats_seg_insert(knl_session_t *session, knl_cursor_t *cursor, knl_seg_stats_desc_t *desc)
{
    ...
    (void)row_put_int64(&ra, MIN(OG_MAX_INT64, desc->logic_reads));
    (void)row_put_int64(&ra, MIN(OG_MAX_INT64, desc->physical_writes));
    (void)row_put_int64(&ra, MIN(OG_MAX_INT64, desc->physical_reads));
    (void)row_put_int64(&ra, MIN(OG_MAX_INT64, desc->itl_waits));
    (void)row_put_int64(&ra, MIN(OG_MAX_INT64, desc->buf_busy_waits));
    (void)row_put_int64(&ra, MIN(OG_MAX_INT64, desc->row_lock_waits));
    ...
    knl_seg_stats_desc_t *desc)
{
    ...
    if (knl_open_sys_temp_cursor(session, cursor, CURSOR_ACTION_UPDATE,
                                 SYS_TMP_SEG_STAT_ID, IDX_OBJECT_ID) != OG_SUCCESS) {
        return OG_ERROR;
    }

    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_BIGINT, &desc->org_scn,
    ...
    if (cursor->eof) {
        if (stats_seg_insert(session, cursor, desc) != OG_SUCCESS) {
            knl_close_cursor(session, cursor);
            return OG_ERROR;
        }
    } else {
...
static status_t stats_insert_record(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity)
{
    ...
    if (stats_update_record(session, cursor, &desc) != OG_SUCCESS) {
        OG_LOG_DEBUG_ERR("insert table %u.%u statistic info failed, org_scn: %llu",
    ...
    if (IS_PART_TABLE(table)) {
        for (i = 0; i < table->part_table->desc.partcnt; i++) {
            ...
            if (stats_update_record(session, cursor, &desc) != OG_SUCCESS) {
                OG_LOG_DEBUG_ERR("insert table %u.%u part_table statistic info failed, org_scn: %llu",
    ...
    for (i = 0; i < table->index_set.count; i++) {
        ...
        if (stats_update_record(session, cursor, &desc) != OG_SUCCESS) {
            OG_LOG_DEBUG_ERR("insert table %u.%u index statistic info failed, org_scn: %llu",
        ...
        for (j = 0; j < idx->part_index->desc.partcnt; j++) {
            ...
            if (stats_update_record(session, cursor, &desc) != OG_SUCCESS) {
                OG_LOG_DEBUG_ERR("insert table %u.%u part_index statistic info failed, org_scn: %llu",
...
status_t stats_temp_insert(knl_session_t *session, struct st_dc_entity *dc_entity)
{
    ...
    if (stats_insert_record(temp_session, cursor, entity) != OG_SUCCESS) {
...
status_t stats_seg_load_entity(knl_session_t *session, knl_scn_t org_scn, seg_stat_t *seg_stat)
{
    ...
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_BIGINT, &org_scn, sizeof(knl_scn_t),
                     IX_COL_OBJECT_UID);

    if (knl_fetch(temp_session, cursor) != OG_SUCCESS) {
...
status_t stats_purge_stats_by_time(knl_session_t *session, int64 max_analyze_time)
{
    ...
    while (!cursor->eof) {
        ...
        if (analyze_time <= max_analyze_time) {
            if (stats_delete_table_stats(session, uid, table_id, OG_FALSE) != OG_SUCCESS) {
                knl_rollback(session, NULL);
                cm_reset_error();
            } else {
                knl_commit(session);
...
static status_t stats_reset_sys_indexpart_by_part(knl_session_t *session, uint32 uid, uint32 oid, uint32 part_id)
{
    knl_cursor_t *cursor = NULL;
    uint32 part;
    row_assist_t ra;
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, &uid, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, OG_TYPE_INTEGER, &oid, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_TABLE_ID);
                                          ...
                                          uint32 part_id)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, &oid, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, &idx_id, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_INDEX_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, &part_id, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_PART_ID);
...
static status_t stats_reset_tableparts_stats(knl_session_t *session, knl_dictionary_t *dc)
{
    table_t *table = DC_TABLE(dc);
    part_table_t *part_table = table->part_table;
    table_part_t *table_part = NULL;
    ...
    for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
        ...
        if (stats_reset_sys_tablepart(session, dc->uid, dc->oid, table_part->desc.part_id) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t stats_reset_indexparts_stats(knl_session_t *session, knl_dictionary_t *dc)
{
    table_t *table = DC_TABLE(dc);
    index_t *idx = NULL;
    ...
    for (uint32 i = 0; i < table->index_set.count; i++) {
        ...
        for (uint32 j = 0; j < part_index->desc.partcnt; j++) {
            ...
            if (stats_reset_sys_indexpart(session, dc->uid, dc->oid, idx->desc.id,
                                          index_part->desc.part_id) != OG_SUCCESS) {
                return OG_ERROR;
            }
        }
    }
...
{
    knl_dictionary_t dc;
    bool32 need_delete = OG_FALSE;
    stats_load_info_t load_info;

    if (knl_open_dc_by_id(session, uid, oid, &dc, OG_TRUE) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (stats_check_analyzing(session, &dc, &need_delete, OG_FALSE) != OG_SUCCESS) {
        dc_close(&dc);
        return OG_ERROR;
    }
    ...
    CM_SAVE_STACK(session->stack);
    knl_cursor_t *cursor = knl_push_cursor(session);
    if (stats_delete_histgram_by_part(session, cursor, &dc, part_id) != OG_SUCCESS) {
    ...
    if (stats_delete_histhead_by_part(session, &dc, part_id) != OG_SUCCESS) {
        stats_set_analyzed(session, &dc, need_delete);
        dc_close(&dc);
        return OG_ERROR;
    }

    if (stats_reset_sys_indexpart_by_part(session, uid, oid, part_id) != OG_SUCCESS) {
        stats_set_analyzed(session, &dc, need_delete);
        dc_close(&dc);
        return OG_ERROR;
    }

    if (stats_reset_sys_tablepart(session, uid, oid, part_id) != OG_SUCCESS) {
        stats_set_analyzed(session, &dc, need_delete);
        dc_close(&dc);
        return OG_ERROR;
    }
    ...
    stats_set_analyzed(session, &dc, need_delete);
    load_info.parent_part_id = part_id;
    load_info.load_subpart = OG_TRUE;
...
static status_t stats_reset_table_stats(knl_session_t *session, knl_dictionary_t *dc, bool32 is_old_nologging)
{
    uint32 uid = dc->uid;
    uint32 oid = dc->oid;
    bool32 need_delete = OG_FALSE;
    ...
    if (stats_drop_hists(session, uid, oid, is_nologging) != OG_SUCCESS) {
        stats_set_analyzed(session, dc, need_delete);
        return OG_ERROR;
    }

    if (stats_reset_sys_column(session, uid, oid) != OG_SUCCESS) {
        stats_set_analyzed(session, dc, need_delete);
        return OG_ERROR;
    }

    if (stats_reset_sys_index(session, uid, oid) != OG_SUCCESS) {
        stats_set_analyzed(session, dc, need_delete);
        return OG_ERROR;
    }

    if (stats_reset_sys_table(session, uid, oid) != OG_SUCCESS) {
        stats_set_analyzed(session, dc, need_delete);
        return OG_ERROR;
    }
    ...
    if (IS_PART_TABLE(table)) {
        if (stats_reset_indexparts_stats(session, dc) != OG_SUCCESS) {
            stats_set_analyzed(session, dc, need_delete);
            return OG_ERROR;
        }

        if (stats_reset_tableparts_stats(session, dc) != OG_SUCCESS) {
            stats_set_analyzed(session, dc, need_delete);
            return OG_ERROR;
        }

        if (cbo_stats != NULL) {
            cbo_stats->global_stats_exist = OG_FALSE;
        }
    }
...
status_t stats_delete_table_stats(knl_session_t *session, const uint32 uid, const uint32 oid, bool32 is_old_nologging)
{
    ...
    if (stats_reset_table_stats(session, &dc, is_old_nologging) != OG_SUCCESS) {
        dc_close(&dc);
        return OG_ERROR;
    }
...
static void stats_calc_global_dml_stats(stats_table_mon_t *global_mon, stats_table_mon_t *part_mon)
{
...
status_t stats_refresh_dc(knl_session_t *session, knl_dictionary_t *dc, stats_load_info_t load_info)
{
    ...
    knl_set_session_scn(session, OG_INVALID_ID64);
    if (cbo_refresh_statistics(session, entity, load_info) != OG_SUCCESS) {
        return OG_ERROR;
    }
    return OG_SUCCESS;
}
...
void stats_dc_invalidate(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    if (lock_table_directly(session, dc, LOCK_INF_WAIT) != OG_SUCCESS) {
        OG_LOG_RUN_WAR("lock table failed when invalidate statistics");
        return;
    }

    if (lock_child_table_directly(session, entity, OG_FALSE) != OG_SUCCESS) {
        unlock_tables_directly(session);
        OG_LOG_RUN_WAR("lock table failed when invalidate statistics");
        return;
    }
                               ...
                               bool32 is_dynamic)
{
    ...
    if (!entity->is_analyzing) {
        entity->is_analyzing = OG_TRUE;
        *need_analyze = OG_TRUE;
        cm_spin_unlock(&entity->entry->lock);
        return OG_SUCCESS;
    } else {
        if (is_dynamic) {
...
void stats_init_stats_option(stats_option_t *stats_option, knl_analyze_tab_def_t *def)
{
    ...
    stats_option->sample_ratio =  def->sample_ratio < OG_REAL_PRECISION
                         ...
                         table_part_t **table_part)
{
    ...
    if (def->part_name.len > 0) {
        if (!part_table_find_by_name(part_table, &def->part_name, table_part)) {
            OG_THROW_ERROR(ERR_PARTITION_NOT_EXIST, "table", T2S(&def->part_name));
            return OG_ERROR;
        }

        if ((*table_part)->desc.not_ready == PARTITON_NOT_READY) {
            OG_THROW_ERROR(ERR_PARTITION_NOT_READY, "table", T2S(&def->part_name));
            return OG_ERROR;
        }
    } else {
        ...
        *table_part = PART_GET_ENTITY(part_table, def->part_no);
        ...
        if (*table_part == NULL) {
            ...
            OG_THROW_ERROR(ERR_PARTITION_NOT_EXIST, "table", T2S(&partno_str));
            return OG_ERROR;
        }
    }
    ...
    uint32 *empty_block)
{
    if ((tab_stats->rownums == 0 && tab_stats->avgrlen != 0) || (tab_stats->avgrlen == 0 && tab_stats->rownums != 0)) {
        OG_LOG_RUN_ERR("stats: fail to estimate empty block according to given parameters");
        return OG_ERROR;
    }
...
static void stats_verfiy_table_stats(knl_cursor_t *cursor, knl_table_set_stats_t *tab_stats)
{
    ...
    if (tab_stats->is_single_part) {
        if (CURSOR_COLUMN_SIZE(cursor, SYS_INDEXPART_COL_ANALYZETIME) != OG_NULL_VALUE_LEN) {
    ...
    tab_stats->rownums = (tab_stats->rownums == OG_INVALID_ID32) ? sys_stats.rownums : tab_stats->rownums;
    tab_stats->blknums = (tab_stats->blknums == OG_INVALID_ID32) ? sys_stats.blknums : tab_stats->blknums;
    tab_stats->avgrlen = (tab_stats->avgrlen == OG_INVALID_ID64) ? sys_stats.avgrlen : tab_stats->avgrlen;
    tab_stats->samplesize = (tab_stats->samplesize == OG_INVALID_ID64) ? sys_stats.samplesize : tab_stats->samplesize;
}
                                             ...
                                             knl_table_set_stats_t *tab_stats)
{
    ...
    uint32 empty_block = 0;
    if (stats_calc_table_empty_block(session, tab_stats, &empty_block) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_INVALID_OPERATION, ", fail to estimate empty_block");
        return OG_ERROR;
    }
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->rownums));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, tab_stats->blknums));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, empty_block));
    (void)row_put_int64(&ra, tab_stats->avgrlen);
    (void)row_put_int64(&ra, MIN(OG_MAX_INT32, tab_stats->samplesize));
    (void)row_put_int64(&ra, OG_INVALID_ID64);
    ...
    return stats_set_analyze_time(session, dc, OG_TRUE);
}
                                                 ...
                                                 knl_table_set_stats_t *tab_stats)
{
    ...
    key = &cursor->scan_range.l_key;
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_part->desc.uid, sizeof(uint32),
                     IX_COL_SYS_TABLEPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_part->desc.table_id, sizeof(uint32),
                     IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &table_part->desc.part_id, sizeof(uint32),
                     IX_COL_SYS_TABLEPART001_PART_ID);
...
static void stats_verfiy_colunm_stats(knl_cursor_t *cursor, knl_column_set_stats_t *col_stats)
{
    ...
    if (CURSOR_COLUMN_SIZE(cursor, SYS_HISTGRAM_ABSTR_COL_ANALYZETIME) != OG_NULL_VALUE_LEN) {
...
static status_t stats_update_sys_histhead_force(knl_session_t *session, knl_dictionary_t *dc, knl_column_set_stats_t
    *col_stats,
                                         table_part_t *table_part, knl_column_t *column)
{
    uint64 part_id = (table_part != NULL) ? table_part->desc.part_id : OG_INVALID_ID32;
    table_t *table = DC_TABLE(dc);
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &column->id, sizeof(uint32),
                     IX_COL_HIST_HEAD_003_COL_ID);
    if (IS_PART_TABLE(table)) {
        knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_BIGINT, &part_id, sizeof(uint64),
            IX_COL_HIST_HEAD_003_SPARE1);
    } else {
    ...
    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_OBJECT_NOT_EXISTS, "column statistics of", column->name);
        return OG_ERROR;
    }
...
static void stats_verfiy_index_stats(knl_cursor_t *cursor, knl_index_set_stats_t *idx_stats)
{
    ...
    if (idx_stats->is_single_part) {
        ...
        if (CURSOR_COLUMN_SIZE(cursor, SYS_INDEX_COLUMN_ID_ANALYZETIME) != OG_NULL_VALUE_LEN) {
    ...
    idx_stats->indlevel = (idx_stats->indlevel == OG_INVALID_ID32) ? sys_stats.indlevel : idx_stats->indlevel;
    idx_stats->numlblks = (idx_stats->numlblks == OG_INVALID_ID32) ? sys_stats.numlblks : idx_stats->numlblks;
    idx_stats->numdist = (idx_stats->numdist == OG_INVALID_ID32) ? sys_stats.numdist : idx_stats->numdist;
    idx_stats->avglblk = (idx_stats->avglblk == OG_INVALID_ID64) ? sys_stats.avglblk : idx_stats->avglblk;
    idx_stats->avgdblk = (idx_stats->avgdblk == OG_INVALID_ID64) ? sys_stats.avgdblk : idx_stats->avgdblk;
    idx_stats->clstfct = (idx_stats->clstfct == OG_INVALID_ID32) ? sys_stats.clstfct : idx_stats->clstfct;
    idx_stats->combndv2 = (idx_stats->combndv2 == OG_INVALID_ID32) ? sys_stats.combndv2 : idx_stats->combndv2;
    idx_stats->combndv3 = (idx_stats->combndv3 == OG_INVALID_ID32) ? sys_stats.combndv3 : idx_stats->combndv3;
    idx_stats->combndv4 = (idx_stats->combndv4 == OG_INVALID_ID32) ? sys_stats.combndv4 : idx_stats->combndv4;
}
    ...
    btree_segment_t *segment, uint32 *empty_block)
{
    ...
    if (!spc_validate_page_id(session, AS_PAGID(segment->tree_info.root))) {
        OG_LOG_RUN_ERR("stats: index %s has been droped", idx->desc.name);
        return OG_ERROR;
    }
    ...
    if (btree_check_segment_scn(page, page_type, segment->seg_scn) != OG_SUCCESS) {
        buf_leave_page(session, OG_FALSE);
        OG_LOG_RUN_ERR("stats: page scn is different from seg scn, index name: %s", idx->desc.name);
        return OG_ERROR;
    }
    ...
    uint32 page_data_size = DEFAULT_PAGE_SIZE(session) - sizeof(btree_page_t) -
        (SPACE_GET(session, segment->space_id))->ctrl->cipher_reserve_size - sizeof(page_tail_t) -
        page->itls * itl_size;
...
static status_t stats_calc_empty_block_index(knl_session_t *session, index_t *idx, knl_index_set_stats_t *idx_stats,
    uint32 *empty_block)
{
    if (stats_check_idx_stats_params(idx_stats) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("stats: fail to estimate empty block according to given parameters");
        return OG_ERROR;
    }
    ...
    if (IS_INVALID_PAGID(btree_entry)) {
        OG_LOG_RUN_ERR("stats: invalid page id, index name: %s", idx->desc.name);
        return OG_ERROR;
    }
    ...
    if (page->type != PAGE_TYPE_BTREE_HEAD || segment->seg_scn != seg_scn) {
        buf_leave_page(session, OG_FALSE);
        OG_LOG_RUN_ERR("stats: current seg scn is different from origin seg scn, index name: %s", idx->desc.name);
        return OG_ERROR;
    }
    ...
    knl_index_set_stats_t *idx_stats, uint32 empty_block)
{
    ...
    (void)row_put_int32(&ra, MIN(OG_MAX_ROOT_LEVEL, idx_stats->indlevel));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, idx_stats->numlblks));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, idx_stats->numdist));
    (void)row_put_real(&ra, idx_stats->avglblk);
    (void)row_put_real(&ra, idx_stats->avgdblk);
    (void)row_put_int64(&ra, OG_INVALID_ID64);
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, empty_block));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, idx_stats->clstfct));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, idx_stats->combndv2));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, idx_stats->combndv3));
    (void)row_put_int32(&ra, MIN(OG_MAX_INT32, idx_stats->combndv4));
...
static status_t stats_update_sys_index_force(knl_session_t *session, knl_dictionary_t *dc, knl_index_set_stats_t *idx_stats,
                                      index_t *index)
{
    knl_scan_key_t *key = NULL;
    ...
    knl_init_index_scan(cursor, OG_TRUE);
    key = &cursor->scan_range.l_key;
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &index->desc.uid, sizeof(uint32),
                     IX_COL_SYS_INDEX_002_USER);
    ...
    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_INDEX_ALREADY_DROPPED, index->desc.name);
        return OG_ERROR;
    }
    ...
    uint32 empty_block = 0;
    if (stats_calc_empty_block_index(session, index, idx_stats, &empty_block) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_INVALID_OPERATION, ", fail to estimate empty_block.");
        return OG_ERROR;
    }
    ...
    knl_index_set_stats_t *idx_stats, btree_segment_t *segment, uint32 *empty_block)
{
    ...
    if (!spc_validate_page_id(session, AS_PAGID(segment->tree_info.root))) {
        OG_LOG_RUN_ERR("stats: part index %s has been droped", idx_part->desc.name);
        return OG_ERROR;
    }
    ...
    if (btree_check_segment_scn(page, page_type, segment->seg_scn) != OG_SUCCESS) {
        buf_leave_page(session, OG_FALSE);
        OG_LOG_RUN_ERR("stats: page scn is different from seg scn, part index: %s", idx_part->desc.name);
        return OG_ERROR;
    }
    ...
    knl_index_set_stats_t *idx_stats, uint32 *empty_block)
{
    ...
    if (IS_INVALID_PAGID(btree_entry)) {
        OG_LOG_RUN_ERR("stats: invalid page id, part index: %s", idx_part->desc.name);
        return OG_ERROR;
    }
    ...
    if (page->type != PAGE_TYPE_BTREE_HEAD || segment->seg_scn != seg_scn) {
        buf_leave_page(session, OG_FALSE);
        OG_LOG_RUN_ERR("stats: current seg scn is different from origin seg scn, part index: %s", idx_part->desc.name);
        return OG_ERROR;
    }
...
static status_t stats_update_sys_indexpart_force(knl_session_t *session, knl_dictionary_t *dc,
                                          knl_index_set_stats_t *idx_stats, index_part_t *idx_part)
{
    ...
    key = &cursor->scan_range.l_key;
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &idx_part->desc.uid, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &idx_part->desc.table_id, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &idx_part->desc.index_id, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_INDEX_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, OG_TYPE_INTEGER, &idx_part->desc.part_id, sizeof(uint32),
                     IX_COL_SYS_INDEXPART001_PART_ID);
    ...
    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_INDEX_ALREADY_DROPPED, idx_part->desc.name);
        return OG_ERROR;
    }
    ...
    uint32 empty_block = 0;
    if (stats_calc_empty_block_part(session, idx_part, idx_stats, &empty_block) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        OG_THROW_ERROR(ERR_INVALID_OPERATION, ", fail to estimate empty_block.");
        return OG_ERROR;
    }
                          ...
                          table_part_t *table_part)
{
    ...
    if (tab_stats->is_single_part) {
        if (stats_update_sys_tablepart_force(session, dc, table_part, tab_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (stats_update_sys_table_force(session, dc, tab_stats) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    if (!entity->stat_exists) {
        ...
        OG_LOG_RUN_ERR("[DC] could not load table statistics %s.%s.", entity->entry->user_name, entity->entry->name);
        return OG_ERROR;
    }
    ...
    knl_column_t *column, text_t *src, text_t *dest)
{
    ...
    if (cbo_alloc_value_mem(session, entity->memory, column, &dest->str) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (cbo_get_stats_values(entity, column, src, dest) != OG_SUCCESS) {
        return OG_ERROR;
    }
                          ...
                          table_part_t *table_part, knl_column_t *column)
{
    ...
    col_stats->max_value.str = (col_stats->max_value.len == OG_NULL_VALUE_LEN) ? max_buf : col_stats->max_value.str;
    col_stats->min_value.str = (col_stats->min_value.len == OG_NULL_VALUE_LEN) ? min_buf : col_stats->min_value.str;

    if (stats_update_sys_histhead_force(session, dc, col_stats, table_part, column) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (table_part != NULL) {
        ...
        if (!part_stats->is_ready) {
            stats_set_load_info(&load_info, entity, OG_TRUE, OG_INVALID_ID32);
            if (cbo_load_table_part_stats(session, entity, table_part->part_no, load_info, OG_TRUE) != OG_SUCCESS) {
                cm_unlatch(&entity->cbo_latch, NULL);
                return OG_ERROR;
            }
        }
    ...
    if (cbo_column == NULL) {
        OG_LOG_RUN_WAR("Failed to modify %s.%s the %d column stats, it is not analyzed",
            entity->entry->user->desc.name, entity->table.desc.name, column->id);
        cm_unlatch(&entity->cbo_latch, NULL);
        return OG_SUCCESS;
    }

    if (stats_set_cbo_column_value(session, entity, column, col_stats, cbo_column) != OG_SUCCESS) {
        cm_unlatch(&entity->cbo_latch, NULL);
        return OG_ERROR;
    }
                         ...
                         index_part_t *idx_part, index_t *index)
{
    ...
    if (idx_stats->is_single_part) {
        if (stats_update_sys_indexpart_force(session, dc, idx_stats, idx_part) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (stats_update_sys_index_force(session, dc, idx_stats, index) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    if (idx_stats->is_single_part) {
        ...
        if (!part_idx_stats->is_ready) {
            if (cbo_load_index_part_stats(session, entity, idx_part->part_no, index->desc.id) != OG_SUCCESS) {
                cm_unlatch(&entity->cbo_latch, NULL);
                return OG_ERROR;
            }
        }
...
void stats_flush_logic_log(knl_session_t *session, knl_dictionary_t *dc, stats_load_info_t *load_info)
{
    ...
    if (knl_begin_auton_rm(session) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Analyze %s.%s statistics write logic log failed ", entity->entry->user->desc.name,
...
{
     *need_analyze = OG_FALSE;

    if (stats_check_analyzing(session, dc, need_analyze, is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    CM_SAVE_STACK(session->stack);
    if (stats_gather_normal_table(session, dc, stats_option, is_dynamic) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
    CM_RESTORE_STACK(session->stack);
...
{
    def->need_analyzed = OG_FALSE;
    def->table_part = NULL;

    if (stats_find_part(session, def, dc, (table_part_t**)&def->table_part) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (stats_check_analyzing(session, dc, &def->need_analyzed, is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (stats_gather_table_part(session, dc, stats_option, (table_part_t *)def->table_part, is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
                             ...
                             bool32 is_dynamic)
{
    ...
    if (stats_gather_index_by_btree(session, dc, def, is_dynamic) != OG_SUCCESS) {
        return OG_ERROR;
    }
