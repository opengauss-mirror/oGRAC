 ...
 *
 * IDENTIFICATION
 * src/kernel/include/mtrl_defs.h
 *
 * -------------------------------------------------------------------------
 */
...
#define MTRL_PAGE_CAPACITY(page) (OG_VMEM_PAGE_SIZE - ((page)->free_begin))
...
typedef struct st_mtrl_context {
    spinlock_t lock;
    uint32 seg_count;
    uint32 open_pages[OG_MAX_MTRL_OPEN_PAGES];
    handle_t session;
    vm_pool_t *pool;
    ...
    mtrl_segment_t *segments[OG_MAX_MATERIALS];
} mtrl_context_t;
...
typedef struct st_mtrl_page {
    ...
    uint16 is_sorted : 1;      // avoid some pages being sorted multiple times in mtrl_sort_segment_parallel.
    uint16 unused : 13;
} mtrl_page_t;
...
typedef struct st_mtrl_cursor {
    ...
    bool32 eof;
    uint32 slot;
    uint32 history[OG_MAX_JOIN_TABLES];
    uint32 count;
...
static inline void mtrl_init_page(mtrl_page_t *page, uint32 id)
{
    ...
    page->has_part_info = OG_FALSE;
    page->page_occupied = OG_FALSE;
    page->is_sorted = OG_FALSE;
    page->unused = 0;
}
...
status_t mtrl_get_column_value(mtrl_row_assist_t *row, bool32 eof, uint32 id, og_type_t datatype,
