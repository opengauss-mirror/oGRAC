 ...
 *
 * IDENTIFICATION
 * src/kernel/include/sysdba_defs.h
 *
 * -------------------------------------------------------------------------
 */
...
typedef struct st_trigger_set {
    ...
    void *items[OG_MAX_TRIGGER_COUNT];
} trigger_set_t;
...
typedef struct st_knl_rsrc_plan {
    ...
    char description[OG_COMMENT_BUFFER_SIZE];
    uint32 type;
}knl_rsrc_plan_t;

typedef struct st_knl_rsrc_plan_rule {
    char plan_name[OG_NAME_BUFFER_SIZE];
    char group_name[OG_NAME_BUFFER_SIZE];
...
typedef struct st_knl_rsrc_group_mapping {
    char attribute[OG_NAME_BUFFER_SIZE];
    char value[OG_VALUE_BUFFER_SIZE];
    char group_name[OG_NAME_BUFFER_SIZE];
...
typedef struct st_object_address {
    ...
    char name[OG_NAME_BUFFER_SIZE]; /* object name */
} object_address_t;
...
status_t knl_get_free_extent(knl_handle_t se, uint32 file_id, page_id_t start, uint32 *extent,
