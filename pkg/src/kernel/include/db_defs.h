...
typedef struct st_knl_dblink_desc {
    ...
    char name[OG_NAME_BUFFER_SIZE];         /* dblink name */
    char user[OG_NAME_BUFFER_SIZE];         /* dblink user */
} knl_dblink_desc_t;
...
typedef struct st_knl_database_def {
    text_t name;                                 // name of database
    SENSI_INFO char sys_password[OG_PASSWORD_BUFFER_SIZE];  // sys pwd of database
