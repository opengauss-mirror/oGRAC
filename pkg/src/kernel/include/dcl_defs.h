...
typedef enum en_sys_privs_id_def {
    ...
    DROP_DATABASE_LINK,
    USE_ANY_TABLESPACE,
    /* OG_SYS_PRIVS_COUNT must placed at the bottom */
    OG_SYS_PRIVS_COUNT
} sys_privs_id;
...
typedef enum en_obj_privs_id_def {
    ...
    /* OG_OBJ_PRIVS_COUNT must placed at the bottom */
    OG_OBJ_PRIVS_COUNT
} obj_privs_id;
...
typedef enum en_user_privs_id_def {
    ...
    /* OG_USER_PRIVS_COUNT must placed at the bottom */
    OG_USER_PRIVS_COUNT
} user_privs_id;
...
typedef struct st_knl_grant_def {
    ...
    text_t type_name;         /* object type name, one of {"table", "view", "sequence", "procedure", "global views"} */
...
typedef struct st_knl_user_def {
    char name[OG_NAME_BUFFER_SIZE];          // username
    SENSI_INFO char password[OG_PASSWORD_BUFFER_SIZE];  // if it comes from sql engine ,it's pwd string before encrypt.
...
typedef struct st_knl_role_def {
    ...
    SENSI_INFO char password[OG_PASSWORD_BUFFER_SIZE]; /* role pwd */
...
typedef struct st_knl_tenant_def {
    char name[OG_TENANT_BUFFER_SIZE];             /* tenant name */
    char default_tablespace[OG_NAME_BUFFER_SIZE]; /* default tablespace */
