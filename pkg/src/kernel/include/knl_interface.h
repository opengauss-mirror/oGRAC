...
typedef struct st_knl_part_key {
    ...
    uint16 offsets[OG_MAX_PARTKEY_COLUMNS];  // column offsets
    uint16 lens[OG_MAX_PARTKEY_COLUMNS];     // column lens
    bool32 closed[OG_MAX_PARTKEY_COLUMNS];   // column include equal or not
} knl_part_key_t;
...
typedef struct st_knl_lnk_dc_callback {
    // input data
    memory_context_t *ogx; // dblink memory context
...
#define IS_LTT_BY_ID(id)     ((id) >= OG_LTT_ID_OFFSET && (id) < OG_DBLINK_ENTRY_START_ID) /* LTT = Local Temporary Table */
