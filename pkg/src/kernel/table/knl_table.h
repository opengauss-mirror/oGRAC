...
#define COLUMN_LIST_BUF_LEN (OG_MAX_INDEX_COLUMNS * 8)
...
typedef struct st_rd_altable_rename_table {
    char user_str[OG_NAME_BUFFER_SIZE];
    char name_str[OG_NAME_BUFFER_SIZE];
    char new_name_str[OG_NAME_BUFFER_SIZE];
...
status_t db_create_index_segment_of_hash_parts(knl_session_t *session, knl_dictionary_t *dc, table_t *table);
