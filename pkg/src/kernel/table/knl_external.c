    ...
    text_t *column, uint16 col_id)
{
    ...
    switch (dc_column->datatype) {
        case OG_TYPE_BOOLEAN:
            if (cm_text2bool(column, &tmp_value.v_bool) != OG_SUCCESS) {
                return OG_ERROR;
            }
            status = row_put_bool(ra, tmp_value.v_bool);
            break;
        case OG_TYPE_UINT32:
            if (cm_text2uint32(column, &val_uint32) != OG_SUCCESS) {
                return OG_ERROR;
            }
            status = row_put_uint32(ra, val_uint32);
            break;
        case OG_TYPE_INTEGER:
            if (cm_text2int(column, &val_int) != OG_SUCCESS) {
                return OG_ERROR;
            }
            status = row_put_int32(ra, val_int);
            break;
        case OG_TYPE_UINT64:
            if (cm_text2uint64(column, &val_ubigint) != OG_SUCCESS) {
                return OG_ERROR;
            }
            status = row_put_uint64(ra, val_ubigint);
            break;
        case OG_TYPE_BIGINT:
            if (cm_text2bigint(column, &val_bigint) != OG_SUCCESS) {
                return OG_ERROR;
            }
            status = row_put_int64(ra, val_bigint);
            break;
        case OG_TYPE_REAL:
            if (cm_text2real(column, &val_double) != OG_SUCCESS) {
                return OG_ERROR;
            }

            if (cm_adjust_double(&val_double, dc_column->precision, dc_column->scale) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        case OG_TYPE_NUMBER2:
            if (cm_text_to_dec(column, &tmp_value.v_dec) != OG_SUCCESS) {
                return OG_ERROR;
            }
            if (cm_adjust_dec(&tmp_value.v_dec, dc_column->precision, dc_column->scale) != OG_SUCCESS) {
                return OG_ERROR;
            }
            status = (OG_IS_NUMBER2_TYPE(dc_column->datatype)) ?
        ...
        case OG_TYPE_TIMESTAMP_LTZ:
            if (cm_text2date(column, NULL, &tmp_value.v_date) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        case OG_TYPE_TIMESTAMP_TZ:
            if (cm_text2timestamp_tz(column, NULL, cm_get_local_tzoffset(), &tmp_value.v_tstamp_tz) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        case OG_TYPE_INTERVAL_DS:
            if (cm_text2dsinterval(column, &tmp_value.v_itvl_ds) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        case OG_TYPE_INTERVAL_YM:
            if (cm_text2yminterval(column, &tmp_value.v_itvl_ym) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        case OG_TYPE_CHAR:
            if (column->len > dc_column->size) {
                OG_THROW_ERROR(ERR_EXCEED_MAX_FIELD_LEN, dc_column->name, column->len, dc_column->size);
                return OG_ERROR;
            }
            if (g_knl_callback.convert_char(session, column, dc_column->size, is_char) != OG_SUCCESS) {
                return OG_ERROR;
            }
    ...
    *is_found = OG_FALSE;
    ...
    while (cm_fetch_text(&line, external_desc->fields_terminator, 0, &column)) {
        if (column_id >= column_count) {
            OG_THROW_ERROR(ERR_LINE_SIZE_TOO_LONG, column_count);
            return OG_ERROR;
        }
        /* the value of column_id is no larger than 65535 */
        if (external_fill_column_to_row(session, cursor, &ra, &column, column_id) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    *is_found = OG_FALSE;
    ...
    for (;;) {
        if (cursor->text.len == 0) {
            if (cm_read_file(cursor->fd, cursor->page_buf, DEFAULT_PAGE_SIZE(session), &read_size) != OG_SUCCESS) {
                OG_LOG_RUN_ERR("[EXTERNAL] failed to read data from %s/%s",
            ...
            if (read_size == DEFAULT_PAGE_SIZE(session)) {
                ...
                if (cursor->text.len == 0) {
                    OG_THROW_ERROR(ERR_VALUE_ERROR, "the size of row is too large.");
                    return OG_ERROR;
                }

                if (cm_seek_file(cursor->fd, (int32)cursor->text.len - read_size, SEEK_CUR) == -1) {
                    OG_THROW_ERROR(ERR_SEEK_FILE, (int32)cursor->text.len - read_size, SEEK_SET, errno);
                    return OG_ERROR;
                }
            }
        }

        if (external_heap_get_row(session, cursor, is_found) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
status_t external_heap_fetch(knl_handle_t handle, knl_cursor_t *cursor)
{
    ...
    for (;;) {
        ...
        if (external_heap_fetch_by_page(session, cursor, &cursor->is_found) != OG_SUCCESS) {
            cm_close_file(cursor->fd);
            cursor->fd = -1;
            return OG_ERROR;
        }
...
static status_t db_write_sys_directory(knl_session_t *session, knl_directory_desc_t *desc)
{
    ...
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_INSERT, SYS_DIRECTORY_ID, OG_INVALID_ID32);
...
static status_t db_delete_sys_directory(knl_session_t *session, char *name)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_STRING, name,
    ...
    if (cursor->eof) {
        OG_THROW_ERROR(ERR_OBJECT_NOT_EXISTS, "directory", name);
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
...
static status_t db_revoke_privs_on_directory(knl_session_t *session, assist_obj_priv_item_t *item)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&own_uid,
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_INTEGER, (void *)&obj_type,
    ...
    while (!cursor->eof) {
        ...
        item->privid = priv_id;
        if (db_revoke_dirpriv_from_grantee(session, grantor_id, grantee_id, grantee_type, item) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
...
static status_t db_update_directory_path(knl_session_t *session, knl_directory_desc_t *desc)
{
    row_assist_t ra;
    knl_cursor_t *cursor = NULL;
...
{
    if (def->name.len > OG_MAX_NAME_LEN - 1) {
        OG_THROW_ERROR(ERR_NAME_TOO_LONG, "directory", def->name.len, OG_MAX_NAME_LEN - 1);
        return OG_ERROR;
    }

    if (def->path.len > OG_MAX_PATH_BUFFER_SIZE - 1) {
        OG_THROW_ERROR(ERR_FILE_PATH_TOO_LONG, OG_MAX_PATH_BUFFER_SIZE - 1);
        return OG_ERROR;
    }

    desc->uid = 0;
    if (cm_text2str(&def->name, desc->name, OG_MAX_NAME_LEN) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (cm_text2str(&def->path, desc->path, OG_MAX_PATH_BUFFER_SIZE) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (strlen(desc->path) == 0 || !cm_dir_exist(desc->path)) {
        OG_THROW_ERROR(ERR_PATH_NOT_EXIST, desc->path);
        return OG_ERROR;
    }

    if (access(desc->path, R_OK) != 0) {
        OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "%s is not an readable folder", desc->path);
        return OG_ERROR;
    }

#ifndef WIN32
    if (cm_verify_file_host(desc->path) != OG_SUCCESS) {
        OG_THROW_ERROR(ERR_FILE_EXEC_PRIV, desc->path);
        return OG_ERROR;
}
#endif
...
static status_t db_create_directory_with_replace(knl_session_t *session, knl_directory_desc_t *desc)
{
    text_t dir_name;
    dc_user_t *user = NULL;
    bool32 has_priv = OG_FALSE;
    dc_obj_priv_entry_t *entry = NULL;
    
    if (db_update_directory_path(session, desc) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (dc_find_objpriv_entry(&user->obj_privs, DB_SYS_USER_ID, &dir_name, OBJ_TYPE_DIRECTORY, &entry)) {
        if (DC_HAS_OBJ_PRIV(entry->priv_item.direct_grant, OG_PRIV_DIRE_READ)) {
            has_priv = OG_TRUE;
        }
    }

    if (!has_priv) {
        if (db_grant_dirpriv_to_user(session, desc->name, session->uid, OG_PRIV_DIRE_READ) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
status_t db_create_directory(knl_session_t *session, knl_directory_def_t *def)
{
    ...
    if (db_prepare_create_directory(def, &desc) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }

    if (db_fetch_directory_path(session, desc.name, NULL, 0, &is_found) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }

    if (is_found && def->is_replace) {
        if (db_create_directory_with_replace(session, &desc) != OG_SUCCESS) {
            dls_spin_unlock(session, &ogx->paral_lock);
            return OG_ERROR;
        }
    } else if (is_found && !def->is_replace) {
        OG_THROW_ERROR(ERR_OBJECT_EXISTS, "directory", desc.name);
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    } else {
        if (db_write_sys_directory(session, &desc) != OG_SUCCESS) {
            dls_spin_unlock(session, &ogx->paral_lock);
            return OG_ERROR;
        }
...
status_t db_drop_directory(knl_session_t *session, knl_drop_def_t *def)
{
    ...
    if (cm_text2str(&def->name, item.objname, OG_MAX_NAME_LEN) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }
    
    if (db_delete_sys_directory(session, item.objname) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }

    /* revoke all privilege on the directory */
    if (db_revoke_privs_on_directory(session, &item) != OG_SUCCESS) {
        dls_spin_unlock(session, &ogx->paral_lock);
        return OG_ERROR;
    }
    ...
    uint32 dire_len, bool32 *is_found)
{
    ...
    uint32 path_len;
    knl_cursor_t *cursor = NULL;
    char name_buffer[OG_FILE_NAME_BUFFER_SIZE] = { 0 };

    *is_found = OG_FALSE;
    size_t dir_name_size = strlen(dir_name);
    ret = memcpy_sp(name_buffer, OG_FILE_NAME_BUFFER_SIZE, dir_name, dir_name_size);
    knl_securec_check(ret);
    ...
    knl_init_index_scan(cursor, OG_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, OG_TYPE_STRING, name_buffer,
