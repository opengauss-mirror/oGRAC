...
#define TEMP_CT_MAX_ROW_SIZE 8000

status_t temp_heap_alloc_page(knl_session_t *session, knl_temp_cache_t *temp_cache, uint32 *curr_vmid);
static void temp_heap_revert_update(knl_session_t *session, undo_row_t *undo_row, row_head_t *row, uint16 *offsets,
                                    uint16 *lens);
...
void temp_mtrl_init_context(knl_session_t *session)
{
    ...
    ogx->seg_count = 0;
    ogx->lock = 0;
    for (i = 0; i < OG_MAX_MTRL_OPEN_PAGES; i++) {
    ...
    ogx->session = (handle_t)session;
    ogx->pool = NULL; /* assign value after getting session->temp_pool in knl_init_session_2 */
    g_knl_callback.init_vmc((handle_t)ogx);

    session->temp_table_count = 0;
    ret = memset_sp(session->temp_table_cache, sizeof(knl_temp_cache_t) * session->temp_table_capacity, 0,
                    sizeof(knl_temp_cache_t) * session->temp_table_capacity);
...
void temp_mtrl_release_context(knl_session_t *session)
{
    ...
    if (ogx->seg_count == 0) {
        int32 ret = memset_sp(ogx->segments, sizeof(ogx->segments), 0, sizeof(ogx->segments));
        knl_securec_check(ret);
        vmc_free(&ogx->vmc);
        return;
    }
...
void temp_page_init(knl_session_t *session, page_head_t *page, uint32 vmid, page_type_t type)
{
    ...
    page->type = type;
    AS_PAGID_PTR(page->next_ext)->vmid = OG_INVALID_ID32;
...
static void temp_heap_validate_page(knl_session_t *session, vm_page_t *vm_page)
{
    ...
    knl_panic_log(page->free_begin >= sizeof(temp_heap_page_t),
                  "page's free size begin is smaller than the size of "
                  "temp heap page's size, panic info: page %u-%u type %u free_begin %u",
    ...
    knl_panic_log(page->free_begin <= page->free_end,
                  "page's free size begin is more than end, panic info: "
                  "page %u-%u type %u free_begin %u free_end %u",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, page->free_begin,
                  page->free_end);
    ...
    for (uint16 i = 0; i < page->dirs; i++) {
        ...
        knl_panic_log(dir->offset < page->free_begin,
                      "dir's offset is more than page's free_begin, panic info: "
                      "page %u-%u type %u free_begin %u dir offset %u",
                      AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, page->free_begin,
                      dir->offset);
        ...
        knl_panic_log(ROW_ITL_ID(row) == OG_INVALID_ID8, "row itl id is valid, panic info: page %u-%u type %u",
                      AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type);
    }
...
status_t buf_enter_temp_page_nolock(knl_session_t *session, uint32 vmid)
{
    ...
    if (vmid == OG_INVALID_ID32) {
        OG_LOG_RUN_ERR("invalid vm id when buffer enter temp page with nolock");
...
static void temp_heap_format_page(knl_session_t *session, knl_temp_cache_t *temp_cache, temp_heap_page_t *page,
                                  uint32 id)
{
    ...
    page->oid = OG_INVALID_ID32;
    page->uid = OG_INVALID_ID16;
    page->seg_scn = OG_INVALID_ID64;
...
static inline void temp_heap_init_row(knl_session_t *session, row_assist_t *ra, char *buf, uint32 column_count,
                                      uint16 flags)
{
    row_init(ra, buf, KNL_MAX_ROW_SIZE(session), column_count);
...
status_t temp_create_segment(knl_session_t *session, uint32 *id)
{
    ...
    for (i = 0; i < ogx->seg_count; i++) {
        segment = ogx->segments[i];
        if (segment->level == OG_INVALID_ID32) {
    ...
    if (i >= session->temp_table_capacity * 2) {
        OG_THROW_ERROR(ERR_TOO_MANY_OBJECTS, session->temp_table_capacity * 2, "temp segments");
        return OG_ERROR;
    }
    ...
    segment->level = 0;
    segment->is_used = OG_TRUE;

    *id = i;
    return OG_SUCCESS;
}

void temp_drop_segment(mtrl_context_t *ogx, uint32 id)
{
    knl_panic_log(ogx->segments[id]->is_used, "current segment is not used, panic info: id %u", id);
    vm_free_list(ogx->session, ogx->pool, &ogx->segments[id]->vm_list);
    ogx->segments[id]->is_used = OG_FALSE;
    ogx->segments[id]->level = OG_INVALID_ID32;
}
...
status_t temp_heap_create_segment(knl_session_t *session, knl_temp_cache_t *temp_table_ptr)
{
    ...
    if (temp_create_segment(session, &table_segid) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (temp_heap_alloc_page(session, temp_table_ptr, &vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to alloc page in heap create segment.");
        return OG_ERROR;
    }
...
static bool32 temp_heap_check_page(knl_session_t *session, knl_cursor_t *cursor, temp_heap_page_t *page,
                                   page_type_t type)
{
    table_t *table = NULL;
...
static void temp_heap_insert_into_page(knl_cursor_t *cursor, temp_heap_page_t *page, row_head_t *row, undo_data_t *undo,
                                       rd_temp_heap_insert_t *rd, uint16 *slot)
{
    ...
    knl_panic_log(page->free_begin <= page->free_end,
                  "page's free size begin is more than end, panic info: "
                  "page %u-%u type %u table %s free_begin %u free_end %u",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type,
                  ((table_t *)cursor->table)->desc.name, page->free_begin, page->free_end);
}
                                  ...
                                  bool32 *is_found)
{
    ...
    *is_found = !row->is_deleted;
    ...
    knl_panic_log(ROW_ITL_ID(row) == OG_INVALID_ID8, "row itl id is valid, panic info: page %u-%u type %u table %s",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type,
                  ((table_t *)cursor->table)->desc.name);
    /* means row->is_link == 0 && row->is_migr == 1 or row->is_link == 1 && row->is_migr == 0 */
    knl_panic_log(row->is_link + row->is_migr < 2, "row is both link and migr, panic info: page %u-%u type %u table %s",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type,
                  ((table_t *)cursor->table)->desc.name);
    return OG_SUCCESS;
}
                                         ...
                                         bool32 *is_found)
{
    ...
    *is_found = OG_FALSE;
    ...
    for (;;) {
        cursor->link_rid.vmid = OG_INVALID_ID32;
        if (cursor->rowid.vm_slot == OG_INVALID_ID16) {
        ...
        if (cursor->rowid.vm_slot == page->dirs) {
            ...
            if (session->stat_sample) {
                cursor->rowid.vmid = OG_INVALID_ID32;
            } else {
        ...
        cursor->rowid_pos.file = cursor->rowid.file;
        cursor->rowid_pos.page = cursor->rowid.page;
        cursor->rowid_pos.vmid = cursor->rowid.vmid;
        cursor->rowid_pos.vm_slot = cursor->rowid.vm_slot;

        if (temp_heap_get_row(session, cursor, page, is_found) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t temp_heap_get_migr_row(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->link_rid.vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open heap migr vm page (%u).", cursor->link_rid.vmid);
        return OG_ERROR;
    }
    ...
    if (!temp_heap_check_page(session, cursor, page, PAGE_TYPE_TEMP_HEAP)) {
        ...
        OG_THROW_ERROR(ERR_OBJECT_ALREADY_DROPPED, "temp table");
        return OG_ERROR;
    }
    ...
    knl_panic_log(row->is_migr == 1, "row is not migr, panic info: page %u-%u type %u table %s",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type,
                  ((table_t *)cursor->table)->desc.name);
    knl_panic_log(cursor->link_rid.vmid != OG_INVALID_ID32,
    ...
    if (row->size != 0) {
        ret = memcpy_sp(cursor->row, TEMP_CT_MAX_ROW_SIZE, row, row->size);
        knl_securec_check(ret);
    }
...
static status_t temp_heap_read_by_rowid(knl_session_t *session, knl_cursor_t *cursor, bool32 *is_found)
{
    ...
    *is_found = OG_FALSE;
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->rowid.vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open heap rowid vm page (%u).", cursor->rowid.vmid);
        return OG_ERROR;
    }
    ...
    if (*is_found && cursor->link_rid.vmid != OG_INVALID_ID32) {
        if (temp_heap_get_migr_row(session, cursor) != OG_SUCCESS) {
            buf_leave_temp_page_nolock(session, OG_FALSE);
            return OG_ERROR;
        }
    }
...
static status_t temp_heap_fetch_by_page(knl_session_t *session, knl_cursor_t *cursor, bool32 *is_found)
{
    ...
    *is_found = OG_FALSE;
    ...
    if (temp_heap_scan_full_page(session, cursor, page, is_found) != OG_SUCCESS) {
        buf_leave_temp_page_nolock(session, OG_FALSE);
        return OG_ERROR;
    }
...
status_t temp_heap_lock_row(knl_session_t *session, knl_cursor_t *cursor, bool32 *is_locked)
{
    ...
    if (temp_heap_try_lock_row(session, cursor) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t temp_heap_fetch(knl_handle_t handle, knl_cursor_t *cursor)
{
    ...
    if (temp_table->table_type == DICT_TYPE_TEMP_TABLE_TRANS && temp_table->hold_rmid != OG_INVALID_ID32 &&
        session->rmid != temp_table->hold_rmid) {
        cursor->eof = OG_TRUE;
        return OG_SUCCESS;
    }

    for (;;) {
        if (cursor->rowid.vmid == OG_INVALID_ID32) {
            cursor->eof = OG_TRUE;
            return OG_SUCCESS;
        }
        if (cursor->eof) {
        ...
        if (temp_heap_fetch_by_page(session, cursor, &cursor->is_found) != OG_SUCCESS) {
            return OG_ERROR;
        }
        ...
        if (temp_heap_lock_row(session, cursor, &cursor->is_found) != OG_SUCCESS) {
            return OG_ERROR;
        }
        return OG_SUCCESS;
    }
...
{
    if (knl_cur->temp_cache == NULL || CURSOR_TEMP_CACHE(knl_cur)->table_segid == OG_INVALID_ID32) {
        knl_cur->eof = OG_TRUE;
        return OG_SUCCESS;
    }
    ...
    for (;;) {
        ...
        if (temp_heap_fetch_by_rowid((knl_session_t *)session, knl_cur) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
status_t temp_heap_fetch_by_rowid(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (temp_heap_read_by_rowid(session, cursor, &cursor->is_found) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
status_t temp_heap_alloc_page(knl_session_t *session, knl_temp_cache_t *temp_cache, uint32 *curr_vmid)
{
    ...
    if (temp_cache == NULL || temp_cache->table_segid == OG_INVALID_ID32) {
    ...
    if (vm_alloc(ogx->session, ogx->pool, &vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to extend segment when heap alloc page.");
        return OG_ERROR;
    }

    if (buf_enter_temp_page_nolock(session, vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open extend vm (%d) when heap alloc page.", vmid);
        return OG_ERROR;
    }
    ...
    *curr_vmid = vmid;
    ...
    knl_panic_log(segment->vm_list.last == vmid,
                  ...
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, segment->vm_list.last,
                  vmid);
    return OG_SUCCESS;
}

static status_t temp_enter_heap_insert(knl_session_t *session, knl_cursor_t *cursor, uint32 cost_size,
                                       uint32 *alloc_vmid)
{
    temp_heap_page_t *page = NULL;
    vm_page_t *vm_page = NULL;
    ...
    if (buf_enter_temp_page_nolock(session, vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open vm page (%u) when enter heap insert.", vmid);
        return OG_ERROR;
    }
    ...
    if (page->free_end < cost_size + page->free_begin + pct_free_size) {
        ...
        if (OG_SUCCESS != temp_heap_alloc_page(session, cursor->temp_cache, &vmid)) {
            OG_LOG_RUN_ERR("failed to find free temp buffer page for enter insert size : %u.", cost_size);
            return OG_ERROR;
        }

        if (buf_enter_temp_page_nolock(session, vmid) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("Fail to open new allocated page (%d) when enter heap insert.", vmid);
            return OG_ERROR;
        }

        vm_page = buf_curr_temp_page(session);
        page = (temp_heap_page_t *)vm_page->data;
        knl_panic_log(page->free_end >= cost_size + page->free_begin,
                      "page's free size is abnormal, panic info: "
...
static void temp_batch_insert_rows(knl_session_t *session, knl_cursor_t *cursor, undo_data_t *undo, uint16 *rows_size)
{
    ...
    for (uint32 i = 0; i < row_count; i++) {
        ...
        if (row->size > TEMP_CT_MAX_ROW_SIZE || page->free_end < page->free_begin + max_cost_size + pct_free_size) {
            break;
        }
    }
    ...
    *rows_size = (uint16)((char *)row - (char *)cursor->row);
    knl_panic_log(page->dirs - batch_undo->begin_slot == batch_undo->count,
                  "invalid temp heap batch insert dirs %u, begin slot %u, batch count %u", (uint32)page->dirs,
                  (uint32)batch_undo->begin_slot, (uint32)batch_undo->count);
...
static status_t temp_heap_try_batch_insert(knl_session_t *session, knl_cursor_t *cursor, uint16 *rows_size)
{
    ...
    if (row->size > TEMP_CT_MAX_ROW_SIZE) {
        OG_THROW_ERROR(ERR_RECORD_SIZE_OVERFLOW, "insert row", row->size, TEMP_CT_MAX_ROW_SIZE);
        return OG_ERROR;
    }
    ...
    if (DB_IS_PRIMARY(&session->kernel->db) && undo_prepare(session, undo.size, need_redo, OG_FALSE) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    *rows_size = 0;
    ...
    uint32 max_cost_size = row->size + sizeof(temp_row_dir_t);
    if (temp_enter_heap_insert(session, cursor, max_cost_size, &vmid) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static void temp_heap_batch_free_dirs(knl_session_t *session, temp_heap_page_t *page, uint16 begin, uint16 end)
{
    ...
    while (dir->next_slot != HEAP_NO_FREE_DIR) {
        ...
        knl_panic_log(dir->next_slot == HEAP_NO_FREE_DIR || dir->next_slot < page->dirs,
                      "temp page invalid next slot %u, page dirs %u.", (uint32)dir->next_slot, (uint32)page->dirs);
        dir = temp_heap_get_dir(page, dir->next_slot);
    }
}

static void temp_heap_undo_insert_row(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot,
                                      uint32 vm_slot)
{
    vm_page_t *vm_page = buf_curr_temp_page(session);
    temp_heap_page_t *page = (temp_heap_page_t *)vm_page->data;
    ...
    row_head_t *row = TEMP_HEAP_GET_ROW(page, dir);
    knl_panic_log(ROW_ITL_ID(row) == OG_INVALID_ID8, "row itl is valid, panic info: page %u-%u type %u",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type);
...
status_t temp_undo_enter_page(knl_session_t *session, uint32 vmid)
{
    ...
    knl_begin_session_wait(session, TEMP_ENTER_PAGE, OG_TRUE);
    for (;;) {
        if (buf_enter_temp_page_nolock(session, vmid) == OG_SUCCESS) {
            break;
        }
    ...
    knl_end_session_wait(session, TEMP_ENTER_PAGE);
    return status;
}
...
void temp_heap_undo_batch_insert(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    vm_page_t *vm_page = buf_curr_temp_page(session);
    temp_heap_page_t *page = (temp_heap_page_t *)vm_page->data;
    knl_panic_log(page->head.type == PAGE_TYPE_TEMP_HEAP, "invalid temp page type %u", (uint32)page->head.type);
...
status_t temp_heap_delete(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (DB_IS_PRIMARY(&session->kernel->db) && OG_SUCCESS != undo_prepare(session, undo.size, need_redo, OG_FALSE)) {
        return OG_ERROR;
    }
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->rowid.vmid) != OG_SUCCESS) {
        cm_pop(session->stack);
        OG_LOG_RUN_ERR("Fail to open vm page (%u) in heap delete.", cursor->rowid.vmid);
        return OG_ERROR;
    }
    ...
    dir->undo_page = undo_page_info->undo_rid.page_id;
    dir->undo_slot = undo_page_info->undo_rid.slot;
    dir->scn = cursor->ssn;
    dir->is_owscn = 0;
    ...
    if (cursor->link_rid.vmid != OG_INVALID_ID32) {
        if (buf_enter_temp_page_nolock(session, (uint32)cursor->link_rid.vmid) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("Fail to open link_rid vm page (%u) in heap delete.", cursor->link_rid.vmid);
            return OG_ERROR;
        }
        vm_page = buf_curr_temp_page(session);
        page = (temp_heap_page_t *)vm_page->data;
...
void temp_heap_undo_delete(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    if (link_rid.vmid != OG_INVALID_ID32) {
        if (temp_undo_enter_page(session, (uint32)link_rid.vmid) != OG_SUCCESS) {
...
static void temp_heap_reorganize_with_update(row_head_t *row, uint16 *offsets, uint16 *lens, knl_update_info_t *info,
                                             row_assist_t *new_ra)
{
    uint32 i;
    uint32 uid;
                                      ...
                                      undo_data_t *undo)
{
    ...
    knl_panic_log(ua->inc_size > 0,
                  "row increased size is abnormal, panic info: page %u-%u type %u table %s "
                  "inc_size %u",
                  cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type,
    ...
    knl_panic_log(ra.head->size == ua->new_size,
                  "ra's size and ua's new_size are not equal, panic info: "
                  "page %u-%u type %u table %s ra size %u new_size %u",
                  cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type,
                  ((table_t *)cursor->table)->desc.name, ra.head->size, ua->new_size);

    // update record across page
    if (temp_enter_heap_insert(session, cursor, cost_size, &vmid) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }
    ...
    knl_panic_log(migr_page->head.type == PAGE_TYPE_TEMP_HEAP,
                  "migr_page type is abnormal, panic info: migr_page %u-%u type %u", AS_PAGID(migr_page->head.id).file,
                  AS_PAGID(migr_page->head.id).page, migr_page->head.type);
    knl_panic_log(migr_page->free_end - migr_page->free_begin >= cost_size,
                  "migr_page's free size is abnormal, "
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->rowid.vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open owner vm page (%u) in heap update migr.", cursor->rowid.vmid);
        cm_pop(session->stack);
        return OG_ERROR;
    }
    ...
    if (old_link_rid.vmid != OG_INVALID_ID32) {
        if (buf_enter_temp_page_nolock(session, (uint32)old_link_rid.vmid) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("Fail to open old_link vm page (%d) in heap update migr.", old_link_rid.vmid);
            cm_pop(session->stack);
            return OG_ERROR;
        }
        ...
        knl_panic_log(old_migr_page->head.type == PAGE_TYPE_TEMP_HEAP,
                      ...
                      AS_PAGID(old_migr_page->head.id).file, AS_PAGID(old_migr_page->head.id).page,
                      old_migr_page->head.type);
...
static void temp_heap_generate_undo_for_update(knl_session_t *session, knl_cursor_t *cursor, temp_heap_page_t *page,
                                               undo_data_t *undo)
{
    temp_row_dir_t *dir = NULL;
    row_head_t *row = NULL;
    ...
    knl_panic_log(undo->size <= TEMP_CT_MAX_ROW_SIZE,
                  "undo size is more than the max limit, panic info: "
                  "page %u-%u type %u table %s undo size %u",
                  cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type,
                  ((table_t *)cursor->table)->desc.name, undo->size);
    dir = temp_heap_get_dir(page, (uint32)cursor->rowid.vm_slot);
    row = TEMP_HEAP_GET_ROW(page, dir);
                                          ...
                                          undo_data_t *undo)
{
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->rowid.vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open vm page (%u) in heap update link row.", cursor->rowid.vmid);
        return OG_ERROR;
    }
    page = TEMP_HEAP_CURR_PAGE(session);
    knl_panic_log(page->head.type == PAGE_TYPE_TEMP_HEAP,
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->link_rid.vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open link_rid vm page (%u) in heap update link row.", cursor->link_rid.vmid);
        return OG_ERROR;
    }
    migr_vm_page = buf_curr_temp_page(session);
    migr_page = (temp_heap_page_t *)migr_vm_page->data;
...
status_t temp_heap_update(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (ua.new_size > TEMP_CT_MAX_ROW_SIZE) {
        OG_THROW_ERROR(ERR_RECORD_SIZE_OVERFLOW, "update row", ua.new_size, TEMP_CT_MAX_ROW_SIZE);
        return OG_ERROR;
    }

    undo.data = (char *)cm_push(session->stack, TEMP_CT_MAX_ROW_SIZE);
    undo.rowid = cursor->rowid;

    /* record two id in temp undo */
    if (ua.undo_size >= cursor->row->size + 2 * sizeof(uint32)) {
        undo.type = UNDO_TEMP_HEAP_UPDATE_FULL;
        ret = memcpy_sp(undo.data, TEMP_CT_MAX_ROW_SIZE, cursor->row, cursor->row->size);
        knl_securec_check(ret);
        undo.size = cursor->row->size;
    } else {
        undo.type = UNDO_TEMP_HEAP_UPDATE;
        heap_get_update_undo_data(session, &ua, &undo, TEMP_CT_MAX_ROW_SIZE);
    }

    undo.size += sizeof(temp_heap_extra_undo_t);
    if (DB_IS_PRIMARY(&session->kernel->db) &&
        undo_prepare(session, undo.size, IS_TEMPTABLE_HAS_REDO(session), OG_FALSE) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }
    ...
    if (buf_enter_temp_page_nolock(session, (uint32)cursor->rowid.vmid) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("Fail to open vm page (%u) in heap update.", cursor->rowid.vmid);
        cm_pop(session->stack);
        return OG_ERROR;
    }
    vm_page = buf_curr_temp_page(session);
    ...
    dir = temp_heap_get_dir(page, (uint32)cursor->rowid.vm_slot);
    row = (row_head_t *)TEMP_HEAP_GET_ROW(page, dir);
    knl_panic_log(ROW_ITL_ID(row) == OG_INVALID_ID8, "row's itl is valid, panic info: page %u-%u type %u",
                  AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type);
                                    ...
                                    uint16 *lens)
{
    ...
    buf = (char *)cm_push(session->stack, TEMP_CT_MAX_ROW_SIZE);
    undo_info = (heap_undo_update_info_t *)cm_push(session->stack, TEMP_CT_MAX_ROW_SIZE);

#ifdef LOG_DIAG
    err = memset_sp(buf, TEMP_CT_MAX_ROW_SIZE, 0, TEMP_CT_MAX_ROW_SIZE);
    knl_securec_check(err);
#endif

    err = memcpy_sp(undo_info, TEMP_CT_MAX_ROW_SIZE, undo_row->data, undo_row->data_size);
    knl_securec_check(err);
    ...
    knl_panic_log(new_ra.head->size <= row_size,
                  "the new_ra's size is more than row_size, panic info: new_ra size %u row_size %u", new_ra.head->size,
                  row_size);
...
static void temp_heap_undo_link_row(knl_session_t *session, undo_row_t *ud_row, rowid_t *rid, uint16 ud_row_size)
{
    temp_heap_page_t *page = NULL;
    temp_row_dir_t *dir = NULL;
    row_head_t *row = NULL;
...
void temp_heap_undo_update(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    if (ud_row->type == UNDO_TEMP_HEAP_UPDATE_FULL) {
        ...
        ret = memcpy_sp(row, TEMP_CT_MAX_ROW_SIZE, ud_row->data, ud_row_size);
        knl_securec_check(ret);
        knl_panic_log(row->size <= row_size,
                      "row size is abnormal, panic info: page %u-%u type %u row's size %u "
                      "row_size %u",
                      AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, row->size, row_size);
        row->flags = flags;
        ROW_SET_ITL_ID(row, OG_INVALID_ID8);
                             ...
                             heap_update_assist_t *ua, temp_heap_page_t *page, uint16 slot)
{
    ...
    if (ua->inc_size > 0) {
        knl_panic_log(page->free_end - page->free_begin >= ua->new_size,
                      "page's free size is abnormal, panic info: "
                      "page %u-%u type %u free_end %u free_begin %u ua's new_size %u",
        ...
        knl_panic_log(dir->offset < page->free_begin,
                      "the dir's offset is more than the page's free_begin, "
                      "panic info: page %u-%u type %u dir offset %u free_begin %u",
                      AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, dir->offset,
                      page->free_begin);
        knl_panic_log(page->free_begin <= page->free_end,
                      "the page's free size begin is more than end, panic info: "
                      "page %u-%u type %u free_begin %u free_end %u",
                      AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, page->free_begin,
                      page->free_end);
        row = TEMP_HEAP_GET_ROW(page, dir);
        ...
        knl_panic_log(ra.head->size > old_size,
                      "current row_size is bigger than old_size when row increased size is "
                      "bigger than 0, panic info: page %u-%u type %u r's size %u old_size %u",
                      AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, ra.head->size,
                      old_size);
    } else {
        temp_heap_init_row(session, &ra, (char *)row, ua->new_cols, flags);
        temp_heap_reorganize_with_update(ori_row, offsets, lens, ua->info, &ra);
        knl_panic_log(ra.head->size <= old_size,
                      "current row_size is bigger than old_size when row increased size "
