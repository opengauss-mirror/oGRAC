 ...
 *
 * IDENTIFICATION
 * src/kernel/table/knl_part_interval.c
 *
 * -------------------------------------------------------------------------
 */
...
status_t part_check_interval_valid(part_key_t *interval_key)
{
    ...
    switch (bits) {
        ...
        default:
            OG_LOG_RUN_ERR("invalid part key bits type %u.", bits);
            status = OG_ERROR;
            break;
    }
...
static uint32 interval_decimal_do_calc_partno(part_table_t *part_table, dec8_t *trans_dec, dec8_t *partkey_dec,
    dec8_t *interval_dec)
{
    dec8_t sub_result;
    dec8_t div_result;
    int32 interval_count;
    (void)cm_dec_subtract(partkey_dec, trans_dec, &sub_result);
    (void)cm_dec_divide(&sub_result, interval_dec, &div_result);
    if (cm_dec_to_int32(&div_result, &interval_count, ROUND_FLOOR) != OG_SUCCESS) {
        cm_reset_error();
        OG_LOG_RUN_ERR("[PART] The number of interval partitions overflow.");
...
static uint32 intval_calc_partno_dec2(part_table_t *part_table, part_decode_key_t *transition_decoder,
    part_decode_key_t *decoder)
{
    dec8_t trans_dec;
    dec8_t partkey_dec;
    ...
    (void)cm_dec_2_to_8(&trans_dec, (const payload_t *)PART_GET_INTERVAL_KEY(transition_decoder),
                        (uint8)PART_GET_INTERVAL_KEY_LEN(transition_decoder));
    (void)cm_dec_2_to_8(&partkey_dec, (const payload_t *)PART_GET_INTERVAL_KEY(decoder),
                        (uint8)PART_GET_INTERVAL_KEY_LEN(decoder));
    (void)cm_dec_2_to_8(&interval_dec, (const payload_t *)PART_GET_INTERVAL_KEY(part_table->desc.interval_key),
                        (uint8)PART_GET_INTERVAL_KEY_LEN(part_table->desc.interval_key));

    return interval_decimal_do_calc_partno(part_table, &trans_dec, &partkey_dec, &interval_dec);
}
...
uint32 part_locate_interval_key(part_table_t *part_table, part_decode_key_t *transition_decoder,
    part_decode_key_t *decoder)
{
    uint32 part_lno = OG_INVALID_ID32;
    ...
    switch (part_table->keycols->datatype) {
        ...
        case OG_TYPE_NUMBER2:
            part_lno = intval_calc_partno_dec2(part_table, transition_decoder, decoder);
            break;
    ...
    return (part_lno >= OG_MAX_PART_COUNT) ? OG_INVALID_ID32 : part_lno;
}
                                          ...
                                          uint32 interval_count, part_interval_bound_t *bound)
{
    ...
    if (OG_MAX_INT64 / (*interval_boundval) < interval_count) {
        OG_THROW_ERROR(ERR_TYPE_OVERFLOW, "BIG INTEGER");
        return OG_ERROR;
    }
   
    temp = (int64)interval_count * (*interval_boundval);
    if (*transition_boundval > 0 && OG_MAX_INT64 - (*transition_boundval) < temp) {
        OG_THROW_ERROR(ERR_TYPE_OVERFLOW, "BIG INTEGER");
        return OG_ERROR;
    }

    if (*transition_boundval < 0 && OG_MIN_INT64 - (*transition_boundval) > temp) {
        OG_THROW_ERROR(ERR_TYPE_OVERFLOW, "BIG INTEGER");
        return OG_ERROR;
    }
...
static status_t part_genr_intval_date_bound(part_decode_key_t *trans_key, part_decode_key_t *intval_key,
    uint32 intval_count, part_interval_bound_t *intval_bound, part_key_t *intval_num)
{
    uint8 bits = part_get_key_bits(intval_num, 0);
    if (bits == PART_KEY_BITS_4) {
        return interval_calc_bound_months(trans_key, intval_key, intval_count, intval_bound);
    }
    interval_calc_bound_days(trans_key, intval_key, intval_count, intval_bound);
    return OG_SUCCESS;
}

static void part_dec_do_genr_bound(dec8_t *trans, dec8_t *intval, uint32 intval_cnt,
                                       part_interval_bound_t *bound)
{
    dec8_t intervals;
    (void)cm_dec8_mul_int64(intval, (int64)intval_cnt, &intervals);
    (void)cm_dec_add(trans, &intervals, &bound->dec_val);
}

static void part_genr_intval_dec2_bound(part_decode_key_t *trans_key, part_decode_key_t *intval_key,
                                              uint32 intval_cnt, part_interval_bound_t *bound)
{
    dec8_t transition_val;
    dec8_t interval_val;
    (void)cm_dec_2_to_8(&transition_val, (const payload_t *)PART_GET_INTERVAL_KEY(trans_key),
                        (uint8)PART_GET_INTERVAL_KEY_LEN(trans_key));
    (void)cm_dec_2_to_8(&interval_val, (const payload_t *)PART_GET_INTERVAL_KEY(intval_key),
                        (uint8)PART_GET_INTERVAL_KEY_LEN(intval_key));
    part_dec_do_genr_bound(&transition_val, &interval_val, intval_cnt, bound);
}
                                             ...
                                             uint32 interval_count, part_interval_bound_t *bound)
{
    ...
    part_dec_do_genr_bound(&transition_val, &interval_val, interval_count, bound);
}
                                               ...
                                               part_key_t *intval_part_key, uint32 part_no)
{
    ...
    switch (part_table->keycols->datatype) {
        ...
        case OG_TYPE_DECIMAL:
            ...
            status = part_put_dec4(intval_part_key, &intval_bound.dec_val);
            break;
        case OG_TYPE_NUMBER2:
            part_genr_intval_dec2_bound(&trans_key.decoder, &intval_key.decoder, intval_count, &intval_bound);
            status = part_put_dec2(intval_part_key, &intval_bound.dec_val);
            break;

        case OG_TYPE_DATE:
        case OG_TYPE_TIMESTAMP:
            if (part_genr_intval_date_bound(&trans_key.decoder, &intval_key.decoder, intval_count, &intval_bound,
                intval_num) != OG_SUCCESS) {
                return OG_ERROR;
            }
        ...
        default:
            OG_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "invalid partition key type %u", part_table->keycols->datatype);
            knl_panic(OG_FALSE);
    }

    if (status != OG_SUCCESS) {
        OG_THROW_ERROR_EX(ERR_INVALID_PART_KEY, "invalid partition key value");
    }
    ...
    bool32 is_left)
{
    ...
    if (table_part != NULL && table_part->is_ready) {
        ...
        key = (part_key_t *)cm_push(session->stack, OG_MAX_PART_COLUMN_SIZE);

        part_key_init(key, 1);
        if (part_calc_interval_part_border(part_table, &interval, key, prev_no) != OG_SUCCESS) {
    ...
    return (result < 0) ? prev_no : ((part_no >= part_table->desc.partcnt) ? OG_INVALID_ID32 : part_no);
}
...
{
    row_assist_t ra;
    char new_name[OG_NAME_BUFFER_SIZE] = { 0 };
    uint32 old_partid = *(uint32 *)CURSOR_COLUMN_DATA(cursor, SYS_TABLEPART_COL_PART_ID);
    errno_t ret = snprintf_s(new_name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1, "_SYS_%llX_P%u",
    ...
    uint32 new_partid = trans_id + (part_pos + 1) * OG_DFT_PARTID_STEP;
...
static status_t db_convert_interval_table_parts_to_range(knl_session_t *session, knl_cursor_t *cursor, table_t *table)
{
    ...
    for (;;) {
        ...
        if (part_convert_interval_range_table(session, cursor, table, trans_id, part_index) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    uint32 trans_id, uint32 part_pos)
{
    ...
    if (IS_COMPART_INDEX(index->part_index)) {
        if (db_upd_parent_idx_partid(session, &index->desc, old_partid, new_partid) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
                                                        ...
                                                        index_t *index)
{
    ...
    for (;;) {
        ...
        if (part_convert_interval_range_index(session, cursor, index, trans_id, part_index) != OG_SUCCESS) {
            return OG_ERROR;
        }
    ...
    uint32 trans_id, uint32 part_pos)
{
    ...
    if (IS_PARENT_LOBPART(&desc)) {
        if (db_update_parent_lobpartid(session, &lob->desc, old_partid, new_partid) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t db_convert_interval_lob_part_to_range(knl_session_t *session, knl_cursor_t *cursor, lob_t *lob)
{
    ...
    for (;;) {
        ...
        if (part_convert_interval_range_lob(session, cursor, lob, trans_id, part_index) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t db_convert_interval_part_to_range(knl_session_t *session, dc_entity_t *entity)
{
    ...
    if (db_convert_interval_table_parts_to_range(session, cursor, table) != OG_SUCCESS) {
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (IS_PART_INDEX(index)) {
            if (db_convert_interval_index_part_to_range(session, cursor, index) != OG_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                return OG_ERROR;
            }
        }
    ...
    for (uint32 i = 0; i < entity->column_count; i++) {
        ...
        if (IS_PART_TABLE(table)) {
            if (db_convert_interval_lob_part_to_range(session, cursor, lob) != OG_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                return OG_ERROR;
            }
        }
    ...
    binary_t *binterval, bool32 is_inertval)
{
    ...
    knl_set_scan_key(desc, l_key, OG_TYPE_INTEGER, &table->desc.uid, sizeof(uint32), IX_COL_SYS_PARTOBJECT001_USER_ID);
    knl_set_scan_key(desc, l_key, OG_TYPE_INTEGER, &table->desc.id, sizeof(uint32), IX_COL_SYS_PARTOBJECT001_TABLE_ID);
    knl_set_key_flag(l_key, SCAN_KEY_LEFT_INFINITE, IX_COL_SYS_PARTOBJECT001_INDEX_ID);
    knl_set_scan_key(desc, r_key, OG_TYPE_INTEGER, &table->desc.uid, sizeof(uint32), IX_COL_SYS_PARTOBJECT001_USER_ID);
    knl_set_scan_key(desc, r_key, OG_TYPE_INTEGER, &table->desc.id, sizeof(uint32), IX_COL_SYS_PARTOBJECT001_TABLE_ID);
    ...
    for (;;) {
        ...
        if (is_inertval) {
            ...
            row_init(&ra, cursor->update_info.data, HEAP_MAX_ROW_SIZE(session), UPDATE_COLUMN_COUNT_THREE);
...
static status_t part_check_transition_part_vaild(part_table_t *part_table, part_key_t *new_interval_key)
{
    knl_part_key_t part_key;
    ...
    if (part_table->desc.not_ready_partcnt > 0) {
        OG_THROW_ERROR(ERR_OPERATIONS_NOT_ALLOW, "convert range partition table to interval partition table \
with not ready partition on the table");
        return OG_ERROR;
    }

    if (OG_IS_DATETIME_TYPE(part_table->keycols->datatype)) {
    ...
    if (bits == PART_KEY_BITS_MAX) {
        OG_THROW_ERROR(ERR_OPERATIONS_NOT_ALLOW, "SET INTERVAL on this table");
        return OG_ERROR;
    }
...
status_t db_altable_set_interval_part(knl_session_t *session, knl_dictionary_t *dc, knl_alt_part_t *def)
{
    ...
    if (part_table->desc.parttype != PART_TYPE_RANGE) {
        OG_THROW_ERROR(ERR_OPERATIONS_NOT_ALLOW, "set interval for other partition table except range");
        return OG_ERROR;
    }
    ...
    if (interval_def->interval.str != NULL) {
        ...
        if (part_check_interval_valid(new_interval_key) != OG_SUCCESS) {
        ...
        if (part_check_transition_part_vaild(part_table, new_interval_key) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    if (PART_CONTAIN_INTERVAL(part_table)) {
        ...
        if (db_convert_interval_part_to_range(session, entity) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
                                           ...
                                           binary_t *bhiboundval, uint32 part_no)
{
    ...
    switch (part_table->keycols->datatype) {
        ...
        case OG_TYPE_DECIMAL:
            ...
            (void)cm_dec_to_text(&inter_bound.dec_val, OG_MAX_DEC_OUTPUT_ALL_PREC, hiboundval);
            return part_put_dec4(inter_part_key, &inter_bound.dec_val);
        case OG_TYPE_NUMBER2:
            part_genr_intval_dec2_bound(&trans_key.decoder, &inter_key.decoder, inter_count, &inter_bound);
            (void)cm_dec_to_text(&inter_bound.dec_val, OG_MAX_DEC_OUTPUT_ALL_PREC, hiboundval);
            return part_put_dec2(inter_part_key, &inter_bound.dec_val);
                                              ...
                                              knl_table_part_desc_t *part_desc, uint32 part_no)
{
    ...
    for (i = 0; i < table->index_set.total_count; i++) {
        ...
        if (part_update_interval_part_count(session, table, part_no, index->desc.id, OG_TRUE) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
                                             ...
                                             uint32 desc_hibdval_max_size, uint32 part_id, uint32 part_no)
{
    ...
    if (part_get_interval_part_spc(session, table, part_desc, part_no) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    part_table_t *part_table, uint32 partno)
{
    ...
    errno_t ret = snprintf_s(part_desc->name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1, "_SYS_P%u", pos);
    knl_securec_check_ss(ret);
}

static uint32 part_calc_interval_part_count(part_table_t *part_table, uint32 part_no)
{
    uint32 part_cnt = OG_INVALID_ID32;

    if (part_no > part_table->desc.partcnt - 1) {
        knl_panic(part_table->desc.partcnt > 0);
        part_cnt = part_no + 1;

        if (part_cnt > OG_MAX_PART_COUNT) {
            return OG_INVALID_ID32;
        }
    } else {
    ...
    part_table_t *part_table)
{
    ...
    errno_t ret = snprintf_s(subpart_desc->name, OG_NAME_BUFFER_SIZE, OG_NAME_BUFFER_SIZE - 1,
    ...
    table_part_t *compart, uint32 part_id)
{
    ...
    desc->is_csf = compart_desc->is_csf;
...
static status_t subpart_add_interval_idxpart(knl_session_t *session, table_t *table, knl_table_part_desc_t *table_desc)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (db_write_sys_indsubpart(session, cursor, &index_desc) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
    }
    ...
    knl_table_part_desc_t *table_desc)
{
    ...
    if (part_write_shadowindex_part(session, dc, &index_subpart, OG_TRUE) != OG_SUCCESS) {
        btree_drop_part_segment(session, &index_subpart);
        return OG_ERROR;
    }
...
static status_t subpart_create_interval_part(knl_session_t *session, knl_dictionary_t *dc, table_part_t *compart)
{
    ...
    if (subpart_generate_interval_name(session, &subpart->desc, part_table) != OG_SUCCESS) {
        return OG_ERROR;
    }
    subpart->desc.parent_partid = compart->desc.part_id;

    if (subpart_add_interval_tabpart(session, &subpart->desc, compart, subpart_id) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (subpart_add_interval_idxpart(session, table, &subpart->desc) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (subpart_add_interval_shwidxpart(session, dc, &subpart->desc) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (subpart_add_interval_lobpart(session, dc, &subpart->desc) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    part_key_t *part_key)
{
    ...
    table_t *table = DC_TABLE(dc);
    part_table_t *part_table = table->part_table;
    char buf[OG_MAX_HIBOUND_VALUE_LEN] = { '\0' };
    
    desc.hiboundval.str = buf;
    desc.bhiboundval.bytes = (uint8 *)cm_push(session->stack, OG_MAX_COLUMN_SIZE);
    knl_panic_log(desc.bhiboundval.bytes != NULL, "bhiboundval's bytes is NULL, panic info: table %s",
                  table->desc.name);
    errno_t ret = memset_sp(desc.bhiboundval.bytes, OG_MAX_COLUMN_SIZE, 0, OG_MAX_COLUMN_SIZE);
    knl_securec_check(ret);
    ...
    if (part_add_interval_table_part(session, dc, &desc, OG_MAX_HIBOUND_VALUE_LEN, part_id, part_no) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }

    if (part_add_interval_index_parts(session, table, &desc, part_no) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }

    if (part_add_interval_shadow_index_part(session, dc, &desc, part_no) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }

    if (part_add_interval_lob_parts(session, dc, &desc, part_no) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }
    cm_pop(session->stack);
    if (part_update_interval_part_count(session, table, part_no, OG_INVALID_ID32, OG_TRUE) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (IS_COMPART_TABLE(part_table)) {
        ...
        if (subpart_create_interval_part(session, dc, part) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    knl_table_part_desc_t *desc, uint32 part_no)
{
    ...
    if (part_get_interval_boundval(part_table, &desc->hiboundval, OG_MAX_HIBOUND_VALUE_LEN, &desc->bhiboundval,
        part_no) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    part_key_t *key, uint32 part_no)
{
    ...
    if (part->desc.groups == NULL) {
        if (dc_alloc_mem(dc_ctx, entity->memory, mem_size, (void **)&part->desc.groups) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    if (decoder->offsets == NULL) {
        if (dc_alloc_mem(dc_ctx, entity->memory, mem_size, (void **)&decoder->offsets) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }

    if (decoder->lens == NULL) {
        if (dc_alloc_mem(dc_ctx, entity->memory, mem_size, (void **)&decoder->lens) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t db_reserve_interval_boundval_memory(knl_session_t *session, knl_dictionary_t *dc, uint32 part_no)
{
    ...
    if (db_cal_interval_bound_size(session, part_table, &desc, part_no) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }
    ...
    if (part->desc.hiboundval.str == NULL) {
        if (dc_alloc_mem(ogx, entity->memory, mem_size, (void **)&part->desc.hiboundval.str) != OG_SUCCESS) {
            cm_pop(session->stack);
            return OG_ERROR;
        }
    }
    ...
    if (part->desc.bhiboundval.bytes == NULL) {
        if (dc_alloc_mem(ogx, entity->memory, mem_size, (void **)&part->desc.bhiboundval.bytes) != OG_SUCCESS) {
            cm_pop(session->stack);
            return OG_ERROR;
        }
    }
    ...
    if (db_reserve_interval_decoder_memory(session, dc, key, part_no) != OG_SUCCESS) {
        cm_pop(session->stack);
        return OG_ERROR;
    }
...
static status_t dc_reserve_subpart_decoder_memory(knl_session_t *session, dc_entity_t *entity, table_part_t *subpart)
{
    ...
    if (subpart->desc.groups == NULL) {
        if (dc_alloc_mem(dc_ctx, entity->memory, mem_size, (void **)&subpart->desc.groups) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static void subpart_generate_default_bound(part_table_t *part_table, part_key_t *bhibound, char *hibound)
{
    ...
    if (part_table->desc.subparttype == PART_TYPE_RANGE) {
        ...
        for (uint32 i = 0; i < subpart_keys; i++) {
            ...
            knl_securec_check(ret);
            if (i != subpart_keys - 1) {
                ret = strcat_sp(hibound, hiboundval_len, ", ");
                knl_securec_check(ret);
            } else {
                ret = strcat_sp(hibound, hiboundval_len, "\0");
                knl_securec_check(ret);
            }
        ...
        ret = strcpy_sp(hibound, OG_MAX_PARTKEY_COLUMNS * OG_MAX_HIBOUND_VALUE_LEN, PART_VALUE_DEFAULT);
        knl_securec_check(ret);
        ret = strcat_sp(hibound, OG_MAX_PARTKEY_COLUMNS * OG_MAX_HIBOUND_VALUE_LEN, "\0");
        knl_securec_check(ret);
...
static status_t dc_reserve_subpart_boundval_memory(knl_session_t *session, dc_entity_t *entity, uint32 part_no)
{
    ...
    char hiboundval_buf[OG_MAX_PARTKEY_COLUMNS * OG_MAX_HIBOUND_VALUE_LEN] = { 0 };
    ...
    if (subpart->desc.hiboundval.str == NULL) {
        if (dc_alloc_mem(ogx, entity->memory, text.len, (void **)&subpart->desc.hiboundval.str) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
    }
    ...
    if (subpart->desc.bhiboundval.bytes == NULL) {
        if (dc_alloc_mem(ogx, entity->memory, part_key->size,
            (void **)&subpart->desc.bhiboundval.bytes) != OG_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return OG_ERROR;
        }
    }
    ...
    if (dc_reserve_subpart_decoder_memory(session, entity, subpart) != OG_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return OG_ERROR;
    }
...
static status_t db_reserve_subtabpart_dc_memory(knl_session_t *session, dc_entity_t *entity, uint32 part_no)
{
    ...
    if (dc_alloc_table_subparts(session, entity, table_part) != OG_SUCCESS) {
        return OG_ERROR;
    }

    table_part->subparts[0] = table->part_table->desc.subpart_cnt;
    if (dc_alloc_table_subpart(session, entity, table->part_table->desc.subpart_cnt) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (dc_reserve_subpart_boundval_memory(session, entity, part_no) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t db_reserve_subidxpart_dc_memory(knl_session_t *session, dc_entity_t *entity, uint32 part_no)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (dc_alloc_index_subparts(session, entity, compart_index) != OG_SUCCESS) {
            return OG_ERROR;
        }

        compart_index->subparts[0] = index->part_index->desc.subpart_cnt;
        if (dc_alloc_index_subpart(session, entity, index, index->part_index->desc.subpart_cnt) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    ...
    if (table->shadow_index != NULL && table->shadow_index->is_valid && IS_PART_INDEX(&table->shadow_index->index)) {
        ...
        if (dc_alloc_index_subparts(session, entity, shwidx_part) != OG_SUCCESS) {
            return OG_ERROR;
        }

        shwidx_part->subparts[0] = shwidx->part_index->desc.subpart_cnt;
        if (dc_alloc_index_subpart(session, entity, shwidx, shwidx->part_index->desc.subpart_cnt) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t db_reserve_sublobpart_dc_memory(knl_session_t *session, dc_entity_t *entity, uint32 part_no)
{
    ...
    for (uint32 i = 0; i < table->desc.column_count; i++) {
        ...
        if (dc_alloc_lob_subparts(session, entity, compart_lob) != OG_SUCCESS) {
            return OG_ERROR;
        }

        compart_lob->subparts[0] = table->part_table->desc.subpart_cnt;
        if (dc_alloc_lob_subpart(session, entity, lob, table->part_table->desc.subpart_cnt) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
static status_t db_reserve_subpart_dc_memory(knl_session_t *session, knl_dictionary_t *dc, uint32 part_no)
{
    ...
    if (db_reserve_subtabpart_dc_memory(session, entity, part_no) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (db_reserve_subidxpart_dc_memory(session, entity, part_no) != OG_SUCCESS) {
        return OG_ERROR;
    }

    if (db_reserve_sublobpart_dc_memory(session, entity, part_no) != OG_SUCCESS) {
        return OG_ERROR;
    }
...
static status_t db_reserve_idxpart_dc_memory(knl_session_t *session, dc_entity_t *entity, uint32 part_no)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (dc_alloc_index_part(session, entity, part_index, part_no) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t db_reserve_lobpart_dc_memory(knl_session_t *session, dc_entity_t *entity, uint32 part_no)
{
    ...
    for (uint32 i = 0; i < table->desc.column_count; i++) {
        ...
        if (dc_alloc_lob_part(session, entity, part_lob, part_no) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
...
status_t db_reserve_interval_dc_memory(knl_session_t *session, knl_dictionary_t *dc, uint32 part_no)
{
    dc_entity_t *entity = DC_ENTITY(dc);
    table_t *table = DC_TABLE(dc);
    part_table_t *part_table = table->part_table;
    ...
    if (db_reserve_idxpart_dc_memory(session, entity, part_no) != OG_SUCCESS) {
        cm_unlatch(&entity->cbo_latch, NULL);
        return OG_ERROR;
    }

    /* for lob part memory */
    if (db_reserve_lobpart_dc_memory(session, entity, part_no) != OG_SUCCESS) {
        cm_unlatch(&entity->cbo_latch, NULL);
        return OG_ERROR;
    }

    /* for table part memory */
    if (dc_alloc_table_part(session, entity, part_table, part_no) != OG_SUCCESS) {
        cm_unlatch(&entity->cbo_latch, NULL);
        return OG_ERROR;
    }
    ...
    if (db_reserve_interval_boundval_memory(session, dc, part_no) != OG_SUCCESS) {
        cm_unlatch(&entity->cbo_latch, NULL);
        return OG_ERROR;
    }

    if (IS_COMPART_TABLE(part_table)) {
        if (db_reserve_subpart_dc_memory(session, dc, part_no) != OG_SUCCESS) {
            cm_unlatch(&entity->cbo_latch, NULL);
            return OG_ERROR;
        }
    }
...
void db_create_interval_update_desc(table_t *table, uint32 part_cnt)
{
    index_t *index = NULL;
    part_table_t *part_table = table->part_table;
...
status_t db_create_interval_part(knl_session_t *session, knl_dictionary_t *dc, uint32 part_no,
                                 part_key_t *part_key)
{
    if (lock_table_shared(session, dc->handle, LOCK_INF_WAIT) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    dls_latch_x(session, &part_table->interval_latch, session->id, &session->stat_interval);

    /* check physical part is created or not */
    if (is_interval_part_created(session, dc, part_no)) {
        dls_unlatch(session, &part_table->interval_latch, &session->stat_interval);
        return OG_SUCCESS;
    }

    if (db_reserve_interval_dc_memory(session, dc, part_no) != OG_SUCCESS) {
        dls_unlatch(session, &part_table->interval_latch, &session->stat_interval);
        return OG_ERROR;
    }

    uint32 part_cnt = part_calc_interval_part_count(part_table, part_no);
    if (part_cnt == OG_INVALID_ID32) {
    ...
    if (part_create_interval_part(session, dc, part_no, part_key) != OG_SUCCESS) {
        knl_end_auton_rm(session, OG_ERROR);
        dls_unlatch(session, &part_table->interval_latch, &session->stat_interval);
        return OG_ERROR;
    }

    if (dc_load_interval_part(session, dc, part_no) != OG_SUCCESS) {
        knl_end_auton_rm(session, OG_ERROR);
        dls_unlatch(session, &part_table->interval_latch, &session->stat_interval);
        return OG_ERROR;
    }

    rd_create_interval_t redo;
    redo.tab_op.op_type = RD_CREATE_INTERVAL;
    redo.tab_op.uid = part_table->desc.uid;
    redo.tab_op.oid = part_table->desc.table_id;
    redo.part_no = part_no;
    redo.part_cnt = part_cnt;
    log_put(session, RD_LOGIC_OPERATION, &redo, sizeof(rd_create_interval_t), LOG_ENTRY_FLAG_NONE);
