...
static status_t spc_open_datafile_common(knl_session_t *session, datafile_t *df, int32 *handle, uint8 is_retry)
{
    uint32 io_flag;
...
{
    return spc_open_datafile_common(session, df, handle, OG_TRUE);
}

status_t spc_open_datafile_no_retry(knl_session_t *session, datafile_t *df, int32 *handle)
{
    return spc_open_datafile_common(session, df, handle, OG_FALSE);
}
...
static status_t df_wait_extend_completed(datafile_t *df)
{
    for (uint32 i = 0; i < DF_PARAL_BUILD_THREAD; i++) {
        df_build_ctx_t *ogx = &df->build_ctx[i];

        while (ogx->status == DF_IS_BUILDING) {
            cm_spin_sleep();
        }

        if (ogx->status == DF_BUILD_FAILED) {
            return OG_ERROR;
        }
    }
    return OG_SUCCESS;
...
static status_t spc_extend_datafile_paral(knl_session_t *session, datafile_t *df, int32 *handle,
    int64 org_size, int64 extend_size_input)
{
    ...
    if (spc_open_datafile(session, df, handle) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[SPACE] failed to open file %s when extending datafile", df->ctrl->name);
        return OG_ERROR;
    }

    if (cm_truncate_file(*handle, org_size + extend_size) != OG_SUCCESS) {
        cm_close_device(df->ctrl->type, handle);
        return OG_ERROR;
    }

    errno_t ret = memset_sp(buf, (size_t)OG_XPURPOSE_BUFFER_SIZE, 0, (size_t)OG_XPURPOSE_BUFFER_SIZE);
    ...
    for (int64 i = 0; i < DF_PARAL_BUILD_THREAD; i++) {
        ...
        if (cm_create_thread(df_build_proc, 0, build_ctx, &build_ctx->thread) != OG_SUCCESS) {
            spc_extend_datafile_clean(df, i);
            cm_close_device(df->ctrl->type, handle);
            OG_LOG_RUN_ERR("[SPACE] failed to start thread when create datafile %s", df->ctrl->name);
            return OG_ERROR;
        }
    }

    if (df_wait_extend_completed(df) != OG_SUCCESS) {
        spc_extend_datafile_clean(df, DF_PARAL_BUILD_THREAD);
        cm_reset_error();
        OG_THROW_ERROR(ERR_CREATE_FILE, df->ctrl->name, errno);
        return OG_ERROR;
    }
...
static status_t spc_extend_datafile_serial(knl_session_t *session, datafile_t *df, int32 *handle, int64 size)
{
    if (spc_open_datafile(session, df, handle) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[SPACE] failed to open file %s when extending datafile", df->ctrl->name);
        return OG_ERROR;
    ...
    return cm_extend_device(df->ctrl->type, *handle, session->kernel->attr.xpurpose_buf, OG_XPURPOSE_BUFFER_SIZE,
        size, session->kernel->attr.build_datafile_prealloc);
}

static status_t spc_build_datafile_serial(knl_session_t *session, datafile_t *df, int32 *handle)
{
    char *buf = session->kernel->attr.xpurpose_buf;

    df->ctrl->type = cm_device_type(df->ctrl->name);
    return cm_build_device(df->ctrl->name, df->ctrl->type, buf, OG_XPURPOSE_BUFFER_SIZE, df->ctrl->size,
...
status_t spc_extend_datafile(knl_session_t *session, datafile_t *df, int32 *handle, int64 size, bool32 need_redo)
{
    ...
    if (size >= DF_BUILD_PARAL_THRES && session->kernel->attr.build_datafile_parallel) {
        if (spc_extend_datafile_paral(session, df, handle, df->ctrl->size, size) != OG_SUCCESS) {
            (void)spc_truncate_datafile(session, df, handle, df->ctrl->size, need_redo);
            return OG_ERROR;
        }
    } else {
        if (spc_extend_datafile_serial(session, df, handle, size) != OG_SUCCESS) {
            (void)spc_truncate_datafile(session, df, handle, df->ctrl->size, need_redo);
            return OG_ERROR;
        }
    }
    ...
    rd_extend_datafile_ograc_t oGRAC_redo;
    if (need_redo) {
        rd_extend_datafile_t *redo = &oGRAC_redo.datafile;
        oGRAC_redo.op_type = RD_SPC_EXTEND_DATAFILE_OGRAC;
        redo->id = df->ctrl->id;
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_BEFORE_SYNC_ABORT, NULL, 0);
...
status_t spc_extend_datafile_ddl(knl_session_t *session, datafile_t *df, int32 *handle, int64 size, bool32 need_redo)
{
    ...
    if (need_redo) {
        ...
        redo->size = df->ctrl->size;
        log_put(session, RD_SPC_EXTEND_DATAFILE, redo, sizeof(rd_extend_datafile_t), LOG_ENTRY_FLAG_NONE);
        if (DB_IS_CLUSTER(session) && !OGRAC_REPLAY_NODE(session)) {
            log_put(session, RD_LOGIC_OPERATION, &oGRAC_redo, sizeof(rd_extend_datafile_ograc_t),
                LOG_ENTRY_FLAG_NONE);
        }
    }
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_BEFORE_LOG_COMMIT_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;
    
    log_commit(session);

    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_BEFORE_EXTEND_DEVICE_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;

    status_t sp_ret = OG_SUCCESS;
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_FAIL, &sp_ret, OG_ERROR);
#ifndef WIN32
    if (size >= DF_BUILD_PARAL_THRES && session->kernel->attr.build_datafile_parallel) {
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_BEFORE_SAVE_DF_CTRL_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;

    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_SAVE_DF_CTRL_FAIL, &sp_ret, OG_ERROR);
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_EXTEND_DATAFILE_AFTER_SYNC_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;
...
{
    rd_truncate_datafile_ograc_t oGRAC_redo;
    rd_truncate_datafile_t *redo = &oGRAC_redo.datafile;

    if (*handle == -1) {
        if (spc_open_datafile(session, df, handle) != OG_SUCCESS) {
            OG_LOG_RUN_ERR("[SPACE] failed to open file %s when truncate datafile", df->ctrl->name);
            return OG_ERROR;
        }
    }
    ...
    oGRAC_redo.op_type = RD_SPC_TRUNCATE_DATAFILE_OGRAC;
    redo->id = df->ctrl->id;
    redo->size = df->ctrl->size;
    ...
    if (cm_truncate_device(df->ctrl->type, *handle, keep_size) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[SPACE] failed to truncate datafile %s", df->ctrl->name);
        return OG_ERROR;
    }
    ...
    if (DB_IS_CLUSTER(session) && !OGRAC_REPLAY_NODE(session)) {
        log_put(session, RD_LOGIC_OPERATION, &oGRAC_redo, sizeof(rd_truncate_datafile_ograc_t),
            LOG_ENTRY_FLAG_NONE);
    }

    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_BEFORE_SYNC_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;
    return OG_SUCCESS;
}
                                   ...
                                   bool32 need_redo)
{
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_BEFORE_LOG_COMMIT_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;

    log_commit(session);

    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_BEFORE_TRUNCATE_DEVICE_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;

    status_t sp_ret = OG_SUCCESS;
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_FAIL, &sp_ret, OG_ERROR);
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_BEFORE_SAVE_DF_CTRL_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;

    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_SAVE_DF_CTRL_FAIL, &sp_ret, OG_ERROR);
    sp_ret = db_save_datafile_ctrl(session, df->ctrl->id);
    SYNC_POINT_GLOBAL_END;
    if (sp_ret != OG_SUCCESS) {
        CM_ABORT(0, "[SPACE] failed to save whole control file when truncate datafile");
    ...
    SYNC_POINT_GLOBAL_START(OGRAC_DDL_TRUNCATE_DATAFILE_AFTER_SYNC_ABORT, NULL, 0);
    SYNC_POINT_GLOBAL_END;
    
    return OG_SUCCESS;
}

static status_t spc_build_datafile_paral(knl_session_t *session, datafile_t *df, int32 *handle)
{
    *handle = -1;
    df->ctrl->type = cm_device_type(df->ctrl->name);
    if (cm_create_device(df->ctrl->name, df->ctrl->type, knl_io_flag(session), handle) != OG_SUCCESS) {
        cm_close_device(df->ctrl->type, handle);
        return OG_ERROR;
    }

    if (spc_extend_datafile_paral(session, df, handle, 0, df->ctrl->size) != OG_SUCCESS) {
        cm_close_device(df->ctrl->type, handle);
        return OG_ERROR;
    }
...
status_t spc_build_datafile(knl_session_t *session, datafile_t *df, int32 *handle)
{
    ...
    if (df->ctrl->size >= DF_BUILD_PARAL_THRES && session->kernel->attr.build_datafile_parallel) {
        if (spc_build_datafile_paral(session, df, handle) != OG_SUCCESS) {
            return OG_ERROR;
        }
    } else {
        if (spc_build_datafile_serial(session, df, handle) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
#else
...
void spc_invalidate_datafile(knl_session_t *session, datafile_t *df, bool32 ckpt_disable)
{
    ...
    if (cm_exist_device(df->ctrl->type, df->ctrl->name)) {
        ...
        if (OG_DROP_DATAFILE_FORMAT_NAME_LEN(name_len) < OG_FILE_NAME_BUFFER_SIZE - 1 && !OGRAC_REPLAY_NODE(session))
            {
            ret = sprintf_s(delete_name, OG_FILE_NAME_BUFFER_SIZE, "%s.delete", df->ctrl->name);
            knl_securec_check_ss(ret);
            if (cm_rename_device(df->ctrl->type, df->ctrl->name, delete_name) != OG_SUCCESS) {
                OG_LOG_RUN_WAR("[SPACE]: failed to rename space datafile %s", df->ctrl->name);
                return;
            }
            ret = memcpy_sp(df->ctrl->name, OG_FILE_NAME_BUFFER_SIZE, delete_name, OG_FILE_NAME_BUFFER_SIZE);
            knl_securec_check(ret);
        }
    }
...
{
    char *buf = (char *)cm_push(session->stack, (uint32)(df->ctrl->block_size + OG_MAX_ALIGN_SIZE_4K));
    ...
    if (db_fdatasync_file(session, session->datafiles[df->ctrl->id]) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("[SPACE] failed to fdatasync datafile %s", df->ctrl->name);
        cm_pop(session->stack);
        return OG_ERROR;
    }
...
void df_init_swap_map_head(knl_session_t *session, datafile_t *df)
{
    ...
    OG_LOG_DEBUG_INF("[SPACE] init swap space %u datafile %u bitmap group.", space->ctrl->id, df->ctrl->id);
}
 ...
 */
static status_t df_search_bitmap(knl_session_t *session, df_map_page_t *map_page, uint16 bits, uint16 *extent_bit)
{
 ...
 */
static status_t df_alloc_extent_from_map(knl_session_t *session, datafile_t *df, page_id_t map_page_id, uint32 extent_size,
                                  page_id_t *extent, bool32 *need_extend)
{
    ...
    if (df_search_bitmap(session, map_page, need_bits, &extent_bit) != OG_SUCCESS) {
        buf_leave_page(session, OG_FALSE);
        return OG_ERROR;
    }
 ...
 * search extent on bitmap bit by bit.
 */
static status_t df_alloc_extent_swap_map(knl_session_t *session, datafile_t *df, page_id_t map_page_id,
    page_id_t *extent, bool32 *need_extend)
{
    df_map_page_t *map_page = NULL;
...
status_t df_alloc_extent(knl_session_t *session, datafile_t *df, uint32 extent_size, page_id_t *extent)
{
    ...
    for (i = 0; i < df->map_head->group_count; i++) {
        ...
        for (j = 0; j < map_group->page_count; j++) {
            if (df_alloc_extent_from_map(session, df, curr_map, extent_size, extent, &need_extend) == OG_SUCCESS) {
                return OG_SUCCESS;
            }
...
status_t df_alloc_swap_map_extent(knl_session_t *session, datafile_t *df, page_id_t *extent)
{
    ...
    for (i = 0; i < df->map_head->group_count; i++) {
        ...
        for (j = 0; j < map_group->page_count; j++) {
            if (df_alloc_extent_swap_map(session, df, curr_map, extent, &need_extend) == OG_SUCCESS) {
                return OG_SUCCESS;
            }
                            ...
                            bool32 *is_last)
{
    ...
    map_page = (df_map_page_t *)CURR_PAGE(session);
    if (df_get_next_free_extent(session, df, map_page, bit_id, &extent_bit, &bit_len, is_last) != OG_SUCCESS) {
        *extent = 0;
        *page_count = 0;
        buf_leave_page(session, OG_FALSE);
        return OG_ERROR;
...
void spc_block_datafile(datafile_t *df, uint32 section_id, uint64 start, uint64 end)
{
    ...
    cm_latch_x(&df->block_latch, OG_INVALID_ID32, NULL);
        ...
        if (end > df->block_start[sec_id] && start < df->block_end[sec_id]) {
            if (DB_IS_CLUSTER(session)) {
                ...
                for (uint32 i = 0; i < session->kernel->db.ctrl.core.node_count; i++) {
                    ...
                    if (dtc_bak_running(session, i, &running) != OG_SUCCESS || running == OG_FALSE) {
                        return OG_FALSE;
                    }
                }
            }
...
status_t spc_get_datafile_name_bynumber(knl_session_t *session, int32 filenumber, char **filename)
{
    ...
    if (filenumber < 0 || filenumber >= (int32)OG_MAX_DATA_FILES) {
        OG_THROW_ERROR(ERR_INVALID_DATAFILE_NUMBER, filenumber, 0, (OG_MAX_DATA_FILES - 1));
        return OG_ERROR;
    }

    df = DATAFILE_GET(session, filenumber);
    if (DF_FILENO_IS_INVAILD(df) || !df->ctrl->used) {
        OG_THROW_ERROR(ERR_DATAFILE_NUMBER_NOT_EXIST, filenumber);
        return OG_ERROR;
    }
    ...
    knl_autoextend_def_t *def)
{
    ...
    if (next_extend_size > maxsize) {
        OG_THROW_ERROR(ERR_DATAFILE_SIZE_NOT_ALLOWED, "NEXT SIZE", df->ctrl->name);
        return OG_ERROR;
    }
...
static status_t spc_alter_precheck_datafile(knl_session_t *session, knl_alterdb_datafile_t *def)
{
    ...
    for (i = 0; i < def->datafiles.count; i++) {
        ...
        if (spc_get_id_by_name(session, name, &id) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (id == OG_INVALID_ID32) {
            OG_THROW_ERROR(ERR_FILE_NOT_EXIST, "data", name->str);
            return OG_ERROR;
        }
        ...
        if (def->autoextend.nextsize > max_file_size) {
            OG_THROW_ERROR(ERR_DATAFILE_SIZE_NOT_ALLOWED, "NEXT SIZE", T2S(name));
            return OG_ERROR;
        }

        if (def->autoextend.maxsize > max_file_size) {
            OG_THROW_ERROR(ERR_DATAFILE_SIZE_NOT_ALLOWED, "MAXSIZE", T2S(name));
            return OG_ERROR;
        }
...
status_t spc_alter_datafile_autoextend(knl_session_t *session, knl_alterdb_datafile_t *def)
{
    ...
    uint32 id = OG_INVALID_ID32;
    datafile_t *df = NULL;
    rd_set_df_autoextend_ograc_t oGRAC_redo;
    rd_set_df_autoextend_t *redo = &oGRAC_redo.rd;
    space_t *space = NULL;
    ...
    if (spc_alter_precheck_datafile(session, def) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    for (uint32 i = 0; i < def->datafiles.count; i++) {
        ...
        oGRAC_redo.op_type = RD_SPC_CHANGE_AUTOEXTEND_OGRAC;
        redo->id = df->ctrl->id;
...
status_t static spc_resize_datafile(knl_session_t *session, space_t *space, datafile_t *df, uint64 new_file_size)
{
    ...
    if (new_file_size < min_keep_size) {
        OG_THROW_ERROR(ERR_DATAFILE_RESIZE_TOO_SMALL);
        dls_spin_unlock(session, &space->lock);
        return OG_ERROR;
    }

    if (new_file_size > df->ctrl->auto_extend_maxsize) {
        OG_THROW_ERROR(ERR_DATAFILE_RESIZE_EXCEED, new_file_size, df->ctrl->auto_extend_maxsize);
        dls_spin_unlock(session, &space->lock);
        return OG_ERROR;
    }
    ...
    if (new_file_size > df->ctrl->size) {
        status = spc_extend_datafile_ddl(session, df, handle, new_file_size - df->ctrl->size, OG_TRUE);
    } else {
        status = spc_truncate_datafile_ddl(session, df, handle, new_file_size, OG_TRUE);
    }
    ckpt_enable(session);

    if (status != OG_SUCCESS) {
        dls_spin_unlock(session, &space->lock);
        OG_LOG_RUN_ERR("[SPACE] failed to resize datafile %s, old size %lld, new size %lld", df->ctrl->name,
...
status_t spc_alter_datafile_resize(knl_session_t *session, knl_alterdb_datafile_t *def)
{
    ...
    for (uint32 i = 0; i < def->datafiles.count; i++) {
        ...
        if (!SPACE_IS_ONLINE(space)) {
            char err_msg[OG_MESSAGE_BUFFER_SIZE] = { 0 };
            errno_t ret = sprintf_s(err_msg, OG_MESSAGE_BUFFER_SIZE, "resize datafile %s failed", df->ctrl->name);
            knl_securec_check_ss(ret);
            OG_THROW_ERROR(ERR_SPACE_OFFLINE, space->ctrl->name, err_msg);
            return OG_ERROR;
        }
        
        if (spc_resize_datafile(session, space, df, def->size) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    return OG_SUCCESS;
...
datafile_t *db_get_next_datafile(knl_session_t *session, uint32 *file_id, uint64 *data_size, uint32 *hwm_start)
{
    ...
    for (; *file_id < OG_MAX_DATA_FILES; (*file_id)++) {
...
void df_build_proc(thread_t *thread)
{
    ...
    df_build_ctx_t *ogx = (df_build_ctx_t *)thread->argument;
    knl_session_t *session = ogx->session;
    datafile_t *df = ogx->df;

    int64 offset = ogx->offset;
    int64 remain_size = ogx->size;
    int32 *handle = DATAFILE_FD(session, df->ctrl->id);

    while (!thread->closed && remain_size > 0) {
        uint32 curr_size = (remain_size > OG_XPURPOSE_BUFFER_SIZE) ? (int32)OG_XPURPOSE_BUFFER_SIZE
                                                                   : (int32)remain_size;
        if (cm_write_device(df->ctrl->type, *handle, offset, ogx->buf, curr_size) != OG_SUCCESS) {
            OG_LOG_RUN_INF("[SPACE] failed to create datafile %s, error code %d", df->ctrl->name, errno);
            ogx->status = DF_BUILD_FAILED;
            return;
        }
...
status_t df_verify_page_by_hwm(knl_session_t *session, rowid_t rowid)
{
    ...
    if (rowid.page >= file_hwm) {
        OG_LOG_DEBUG_WAR("page is invaild, page id is not less than datafile hwm");
        return OG_ERROR;
    }
    return OG_SUCCESS;
}
...
{
    if (page_id.file >= OG_MAX_DATA_FILES) {
        OG_LOG_DEBUG_WAR("page is invaild, file id(%u) is larger than max datafile id", page_id.file);
        return OG_ERROR;
    }

    datafile_t *df = &session->kernel->db.datafiles[page_id.file];
    if (!df->ctrl->used || !DATAFILE_IS_ONLINE(df)) {
        OG_LOG_DEBUG_WAR("page is invaild, file(%u) is offline", page_id.file);
        return OG_ERROR;
    }
    ...
    if (page_id.page >= file_hwm) {
        OG_LOG_DEBUG_WAR("page is invaild, page id(%u) is not less than datafile hwm(%u)",
...
status_t df_verify_pageid_by_size(knl_session_t *session, page_id_t page_id)
{
    ...
    if (page_id.page >= (uint32)max_page_count) {
        OG_LOG_DEBUG_WAR("page is invaild, page id(%u) is not less than datafile max page count(%u)",
