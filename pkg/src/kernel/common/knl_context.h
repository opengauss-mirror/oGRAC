...
typedef struct st_knl_attr {
    ...
    uint16 repl_port;
    char repl_trust_host[OG_HOST_NAME_BUFFER_SIZE * OG_MAX_LSNR_HOST_COUNT];
    ...
    char raft_peer_ids[OG_HOST_NAME_BUFFER_SIZE];
    char raft_local_addr[OG_HOST_NAME_BUFFER_SIZE];
    char raft_peer_addrs[OG_RAFT_PEERS_BUFFER_SIZE];
    char raft_kudu_dir[OG_FILE_NAME_BUFFER_SIZE];
    char raft_priority_type[OG_FILE_NAME_BUFFER_SIZE];
    char raft_priority_level[OG_FILE_NAME_BUFFER_SIZE];
    char raft_layout_info[OG_FILE_NAME_BUFFER_SIZE];
    char raft_pending_cmds_buffer_size[OG_MAX_NAME_LEN];
    char raft_send_buffer_size[OG_MAX_NAME_LEN];
    char raft_receive_buffer_size[OG_MAX_NAME_LEN];
    char raft_tls_dir[OG_FILE_NAME_BUFFER_SIZE];
    char raft_token_verify[OG_FILE_NAME_BUFFER_SIZE];
    char raft_max_size_per_msg[OG_MAX_NAME_LEN];
    char raft_entry_cache_memory_size[OG_MAX_NAME_LEN];
    char raft_mem_threshold[OG_MAX_NAME_LEN];
    char raft_election_timeout[OG_MAX_NAME_LEN];
    ...
    arch_attr_t arch_attr[OG_MAX_ARCH_DEST];
    char pwd_alg[OG_NAME_BUFFER_SIZE];
    char sys_pwd[OG_PASSWORD_BUFFER_SIZE]; // encrypted data
    ...
    uint32 idx_auto_rebuild_start_date; // total seconds from 00:00:00, invalid OG_INVALID_ID32
...
typedef struct st_knl_instance {
    ...
    vm_pool_t temp_pool[OG_MAX_BUF_POOL_NUM];

    switch_ctrl_t switch_ctrl;
    volatile bool32 record_backup_trigger[OG_MAX_PHYSICAL_STANDBY];

    uint32 reserved_sessions;
    atomic32_t running_sessions;
    atomic32_t assigned_sessions;
    knl_session_t *sessions[OG_MAX_SESSIONS];
    knl_rm_t *rms[OG_MAX_RMS];
    uint32 rm_count;
    char instance_name[OG_NAME_BUFFER_SIZE];
