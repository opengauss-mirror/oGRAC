 ...
 *
 * IDENTIFICATION
 * src/kernel/common/knl_sort_page.c
 *
 * -------------------------------------------------------------------------
 */
...
#define MTRL_PRINT_PAGE(ogx, segment, page)                  \
    if ((ogx)->print_page != NULL) {                      \
        (ogx)->print_page (segment, (char *)(page));      \
    }
...
static status_t mtrl_sort_move_left(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    char *pivot, int32 r_ind, int32 *l_ind)
{
    ...
    while (*l_ind <= r_ind) {
        ...
        if (ogx->sort_cmp(segment, row_l, pivot, &result) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (result > 0) {
            break;
...
static status_t mtrl_sort_move_right(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    char *pivot, int32 l_ind, int32 *r_ind)
{
    ...
    while (l_ind < *r_ind) {
        ...
        if (ogx->sort_cmp(segment, row_r, pivot, &result) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (result <= 0) {
            break;
...
static status_t mtrl_sort_span(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    qsort_span_t *span, int32 *new_ind)
{
    ...
    MTRL_PRINT_PAGE(ogx, segment, page);
    ...
    while (l_ind <= r_ind) {
        if (mtrl_sort_move_left(ogx, segment, page, pivot, r_ind, &l_ind) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (l_ind >= r_ind) {
            break;
        ...
        if (mtrl_sort_move_right(ogx, segment, page, pivot, l_ind, &r_ind) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (l_ind >= r_ind) {
            break;
...
status_t mtrl_sort_page(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page)
{
    int32 new_ind;
    qsort_stack_t stack;
    qsort_span_t span;
    ...
    while (stack.depth > 0) {
        ...
        if (mtrl_sort_span(ogx, segment, page, &span, &new_ind) != OG_SUCCESS) {
            return OG_ERROR;
        }
        QSORT_POP(&stack);
...
static status_t mtrl_binary_insert_sort(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page, int32 left,
    int32 right)
{
    int32 tmp_left;
    int32 tmp_right;
    ...
    for (int32 i = left + 1; i <= right; i++) {
        ...
        if (ogx->sort_cmp(segment, row_right, row_i, &cmp) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (cmp <= 0) {
            continue;
        ...
        while (tmp_left <= tmp_right) {
            ...
            if (ogx->sort_cmp(segment, row_mid, row_i, &cmp) != OG_SUCCESS) {
                return OG_ERROR;
            }
            if (cmp > 0) {
                tmp_right = mid - 1;
...
static status_t mtrl_five_points_check_equal(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    int32 e1, int32 e2, int32 e3, int32 e4, int32 e5, bool8 *equal)
{
    int32 cmp = 0;
    char *row_l = MTRL_GET_ROW(page, e1);
    char *row_r = MTRL_GET_ROW(page, e2);
    if (ogx->sort_cmp(segment, row_l, row_r, &cmp) != OG_SUCCESS) { // e1 & e2
        return OG_ERROR;
    }
    if (cmp == 0) {
        *equal = OG_TRUE;
    ...
    if (ogx->sort_cmp(segment, row_l, row_r, &cmp) != OG_SUCCESS) { // e2 & e3
        return OG_ERROR;
    }
    if (cmp == 0) {
        *equal = OG_TRUE;
    ...
    if (ogx->sort_cmp(segment, row_l, row_r, &cmp) != OG_SUCCESS) { // e3 & e4
        return OG_ERROR;
    }
    if (cmp == 0) {
        *equal = OG_TRUE;
    ...
    if (ogx->sort_cmp(segment, row_l, row_r, &cmp) != OG_SUCCESS) { // e4 & e5
        return OG_ERROR;
    }
    if (cmp == 0) {
        *equal = OG_TRUE;
...
static status_t mtrl_adaptive_dual_pivots_swap(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    char *pivot1, char *pivot2, int32 *less, int32 *great)
{
    ...
    for (int32 k = *less; k <= *great; k++) {
        ...
        if (ogx->sort_cmp(segment, row_k, pivot1, &cmp1) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (cmp1 < 0) {
        ...
        if (ogx->sort_cmp(segment, row_k, pivot2, &cmp2) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (cmp2 <= 0) {
            continue;
        ...
        if (ogx->sort_cmp(segment, row_g, pivot2, &cmp2) != OG_SUCCESS) {
            return OG_ERROR;
        }
        while (cmp2 > 0) {
            if ((*great)-- == k) {
        ...
        if (ogx->sort_cmp(segment, row_g, pivot1, &cmp1) != OG_SUCCESS) {
            return OG_ERROR;
        }
...
static status_t mtrl_adaptive_dual_pivots_center_swap(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    char *pivot1, char *pivot2, int32 *less, int32 *great)
{
    char *row_k = NULL;
    char *row_g = NULL;
...
static status_t mtrl_adaptive_dual_pivots_center(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    char *pivot1, char *pivot2, int32 *less, int32 *great)
{
    char *row_less = NULL;
    char *row_great = NULL;
    ...
    if (ogx->sort_cmp(segment, row_less, pivot1, &cmp1) != OG_SUCCESS) {
        return OG_ERROR;
    }
    while (cmp1 == 0) {
        (*less)++;
    ...
    if (ogx->sort_cmp(segment, row_great, pivot2, &cmp2) != OG_SUCCESS) {
        return OG_ERROR;
    }
    while (cmp2 == 0) {
        (*great)--;
    ...
    return mtrl_adaptive_dual_pivots_center_swap(ogx, segment, page, pivot1, pivot2, less, great);
}
...
static status_t mtrl_adaptive_sort_dual_pivots(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    qsort_span_t *span, qsort_stack_t *stack, int32 e2, int32 e4)
{
    ...
    if (mtrl_adaptive_dual_pivots_swap(ogx, segment, page, pivot1, pivot2, &less, &great) != OG_SUCCESS) {
        return OG_ERROR;
    }
    ...
    if (less < e2 - seventh && e4 + seventh < great) {  // If center part is too large (comprises > 4/7 of the array)
        if (mtrl_adaptive_dual_pivots_center(ogx, segment, page, pivot1, pivot2, &less, &great) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    if (less < great) {
...
static status_t mtrl_three_ways_qsort_swap(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    int32 *i, int32 *j)
{
    ...
    for (int32 k = (*i) + 1; k <= *j; k++) {
        row_k = MTRL_GET_ROW(page, k);
        if (ogx->sort_cmp(segment, row_k, pivot, &cmp_k) != OG_SUCCESS) {
            return OG_ERROR;
        }
        if (cmp_k == 0) {
            continue;
        ...
        if (ogx->sort_cmp(segment, row_j, pivot, &cmp_j) != OG_SUCCESS) {
            return OG_ERROR;
        }
        while (cmp_j > 0) {
            if (--(*j) < k) {
...
static status_t mtrl_adaptive_sort_three_ways(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    qsort_span_t *span, qsort_stack_t *stack)
{
    int32 i = span->left;
    int32 j = span->right;
    if (mtrl_three_ways_qsort_swap(ogx, segment, page, &i, &j) != OG_SUCCESS) {
        return OG_ERROR;
    }
    QSORT_POP(stack);
    if (span->left < i - 1) {
...
static status_t mtrl_adaptive_sort_span(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
    qsort_span_t *span, qsort_stack_t *stack)
{
    ...
    if (len < MTRL_ADAPTIVE_SORT_THRESHOLD) {  // if len >= 4 * 7, e1 - left >= 5
        QSORT_POP(stack);
        return mtrl_binary_insert_sort(ogx, segment, page, left, right);
    }
    int32 seventh = (len / 8) + (len / 64) + 1;  // leng / 8 + len / 64 + 1 �� len / 7
    int32 e3 = (left + right) / 2;
    int32 e2 = e3 - seventh;
    int32 e1 = e2 - seventh;
    int32 e4 = e3 + seventh;
    int32 e5 = e4 + seventh;
    bool8 equal = OG_FALSE;
    ...
    if (mtrl_binary_insert_sort(ogx, segment, page, left, left + 4) != OG_SUCCESS) {
        return OG_ERROR;
    }
    MOVE_FIVE_POINTS(page, left, e1, e2, e3, e4, e5);

    if (mtrl_five_points_check_equal(ogx, segment, page, e1, e2, e3, e4, e5, &equal) != OG_SUCCESS) {
        return OG_ERROR;
    }
    if (!equal) {
        return mtrl_adaptive_sort_dual_pivots(ogx, segment, page, span, stack, e2, e4);
    }
    mtrl_swap_dir(page, left, e3);
    return mtrl_adaptive_sort_three_ways(ogx, segment, page, span, stack);
}

// It combines Binary-Insertion SORT, Dual-Pivot QSORT and Three-Way QSORT.
status_t mtrl_adaptive_sort_page(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page)
{
    qsort_stack_t stack;
    qsort_span_t span;
    if (page->rows <= 1) {
    ...
    while (stack.depth > 0) {
        span = QSORT_CURR(&stack);
        if (mtrl_adaptive_sort_span(ogx, segment, page, &span, &stack) != OG_SUCCESS) {
            return OG_ERROR;
        }
    }
    return OG_SUCCESS;
...
static status_t mtrl_binsearch(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page, char *row,
                               uint32 *slot)
{
    int32 result;
    int32 begin;
    ...
    if (ogx->sort_cmp(segment, row, cmp_row, &result) != OG_SUCCESS) {
        return OG_ERROR;
    }
    if (result >= 0) {
        *slot = page->rows;
...
status_t mtrl_insert_sorted_page(mtrl_context_t *ogx, mtrl_segment_t *segment, mtrl_page_t *page,
                                 char *row, uint16 row_size, uint32 *slot)
{
    ...
    if (mtrl_binsearch(ogx, segment, page, row, &pos) != OG_SUCCESS) {
        return OG_ERROR;
    }
