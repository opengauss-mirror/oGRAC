 ...
 *
 * IDENTIFICATION
 * src/kernel/replication/repl_raft.h
 *
 * -------------------------------------------------------------------------
 */
...
#define RAFT_ASYNC_LOG_NEXT(ogx, pos) (((pos) + 1) % (ogx)->logwr_async_buf_num)


#ifndef WIN32
#define DB_IS_RAFT_ENABLED(kernel) ((kernel)->attr.enable_raft)
#else
#define DB_IS_RAFT_ENABLED(kernel) (OG_FALSE)
#endif
...
typedef struct st_raft_context {
    ...
    char priority_type[OG_FILE_NAME_BUFFER_SIZE];
    char priority_level[OG_FILE_NAME_BUFFER_SIZE];
    char layout_info[OG_FILE_NAME_BUFFER_SIZE];
} raft_context_t;
