...
#define GBP_CONNEOG_TIMEOUT         3000 // 3s
...
#define GBP_MAX_READ_WAIT_TIME      3000 // 3s
...
typedef struct st_gbp_context {
    ...
    atomic_t dtc_read_failed;               /* DTC GBP read/apply failed; owner must READ_END before fallback */
    uint32 dtc_read_failed_node;
    uint32 dtc_read_failed_result;
    atomic_t dtc_gbp_fallback_required;     /* Persistent DTC partial fallback marker; cleared by owner only */
    uint32 dtc_gbp_fallback_node;
    uint32 dtc_gbp_fallback_result;
    uint32 dtc_gbp_fallback_reason;
...
typedef enum en_gbp_dtc_fallback_reason {
    GBP_DTC_FALLBACK_NONE = 0,
    GBP_DTC_FALLBACK_READ_FAILED = 1,
    GBP_DTC_FALLBACK_PAGE_READ = 2,
    GBP_DTC_FALLBACK_VERIFY_MISS = 3,
    GBP_DTC_FALLBACK_TAIL_INCOMPLETE = 4,
    GBP_DTC_FALLBACK_RECOVER_CHECK = 5,
} gbp_dtc_fallback_reason_e;
