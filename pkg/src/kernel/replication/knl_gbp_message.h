 ...
 *
 * IDENTIFICATION
 * src/kernel/replication/knl_gbp_message.h
 *
 * -------------------------------------------------------------------------
 */
 ...
 #define GBP_REQ_READ_META_CHUNK     22000   /* read page_id/page_lsn metadata snapshot from GBP */
 #define GBP_REQ_BATCH_PAGE_READ_SELECTED 23000 /* batch read selected page ids from GBP */
 ...
 /* writer_inst_id / writer_global_seq identify multi-writer page versions for peer GBPS. */
 ...
 #define GBP_META_CHUNK_NUM 1024

 typedef struct st_gbp_meta_item {
     page_id_t page_id;
     uint64 page_lsn;
     uint32 page_pcn;
     uint32 source_node;
 ...
 typedef struct st_gbp_read_meta_req {
     gbp_msg_hdr_t header;
     uint64 epoch;
     uint64 cursor;
     uint32 max_count;
 ...
 typedef struct st_gbp_read_meta_resp {
     gbp_msg_hdr_t header;
 ...
 } gbp_read_meta_resp_t;

 typedef struct st_gbp_selected_page_req {
     page_id_t page_id;
     uint64 selected_lsn;
 } gbp_selected_page_req_t;

 typedef struct st_gbp_batch_selected_read_req {
     gbp_msg_hdr_t header;
     uint32 count;
     uint32 reserved;
     gbp_selected_page_req_t pages[GBP_BATCH_PAGE_NUM];
 } gbp_batch_selected_read_req_t;
