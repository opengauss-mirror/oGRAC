                                       ...
                                       drc_lock_mode_e mode)
{
    ...
    if (size > DEFAULT_PAGE_SIZE(session)) {
        ...
        if (rbp_need_wait_before_remote_overwrite(session, ctrl)) {
            ...
            /*
             * Keep the DCS hot path non-blocking: snapshot the pending RBP image
             * while the old page is still latched, then overwrite the live ctrl.
             * If snapshot memory is unavailable, RBP falls back through gap/reset.
             */
            if (!rbp_try_detach_pending_page(session, ctrl)) {
                OG_LOG_RUN_WAR("[DCS][RBP] pending EDP snapshot detach failed before remote overwrite: "
                               "page=%u-%u old_lsn=%llu new_lsn=%llu, continue with RBP gap",
                               old_page_id.file, old_page_id.page, old_lsn, new_lsn);
            } else {
                OG_LOG_DEBUG_INF("[DCS][RBP] detached pending EDP before remote overwrite: "
                                 "page=%u-%u old_lsn=%llu new_lsn=%llu",
                                 old_page_id.file, old_page_id.page, old_lsn, new_lsn);
            }
            knl_panic_log(IS_SAME_PAGID(old_page_id, ctrl->page_id),
                          "[DCS][RBP] ctrl changed before remote page overwrite, old page %u-%u current page %u-%u",
