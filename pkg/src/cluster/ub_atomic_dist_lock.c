...
ub_lock_result_t ub_rw_lock_x_lock(ub_rw_lock_t *lock, const ub_lock_policy_t *policy, const ub_location_t *location)
{
    ...
    for (;;) {
        int expected = 0;
        if (atomic_compare_exchange_weak_explicit(&lock->state, &expected, INT32_MIN,
                                                  memory_order_acquire, memory_order_relaxed)) {
...
ub_lock_result_t ub_rw_lock_sx_lock(ub_rw_lock_t *lock, const ub_lock_policy_t *policy,
