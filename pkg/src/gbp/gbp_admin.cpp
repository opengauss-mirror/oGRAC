...
static bool parse_nonnegative_int_exact(const std::string& text, int& out) {
    ...
    try {
        ...
        if (parsed != text.size() || value < 0 || value > std::numeric_limits<int>::max()) return false;
...
static bool parse_file_page(const std::string& text, int& file_no, int& page_no) {
    ...
    if (!parse_nonnegative_int_exact(s.substr(0, pos), file_no)) return false;
    if (!parse_nonnegative_int_exact(s.substr(pos + 1), page_no)) return false;
    if (file_no > 65535) return false;
    return true;
}
...
static std::string hex_preview(const char* data, size_t len, size_t limit) {
    ...
    for (size_t i = 0; i < n; ++i) os << std::setw(2) << static_cast<unsigned>(static_cast<unsigned char>(data[i]));
    return os.str();
}
...
static std::string admin_query_page(GbpServerState& state, int file_no, int page_no) {
    ...
    std::optional<PageRecord> hit_rec;
    {
        ...
        if (it != shard.page_cache.end()) hit_rec = it->second;
    }
    if (!hit_rec) {
    ...
    cks_os << std::hex << std::setw(4) << std::setfill('0') << cks;
...
static std::string admin_dump_page(GbpServerState& state, int file_no, int page_no) {
    ...
    uint16_t cks = 0;
    page_diag_from_block(page_block_cstr(hit), lsn, pcn, cks);
    uint32_t tr_asn = 0, tr_blk = 0, tr_rst = 0, lr_asn = 0, lr_blk = 0, lr_rst = 0;
    uint64_t tr_lfn = 0, tr_lsn = 0, lr_lfn = 0, lr_lsn = 0;
    ...
    os << "DUMP_BEGIN\n"
       ...
       << "page_lsn=" << lsn << "\n"
       << "page_pcn=" << pcn << "\n"
       << "page_checksum=0x" << std::hex << std::setw(4) << std::setfill('0') << cks << std::dec << "\n"
       << "trunc_rst=" << tr_rst << "\n"
       << "trunc_asn=" << tr_asn << "\n"
       << "trunc_blk=" << tr_blk << "\n"
       << "trunc_lfn=" << tr_lfn << "\n"
    ...
    os << "trunc_hex=" << hex_preview(trunc_bytes, LOG_POINT_SIZE, LOG_POINT_SIZE) << "\n"
       ...
       << "page_header_hex=" << hex_preview(page_block_cstr(hit), GBP_PAGE_SIZE, 96) << "\n";
    const char* block = page_block_cstr(hit);
    if (block) {
        os << "page_tail_hex=" << hex_preview(block + GBP_PAGE_SIZE - 64, 64, 64) << "\n";
    } else {
        os << "page_tail_hex=\n";
    }
    os << "DUMP_END\n";
...
static std::string admin_window(GbpServerState& state) {
    ...
    if (!reset_diag.empty()) {
        os << reset_diag.substr(3) << "\n";
    }
    if (!frontier_diag.empty()) {
        os << frontier_diag.substr(3) << "\n";
    }
    os << "WINDOW_END\n";
    return os.str();
}
