...
struct PidBytesLess {
    bool operator()(const std::array<uint8_t, 8>& a, const std::array<uint8_t, 8>& b) const {
        return std::memcmp(a.data(), b.data(), 8) < 0;
    }
};

using MetaIndexMap = std::map<std::array<uint8_t, 8>, MetaSnapshotRow, PidBytesLess>;

inline std::array<uint8_t, 8> meta_index_key_from_pid_key(uint64_t pid_key) {
...
public:
    ...
    explicit LfnBucketIndex(const Config& /*cfg*/) {}

    LfnBucketIndex(const LfnBucketIndex& other) : by_lfn_(other.by_lfn_) { rebuild_by_pid_(); }
    ...
    std::vector<uint64_t> collect_purge_pids(uint64_t through_lfn, int budget) const {
        ...
        for (const auto& entry : by_lfn_) {
            if (entry.first > through_lfn) break;
            picked.push_back(entry.second);
            if (static_cast<int>(picked.size()) >= budget) break;
        }
        return picked;
    }
...
public:
    ...
    void pop(uint64_t pid) { present_.erase(pid); }
    ...
    void rebuild(const std::vector<uint64_t>& pids) {
        clear();
        for (uint64_t pid : pids) enqueue(pid);
    }
...
private:
    void compact_if_needed() {
        if (pos_ < 4096 || pos_ * 2 < order_.size()) return;
        ...
        for (size_t i = pos_; i < order_.size(); ++i) {
            ...
            auto it = present_.find(pid);
            if (it != present_.end() && it->second == gen) kept.emplace_back(pid, gen);
        }
...
struct EvictState {
    ...
    bool wait_stable(int timeout_ms) {
        ...
        while (job_running || !purge_stable) {
            ...
            const auto chunk = remaining > std::chrono::milliseconds(50) ? std::chrono::milliseconds(50) : remaining;
...
public:
    ...
    WindowSnap snap{};
    std::map<std::array<uint8_t, 8>, MetaSnapshotRow, PidBytesLess> meta_index_;
...
public:
    ...
    GbpShard& shard(uint32_t qid) { return *shards_[qid % OG_GBP_SESSION_COUNT]; }
    ...
    SelectedReadDiag& selected_read_diag() { return selected_read_diag_; }
