...
inline bool gbp_mkdirs_for_file(const std::string& file, std::string& err) {
    ...
    size_t start = 0;
    if (dir.size() >= 2 && dir[1] == ':') start = 2;
    while (start < dir.size() && (dir[start] == '/' || dir[start] == '\\')) start++;
...
inline bool gbp_init_log_file(const std::string& file, std::string& err) {
    ...
    if (!gbp_mkdirs_for_file(file, err)) return false;
...
inline int log_point_cmp(const log_point_t& left, const log_point_t& right, bool lsn_only) {
    if (lsn_only) {
        if (left.lsn > right.lsn) return 1;
        if (left.lsn < right.lsn) return -1;
        return 0;
    }
    if (left.rst_id > right.rst_id) return 1;
    if (left.rst_id < right.rst_id) return -1;
    if (left.asn > right.asn) return 1;
    if (left.asn < right.asn) return -1;
    if (left.block_id > right.block_id) return 1;
    if (left.block_id < right.block_id) return -1;
    return 0;
}
...
inline void page_diag_from_block(const char* blk, uint64_t& lsn, uint32_t& pcn, uint16_t& cks) {
    ...
    cks = 0;
    if (!blk) return;
    std::memcpy(&lsn, blk + 16, 8);
    std::memcpy(&pcn, blk + 24, 4);
    std::memcpy(&cks, blk + GBP_PAGE_SIZE - 8, 2);
}
...
inline SmbDecision smb_should_replace(uint32_t stored_inst, uint64_t stored_seq,
    ...
    if (incoming_seq > stored_seq) return {true, "newer_seq"};
    if (incoming_seq < stored_seq) return {false, "stale_seq"};
    if (incoming_inst == stored_inst) return {true, "idem_same_writer"};
