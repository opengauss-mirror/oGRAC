...
typedef enum en_cs_packet_version {
    ...
    CS_VERSION_6,     /* support login package client kind like jdbc/ogconn/... */
    CS_VERSION_7,     /* support more efficient encode and decode param */
    CS_VERSION_8,     /* 1. enum value of OG_TYPE_TIMESTAMP_TZ be changed (18->32)
    ...
    CS_VERSION_16,    /* support autotrace, only for ogsql client */
    ...
    CS_VERSION_23,    /* 1. support send the local IP address after the replication connected
                         ...
                         3. support send message with text format instead of str(login ack or send result error or pl
                             warning)
    ...
    CS_VERSION_24,    /* support jsonb
                         1. load: add convert_jsonb parameter
                         2. exp:  print the col_type of jsonb columns is 'JSONB',
                                  and not allowed exp jsonb data with TXT
                         3. support: exp/imp support subpartion format csf */
    CS_VERSION_25,    /* 1. support: exp indexes support nologging attribute */
    /* remember to modify and CS_LOCAL_VERSION if add new version !!! */
} cs_packet_version_t;
...
#define CS_LOCAL_VERSION (uint32) (CS_VERSION_25 + \
...
#define CS_FLAG_OGSQL_IN_ALTPWD       0x0400 // OGSQL supports interactive password change for expired accounts
#define OG_FLAG_CN_USE_ROUTE         0x0800 // CN direct route information
#define OG_FLAG_ALLOWED_BATCH_ERRS   0x1000 // whether has allowed errors or not
#define OG_FLAG_CREATE_TABLE_AS      0x2000 // whether create table as select
#define OG_FLAG_REMOTE_AS_SYSDBA     0x4000 // support for remote connect as sysdba
#define OG_FLAG_EXTEND               0x8000 // already used for extend flag packet
...
typedef struct tagcs_packet {
    ...
    uint32 buf_size;
    char *buf;
    char init_buf[OG_MAX_PACKET_SIZE];
} cs_packet_t;
...
#define CS_CREATE_TABLE_AS(flags)    ((flags) & OG_FLAG_CREATE_TABLE_AS)

/*
 * check the send-buffer size, extend the buffer dynamicly if need.
 * default buffer size is OG_MAX_PACKET_SIZE;
 * if max_buf_size == buf_size == OG_MAX_PACKET_SIZE, use default buffer, not extend;
...
static status_t cs_try_realloc_send_pack(cs_packet_t *pack, uint32 expect_size)
{
    ...
    if (!CS_HAS_REMAIN(pack, expect_size)) {
        if (OG_MAX_UINT32 - pack->head->size < (uint32)CM_ALIGN_8K(expect_size)) {
            OG_THROW_ERROR(ERR_NUM_OVERFLOW);
            return OG_ERROR;
        }
        // extend memory align 8K
        if (OG_MAX_UINT32 - pack->head->size < CM_ALIGN_8K(expect_size)) {
            OG_THROW_ERROR(ERR_NUM_OVERFLOW);
            return OG_ERROR;
        }

        if (pack->head->size + expect_size > pack->max_buf_size) {
            OG_THROW_ERROR(ERR_FULL_PACKET, "send", pack->head->size + expect_size, pack->max_buf_size);
            return OG_ERROR;
        }
        ...
        if (new_buf == NULL) {
            OG_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)new_buf_size, "large packet buffer");
            return OG_ERROR;
        }
...
#define CM_CHECK_SEND_PACK_FREE(pack, len) OG_RETURN_IFERR(cs_try_realloc_send_pack(pack, len))

#define CM_CHECK_RECV_PACK_FREE(pack, len)                                                    \
    {                                                                                         \
        if (!CS_HAS_RECV_REMAIN(pack, len)) {                                                 \
            OG_THROW_ERROR(ERR_PACKET_READ, (pack)->head->size, (pack)->offset, (uint32)(len)); \
            return OG_ERROR;                                                                  \
        }                                                                                     \
    }
...
static inline void cs_init_packet(cs_packet_t *pack, uint32 options)
{
    CM_POINTER(pack);
    pack->offset = 0;
    pack->max_buf_size = OG_MAX_PACKET_SIZE;
    pack->buf_size = OG_MAX_PACKET_SIZE;
...
static inline status_t cs_try_realloc_packet_buffer(cs_packet_t *pack, uint32 offset)
{
    ...
    if (pack->head->size > pack->buf_size) {
        ...
        if (pack->head->size > pack->max_buf_size || new_buf_size > pack->max_buf_size) {
            OG_THROW_ERROR(ERR_FULL_PACKET, "request", new_buf_size, pack->max_buf_size);
            return OG_ERROR;
        }
        char *new_buf = (char *)malloc(new_buf_size);
        if (new_buf == NULL) {
...
static inline status_t cs_put_timestamp(cs_packet_t *pack, const struct timeval *ts)
{
    ...
    uint32        offset;
    OG_RETURN_IFERR(cs_reserve_space(pack, sizeof(cs_timeval_t), &offset));
...
{
    CM_POINTER2(pack, text);
    OG_RETURN_IFERR(cs_get_int32(pack, (int32 *)&text->len));
...
{
    cs_timeval_t *tmp_tv = NULL;
    OG_RETURN_IFERR(cs_get_data(pack, sizeof(cs_timeval_t), (void **)&tmp_tv));
#ifdef WIN32
    if (CS_DIFFERENT_ENDIAN(pack->options)) {
...
{
    // bit[0] can be 0 1
    return ((flag & 0x01) == 0x01) ? (uint8)OG_TRUE : (uint8)OG_FALSE;
}
...
static inline uint8 cs_get_param_direction(uint8 flag)
{
    ...
    if ((flag & 0x40) == 0x40) {
        return ((flag & 0x80) == 0x80) ? OG_INOUT_PARAM : OG_INPUT_PARAM;
    } else {
        return ((flag & 0x80) == 0x80) ? OG_OUTPUT_PARAM : OG_INPUT_PARAM;
    }
}
