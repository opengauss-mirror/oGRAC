...
static bool32 cs_create_tcp_link(socket_t sock_ready, cs_pipe_t *pipe)
{
    ...
    OG_LOG_DEBUG_INF("[LSNR] Get remote tcp message from host %s by local %s",
    ...
    if (link->sock == CS_INVALID_SOCKET) {
        cs_check_file_error();
        OG_LOG_RUN_INF("Failed to accept connection request, OS error:%d", cm_get_os_error());
        return OG_FALSE;
    }
...
static bool32 cs_create_ssl_link(socket_t sock_ready, cs_pipe_t *pipe)
{
    ...
    /* set default options of sock */
    cs_set_io_mode(link->tcp.sock, OG_TRUE, OG_TRUE);
    cs_set_buffer_size(link->tcp.sock, OG_TCP_DEFAULT_BUFFER_SIZE, OG_TCP_DEFAULT_BUFFER_SIZE);
    cs_set_keep_alive(link->tcp.sock, OG_TCP_KEEP_IDLE, OG_TCP_KEEP_INTERVAL, OG_TCP_KEEP_COUNT);
    cs_set_linger(link->tcp.sock, 1, 1);
    
    link->tcp.closed = OG_FALSE;
    return OG_TRUE;
}
...
static bool32 cs_create_rdma_link(socket_t sock_ready, cs_pipe_t *pipe)
{
    ...
    if (link->sock == CS_INVALID_SOCKET) {
        cs_check_file_error();
        OG_LOG_RUN_INF("Failed to accept RDMA connection request, OS error:%d",
                       cm_get_os_error());
        return OG_FALSE;
    }
...
static void cs_try_ssl_accept(tcp_lsnr_t *lsnr, cs_pipe_t *pipe)
{
    socket_t sock_ready;
    int32 loop;
    int32 ret;
    struct epoll_event evnts[OG_MAX_LSNR_HOST_COUNT];

    ret = epoll_wait(lsnr->epoll_fd, evnts, (int)lsnr->sock_count, OG_POLL_WAIT);
    if (ret == 0) {
        return;
    }
    if (ret < 0) {
    ...
    if (ret > OG_MAX_LSNR_HOST_COUNT) {
        OG_LOG_RUN_ERR("the ret %d is illegal!", ret);
        return;
    }
...
void cs_try_rdma_accept(rdma_lsnr_t *lsnr, cs_pipe_t *pipe)
{
    ...
    /* try poll one connect request in time OG_POLL_WAIT */
    ret = cm_rdma_poll(&rdma_fd, 1, OG_POLL_WAIT);
    if (ret > 0 && ((uint16)rdma_fd.revents & (POLLERR | POLLHUP))) {
        OG_LOG_DEBUG_ERR("Failed to poll RDMA connection");
        return;
    }
    ...
    if (ret < 0) {
        if (cm_get_os_error() != EINTR) {
            ...
            OG_LOG_RUN_ERR("Failed to wait for RDMA connection request, OS error:%d", cm_get_os_error());
        }
        return;
    }
    ...
    if (lsnr->status != LSNR_STATUS_RUNNING) {
        ...
        OG_LOG_RUN_WAR("RDMA listener not in running status");
        return;
    }
    (void)lsnr->action(lsnr, pipe);
}
...
static void srv_ssl_lsnr_proc(thread_t *thread)
{
    ...
    OG_LOG_RUN_INF("ssl-lsnr thread started");
    ...
    OG_LOG_RUN_INF("ssl-lsnr thread closed");
}
...
static void srv_tcp_lsnr_proc(thread_t *thread)
{
    ...
    OG_LOG_RUN_INF("tcp-lsnr thread started");
    ...
    OG_LOG_RUN_INF("tcp-lsnr thread closed");
}
...
static status_t cs_create_one_lsnr_sock(tcp_lsnr_t *lsnr, const char *host, int32 *slot_id, bool32 is_replica)
{
    ...
    OG_RETURN_IFERR(cm_ipport_to_sockaddr(host, lsnr->port, &sock_addr));
    
    OG_RETURN_IFERR(cs_alloc_sock_slot(lsnr, slot_id));
    sock = &lsnr->socks[*slot_id];
    if (cs_create_socket(SOCKADDR_FAMILY(&sock_addr), sock) != OG_SUCCESS) {
        return OG_ERROR;
    }
    if (is_replica) {
        cs_set_buffer_size(*sock, OG_TCP_DEFAULT_BUFFER_SIZE, OG_TCP_DEFAULT_BUFFER_SIZE);
    }
    ...
    if (cs_tcp_try_connect(host, lsnr->port)) {
        cs_close_socket(*sock);
        *sock = CS_INVALID_SOCKET;
        OG_THROW_ERROR(ERR_TCP_PORT_CONFLICTED, host, (uint32)lsnr->port);
        return OG_ERROR;
    }
    ...
    if (code != 0) {
        cs_close_socket(*sock);
        *sock = CS_INVALID_SOCKET;
        OG_THROW_ERROR(ERR_SOCKET_BIND, host, (uint32)lsnr->port, cm_get_os_error());
        return OG_ERROR;
    }
...
static status_t cs_create_rdma_lsnr_sock(rdma_lsnr_t *lsnr, const char *host)
{
    ...
    *sock = CS_INVALID_SOCKET;
    ...
    if (cs_create_rdma_socket(SOCKADDR_FAMILY(&sock_addr), sock) != OG_SUCCESS) {
        return OG_ERROR;
    }
    cs_rdma_set_io_mode(*sock, OG_TRUE, OG_TRUE);
    option = 1;
...
status_t cs_add_lsnr_ipaddr(tcp_lsnr_t *lsnr, const char *ip_addr, int32 *slot_id)
{
    ...
    errno_t errcode;
    *slot_id = OG_MAX_LSNR_HOST_COUNT;
    if (cs_create_one_lsnr_sock(lsnr, ip_addr, slot_id, OG_FALSE) != OG_SUCCESS) {
        if (*slot_id != OG_MAX_LSNR_HOST_COUNT) {
    ...
    if (0 != epoll_ctl(lsnr->epoll_fd, EPOLL_CTL_ADD, ev.data.fd, &ev)) {
        OG_THROW_ERROR(ERR_SOCKET_LISTEN, "add socket for listening to epool fd", cm_get_os_error());
        cs_close_socket(lsnr->socks[*slot_id]);
        lsnr->socks[*slot_id] = CS_INVALID_SOCKET;
    ...
    OG_LOG_RUN_INF("[LSNR] add lsnr ip %s, current valid listening sock count = %lld", lsnr->host[*slot_id],
                   lsnr->sock_count);
    return OG_SUCCESS;
}
...
status_t cs_delete_lsnr_slot(tcp_lsnr_t *lsnr, int32 slot_id)
{
    if (0 != epoll_ctl(lsnr->epoll_fd, EPOLL_CTL_DEL, (int)lsnr->socks[slot_id], NULL)) {
        OG_THROW_ERROR(ERR_SOCKET_LISTEN, "delete socket for listening to epool fd", cm_get_os_error());
        return OG_ERROR;
    }
    cs_close_socket(lsnr->socks[slot_id]);
    lsnr->socks[slot_id] = CS_INVALID_SOCKET;
    (void)cm_atomic_dec(&lsnr->sock_count);
    OG_LOG_RUN_INF("[LSNR] delete lsnr ip %s, leave valid listening sock count = %lld", lsnr->host[slot_id],
                   lsnr->sock_count);
    return OG_SUCCESS;
}
...
status_t cs_create_lsnr_socks(tcp_lsnr_t *lsnr, bool32 is_replica)
{
    ...
    for (loop = 0; loop < OG_MAX_LSNR_HOST_COUNT; loop++) {
        if (host[loop][0] != '\0') {
            if (cs_create_one_lsnr_sock(lsnr, host[loop], &slot_id, is_replica) != OG_SUCCESS) {
...
status_t cs_create_rdma_lsnr(rdma_lsnr_t *lsnr)
{
    ...
    // for RDMA, multiple listenning sockets are not supported now
    if (host[0] != '\0') {
        if (cs_create_rdma_lsnr_sock(lsnr, host) != OG_SUCCESS) {
...
status_t cs_lsnr_init_epoll_fd(tcp_lsnr_t *lsnr)
{
    ...
    if (-1 == lsnr->epoll_fd) {
        OG_THROW_ERROR(ERR_SOCKET_LISTEN, "create epoll fd for listener", cm_get_os_error());
        return OG_ERROR;
    }
...
status_t cs_start_ssl_lsnr(tcp_lsnr_t *lsnr, connect_action_t action, bool32 is_replica)
{
    ...
    if (cs_create_lsnr_socks(lsnr, is_replica) != OG_SUCCESS) {
        OG_LOG_RUN_ERR("failed to create lsnr sockets for listener type %d", lsnr->type);
        return OG_ERROR;
    }

    if (cs_lsnr_init_epoll_fd(lsnr) != OG_SUCCESS) {
        cs_close_lsnr_socks(lsnr);
        OG_LOG_RUN_ERR("failed to init epoll fd for listener type %d", lsnr->type);
        return OG_ERROR;
    }

    lsnr->status = LSNR_STATUS_RUNNING;
    if (cm_create_thread(srv_ssl_lsnr_proc, 0, lsnr, &lsnr->thread) != OG_SUCCESS) {
        cs_close_lsnr_socks(lsnr);
        (void)epoll_close(lsnr->epoll_fd);
        lsnr->status = LSNR_STATUS_STOPPED;
        OG_LOG_RUN_ERR("failed to create accept thread for listener type %d", lsnr->type);
        return OG_ERROR;
    }
    return OG_SUCCESS;
}
...
status_t cs_start_tcp_lsnr(tcp_lsnr_t *lsnr, connect_action_t action, bool32 is_replica)
{
    ...
    lsnr->status = LSNR_STATUS_RUNNING;
    if (cm_create_thread(srv_tcp_lsnr_proc, 0, lsnr, &lsnr->thread) != OG_SUCCESS) {
        cs_close_lsnr_socks(lsnr);
        (void)epoll_close(lsnr->epoll_fd);
        lsnr->status = LSNR_STATUS_STOPPED;
        OG_LOG_RUN_ERR("failed to create accept thread for listener type %d", lsnr->type);
...
static void cs_uds_lsnr_proc(thread_t *thread)
{
    ...
    if (errcode != EOK) {
        ...
        OG_LOG_RUN_INF("uds-lsnr thread start failed");
        return;
    }
    ...
    OG_LOG_RUN_INF("uds-lsnr thread started");
    ...
    OG_LOG_RUN_INF("uds-lsnr thread closed");
}
...
{
    int32 loop = 0;
    char(*name)[OG_UNIX_PATH_MAX] = lsnr->names;

    lsnr->sock_count = 0;
    for (loop = 0; loop < OG_MAX_LSNR_HOST_COUNT; loop++) {
        if (name[loop][0] != '\0') {
            if (cs_uds_create_listener(lsnr->names[loop], &lsnr->socks[loop],
                lsnr->permissions) != OG_SUCCESS) {
                cs_close_uds_socks(lsnr);
                return OG_ERROR;
            }
            (void)cm_atomic_inc(&lsnr->sock_count);
...
status_t cs_start_uds_lsnr(uds_lsnr_t *lsnr, uds_connect_action_t action)
{
    ...
    if (status != OG_SUCCESS) {
        OG_LOG_RUN_ERR("create domain socket failed. error code is %d.", cm_get_os_error());
        return OG_ERROR;
    }

    if (cs_uds_lsnr_init_epoll_fd(lsnr) != OG_SUCCESS) {
        cs_close_uds_socks(lsnr);
        OG_LOG_RUN_ERR("failed to init epoll fd");
        return OG_ERROR;
    }

    lsnr->status = LSNR_STATUS_RUNNING;
    if (cm_create_thread(cs_uds_lsnr_proc, 0, lsnr, &lsnr->thread) != OG_SUCCESS) {
        cs_close_uds_socks(lsnr);
        (void)epoll_close(lsnr->epoll_fd);

        lsnr->status = LSNR_STATUS_STOPPED;
        OG_LOG_RUN_ERR("failed to create accept thread");
        return OG_ERROR;
    }
    return OG_SUCCESS;
}
