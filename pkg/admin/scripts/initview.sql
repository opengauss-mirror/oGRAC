...
AS
  SELECT U.NAME, T.NAME, C.NAME, DECODE(C.FLAGS & 4096, 4096, 'ROWID', TYPE_ID2NAME(C.DATATYPE)) || DECODE(C.FLAGS & 1024, 1024, '[]', ''),
...
AS
  SELECT T.NAME, C.NAME, DECODE(C.FLAGS & 4096, 4096, 'ROWID', TYPE_ID2NAME(C.DATATYPE)) || DECODE(C.FLAGS & 1024, 1024, '[]', ''),
  C.BYTES, C.PRECISION,  C.SCALE, DECODE(C.NULLABLE, 1, 'Y', 'N'), C.ID, 
  C.DEFAULT_TEXT, C.NUM_DISTINCT, C.LOW_VALUE, C.HIGH_VALUE, 0, NULL, C.HISTOGRAM,
  DECODE(C.FLAGS & 2048, 2048, 'TRUE', 'FALSE') 
...
AS
  ...
  LEFT JOIN SYS.SYS_HISTGRAM_ABSTR HH ON C.USER# = HH.USER# AND C.TABLE# = HH.TAB# AND C.ID = HH.COL# AND (HH.SPARE1 IS NULL OR HH.SPARE1 = 4294967295)
  UNION ALL
  SELECT V.NAME, C.NAME, DECODE(C.FLAGS & 4096, 4096, 'ROWID', TYPE_ID2NAME(C.DATATYPE)) || DECODE(C.FLAGS & 1024, 1024, '[]', ''), C.BYTES, C.PRECISION, C.SCALE, DECODE(C.NULLABLE, 1, 'Y', 'N'), C.ID, NULL,NULL::BINARY_INTEGER,NULL,NULL,
...
AS
  ...
  LEFT JOIN SYS.SYS_HISTGRAM_ABSTR HH ON C.USER# = HH.USER# AND C.TABLE# = HH.TAB# AND C.ID = HH.COL# AND (HH.SPARE1 IS NULL OR HH.SPARE1 = 4294967295) 
    where (U.NAME = user or 
           ...
           DBE_DIAGNOSE.has_obj_privs(user, U.NAME, '"'||T.NAME||'"', 'TABLE')) AND DBE_DIAGNOSE.TENANT_CHECK(0, U.TENANT_ID)
  UNION ALL
  SELECT U.NAME, V.NAME, C.NAME, DECODE(C.FLAGS & 4096, 4096, 'ROWID', TYPE_ID2NAME(C.DATATYPE)) || DECODE(C.FLAGS & 1024, 1024, '[]', ''), C.BYTES, C.PRECISION, C.SCALE, DECODE(C.NULLABLE, 1, 'Y', 'N'), C.ID, NULL,NULL::BINARY_INTEGER,NULL,NULL,
  NULL::BINARY_INTEGER,NULL::BINARY_INTEGER,0, NULL::CHAR,NULL::BINARY_INTEGER,NULL::TIMESTAMP,NULL,NULL::CHAR
...
AS
  SELECT U.NAME, V.NAME, C.ID, C.NAME, DECODE(C.FLAGS & 4096, 4096, 'ROWID', TYPE_ID2NAME(C.DATATYPE)) || DECODE(C.FLAGS & 1024, 1024, '[]', ''), C.BYTES, C.PRECISION, C.SCALE, DECODE(C.NULLABLE, 1, 'Y', 'N')
  FROM SYS.SYS_USERS U, SYS.SYS_VIEWS V, SYS.SYS_VIEW_COLS C 
...
AS
  SELECT V.NAME, C.ID, C.NAME, DECODE(C.FLAGS & 4096, 4096, 'ROWID', TYPE_ID2NAME(C.DATATYPE)) || DECODE(C.FLAGS & 1024, 1024, '[]', ''), C.BYTES, C.PRECISION, C.SCALE, DECODE(C.NULLABLE, 1, 'Y', 'N')
  FROM SYS.DV_ME ME, SYS.SYS_VIEWS V, SYS.SYS_VIEW_COLS C
...
AS
 SELECT
    ...
    C.ID,
    C.DEFAULT_TEXT,
    DECODE(LOCATE(',-' || (C.ID + 1)::VARCHAR(128) || ',', ',' || I.COL_LIST || ','), 0, 'N', 'Y')
