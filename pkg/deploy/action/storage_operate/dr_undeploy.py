...
class UNDeploy(object):
    ...
    def delete_hyper_metro_domain(self):
        ...
        metadata_dr_deploy_path = os.path.join(_paths.remote_data,
                                               f"metadata_{self.dr_deploy_info.get('storage_metadata_fs')}")
        metadata_dr_deploy_config = os.path.join(metadata_dr_deploy_path, "dr_deploy_param.json")
        dr_deploy_config = os.path.join(_paths.config_dir, "dr_deploy_param.json")
    ...
    def do_stop(self):
        ...
        if not os.path.exists(stop_flag_file):
            ...
            cmd = "sh %s/appctl.sh stop" % ctl_file_path
    ...
    def do_uninstall(self):
        ctl_file_path = os.path.join(CURRENT_PATH, "../")
        cmd = "sh %s/appctl.sh uninstall override" % ctl_file_path
        return_code, output, stderr = exec_popen(cmd, timeout=180)
        if return_code:
            err_msg = "Failed to execute uninstall, stderr:%s, output:%s" % (stderr, output)
            raise Exception(err_msg)
        if os.path.exists(_paths.ograc_home):
            shutil.rmtree(_paths.ograc_home)
    ...
    def clean_dr_config_file(self):
        ...
        if self.deploy_params.get("deploy_mode") == "dbstor":
            ...
            clean_cmd = "rm -rf " + os.path.join(_paths.remote_data, f"metadata_{fs_name}", "dr_deploy_param.json")
        try:
