...
def setup_logger(log_file, name="ograc_deploy", level=logging.INFO,
                 console_info=True):
    ...
    console_level = logging.INFO if console_info else logging.ERROR
...
def get_logger(module="deploy", log_file=None):
    if log_file is None:
        try:
            from config import cfg
            log_file = cfg.logs.log_file(module)
        except Exception:
            log_file = f"/opt/ograc/log/{module}/{module}.log"
    return setup_logger(log_file, name=f"ograc_{module}")
