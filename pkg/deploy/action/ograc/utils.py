...
def exec_popen(cmd, timeout=1800):
    ...
    try:
        stdout_b, stderr_b = pobj.communicate(
    ...
    stdout = stdout_b.decode(errors="replace").rstrip(os.linesep)
    stderr = stderr_b.decode(errors="replace").rstrip(os.linesep)
    return pobj.returncode, stdout, stderr


def run_cmd(cmd, timeout=1800, error_msg="Command failed"):
