...
from kernel_param_utils import (
    is_generated_kernel_parameter,
    iter_normalized_kernel_parameters,
...
class ConfigChecker:
    ...
    def kernel_parameters(value):
        is_valid, error_message = validate_kernel_parameters(value)
        if not is_valid:
...
class CheckInstallConfig(CheckBase):
    ...
    def _is_generated_kernel_parameter(key):
        return is_generated_kernel_parameter(key)
    ...
    def _apply_kernel_parameters(self, install_conf):
        ...
        for index, (param_name, param_value) in enumerate(
                iter_normalized_kernel_parameters(kernel_params), start=3):
